...
class DocxProfessionalGenerator:
    ...
    def generate_client_proposal(
        ...
        proposal_data = self._structure_proposal_data(lead_data, property_matches, market_analysis)
        ...
        output_path = self._save_document(
            doc, f"client_proposal_{lead_data.get('lead_id', 'unknown')}_{datetime.now().strftime('%Y%m%d')}"
        )
    ...
    def generate_api_documentation(
        self, service_paths: List[str], include_examples: bool = True, include_testing: bool = True
    ) -> Tuple[Path, DocumentMetadata]:
        """
        ...
        # Save document
        output_path = self._save_document(doc, f"api_documentation_{datetime.now().strftime('%Y%m%d')}")
    ...
    def generate_compliance_report(
        ...
        # Save document
        output_path = self._save_document(doc, f"compliance_report_{datetime.now().strftime('%Y%m%d')}")
    ...
    def generate_contract_amendment(
        self, original_contract: Dict[str, Any], amendments: List[Dict[str, Any]], legal_review: bool = False
    ) -> Tuple[Path, DocumentMetadata]:
        """
        ...
        output_path = self._save_document(doc, f"contract_amendment_{contract_id}_{datetime.now().strftime('%Y%m%d')}")
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    class_info = {"name": node.name, "docstring": ast.get_docstring(node), "methods": []}
    ...
    def _structure_proposal_data(
        self, lead_data: Dict[str, Any], property_matches: List[Dict[str, Any]], market_analysis: Dict[str, Any]
    ) -> ClientProposalData:
    ...
    def _analyze_pricing_strategy(self, property_matches: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        prices = [
            match.get("property", {}).get("price", 0)
            for match in property_matches
            if match.get("property", {}).get("price")
        ]
...
def demo_docx_generation():
    ...
    try:
        ...
        proposal_path, proposal_metadata = generator.generate_client_proposal(
            sample_lead, sample_properties, sample_market, template_type="luxury_proposal"
        )
        ...
        api_docs_path, api_metadata = generator.generate_api_documentation(service_paths, include_examples=True)
