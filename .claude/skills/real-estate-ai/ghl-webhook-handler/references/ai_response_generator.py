...
class RealEstateAIResponseGenerator:
    ...
    async def _generate_claude_response(
        ...
        messages = self._build_conversation_context(conversation_history, question_type, contact_data, lead_context)
...
- Ignore what they just told you"""
    ...
    def _get_question_context(
        self, question_type: str, contact_data: Dict[str, Any], lead_context: Optional[Dict[str, Any]]
    ) -> str:
        ...
        contexts = {
            ...
            "location": f"which neighborhoods or areas they're interested in. This is Rancho Cucamonga market focused.",
            ...
            "preapproval": f"their financing situation - are they pre-approved? This is crucial for serious buyers.",
    ...
    def _personalize_cached_response(self, cached_response: str, contact_data: Dict[str, Any]) -> str:
        ...
        if (
            contact_name
            and len(cached_response) < 140
            and not any(indicator in cached_response.lower() for indicator in ["hey", "hi", contact_name.lower()])
        ):
            return f"{contact_name}, {cached_response.lower()}"

        return cached_response

    def generate_handoff_message(self, contact_data: Dict[str, Any], qualification_summary: Dict[str, Any]) -> str:
    ...
    async def generate_batch_responses(self, requests: List[Dict[str, Any]]) -> List[str]:
        ...
        tasks = [self.generate_qualification_response(**request) for request in requests]
        ...
        for i, response in enumerate(responses):
            if isinstance(response, Exception):
                ...
                fallback = self._get_fallback_response(
                    requests[i].get("question_type", "follow_up"), requests[i].get("contact_data", {})
                )
