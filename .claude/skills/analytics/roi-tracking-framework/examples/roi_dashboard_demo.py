...
def create_trend_data():
    ...
    return pd.DataFrame({"date": dates, "weekly_savings": weekly_savings, "cumulative_savings": cumulative_savings})
...
def main():
    ...
    st.set_page_config(
        page_title="ROI Tracking Dashboard", page_icon="📈", layout="wide", initial_sidebar_state="expanded"
    )
    ...
    with col1:
        st.metric("Annual Savings", f"${annual_savings:,}", f"+${monthly_savings:,}/month")

    with col2:
        st.metric("ROI", f"{roi_percentage:.0f}%", "Return on Investment")

    with col3:
        st.metric("Payback Period", f"{payback_months:.1f} months", "Time to break even")

    with col4:
        net_value = annual_savings - total_investment
        st.metric("Net Annual Value", f"${net_value:,}", f"${total_investment:,} invested")
    ...
    fig_trends.update_layout(
        title="Cumulative Savings vs Investment", xaxis_title="Date", yaxis_title="Amount ($)", hovermode="x unified"
    )
    ...
    with col1:
        ...
        fig_pie = px.pie(values=values, names=categories, title="Annual Savings by Category")
    ...
    styled_df = performance_df.style.format(
        {"Investment": "${:,.0f}", "Annual Savings": "${:,.0f}", "ROI (%)": "{:.0f}%", "Payback (months)": "{:.1f}"}
