...
class MetricsTracker:
    ...
    def _update_skill_metrics(self, tool_name: str, success: bool, duration_ms: int) -> None:
        ...
        if today not in metrics["daily_summary"]:
            metrics["daily_summary"][today] = {"total_operations": 0, "successful_operations": 0, "tools_used": set()}
    ...
    def _calculate_skill_effectiveness(self, metrics: Dict) -> Dict:
        ...
        for tool_name, tool_data in metrics["tools"].items():
            ...
            effectiveness[tool_name] = {
                ...
                "performance": "fast"
                if tool_data["avg_duration_ms"] < 500
                else "medium"
                if tool_data["avg_duration_ms"] < 2000
                else "slow",
            }
    ...
    def check_rate_limit(self, operation: str, limit: int, window_seconds: int) -> Dict:
        ...
        if operation not in rate_limits:
            rate_limits[operation] = {"window_start": now, "count": 0, "limit": limit, "window_seconds": window_seconds}
    ...
    def generate_report(self, days: int = 7) -> str:
        ...
        # Tool usage table
        for tool_name, tool_data in sorted(metrics["tools"].items(), key=lambda x: x[1]["total_calls"], reverse=True):
            success_rate = (
                (tool_data["successful_calls"] / tool_data["total_calls"] * 100) if tool_data["total_calls"] > 0 else 0
            )
        ...
        for tool_name, effectiveness in sorted(
            metrics["skill_effectiveness"].items(), key=lambda x: x[1]["success_rate"], reverse=True
        ):
            report_lines.append(
                f"{tool_name:20s} | "
        ...
        for date_str, daily_summary in metrics["daily_summary"].items():
            ...
            if date_obj >= cutoff_date:
                success_rate = (
                    (daily_summary["successful_operations"] / daily_summary["total_operations"] * 100)
                    if daily_summary["total_operations"] > 0
                    else 0
                )
        ...
        # Find slowest tools
        slow_tools = [(name, data["avg_duration_ms"]) for name, data in metrics["tools"].items()]
        ...
        reliable_tools = [(name, eff["success_rate"]) for name, eff in metrics["skill_effectiveness"].items()]
    ...
    def export_csv(self, output_file: str = "metrics-export.csv") -> None:
        ...
        with open(output_path, "w", newline="") as csvfile:
            ...
            for tool_name, tool_data in metrics["tools"].items():
                success_rate = (
                    (tool_data["successful_calls"] / tool_data["total_calls"] * 100)
                    if tool_data["total_calls"] > 0
                    else 0
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Track and analyze skill/tool usage metrics")
    # Automated metrics collection mode (from PostToolUse hooks)
    parser.add_argument("--auto", action="store_true", help="Enable automated metrics collection mode (from hooks)")

    # Tool tracking arguments
    parser.add_argument("--tool", help="Tool name (Read, Write, Edit, Bash, Skill, etc.)")
    parser.add_argument(
        "--success", type=lambda x: x.lower() == "true", help="Whether operation succeeded (true/false)"
    )
    ...
    parser.add_argument("--limit", type=int, default=10, help="Rate limit count (default: 10)")
    parser.add_argument("--window", type=int, default=300, help="Rate limit window in seconds (default: 300)")
