...
class ProductSettings(BaseSettings):
    ...
    stripe_meter_id: Optional[str] = Field(
        default=None,
        description="Stripe meter ID for usage billing"
    )
    ...
    def validate_database_url(cls, v: str) -> str:
        ...
        if v.startswith("postgresql://"):
            ...
            raise ValueError(
                f"Database URL must use postgresql+asyncpg:// driver, got: {v[:30]}..."
            )
        return v
    ...
    def validate_stripe_key(cls, v: Optional[str]) -> Optional[str]:
        ...
        if not (v.startswith("sk_live_") or v.startswith("sk_test_")):
            raise ValueError(
                "Stripe API key must start with 'sk_live_' or 'sk_test_'"
            )
        return v
