...
class RealTimeDemo:
    ...
    async def _generate_lead_event(self, current_hour: int):
        ...
        await self.event_publisher.publish_lead_update(lead_id=lead_id, lead_data=lead_data, action=action)
    ...
    async def _generate_commission_event(self, current_hour: int):
        ...
        await self.event_publisher.publish_commission_update(
            deal_id=deal_id, commission_amount=float(amount), pipeline_status=status
        )
    ...
    async def _generate_performance_event(self, current_hour: int, include_metrics: bool = False):
        ...
        await self.event_publisher.publish_performance_update(
            metric_name=metric_name, metric_value=metric_value, metric_unit=unit, comparison=comparison
        )
    ...
    async def _generate_system_event(self, current_hour: int):
        ...
        await self.event_publisher.publish_system_alert(
            alert_type=alert_type, message=message, severity=severity, details=details
        )
...
async def main():
    ...
    parser.add_argument(
        "--scenario",
        choices=["business_day", "high_activity", "performance_demo", "custom"],
        default="business_day",
        help="Demo scenario to run",
    )
    parser.add_argument("--duration", type=int, default=10, help="Duration in minutes for time-based scenarios")
    parser.add_argument("--events", type=int, default=20, help="Number of events for custom scenario")
    parser.add_argument(
        "--interval", type=float, default=2.0, help="Interval between events in seconds for custom scenario"
    )
