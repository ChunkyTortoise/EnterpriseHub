...
class Service6Deployer:
    ...
    async def _deploy_environment_validation(self) -> bool:
        ...
        required_vars = ["DATABASE_URL", "REDIS_URL", "CLAUDE_API_KEY", "JWT_SECRET_KEY"]
    ...
    async def _deploy_monitoring_setup(self) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[green]✓ Monitoring active - {system_status.get('metrics', {}).get('total_services', 0)} services monitored[/green]"
            )
        ...
        try:
            ...
            if "security" in self.services:
                ...
                try:
                    ...
                    rate_limit_ok = await security.check_rate_limit(
                        mock_request, limit=10, limit_type=RateLimitType.PER_IP
                    )
...
async def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Run deployment validation without making changes")
    parser.add_argument("--skip-external", action="store_true", help="Skip external service validation")
