...
def index_frontend_assets():
    ...
    if nextjs_root.exists():
        for file_path in nextjs_root.glob("**/*.tsx"):
            ...
            registry.register_document(name=f"nextjs_{name}", content=content, tags=tags + ["nextjs", "typescript"])
    ...
    if streamlit_root.exists():
        for file_path in streamlit_root.glob("*.py"):
            ...
            registry.register_document(
                name=f"streamlit_{name}", content=content, tags=["streamlit", "python", "legacy-reference"]
            )
