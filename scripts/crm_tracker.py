...
VALID_STATUSES = ["lead", "contacted", "proposal_sent", "negotiation", "won", "lost", "inactive"]
...
def load_data() -> Dict:
    ...
    if not STORAGE_FILE.exists():
        return {"prospects": [], "metadata": {"created": datetime.now().isoformat(), "version": "1.0"}}
...
def cmd_pipeline(args):
    ...
    if counts.get("lead", 0) + counts.get("won", 0) > 0:
        conversion_rate = (
            counts.get("won", 0) / (counts.get("lead", 0) + counts.get("won", 0) + counts.get("lost", 0)) * 100
            if (counts.get("lead", 0) + counts.get("won", 0) + counts.get("lost", 0)) > 0
            else 0
        )
...
def cmd_export(args):
    ...
    with open(output_file, "w", newline="") as csvfile:
        fieldnames = ["company", "status", "source", "rate", "contact", "deal_size", "created", "updated", "notes"]
...
def main():
    ...
    add_parser.add_argument(
        "--source", required=True, choices=["upwork", "fiverr", "referral", "linkedin", "direct"], help="Lead source"
    )
