...
class ProductionConfigValidator:
    ...
    def load_config(self):
        ...
        try:
            ...
            self.results.append(
                ValidationResult(
                    "Configuration", "File Loading", "fail", f"Error loading configuration: {e}", "critical"
                )
            )
    ...
    def validate_environment_basics(self):
        ...
        for key, expected in required_basics.items():
            ...
            if not value:
                self.results.append(
                    ValidationResult(
                        "Environment", key, "fail", f"Required environment variable missing: {key}", "high"
                    )
                )
                ...
                if value not in expected:
                    ...
                    self.results.append(
                        ValidationResult("Environment", key, "pass", f"{key} correctly set to '{value}'")
                    )
            elif value != expected:
                self.results.append(
                    ValidationResult(
                        "Environment", key, "fail", f"{key} must be '{expected}' in production, got '{value}'", "high"
                    )
                )
            else:
                self.results.append(ValidationResult("Environment", key, "pass", f"{key} correctly set to '{value}'"))
    ...
    def validate_database_config(self):
        ...
        if not db_url:
            self.results.append(
                ValidationResult(
                    "Database", "Primary Connection", "fail", "DATABASE_URL is required for production", "critical"
                )
            )
        else:
            # Parse database URL
            try:
                ...
                if "sslmode=require" not in db_url:
                    ...
                    self.results.append(
                        ValidationResult(
                            "Database", "SSL Configuration", "pass", "SSL mode correctly configured for production"
                        )
                    )
                ...
                self.results.append(
                    ValidationResult("Database", "Primary Connection", "pass", "Database URL format is valid")
                )

            except Exception as e:
                self.results.append(
                    ValidationResult(
                        "Database", "Primary Connection", "fail", f"Invalid DATABASE_URL format: {e}", "high"
                    )
                )
        ...
        if not read_db_url:
            ...
            self.results.append(
                ValidationResult("Database", "Read Replica", "pass", "Read replica configured for analytics queries")
            )

        # Check pool configuration
        pool_configs = {"DB_POOL_SIZE": (10, 50), "DB_MAX_OVERFLOW": (20, 100), "DB_POOL_TIMEOUT": (10, 60)}
            ...
            if value:
                try:
                    ...
                    if min_val <= int_value <= max_val:
                        self.results.append(
                            ValidationResult(
                                "Database", config_key, "pass", f"{config_key} appropriately set to {int_value}"
                            )
                        )
                    else:
                        self.results.append(
                    ...
                    self.results.append(
                        ValidationResult(
                            "Database", config_key, "fail", f"{config_key} must be a valid integer", "medium"
                        )
                    )
    ...
    def validate_redis_config(self):
        ...
        if not redis_url:
            self.results.append(
                ValidationResult(
                    "Redis", "Connection URL", "fail", "REDIS_URL is required for production caching", "critical"
                )
            )
            return
        ...
        if "default:" not in redis_url and "@" not in redis_url:
            self.results.append(
                ValidationResult(
                    "Redis", "Authentication", "fail", "Redis must have authentication enabled in production", "high"
                )
            )
        else:
            self.results.append(ValidationResult("Redis", "Authentication", "pass", "Redis authentication configured"))
        ...
        if cluster_nodes:
            ...
            if node_count >= 3:
                self.results.append(
                    ValidationResult(
                        "Redis", "Cluster Configuration", "pass", f"Redis cluster with {node_count} nodes configured"
                    )
                )
            else:
                self.results.append(
            ...
            if ttl_value:
                try:
                    ...
                    if min_ttl <= ttl_int <= max_ttl:
                        self.results.append(
                            ValidationResult("Redis", ttl_key, "pass", f"TTL appropriately set to {ttl_int} seconds")
                        )
                    else:
                        self.results.append(
                            ValidationResult(
                    ...
                    self.results.append(
                        ValidationResult("Redis", ttl_key, "fail", f"{ttl_key} must be a valid integer", "medium")
                    )
    ...
    def validate_ai_services(self):
        ...
        if not anthropic_key:
            ...
            self.results.append(
                ValidationResult("AI Services", "Anthropic API Key", "pass", "Anthropic API key format appears valid")
            )
        ...
        if timeout:
            try:
                ...
                if timeout_int < 10 or timeout_int > 60:
                    ...
                    self.results.append(
                        ValidationResult(
                            "AI Services", "API Timeout", "pass", f"API timeout appropriately set to {timeout_int}s"
                        )
                    )
            except ValueError:
                self.results.append(
                    ValidationResult(
                        "AI Services", "API Timeout", "fail", "ANTHROPIC_TIMEOUT must be a valid integer", "medium"
                    )
                )
    ...
    def validate_security_config(self):
        ...
        if not jwt_secret:
            self.results.append(
                ValidationResult(
                    "Security", "JWT Secret", "fail", "JWT_SECRET_KEY is required for authentication", "critical"
                )
            )
            ...
            self.results.append(
                ValidationResult(
                    "Security", "JWT Secret", "pass", f"JWT secret length appropriate ({len(jwt_secret)} chars)"
                )
            )
        ...
        if not encryption_key:
            ...
            self.results.append(
                ValidationResult("Security", "Encryption Key", "pass", "Encryption key length is correct")
            )
        ...
        if "localhost" in cors_origins or "127.0.0.1" in cors_origins:
            ...
            self.results.append(
                ValidationResult(
                    "Security", "CORS Configuration", "warning", "CORS_ORIGINS not properly configured", "medium"
                )
            )
        else:
            self.results.append(
                ValidationResult("Security", "CORS Configuration", "pass", "CORS origins appear properly configured")
            )
        ...
        if rate_limit:
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Security", "Rate Limiting", "fail", "API_RATE_LIMIT must be a valid integer", "medium"
                    )
                )
    ...
    def validate_ghl_integration(self):
        ...
        if not location_id:
            ...
            self.results.append(
                ValidationResult("GHL Integration", "Location ID", "pass", "GHL location ID is configured")
            )

        if not api_key:
            self.results.append(
                ValidationResult(
                    "GHL Integration", "API Key", "fail", "GHL_API_KEY is required for platform integration", "critical"
                )
            )
            ...
            self.results.append(
                ValidationResult("GHL Integration", "API Key", "pass", "GHL API key format appears valid")
            )
        ...
        if not webhook_secret:
            ...
            self.results.append(
                ValidationResult(
                    "GHL Integration", "Webhook Secret", "pass", "GHL webhook secret length is appropriate"
                )
            )
    ...
    def validate_performance_config(self):
        ...
        if ml_timeout:
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Performance", "ML Timeout", "fail", "ML_PREDICTION_TIMEOUT must be a valid integer", "medium"
                    )
                )
    ...
    def validate_monitoring_config(self):
        ...
        if metrics_enabled != "true":
            ...
            self.results.append(
                ValidationResult("Monitoring", "Metrics Collection", "pass", "Metrics collection enabled")
            )

        # Check Prometheus configuration
        prometheus_enabled = self.config.get("PROMETHEUS_ENABLED", "").lower()
        if prometheus_enabled == "true":
            self.results.append(ValidationResult("Monitoring", "Prometheus", "pass", "Prometheus monitoring enabled"))
            ...
            if not metrics_path.startswith("/"):
                self.results.append(
                    ValidationResult(
                        "Monitoring", "Metrics Path", "fail", "PROMETHEUS_METRICS_PATH must start with '/'", "medium"
                    )
                )
        else:
            self.results.append(
        ...
        if log_format != "json":
            ...
            self.results.append(
                ValidationResult("Monitoring", "Log Format", "pass", "Structured JSON logging configured")
            )
        ...
        if log_retention:
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Monitoring", "Log Retention", "fail", "LOG_RETENTION_DAYS must be a valid integer", "medium"
                    )
                )
    ...
    def validate_feature_flags(self):
        ...
        for flag, recommended in feature_flags.items():
            ...
            if value != recommended:
                ...
                self.results.append(ValidationResult("Features", flag, "pass", f"{flag} properly enabled"))

        # Check debug features are disabled
        debug_features = ["ENABLE_DEBUG_ENDPOINTS", "ENABLE_PERFORMANCE_PROFILING"]
        ...
        for flag in debug_features:
            ...
            if value == "true":
                self.results.append(
                    ValidationResult(
                        "Features", flag, "fail", f"{flag} must be 'false' in production for security", "high"
                    )
                )
            else:
                self.results.append(
                    ValidationResult("Features", flag, "pass", f"{flag} correctly disabled for production")
                )
    ...
    def print_results(self):
        ...
        for category, results in categories.items():
            ...
            for result in results:
                ...
                print(f"  {status_color}{status_icon}{Colors.END} {severity_indicator} {result.item}: {result.message}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Validate Jorge's BI Dashboard production configuration")
    parser.add_argument("--config", "-c", default=".env.bi.production", help="Path to production configuration file")
