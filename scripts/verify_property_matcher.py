...
def test_case(label, budget, beds=None, neighborhood=None, expected_min=1):
    results = matcher.find_buyer_matches(budget=budget, beds=beds, neighborhood=neighborhood, limit=5)
    ...
    for r in results:
        print(
            f"    - {r['id']}: ${r['price']:,}, {r.get('beds', 0)}bd, "
...
results.append(test_case("Entry-level ($600k, 3+ beds)", budget=600000, beds=3, expected_min=3))

# Test 2: Entry-level budget, 4+ beds
results.append(test_case("Entry-level ($600k, 4+ beds)", budget=600000, beds=4, expected_min=1))

# Test 3: Mid-market with neighborhood filter
results.append(
    test_case("Mid-market ($800k, Etiwanda neighborhood)", budget=800000, neighborhood="Etiwanda", expected_min=1)
)

# Test 4: Luxury, 6+ beds
results.append(test_case("Luxury ($1.5M, 6+ beds)", budget=1500000, beds=6, expected_min=1))

# Test 5: Budget too low for any property
results.append(test_case("Below market ($300k) — expect 0 matches", budget=300000, beds=3, expected_min=0))

# Test 6: Victoria neighborhood preference
results.append(
    test_case("Victoria neighborhood ($650k, 3+ beds)", budget=650000, beds=3, neighborhood="Victoria", expected_min=1)
)

# Test 7: Terra Vista mid-to-high
results.append(
    test_case("Terra Vista ($950k, 5+ beds)", budget=950000, beds=5, neighborhood="Terra Vista", expected_min=1)
)
