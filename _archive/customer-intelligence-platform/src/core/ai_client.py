...
class AIClient:
    ...
    def __init__(
        ...
        Args:
            provider: LLM provider ("gemini" or "claude"). Defaults to environment.
            model: Specific model name. Defaults to provider defaults.
            api_key: Optional API key for multi-tenancy.
        """
        provider_val = provider or os.getenv("DEFAULT_LLM_PROVIDER", "claude")
        ...
        if model:
            ...
            self.model = os.getenv("PERPLEXITY_MODEL", "llama-3.1-sonar-small-128k-online")
        else:
...
def get_available_providers() -> Dict[str, bool]:
    ...
    providers["perplexity"] = bool(os.getenv("PERPLEXITY_API_KEY"))
    return providers


def get_ai_client() -> AIClient:
    """Get a default AI client instance."""
    return AIClient(provider=LLMProvider.CLAUDE)
