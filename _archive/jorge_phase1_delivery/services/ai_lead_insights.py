...
class AILeadInsightsService:
    ...
    def _detect_urgency_signals(self, conversations: List) -> Dict:
        ...
        urgency_keywords = [
            'asap', 'urgent', 'quickly', 'soon', 'immediate', 'right away',
            'this week', 'today', 'tomorrow', 'need to move', 'deadline',
            'losing my lease', 'closing date', 'already sold', 'cash buyer'
        ]
    ...
    def _identify_hidden_objections(self, conversations: List) -> List[str]:
        ...
        price_keywords = ['expensive', 'afford', 'budget', 'too much', 'price', 'cheaper']
        for conv in conversations:
            message = conv.get('message', '').lower()
        ...
        trust_keywords = ['reviews', 'references', 'credentials', 'license', 'experience']
        for conv in conversations:
            message = conv.get('message', '').lower()
    ...
    def _detect_competitive_signals(self, conversations: List) -> Dict:
        ...
        competitive_keywords = [
            'other agent', 'another realtor', 'comparing', 'other offers',
            'someone else', 'different agent', 'multiple offers'
        ]
    ...
    def get_daily_insights_summary(self, tenant_id: str, leads: List[Dict]) -> Dict:
        ...
        return {
            ...
            'top_opportunities': [i.to_dict() for i in sorted(opportunities, key=lambda x: x.confidence, reverse=True)[:5]],
...
if __name__ == "__main__":
    ...
    sample_lead = {
        'lead_id': 'L123',
        'conversations': [
            {'sender': 'agent', 'message': 'Hey! Are you looking to buy or sell?', 'timestamp': '2026-01-05T10:00:00'},
            {'sender': 'lead', 'message': 'Looking to sell ASAP, already found my next place', 'timestamp': '2026-01-05T10:02:00'},
            {'sender': 'agent', 'message': 'Got it! Cash offer or list for top dollar?', 'timestamp': '2026-01-05T10:03:00'},
            {'sender': 'lead', 'message': 'What would be faster? Need to move in 2 weeks', 'timestamp': '2026-01-05T10:05:00'},
        ],
        'score': 8.5,
        'metadata': {
            'lead_id': 'L123',
