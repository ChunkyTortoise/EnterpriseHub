...
class PredictiveBuyerScoring:
    ...
    def _score_engagement(self, behavioral_data: Dict[str, Any]) -> float:
        ...
        if visits >= 10: score += 20
        elif visits >= 5: score += 15
        elif visits >= 2: score += 10
        elif visits >= 1: score += 5
        
        # Property views
        views = behavioral_data.get('property_views', 0)
        if views >= 15: score += 25
        elif views >= 8: score += 20
        elif views >= 4: score += 15
        elif views >= 1: score += 10
        
        # Saved properties
        saved = behavioral_data.get('saved_properties', 0)
        if saved >= 5: score += 20
        elif saved >= 3: score += 15
        elif saved >= 1: score += 10
        
        # Email opens
        opens = behavioral_data.get('email_opens', 0)
        if opens >= 10: score += 15
        elif opens >= 5: score += 10
        elif opens >= 1: score += 5
        
        # Recent activity (bonus)
        last_activity = behavioral_data.get('last_activity_days', 999)
        if last_activity <= 1: score += 20
        elif last_activity <= 3: score += 10
        elif last_activity <= 7: score += 5
    ...
    def _score_financial_readiness(self, lead_data: Dict[str, Any]) -> float:
        ...
        if down_payment >= 20: score += 20
        elif down_payment >= 10: score += 15
        elif down_payment >= 5: score += 10
        elif down_payment > 0: score += 5
        
        # Credit score (if available)
        credit = lead_data.get('credit_score', 0)
        if credit >= 740: score += 15
        elif credit >= 680: score += 10
        elif credit >= 620: score += 5
    ...
    def _score_urgency(
        ...
        if timeline == 'immediate': score += 40
        elif timeline == '1-3 months': score += 30
        elif timeline == '3-6 months': score += 20
        elif timeline == '6-12 months': score += 10
        ...
        if recent_searches >= 10: score += 15
        elif recent_searches >= 5: score += 10
        elif recent_searches >= 1: score += 5
    ...
    def _score_property_match(
        ...
        if search_patterns == 'very_consistent': score += 30
        elif search_patterns == 'consistent': score += 20
        elif search_patterns == 'somewhat_consistent': score += 10
    ...
    def _score_communication(self, behavioral_data: Dict[str, Any]) -> float:
        ...
        if response_rate >= 80: score += 35
        elif response_rate >= 60: score += 25
        elif response_rate >= 40: score += 15
        elif response_rate >= 20: score += 5
        
        # Phone calls answered
        calls_answered = behavioral_data.get('calls_answered', 0)
        if calls_answered >= 3: score += 25
        elif calls_answered >= 1: score += 15
        
        # Two-way conversations
        conversations = behavioral_data.get('two_way_conversations', 0)
        if conversations >= 3: score += 25
        elif conversations >= 1: score += 15
    ...
    def _classify_lead(self, score: float) -> str:
        ...
        if score >= 80: return "🔥 HOT - Ready to Buy"
        elif score >= 60: return "🎯 WARM - High Intent"
        elif score >= 40: return "📍 QUALIFIED - Nurture"
        elif score >= 20: return "🌱 COLD - Long-term"
        else: return "❄️ ICE COLD - Low Priority"
