...
class DatabaseValidator:
    ...
    async def validate_schema(self):
        ...
        # Check triggers
        await self.run_validation(
            "Schema", "Automated Triggers", "SELECT tgname FROM pg_trigger WHERE tgname NOT LIKE '%_oid'", 10
        )
    ...
    async def validate_security(self):
        ...
        # Check roles
        await self.run_validation(
            "Security", "Database Roles", "SELECT rolname FROM pg_roles WHERE rolname LIKE 'service6_%'", 5
        )
    ...
    async def validate_data_quality(self):
        ...
        if self.environment != "production":
            await self.run_validation(
                "Data Quality", "Sample Data", "SELECT COUNT(*) FROM leads WHERE deleted_at IS NULL", 5
            )
    ...
    async def validate_performance(self):
        ...
        await self.run_validation(
            "Performance", "Materialized Views", "SELECT matviewname FROM pg_matviews WHERE schemaname = 'public'", 1
        )
