    ...
    try:
        with st.spinner(f"Fetching data for {ticker}..."):
            ...
            if df is None or df.empty:
                st.error(
                    f"❌ No data found for {ticker}. Please verify the ticker symbol."
                )
                return
        ...
        st.error(f"❌ {str(e)}")
        st.info(
            "💡 **Tip:** Make sure you're using the correct ticker symbol (e.g., AAPL for Apple Inc.)"
        )
...
def _create_technical_chart(df: pd.DataFrame, ticker: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            x=df.index, y=df["MA20"], line=dict(color="orange", width=1), name="MA 20"
        ),
        row=1,
        col=1,
    )
