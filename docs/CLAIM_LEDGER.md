...
| Historical larger test-count claims are collectible/passing. | Local `python3 scripts/ci/collect_tests_summary.py` collected 7,740 tests on 2026-05-17. Targeted eval/orchestrator tests pass; some targeted health/security suites historically needed repair. | Credible but needs context | "Large test suite: 7,740 tests collected locally on 2026-05-17; publish the exact command and date with any count." |
...
| Reviewer checks require production secrets. | `make verify-reviewer`, `make verify-public`, and `make verify-focused` are documented as secret-free reviewer checks. `scripts/ci/secret_scan.py` and `scripts/ci/tracked_artifact_policy.py` guard against tracked secrets and local artifacts. | Strong | "Reviewer checks do not require production secrets; deeper local integrations use private env vars or untracked `.env` files." |
