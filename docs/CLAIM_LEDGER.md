...
| 8,212 tests are collectible/passing. | Local `pytest --collect-only --override-ini='addopts='` collected 7,721 tests with 38 skipped on 2026-04-29. Targeted eval/orchestrator tests pass; some targeted health/security suites fail. | Credible but needs context | "Large test suite: 7,721 tests collected locally on 2026-04-29; targeted eval/orchestrator tests pass, with health/webhook suites still needing repair." |
...
| Health checks are production-ready. | Health modules exist; targeted `tests/api/test_health_routes.py` failed 7/14 locally with DB/env assumptions and 500s. | Credible but needs context | "Health endpoints exist; local test fixtures and fail-closed behavior need tightening." |
