...
def upgrade() -> None:
    ...
    experiment_status_enum = postgresql.ENUM(
        "draft", "active", "paused", "completed", "archived", name="experiment_status", create_type=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            postgresql.JSONB(),
            nullable=True,
            server_default=sa.text('\'{"A": 0.5, "B": 0.5}\'::jsonb'),
        ),
...
def downgrade() -> None:
    ...
    # Drop ENUM type
    experiment_status_enum = postgresql.ENUM(
        "draft", "active", "paused", "completed", "archived", name="experiment_status", create_type=False
    )
