...
class DataQualityChecker:
    ...
    def check_regex(self, field_name: str, value: Any, pattern: str) -> QualityCheckResult:
        ...
        return QualityCheckResult(
            ...
            check_type="regex",
            passed=passed,
            message=f"{field_name} matches pattern"
            if passed
            else f"{field_name} does not match {pattern}",
        )
    ...
    def check_range(
        ...
        if min_val is not None and num < min_val:
            return QualityCheckResult(
                ...
                check_type="range",
                passed=False,
                message=f"{field_name}={num} < min {min_val}",
            )
        if max_val is not None and num > max_val:
            return QualityCheckResult(
                ...
                check_type="range",
                passed=False,
                message=f"{field_name}={num} > max {max_val}",
            )
        return QualityCheckResult(
            field_name=field_name,
            check_type="range",
            passed=True,
            message=f"{field_name}={num} in range",
        )
    ...
    def check_unique(self, field_name: str, values: list[Any]) -> QualityCheckResult:
        ...
        return QualityCheckResult(
            ...
            check_type="unique",
            passed=passed,
            message=f"{field_name}: {unique_count}/{total} unique"
            if passed
            else f"{field_name}: {total - unique_count} duplicates found",
        )

    def check_freshness(
        self, field_name: str, timestamp: datetime, max_age: timedelta
    ) -> QualityCheckResult:
    ...
    def run_checks(self, data: dict[str, Any], checks: list[dict]) -> QualityReport:
        results = []
        for check in checks:
            ...
            if check_type == "not_null":
                ...
                results.append(
                    self.check_regex(field_name, data.get(field_name), params["pattern"])
                )
