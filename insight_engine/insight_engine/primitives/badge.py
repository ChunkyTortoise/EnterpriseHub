...
def render_obsidian_badge(text: str, config: BadgeConfig = BadgeConfig(), custom_icon: Optional[str] = None):
    ...
    glow_style = (
        f"box-shadow: 0 0 15px {variant_config['glow']}, 0 0 5px {variant_config['glow']};"
    ...
    icon_html = (
        f'<span style="margin-right: 0.4rem; display: inline-flex; align-items: center;">{icon(badge_icon, color=variant_config["color"], size=size_config["icon_size"])}</span>'
        if badge_icon
        else ""
    )
...
def lead_temperature_badge(temperature: Literal["hot", "warm", "cold"], text: str = None):
    ...
    config = BadgeConfig(
        variant=temperature, glow_effect=(temperature == "hot"), pulse_animation=(temperature == "hot"), show_icon=True
    )
