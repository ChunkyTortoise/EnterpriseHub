...
class LLMClient:
    ...
    def _init_perplexity(self) -> None:
        ...
        try:
            ...
            self._client = httpx.Client(
                base_url="https://api.perplexity.ai", headers={"Authorization": f"Bearer {api_key}"}, timeout=30.0
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.provider == LLMProvider.GEMINI:
            ...
            response = self._generate_perplexity(
                prompt, system_prompt, history, max_tokens, temperature, model_override=target_model
            )
        elif self.provider == LLMProvider.OPENROUTER:
            response = self._generate_openrouter(
                prompt, system_prompt, max_tokens, temperature, model_override=target_model
            )
        else:
            response = self._generate_claude(
                prompt, system_prompt, history, max_tokens, temperature, model_override=target_model
            )
            ...
            if self.provider == LLMProvider.MOCK:
                ...
                if "dojo sensei" in prompt_lower or "dojo sensei" in system_lower:
                    ...
                    content = json.dumps(
                        {
                            ...
                            "coaching_tips": [
                                "No changes needed. This is production-grade performance for elite global clients."
                            ],
                        }
                    )
                    ...
                    if "arbitrageur" in system_lower or "sophisticated" in system_lower:
                        arbitrageur_responses = [
                            "I'm liquidating my London portfolio to pivot into Singapore and Rancho Cucamonga. I need 12% cash-on-cash yield or I'm walking. Also, I'm under strict GDPR and PDPA data mandates. Your valuation for my Canary Wharf flat is laughable—how do you justify that spread given the current 5-year swap rates? If your compliance isn't airtight, we're done.",
                            "15% spread in Rancho Cucamonga? That's interesting. But how are you handling the vacancy risk in those emerging corridors? And I want a guarantee that my data stays within Singapore jurisdiction for the APAC leg.",
                        ...
                        if "arbitrage" in prompt_lower or "yield" in prompt_lower:
                            ...
                            content = "I'm looking for high-yield properties in Rancho Cucamonga. Don't show me anything with a cap rate below 8%. Also, I'm only paying 4% commission, take it or leave it."
                    ...
                    "you are an elite real estate ai agent" in system_lower
                    or "psychologist" in system_lower
                    or "jorge" in system_lower
                ):
                    # Trainee (Agent)
                    turn_count = len(history) // 2
                    ...
                    if "the gauntlet" in system_lower or "sophisticated arbitrageur" in system_lower:
                        gauntlet_responses = [
                            ...
                            "Our data indicates a 15% yield spread in Rancho Cucamonga's emerging tech corridors that counters current Singapore stamp duty drag. Shall we review the net exit metrics?",
                        ...
                        if (
                            "sketchy" in prompt_lower
                            or "people like us" in prompt_lower
                            or "good neighborhood" in prompt_lower
                        ):
                            ...
                            content = "We should focus on properties that meet your specific requirements for square footage and amenities in Rancho Cucamonga's high growth zones."
                ...
                if images:
                    for img_b64 in images:
                        full_prompt_parts.append({"mime_type": "image/jpeg", "data": img_b64})
                ...
                if cached_content:
                    ...
                    response = await model.generate_content_async(
                        full_prompt_parts, generation_config=gen_config, cached_content=cache
                    )
                else:
                    response = await model.generate_content_async(full_prompt_parts, generation_config=gen_config)
                ...
                result = await self._agenerate_perplexity(
                    prompt, system_prompt, history, max_tokens, temperature, model_override=target_model
                )
            else:
                ...
                if self.enhanced_caching_enabled and self.enhanced_caching and system_prompt:
                    try:
                        ...
                        if cache_candidates:
                            system_blocks = self.enhanced_caching.build_cached_messages(cache_candidates, location_id)

                            # Log caching details
                            total_cached_tokens = sum(c.token_count for c in cache_candidates if c.should_cache)
                            logger.info(
                                f"Enhanced caching: {len(cache_candidates)} candidates, {total_cached_tokens} tokens cached"
                            )
                        else:
    ...
    async def astream(
        self, prompt: str, system_prompt: Optional[str] = None, complexity: Optional[TaskComplexity] = None, **kwargs
    ...
    def _generate_gemini(
        ...
        **kwargs,
        ...
        if cached_content:
            ...
            cache = caching.CachedContent.get(cached_content)
            response = model.generate_content(full_prompt, generation_config=gen_config, cached_content=cache)
        else:
    ...
    def create_context_cache(
        self, content: str, display_name: str, ttl_minutes: int = 60, system_instruction: Optional[str] = None
    ) -> str:
        """
    ...
    def _stream_gemini(
        self, prompt: str, system_prompt: Optional[str], model_override: Optional[str] = None
    ) -> Generator[str, None, None]:
...
def get_llm_client() -> LLMClient:
    ...
    # ROADMAP-091: Implement proper singleton or factory pattern for LLM client
