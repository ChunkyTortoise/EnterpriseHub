...
class ServiceRegistry:
    ...
    def __init__(self, location_id: Optional[str] = None, demo_mode: bool = False, api_key: Optional[str] = None):
        """
        ...
        logger.info(f"ServiceRegistry initialized (location={self.location_id}, demo_mode={self.demo_mode})")
    ...
    def _get_service(self, service_name: str, module_name: str, class_name: str) -> Any:
        ...
        if service_name not in self._services:
            try:
                # Dynamic import
                module = __import__(f"ghl_real_estate_ai.services.{module_name}", fromlist=[class_name])
                ...
                if service_name == "demo_mode":
                    ...
                    try:
                        self._services[service_name] = service_class(location_id=self.location_id)
                    except TypeError:
            ...
            if recommender:
                try:
                    recommendations = recommender.get_recommendations(
                        lead_data.get("id"), context={"score": score_result.get("score")}
                    )
                except Exception as e:
    ...
    def calculate_commissions(self, deals: List[Dict[str, Any]], agent_id: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_deal_closer_suggestions(self, lead_id: str, conversation_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            if self.demo_mode:
                return self.demo_mode_service.get_deal_closer_suggestions(lead_id, conversation_history)
    ...
    def prepare_meeting(self, lead_id: str, meeting_type: str = "showing") -> Dict[str, Any]:
        """
    ...
    def get_revenue_metrics(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_competitive_analysis(self, property_id: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def _get_safe_dashboard_fallback(self) -> Dict[str, Any]:
        ...
        return {
            "revenue_metrics": {"total": 0, "trend": "stable", "forecast": 0},
            "lead_metrics": {"total": 0, "conversion_rate": 0, "velocity": 0},
            "performance_metrics": {"team_performance": [], "top_performers": []},
            "alert_summary": {"critical": 0, "warnings": 0, "info": 1, "messages": ["Dashboard in demo mode"]},
            "demo_mode": True,
        }
    ...
    def _get_safe_revenue_fallback(self) -> Dict[str, Any]:
        ...
        return {"total_revenue": 0, "revenue_by_source": {}, "trends": [], "forecasts": [], "demo_mode": True}

    def _get_safe_competitive_fallback(self) -> Dict[str, Any]:
        """Return safe default competitive analysis."""
        return {"market_position": "unknown", "competitors": [], "insights": [], "demo_mode": True}
...
def get_service_registry(
    ...
    if force_new or _service_registry_instance is None:
        _service_registry_instance = ServiceRegistry(
            location_id=location_id, demo_mode=demo_mode if demo_mode is not None else False, api_key=api_key
        )
