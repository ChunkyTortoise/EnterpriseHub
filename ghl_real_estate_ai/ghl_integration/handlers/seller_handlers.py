...
async def handle_listing_created(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Update GHL
        await _update_ghl_seller_fields(contact_id, {
...
async def handle_seller_response(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Emit event
        await _emit_seller_event("seller.response.processed", {
            "contact_id": contact_id,
            "stage": next_stage,
...
def _extract_address(data: Dict[str, Any]) -> Optional[str]:
    ...
    if message:
        ...
        address_pattern = r'\d+\s+[A-Za-z]+\s+(?:Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Boulevard|Blvd|Lane|Ln|Way|Court|Ct)'
