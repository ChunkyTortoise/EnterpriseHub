...
class DatabaseConnectionManager:
    ...
    async def bulk_insert(
        ...
        conflict_resolution: str = "DO NOTHING"
    ) -> int:
        """
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        leaked_connections = [
            conn_id for conn_id, start_time in self.active_connections.items()
            if current_time - start_time > self.connection_leak_threshold
        ]
        
        if leaked_connections:
            metrics['warnings'] = [f"Potential connection leak detected: {len(leaked_connections)} long-running connections"]
