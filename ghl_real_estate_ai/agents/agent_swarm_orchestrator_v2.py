...
class AgentSwarmOrchestratorV2:
        ...
        try:
            ...
            for deal in closed_deals:
                await lead_intelligence_swarm.learn_from_outcomes(
                    deal['contact_id'], 
                    {"is_conversion": True, "revenue": deal.get('value', 0), "tenant_id": tenant_id}
                )
            ...
            for feedback in human_feedback:
                ...
                await lead_intelligence_swarm.adjust_agent_weights({
                    feedback['agent_id']: feedback['rating']
                })
                agent.actions_taken += 1
            ...
            for lead in active_seller_leads:
                ...
                if research_results:
                    ...
                    script_response = await self.llm.agenerate(
                        prompt=prompt,
                        system_prompt="You are a real estate arbitrage specialist.",
                        temperature=0.7
                    )
                    ...
                    logger.info(f"[{tenant_id}] {agent.name}: Generated sanitized leverage script for lead in {zip_code}")
                    agent.actions_taken += 1
            ...
            for contact_id in stalled_leads:
                ...
                if optimizer_insight:
                    ...
                    if drift_data.get("is_softening", False):
                        logger.warning(f"🎯 [{tenant_id}] {agent.name}: TRIGGERING AUTOMATIC TAKE-AWAY CLOSE for {contact_id}")
    ...
    async def trigger_take_away_sms(self, contact_id: str, reason: str, tenant_id: str = "default_tenant"):
        ...
        sanitized_message = self.governance.enforce(raw_message, {"contact_id": contact_id, "task": "take_away_close", "tenant_id": tenant_id})
        ...
        try:
            ...
            for pattern in market_intel.migration_patterns:
                ...
                prediction = await market_intel.predict_cross_market_demand(
                    pattern.source_market, 
                    pattern.destination_market
                )
                
                if prediction["demand_score"] > 70:
                    logger.info(f"🚀 [{tenant_id}] {agent.name}: High Demand detected for {prediction['destination_market']} from {prediction['source_market']}")
                    ...
                    for lead in potential_migrants:
                        ...
                        sanitized_pitch = self.governance.enforce(
                            ...
                            {"market_id": pattern.destination_market, "task": "relocation_pitch", "tenant_id": tenant_id}
                        )
    ...
    async def run_portfolio_manager(self, tenant_id: str = "default_tenant"):
        ...
        try:
            ...
            for deal in dormant_deals:
                ...
                logger.warning(f"🚀 [{tenant_id}] {agent.name}: TRIGGERING RESURRECTION for {deal['name']} ({deal['id']})")
