...
from ..multi_agent.agent_coordinator import BaseAgent, AgentRole, ClientRequest, ClientPriority, AgentAssignment
...
class IntelligentLoadBalancer:
    ...
    async def assign_optimal_agent(self,
                                 ...
                                 constraints: Optional[Dict[str, Any]] = None) -> LoadBalancingDecision:
        """
        ...
        try:
            ...
            optimal_assignment = await self._select_optimal_assignment(
                client_request, agent_scores
            )
    ...
    async def rebalance_workload(self,
                               ...
                               rebalancing_strategy: Optional[LoadBalancingStrategy] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if workload_analysis['imbalance_score'] > self.rebalancing_threshold:
                ...
                execution_results = await self._execute_rebalancing_actions(
                    rebalancing_result['rebalancing_actions']
                )
    ...
    async def predict_capacity_needs(self,
                                   ...
                                   scenario_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Capacity prediction completed - {len(capacity_prediction['bottleneck_risks'])} risks identified")
    ...
    async def optimize_for_jorge_metrics(self,
    ...
    async def _score_agents_for_request(self,
                                      request: ClientRequest,
                                      agents: List[BaseAgent]) -> List[AgentScore]:
        ...
        try:
            ...
            for agent in agents:
                ...
                factor_scores[AssignmentFactor.EXPERTISE_MATCH] = await self._calculate_expertise_match(
                    agent, request
                )
                ...
                factor_scores[AssignmentFactor.CURRENT_WORKLOAD] = await self._calculate_workload_score(
                    agent, request
                )
