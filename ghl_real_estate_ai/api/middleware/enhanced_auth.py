...
class EnhancedJWTAuth:
    ...
    def _get_secure_secret_key(self) -> str:
        ...
        if not secret_key:
            if getattr(settings, 'environment', 'development') == 'production':
                ...
                raise ValueError(
                    f"JWT_SECRET_KEY environment variable is required in production. "
                    f"Error ID: {error_id}"
                )
    ...
    async def check_rate_limit(self, identifier: str, request: Request) -> None:
        ...
        try:
            ...
            logger.error(
                f"RATE_LIMIT_CHECK_ERROR: Error checking rate limit",
                extra={
                    "identifier": identifier,
                    "error": str(e)
                }
        ...
        try:
            ...
            if not token_id:
                raise AuthenticationError(
                    "Token missing unique identifier",
                    error_id="TOKEN_MISSING_JTI"
                )
    ...
    async def _is_token_blacklisted(self, token_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"BLACKLIST_CHECK_ERROR: Error checking token blacklist",
                extra={
                    "token_id": token_id,
                    "error": str(e)
                }
    ...
    async def blacklist_token(self, token_id: str, expires_at: datetime) -> None:
        ...
        try:
            ...
            logger.error(
                f"TOKEN_BLACKLIST_ERROR: Error blacklisting token",
                extra={
                    "token_id": token_id,
                    "error": str(e)
                }
    ...
    try:
        ...
        if not user_id:
            raise AuthenticationError(
                ...
                error_id="TOKEN_MISSING_USER_ID"
            )
