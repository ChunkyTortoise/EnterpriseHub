...
class ARVisualizationService:
    ...
    async def generate_property_ar_overlays(
        self, property_id: str, user_location: Optional[Dict[str, float]], device_capabilities: Dict[str, Any]
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to generate AR overlays"
            )

    async def generate_vr_tour_waypoints(
        self, property_id: str, quality_preference: str = "balanced"
    ) -> List[VRTourWaypoint]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to generate VR tour")

    async def create_spatial_anchor(
        self, property_id: str, anchor_data: Dict[str, Any], user_id: str
    ) -> ARSpatialAnchor:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create spatial anchor"
            )
    ...
    async def _generate_ar_property_insights(
        self, property_data: Dict[str, Any], user_location: Optional[Dict[str, float]]
    ) -> Dict[str, Any]:
        """Generate AI insights for AR overlay."""
        try:
            # Use Claude assistant to generate contextual insights
            context = {"property": property_data, "user_location": user_location, "ar_context": True}
            ...
            return {
                ...
                "concerns": ["Slightly above market average", "Limited public transportation"],
                "investment_score": 85,
    ...
    async def _generate_vr_waypoint_insights(
        self, property_data: Dict[str, Any], room_name: str, room_description: str
    ) -> str:
...
async def setup_property_visualization(
    request: PropertyVisualizationRequest, current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if request.visualization_type in ["ar_overlay", "mixed_reality"]:
            ...
            ar_overlays = await ar_visualization_service.generate_property_ar_overlays(
                request.property_id, request.user_location, request.device_capabilities
            )
        ...
        if request.visualization_type in ["vr_tour", "mixed_reality"]:
            ...
            vr_waypoints = await ar_visualization_service.generate_vr_tour_waypoints(
                request.property_id, request.quality_preference
            )
        ...
        device_optimizations = {
            ...
            "lighting_quality": "realtime"
            if request.device_capabilities.get("supports_realtime_lighting", False)
            else "baked",
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Visualization setup failed")
    ...
    try:
        ...
        model = Property3DModel(
            ...
            interactive_elements=[
                ...
                {"id": "windows", "type": "window_group", "count": 12, "interaction": "tap_for_interior_view"},
            ],
        )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="3D model retrieval failed")


@router.post("/anchors/create", response_model=ARSpatialAnchor)
async def create_spatial_anchor(
    anchor_request: Dict[str, Any] = Body(...), current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    Create persistent spatial anchor for AR content.
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Spatial anchor creation failed")


@router.get("/anchors/property/{property_id}")
async def get_property_anchors(property_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    """
    ...
    try:
        ...
        return {"property_id": property_id, "anchors": [anchor.dict() for anchor in anchors], "count": len(anchors)}

    except Exception as e:
        logger.error(f"Property anchors retrieval error: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Property anchors retrieval failed"
        )


@router.post("/mapping/upload")
async def upload_spatial_mapping(
    mapping_data: SpatialMappingData, current_user: Dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Spatial mapping upload failed")


@router.get("/tour/{property_id}/waypoints")
async def get_vr_tour_waypoints(
    property_id: str,
    quality: str = Query(default="balanced", pattern="^(low|balanced|high|ultra)$"),
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    """
    ...
    """
    try:
        waypoints = await ar_visualization_service.generate_vr_tour_waypoints(property_id, quality)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="VR tour waypoints retrieval failed"
        )
...
async def check_device_capabilities(
    ...
    try:
        ...
        device_caps = capabilities.get(
            device_type.lower(), {"ar_supported": False, "vr_supported": False, "recommended_quality": "low"}
        )
        ...
        return {
            ...
            "features_available": [
                feature
                for feature, supported in device_caps.items()
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Device capabilities check failed"
        )
