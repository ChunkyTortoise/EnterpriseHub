...
class MobileAuthService:
    ...
    async def authenticate_mobile_user(
        ...
        try:
            ...
            access_token = self.jwt_auth.create_access_token(
                access_token_data,
                expires_delta=timedelta(minutes=MOBILE_ACCESS_TOKEN_EXPIRE_MINUTES)
            )
            ...
            return MobileTokenResponse(
                ...
                user_info={
                    ...
                    "ghl_api_key": user_data.get("ghl_api_key", "")[:10] + "..." if user_data.get("ghl_api_key") else None
                },
        ...
        try:
            ...
            if not device_registration:
                raise HTTPException(
                    ...
                    detail="Device not registered for biometric authentication"
                )
            ...
            challenge_data = {
                ...
                "created_at": datetime.now(timezone.utc).isoformat(),
                "expires_at": (datetime.now(timezone.utc) + timedelta(minutes=BIOMETRIC_TOKEN_EXPIRE_MINUTES)).isoformat()
            }
            
            # Store challenge
            await cache.set(
                f"biometric_challenge:{challenge_token}",
                challenge_data,
                ttl=BIOMETRIC_TOKEN_EXPIRE_MINUTES * 60
            )
    ...
    async def _update_device_last_used(self, device_id: str, user_id: str):
        ...
        if device_registration:
            ...
            await cache.set(
                f"device_registration:{device_id}:{user_id}",
                device_registration,
                ttl=None
            )
...
async def get_device_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Device status check failed"
        )
...
async def unregister_device(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Device unregistration failed"
        )