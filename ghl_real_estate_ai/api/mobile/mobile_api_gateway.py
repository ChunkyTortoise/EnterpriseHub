...
async def get_mobile_dashboard(
    current_user=Depends(get_current_user), device_type: str = Header(default="ios", description="ios or android")
):
    """
            ...
            if _is_within_radius(
                ...
                mobile_lead = {
                    ...
                    "distance_miles": _calculate_distance(
                        lead.get("latitude"), lead.get("longitude"), location.latitude, location.longitude
                    ),
...
async def quick_update_lead(lead_id: str, update_data: MobileLeadUpdate, current_user=Depends(get_current_user)):
    """
...
async def get_property_swipe_stack(
    lead_id: Optional[str] = None, limit: int = Query(10, ge=5, le=20), current_user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        mls_repo = MLSAPIRepository(
            {"api_base_url": "https://api.mlsgrid.com/v2", "api_key": "mock_key", "provider": "generic"}
        )
...
@router.post("/sync")
async def offline_sync(sync_request: SyncRequest, current_user=Depends(get_current_user)):
    """
        ...
        for operation in sync_request.pending_operations:
            try:
                ...
                if op_type == "lead_update":
                    ...
                    if not server_lead:
                        sync_results.append(
                            {"operation_id": operation.get("id"), "status": "failed", "error": "Not found"}
                        )
                        continue
    ...
    try:
        ...
        async with db.get_connection() as conn:
            ...
            for row in lead_rows:
                updates.append({"type": "lead_update", "data": dict(row), "timestamp": row["updated_at"]})
            ...
            for row in comm_rows:
                updates.append({"type": "new_communication", "data": dict(row), "timestamp": row["sent_at"]})
...
async def _get_mobile_trends(location_id: str, period: str) -> Dict[str, Any]:
    ...
    try:
        ...
        async with db.get_connection() as conn:
            ...
            prev_leads = await conn.fetchval(
                "SELECT COUNT(*) FROM leads WHERE created_at > $1 AND created_at <= $2", prev_cutoff, cutoff
            )

            lead_trend = (
                "increasing" if current_leads > prev_leads else "decreasing" if current_leads < prev_leads else "stable"
            )

            # 2. Response Trend
            current_responses = await conn.fetchval(
                "SELECT COUNT(*) FROM communication_logs WHERE direction = 'inbound' AND sent_at > $1", cutoff
            )
            prev_responses = await conn.fetchval(
                "SELECT COUNT(*) FROM communication_logs WHERE direction = 'inbound' AND sent_at > $1 AND sent_at <= $2",
                prev_cutoff,
                cutoff,
            )
