...
class FeatureTrendResponse(BaseModel):
    ...
    summary_stats: Dict[str, float] = Field(
        default_factory=dict,
        description="Summary statistics (mean, std, trend_direction)"
    )
...
class MarketHeatmapRequest(BaseModel):
    ...
    metric_type: AnalyticsMetric = Field(
        default=AnalyticsMetric.LEAD_DENSITY,
        description="Market metric to visualize"
    )
    time_range_days: int = Field(
        default=30,
        ge=1,
    ...
    granularity: Granularity = Field(
        default=Granularity.NEIGHBORHOOD,
        description="Geographic granularity level"
    )
    custom_bounds: Optional[Dict[str, float]] = Field(
        None,
        description="Custom geographic bounds (north, south, east, west)"
    )
        ...
        schema_extra = {
            "example": {
                ...
                "metrics": {
                    ...
                    "hot_zones": [
                        {"name": "Downtown", "score": 95.2, "reason": "high_density_high_conversion"}
                    ]
                },
            ...
            "example": {
                ...
                "data": {
                    ...
                    "contributing_factors": [
                        "high_lead_density",
                        "above_avg_conversion",
                        "premium_property_values"
                    ],
                    "recommended_actions": [
                        "increase_ad_spend",
                        "deploy_premium_agent",
                        "schedule_market_visit"
                    ]
                },
                "timestamp": "2024-01-01T12:00:00Z",
                "priority": "critical"
