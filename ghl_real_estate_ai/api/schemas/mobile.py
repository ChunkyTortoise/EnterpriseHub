...
def build_mobile_error_response(
    ...
    return MobileErrorResponse(
        error_code=error_code, message=message, details=details, retry_after=retry_after, support_id=support_id
    )
...
def build_mobile_success_response(
    ...
    return MobileSuccessResponse(data=data, metadata=metadata)


def build_mobile_list_response(
    data: List[Dict[str, Any]], pagination: MobilePaginationMetadata, filters_applied: Optional[Dict[str, Any]] = None
) -> MobileListResponse:
    """Build standardized mobile list response."""
    return MobileListResponse(data=data, pagination=pagination, filters_applied=filters_applied)
