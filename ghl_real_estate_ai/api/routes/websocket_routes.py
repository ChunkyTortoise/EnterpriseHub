...
async def get_active_connections(current_user = Depends(get_current_user)):
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403,
            detail="Admin access required to view connection information"
        )
...
async def broadcast_event(
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403,
            detail="Admin access required to broadcast events"
        )
    ...
    try:
        ...
        if target_roles_str:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid role in target_roles. Valid roles: {valid_roles}"
                )
        ...
        logger.info(f"Admin {current_user.username} broadcasted {event_type.value} event to {len(target_roles) if target_roles else 'all'} roles")
...
async def test_websocket_connection(current_user = Depends(get_current_user)):
    ...
    try:
        ...
        if len(user_connections) == 0:
            test_results["recommendations"].append("No active WebSocket connections. Consider connecting to enable real-time updates.")
        elif len(user_connections) > 3:
            test_results["recommendations"].append(f"Multiple connections detected ({len(user_connections)}). Consider closing unused connections for better performance.")
...
async def start_websocket_services(current_user = Depends(get_current_user)):
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403,
            detail="Admin access required for service management"
        )
