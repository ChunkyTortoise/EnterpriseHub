...
from ghl_real_estate_ai.services.lead_sequence_state_service import SequenceDay, get_sequence_service
...
async def get_sequence_status(lead_id: str, _auth=Depends(require_auth)):
    ...
    try:
        ...
        progress = {
            ...
            "day_3_delivered_at": sequence_state.day_3_delivered_at.isoformat()
            if sequence_state.day_3_delivered_at
            else None,
            "day_7_delivered_at": sequence_state.day_7_delivered_at.isoformat()
            if sequence_state.day_7_delivered_at
            else None,
            "day_14_delivered_at": sequence_state.day_14_delivered_at.isoformat()
            if sequence_state.day_14_delivered_at
            else None,
            "day_30_delivered_at": sequence_state.day_30_delivered_at.isoformat()
            if sequence_state.day_30_delivered_at
            else None,
        ...
        if not sequence_state.day_3_completed:
            next_action = {
                "action": "Day 3 SMS",
                "scheduled_for": sequence_state.next_scheduled_at.isoformat()
                if sequence_state.next_scheduled_at
                else None,
            }
...
async def manual_trigger(
    ...
    try:
        ...
        success = await lead_scheduler._execute_sequence_action(request.lead_id, sequence_day, request.action_type)
...
async def health_check(scheduler_service=Depends(get_scheduler_service)):
    ...
    try:
        ...
        if is_healthy:
            ...
            raise HTTPException(status_code=503, detail="Scheduler is not healthy")
