...
async def get_state_delta(current_user: dict = Depends(enterprise_auth_service.get_current_enterprise_user)):
    ...
    return {
        "type": "STATE_DELTA",
        "delta": sync_service.get_delta()
    }

@router.post("/mock-update")
async def mock_agent_update(path: str, value: Any, current_user: dict = Depends(enterprise_auth_service.get_current_enterprise_user)):
...
@router.post("/thought")
async def record_thought(agent_id: str, thought: str, status: str = "Success", current_user: dict = Depends(enterprise_auth_service.get_current_enterprise_user)):
