...
from ...prediction.jorge_prediction_engine import JorgePredictionEngine, PredictionType, TimeFrame, PredictionContext
...
async def predict_client_behavior(
    ...
    try:
        ...
        behavioral_patterns = await client_analyzer.assess_behavioral_patterns(
            request.client_id, 30
        )

        financial_profile = await client_analyzer.evaluate_financial_readiness(
            request.client_id, request.context
        )
...
async def predict_deal_outcome(
    ...
    try:
        ...
        deal_forecast = await deal_predictor.predict_deal_success(
            request.deal_id,
            deal_data,
            DealStage(request.current_stage),
            request.context
        )
        ...
        for req in request.requests:
            try:
                ...
                if prediction_type == "market_movement":
                    prediction = await predict_market_movement(
                        MarketMovementRequest(**parameters),
                        current_user
                    )
                elif prediction_type == "client_behavior":
                    prediction = await predict_client_behavior(
                        ClientBehaviorRequest(**parameters),
                        current_user
                    )
                elif prediction_type == "deal_outcome":
                    prediction = await predict_deal_outcome(
                        DealOutcomeRequest(**parameters),
                        current_user
                    )
                elif prediction_type == "business_forecast":
                    prediction = await generate_business_forecast(
                        BusinessForecastRequest(**parameters),
                        current_user
                    )
                else:
...
async def update_prediction_accuracy(
    ...
    try:
        ...
        await prediction_engine.update_prediction_accuracy(
