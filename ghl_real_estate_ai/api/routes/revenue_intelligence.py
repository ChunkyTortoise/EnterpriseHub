...
async def analyze_deal_probabilities(
    ...
    try:
        ...
        deal_scores = await engine.calculate_deal_probability_scores(
            lead_ids=request.lead_ids, include_pipeline_analysis=request.include_pipeline_analysis
        )

        # Calculate pipeline summary metrics
        total_value = sum(float(deal.deal_value) for deal in deal_scores)
        weighted_probability = (
            sum(deal.closing_probability * float(deal.deal_value) for deal in deal_scores) / total_value
            if total_value > 0
            else 0
        )
        ...
        pipeline_summary = {
            ...
            "avg_time_to_close": sum(deal.time_to_close_days for deal in deal_scores) / len(deal_scores)
            if deal_scores
            else 0,
        }
    ...
    try:
        ...
        if not request.current_revenue:
            current_forecast = await engine.forecast_revenue_advanced(
                timeframe=request.timeframe, revenue_stream=RevenueStreamType.TOTAL_REVENUE
            )
        else:
        ...
        optimization_plan = await engine.generate_revenue_optimization_plan(
            current_forecast=current_forecast, target_growth=request.target_growth
        )
    ...
    try:
        ...
        executive_summary = {
            ...
            "pipeline_health": {
                ...
                "avg_probability": sum(forecast.deal_probability_scores.values())
                / len(forecast.deal_probability_scores)
                if forecast.deal_probability_scores
                else 0,
            },
            "jorge_methodology": {
