...
def convert_roi_calculation_to_dict(roi_calc: RealTimeROICalculation) -> Dict[str, Any]:
    ...
    data = {
        ...
        "projected_lifetime_value": float(roi_calc.projected_lifetime_value) if roi_calc.projected_lifetime_value else None
    }
    return data
...
async def create_justification_documentation(
    request: JustificationDocumentRequest,
    value_engine = Depends(get_value_engine)
):
    """
    ...
    try:
        ...
        dashboard_data = {
            ...
            "performance_indicators": {
                "payback_period_days": roi_calculation.payback_period_days,
                "projected_annual_value": float(roi_calculation.projected_annual_value) if roi_calculation.projected_annual_value else None,
