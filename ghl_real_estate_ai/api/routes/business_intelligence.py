...
async def get_revenue_intelligence(
    ...
    try:
        ...
        revenue_data = await bi_cache.get_revenue_pipeline(
            location_id=location_id,
            forecast_days=forecast_days if include_forecast else 0
        )
...
async def get_bot_performance_matrix(
    ...
    try:
        ...
        bot_metrics = await bi_cache.get_bot_performance_matrix(
            location_id=location_id,
            timeframe=timeframe
        )
...
async def detect_anomalies(
    ...
    try:
        ...
        anomalies = await _detect_performance_anomalies(
            metrics_data,
            timeframe,
            sensitivity
        )
...
async def _get_kpi_trends(location_id: str, timeframe: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for i in range(days):
        ...
        revenue_trend.append({
            "hour": date.strftime("%Y-%m-%d"),
            "value": 15000 + (i * 500) + (i % 3 * 1000)
        })
        leads_trend.append({
            "hour": date.strftime("%Y-%m-%d"),
            "value": 50 + (i * 2) + (i % 2 * 5)
