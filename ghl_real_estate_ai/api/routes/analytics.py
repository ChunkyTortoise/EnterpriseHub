...
from ghl_real_estate_ai.services.advanced_analytics import (
    ABTestManager,
...
class ExperimentCreate(BaseModel):
    ...
    metric: str = Field(
        default="conversion_rate", description="Primary metric to optimize"
    )
...
async def get_campaign_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch campaigns: {str(e)}"
        )
...
async def get_campaign_details(location_id: str, campaign_id: str, current_user: dict = Depends(enterprise_auth_service.get_current_enterprise_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch campaign details: {str(e)}"
        )
