...
async def lead_generation_performance_analysis(
    ...
    try:
        ...
        insights = await generation_engine.analyze_lead_generation_performance(
            location_id=location_id, analysis_period_days=request.analysis_period_days
        )
        ...
        response_data = {
            ...
            "alerts": {"performance": insights.performance_alerts, "budget": insights.budget_optimization_alerts},
            "confidence_level": insights.confidence_level,
            "generated_at": insights.generated_at.isoformat(),
        }

        logger.info(
            f"Generation analysis complete: Grade {insights.overall_performance_grade} [{processing_time:.1f}ms]"
        )
...
async def get_lead_scoring_history(
    ...
    try:
        ...
        history = await scoring_service.get_lead_score_history(
            contact_id=contact_id, location_id=location_id, days_back=days_back
        )
...
async def bulk_lead_scoring(
    ...
    try:
        ...
        results = await scoring_service.bulk_score_leads(
            contact_ids=contact_id_list, location_id=location_id, batch_size=batch_size
        )
        ...
        for contact_id, score in results.items():
            if isinstance(score, Exception):
                formatted_results[contact_id] = {"success": False, "error": str(score)}
            else:
