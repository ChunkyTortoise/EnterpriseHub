...
async def track_client_journey(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid journey stage '{request.current_stage}'. Valid stages: {valid_stages}"
            )
        ...
        response_data = {
            ...
            'timing': {
                ...
                'expected_completion_date': health_metrics.expected_completion_date.isoformat() if health_metrics.expected_completion_date else None,
...
async def progress_client_milestone(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid milestone type '{request.milestone_type}'. Valid types: {valid_types}"
            )
        ...
        response_data = {
            ...
            'timing': {
                'expected_completion': milestone.expected_completion.isoformat() if milestone.expected_completion else None,
        ...
        logger.info(f"Milestone progressed: {milestone_type.value} -> {milestone.completion_status} [{processing_time:.1f}ms]")
...
async def personalize_client_journey(
    location_id: str = Path(..., description="Location/tenant identifier"),
    request: JourneyPersonalizationRequest = ...
) -> StandardResponse:
    """
...
async def get_client_journey_insights(
    ...
    try:
        ...
        insights = await journey_service.get_journey_insights(
            client_id=client_id,
            location_id=location_id
        )
...
async def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'status': 'healthy',
            'services': {
                'client_journey_mapping': 'available',
