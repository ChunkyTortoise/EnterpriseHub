...
class ThreatDetectionRequest(BaseModel):
    ...
    sensitivity_level: str = Field("medium", pattern="^(low|medium|high)$")
...
class CompetitiveResponseRequest(BaseModel):
    ...
    response_type: str = Field(..., pattern="^(pricing|marketing|outreach|positioning)$")
    automation_level: str = Field("manual", pattern="^(manual|assisted|automatic)$")
...
async def generate_intelligence_report(
    ...
    analysis_period: str = Query("30_days", pattern="^(7_days|30_days|90_days|1_year)$"),
    report_format: str = Query("summary", pattern="^(summary|detailed|executive)$"),
    user: dict = Depends(require_permission("read:competitive_data")),
):
    """
