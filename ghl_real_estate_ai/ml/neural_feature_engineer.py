...
from ghl_real_estate_ai.ml.feature_engineering import (
    FeatureEngineer, ConversationFeatures, MarketFeatures
)
...
class NeuralFeatureEngineer:
    ...
    async def extract_comprehensive_features(
        ...
        try:
            ...
            structured_features, structured_names, struct_metadata = await self._extract_structured_features(data, data_type)
            ...
            temporal_features, temporal_names, temporal_metadata = await self._extract_temporal_features(data, data_type)
            ...
            logger.debug(f"Feature extraction completed in {extraction_time:.2f}ms: "
    ...
    async def _extract_structured_features(
        ...
        try:
            ...
            metadata = {
                "numerical": num_metadata,
                "categorical": cat_metadata,
    ...
    async def _extract_text_features(
        ...
        try:
            ...
            if data_type == "property":
                ...
                if preferences:
                    pref_text = f"Looking for {preferences.get('property_type', '')} " \
    ...
    async def _extract_image_features(
        ...
        try:
            ...
            all_image_features = np.concatenate([
                ...
                mock_spatial_features,
                mock_architectural_features
            ])
    ...
    async def _extract_temporal_features(
        ...
        try:
            ...
            metadata = {
                "total_temporal_features": len(temporal_features),
                "has_time_data": bool(temporal_features)
            }
    ...
    async def _extract_geospatial_features(
        ...
        try:
            ...
            if data_type == "property":
                ...
                feature_names.extend(["neighborhood_desirability", "neighborhood_safety", 
                                    "neighborhood_schools", "neighborhood_commute"])
                ...
                location_types = ["downtown", "suburban", "urban", "rural", "waterfront", 
                                "mountain", "golf", "historic", "new development", "gated"]
            ...
            metadata = {
                ...
                "total_geo_features": len(geo_features)
            }
        ...
        try:
            ...
            if data_type == "property":
                ...
                feature_names.extend([
                    "price_per_sqft_norm", "price_per_bedroom_norm", 
                    "bathroom_bedroom_ratio", "property_age_norm", "sqft_per_bedroom"
                ])
                ...
                feature_names.extend([
                    "cash_buyer", "investor", "first_time_buyer", 
                    "relocation", "upgrade", "downsize"
                ])
                ...
                feature_names.extend([
                    "immediate_timeline", "monthly_timeline", 
                    "preapproved", "budget_level"
                ])
            ...
            feature_names.extend([
                ...
                "interest_rate_level", "inventory_level", "appreciation_rate"
            ])
    ...
    def _extract_traditional_text_features(self, text: str) -> Tuple[List[float], List[str]]:
        ...
        real_estate_keywords = [
            "pool", "garage", "garden", "modern", "updated", "spacious",
            "quiet", "convenient", "schools", "shopping", "downtown",
            "neighborhood", "investment", "opportunity", "motivated",
            "negotiable", "price", "value", "location", "move-in"
        ]
...
async def test_neural_feature_engineer():
    ...
    property_features = await neural_engineer.extract_comprehensive_features(
    ...
    client_features = await neural_engineer.extract_comprehensive_features(
        client_data, "client"
    )
    ...
    if property_features.feature_importance:
        top_important = sorted(
            property_features.feature_importance.items(),
            key=lambda x: x[1],
            reverse=True
        )[:5]
