...
class EmergencyDealRescue:
    ...
    async def _analyze_conversation_signals(self, conversation_context: Dict) -> List[ChurnSignal]:
        ...
        for i, message in enumerate(recent_messages):
            ...
            negative_patterns = [
                'concerned about', 'worried', 'not sure', 'hesitant',
                'changed my mind', 'second thoughts', 'maybe we should',
                'found another', 'different agent', 'thinking of waiting'
            ]
    ...
    async def generate_rescue_alert(self, deal_id: str, conversation_context: Optional[Dict] = None) -> Optional[RescueAlert]:
        ...
        return RescueAlert(
            ...
            escalation_timeline=f"within {urgency_expiry[risk_profile.urgency_level]} hours" if risk_profile.urgency_level != RescueUrgencyLevel.LOW else None,
    ...
    def _should_generate_alert(self, risk_profile: DealRiskProfile) -> bool:
        ...
        if (risk_profile.urgency_level == RescueUrgencyLevel.MEDIUM and
            risk_profile.deal_value > self.value_thresholds['high_alert']):
            return True
    ...
    async def _generate_rescue_recommendations(self, risk_profile: DealRiskProfile) -> List[RescueRecommendation]:
        ...
        for signal in primary_signals[:3]:  # Top 3 signals
            ...
            if signal.signal_type == ChurnSignalType.COMMUNICATION_SILENCE:
                recommendations.append(RescueRecommendation(
    ...
    async def _generate_alert_content(self, risk_profile: DealRiskProfile,
                                    recommendations: List[RescueRecommendation]) -> Dict[str, Any]:
        ...
        buyer_psychology = "Buyer may be experiencing decision fatigue and uncertainty. Focus on reassurance and clear next steps."
