...
class SemanticResponseCache:
    ...
    async def get_or_set(
        self, query_text: str, compute_function, context_tags: List[str] = None, user_id: str = None, ttl: int = None
    ...
    async def _check_semantic_match(self, query: SemanticQuery) -> Tuple[Optional[CachedResponse], float]:
        ...
        for cached_key, cached_embedding in self.similarity_index[-1000:]:  # Check last 1000 for performance
            similarity = self.embedding_service.calculate_similarity(query.embedding, cached_embedding)
    ...
    async def _cache_response(self, query: SemanticQuery, response: Any, ttl: int):
    ...
    async def get_similar_queries(self, query_text: str, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        for query_hash, cached_query in self.query_cache.items():
            if cached_query.embedding:
                similarity = self.embedding_service.calculate_similarity(query_embedding, cached_query.embedding)
...
def create_semantic_cache(
    ...
    **kwargs,
    ...
    return SemanticResponseCache(
        embedding_service=embedding_service, similarity_threshold=similarity_threshold, **kwargs
    )
