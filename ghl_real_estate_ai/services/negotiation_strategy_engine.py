...
class NegotiationStrategyEngine:
    ...
    async def generate_negotiation_strategy(
        ...
        offer_analysis = await self._calculate_optimal_offer(
            property_data, seller_psychology, market_leverage
        )
        
        # Determine primary negotiation tactic
        primary_tactic = self._determine_primary_tactic(
            seller_psychology, market_leverage, buyer_profile
        )
        
        # Generate strategic elements
        strategic_elements = await self._generate_strategic_elements(
            seller_psychology, market_leverage, primary_tactic
        )
    ...
    def _generate_price_focused_opening(self, psychology: SellerPsychologyProfile, offer_analysis: Dict) -> str:
        ...
        if psychology.flexibility_score > 70:
            ...
            return f"Present {percentage:.1f}% offer as market-driven, not negotiation tactic. Use third-party validation."
    ...
    def _generate_timeline_focused_opening(self, psychology: SellerPsychologyProfile) -> str:
        ...
        if psychology.urgency_level == UrgencyLevel.CRITICAL:
            return "Lead with ability to close quickly and accommodate seller timeline. Price as secondary consideration."
        else:
    ...
    def _generate_pricing_rationale(
        ...
        if market_adj < -0.03:
            rationale_parts.append(f"Market leverage ({leverage.overall_leverage_score}%) supports aggressive positioning")
        if leverage.price_positioning == "overpriced":
