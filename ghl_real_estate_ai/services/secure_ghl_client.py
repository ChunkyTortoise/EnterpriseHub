...
class SecureGHLClient:
    ...
    def __init__(self, api_key: str = None, location_id: str = None):
        ...
        if not self._validate_api_key():
            ...
            raise GHLSecurityError("Invalid GHL API key format - possible injection attempt", error_id=error_id)
    ...
    async def secure_get_conversations(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        if settings.test_mode or self.api_key == "dummy":
            logger.info("TEST_MODE: Returning mock conversation data", extra={"test_mode": True, "limit": limit})
            return [
                {
                    "id": f"test_conv_{i}",
        ...
        try:
            ...
            logger.error(
                ...
                extra={"error_id": error_id, "endpoint": endpoint, "timeout_seconds": 30, "error": str(e)},
            )
            raise GHLAPIError(f"GHL API request timed out after 30 seconds", error_id=error_id, status_code=408)
            ...
            if e.response.status_code == 401:
                ...
                raise GHLSecurityError("GHL API authentication failed - check API key", error_id=error_id)
            elif e.response.status_code == 403:
                logger.error(
                    f"GHL_AUTHORIZATION_FAILED: Insufficient permissions",
                    extra={"error_id": error_id, "status_code": 403, "endpoint": endpoint},
                )
                raise GHLSecurityError("GHL API access forbidden - insufficient permissions", error_id=error_id)
                ...
                raise GHLAPIError(
                    "GHL API rate limit exceeded - please retry later", error_id=error_id, status_code=429
                )
            else:
                logger.error(
            ...
            raise GHLAPIError(f"Unexpected error occurred: {type(e).__name__}: {e}", error_id=error_id)
    ...
    async def secure_send_message(
        ...
        if not self._validate_contact_id(contact_id):
            ...
            raise GHLSecurityError("Invalid contact ID format - possible injection attempt", error_id=error_id)
        ...
        try:
            ...
            raise GHLAPIError(f"Unexpected error sending message: {type(e).__name__}: {e}", error_id=error_id)
...
__all__ = ["SecureGHLClient", "GHLAPIError", "GHLSecurityError"]
