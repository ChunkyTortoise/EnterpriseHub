...
class StreamlitPerformanceOptimizer:
    ...
    def _generate_cache_key(self, name: str, args: tuple, kwargs: dict, key_func: Optional[Callable]) -> str:
        ...
        if key_func:
            ...
            key_data = {"args": args, "kwargs": kwargs, "session_id": self._get_session_id()}
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    future = asyncio.run_coroutine_threadsafe(self.cache_service.get(key), loop)
    ...
    def _should_load_component(self, name: str) -> bool:
        ...
        if interaction_key in session_state:
            ...
            if isinstance(last_interaction, datetime) and (datetime.now() - last_interaction).seconds < 60:
                return True
    ...
    def register_lazy_component(self, name: str, loader_func: Callable, priority: int = 1):
        ...
        self.lazy_components[name] = {"loader": loader_func, "priority": priority, "loaded": False, "load_time": None}
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        overall_stats = {
            ...
            "avg_render_time_ms": round(sum(m.avg_render_time_ms for m in all_metrics) / len(all_metrics), 2)
            if all_metrics
            else 0,
            "overall_cache_hit_rate": round(
                sum(m.cache_hits for m in all_metrics)
                / max(sum(m.cache_hits + m.cache_misses for m in all_metrics), 1)
                * 100,
                2,
            ),
