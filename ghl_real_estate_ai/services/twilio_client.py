...
class TwilioAPIException(Exception):
    ...
    def __init__(self, message: str, error_code: int = None, status_code: int = None, details: Dict = None):
...
class TwilioClient:
    ...
    def __init__(
        self, config: TwilioConfig = None, cache_service: CacheService = None, database_service: DatabaseService = None
    ):
    ...
    async def _ensure_session(self):
        ...
        if not self.session:
            ...
            self.session = aiohttp.ClientSession(
                timeout=timeout, auth=auth, connector=aiohttp.TCPConnector(limit=50, ttl_dns_cache=300)
            )
    ...
    async def validate_phone_number(self, phone_number: str) -> PhoneNumberInfo:
        ...
        try:
            ...
            lookup = self.sync_client.lookups.v1.phone_numbers(phone_number).fetch(type=["carrier"])
            ...
            if e.code == 20404:  # Invalid phone number
                return PhoneNumberInfo(phone_number=phone_number, country_code="", national_format="", valid=False)
            else:
    ...
    async def send_sms(
        self, to: str, message: str, lead_id: str = None, campaign_id: str = None, template_id: str = None
    ) -> SMSMessage:
        """
    ...
    async def send_templated_sms(
        ...
        return await self.send_sms(
            to=to, message=message, lead_id=lead_id, campaign_id=campaign_id, template_id=template_id
        )
    ...
    async def process_incoming_webhook(self, request: Request, webhook_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if any(keyword in body for keyword in opt_out_keywords):
                ...
                # Send confirmation
                try:
                    await self.send_sms(to=from_number, message="You've been unsubscribed. Reply START to resubscribe.")
                except Exception as e:
                ...
                try:
                    await self.send_sms(
                        to=from_number, message="Welcome back! You're now subscribed to receive messages."
                    )
                except Exception as e:
                    ...
                    await log_communication(
                        {
                            ...
                            "status": "received",
                            "metadata": {"twilio_sid": message_sid, "sender": from_number, "recipient": to_number},
                        }
                    )
    ...
    async def send_bulk_sms(self, recipients: List[Dict[str, Any]], batch_size: int = 10) -> List[Dict[str, Any]]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            phone_number = self.sync_client.incoming_phone_numbers.list(phone_number=self.config.phone_number)
    ...
    async def get_usage_stats(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_records = self.sync_client.usage.records.list(
                category="sms", start_date=start_date.date(), end_date=end_date.date()
            )
