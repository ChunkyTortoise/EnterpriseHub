...
class AdvancedWorkflowEngine:
            ...
            while current_step and execution_context['status'] == 'running':
                ...
                if self.state_manager:
                    await self.state_manager.save_execution_state(
                        execution_context['id'], execution_context
                    )
    ...
    async def _handle_conditional_split(
        self,
        step: WorkflowStep,
        execution_context: Dict[str, Any]
    ) -> bool:
    ...
    async def _handle_score_update(
        self,
        step: WorkflowStep,
        execution_context: Dict[str, Any]
    ) -> bool:
    ...
    async def pause_workflow(self, execution_id: str):
        ...
        if execution_id in self.active_executions:
            ...
            if self.state_manager:
                await self.state_manager.save_execution_state(
                    execution_id, self.active_executions[execution_id]
                )
    ...
    async def cleanup_completed_executions(self, max_age_hours: int = 24):
        ...
        for execution_id, context in self.active_executions.items():
            if (context['status'] in ['completed', 'failed', 'stopped'] and
                context.get('completed_at', datetime.now()) < cutoff_time):
