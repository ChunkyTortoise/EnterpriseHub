...
class VossScoringEngine:
    ...
    def calculate_frs(self, 
                      motivation: float, 
                      timeline: float, 
                      condition_acceptance: float, 
                      price_realism: float) -> float:
        """
    ...
    def get_jorge_qualification_pillars(self, nlp_insights: Dict[str, Any]) -> Dict[str, float]:
        ...
        motivation = 50.0
        if any(word in nlp_insights.get("text", "").lower() for word in ["must", "need", "relocating", "divorce", "probate"]):
            motivation = 90.0
