...
class SellerPsychologyAnalyzer:
    ...
    async def analyze_seller_psychology(
        ...
        await self.cache_service.set(
            cache_key, 
            psychology_profile.model_dump(), 
            ttl=14400
        )
        
        logger.info(f"Seller psychology analysis complete: {psychology_profile.motivation_type}, urgency: {psychology_profile.urgency_level}")
    ...
    async def _analyze_urgency_indicators(
        ...
        if listing_history.price_drops:
            recent_drops = [d for d in listing_history.price_drops 
    ...
    async def _analyze_negotiation_flexibility(
        ...
        return {
            ...
            "negotiation_openness": "high" if flexibility_score > 70 else "moderate" if flexibility_score > 40 else "low"
        }
    ...
    def _assess_engagement_quality(
        self, view_to_showing_ratio: float, showing_to_offer_ratio: float
    ) -> str:
