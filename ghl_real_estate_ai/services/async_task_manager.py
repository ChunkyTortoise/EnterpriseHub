...
class TaskWorker:
    ...
    async def _process_task(self, task: Task):
        ...
        try:
            ...
            result = await asyncio.wait_for(self._execute_task(task), timeout=task.timeout_seconds)
    ...
    async def _handle_task_failure(self, task: Task, error_msg: str, duration_ms: float):
        ...
        if current_retries < task.max_retries:
            ...
            logger.warning(
                f"Task {task.task_id} failed, scheduling retry {current_retries + 1}/{task.max_retries} in {delay_seconds}s"
            )
        else:
...
class AsyncTaskManager:
    ...
    async def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "workers": {"total": len(self.workers), "active": active_workers, "idle": idle_workers},
...
def background_task(priority: TaskPriority = TaskPriority.NORMAL, max_retries: int = 3):
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            task_id = await task_manager.submit_task(func, *args, priority=priority, max_retries=max_retries, **kwargs)
            return task_id
