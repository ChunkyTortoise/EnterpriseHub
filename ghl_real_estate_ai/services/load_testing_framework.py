...
class VirtualUser:
    ...
    def __init__(self, user_id: str, scenario: LoadTestScenario, session: aiohttp.ClientSession, results: LoadTestResults):
    ...
    async def _user_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if (self.scenario.duration_seconds and 
                    time.time() - scenario_start > self.scenario.duration_seconds):
                    break
...
class LoadTestRunner:
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                monitoring_task = asyncio.create_task(
                    self._monitor_test_progress(test_id, config.report_interval_s)
                )
    ...
    async def _monitor_test_progress(self, test_id: str, report_interval_s: int):
        ...
        while test_id in self.running_tests and self.running_tests[test_id].status == TestStatus.RUNNING:
            try:
                ...
                recent_requests = [
                    m for m in results.request_metrics
    ...
    async def _send_test_metrics(self, test_id: str, rps: float, errors: int, recent_requests: List[RequestMetrics]):
        ...
        try:
            ...
            self.monitoring_system.set_gauge('load_test_active_users', len(self.virtual_users.get(test_id, [])), {'test_id': test_id})
            
            # Record response time metrics
            if recent_requests:
                response_times = [r.response_time_ms for r in recent_requests]
                self.monitoring_system.record_histogram('load_test_response_time', statistics.mean(response_times), {'test_id': test_id})
    ...
    def _analyze_performance(self, results: LoadTestResults) -> Dict[str, Any]:
        ...
        analysis['overall_passed'] = all(
            analysis[key]['passed'] 
            for key in ['response_time_analysis', 'error_rate_analysis', 'throughput_analysis']
        )
    ...
    async def _export_detailed_metrics(self, test_id: str):
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'user_id', 'scenario', 'url', 'method', 
...
def create_api_load_test_config(base_url: str, max_users: int = 100) -> LoadTestConfig:
    ...
    dashboard_scenario = LoadTestScenario(
        ...
        requests=[
            ...
            {
                'url': f"{base_url}/api/analytics/charts/conversion",
                'method': 'GET'
            },
            {
                'url': f"{base_url}/api/analytics/charts/revenue",
                'method': 'GET'
            }
        ],
