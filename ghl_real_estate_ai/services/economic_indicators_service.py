...
from ghl_real_estate_ai.services.sentiment_types import DataSourceInterface, SentimentSignal, SentimentTriggerType
...
class EconomicIndicatorsService(DataSourceInterface):
    ...
    async def _analyze_mortgage_rates(
        self, indicators: List[EconomicIndicator], location: str
    ...
    async def _analyze_employment_trends(
        self, indicators: List[EconomicIndicator], location: str
    ) -> List[SentimentSignal]:
    ...
    async def _analyze_housing_costs(self, indicators: List[EconomicIndicator], location: str) -> List[SentimentSignal]:
        ...
        try:
            housing_indicators = [
                ...
                if any(term in ind.indicator_name.lower() for term in ["tax", "insurance", "housing", "home"])
            ]
