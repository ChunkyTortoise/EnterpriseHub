...
class LuxuryLeadScoringEngine:
    ...
    async def _analyze_net_worth_indicators(self, lead_data: Dict[str, Any]) -> NetWorthIndicators:
        ...
        if search_history:
            ...
            indicators.estimated_net_worth = max(
                indicators.estimated_net_worth, search_analysis.get("min_net_worth_estimate", 0)
            )
    ...
    async def _analyze_professional_indicators(self, profession: str, company: str) -> Dict[str, Any]:
        ...
        profession_analysis = {"income_estimates": {}, "investment_sophistication": 0, "net_worth_contribution": 0}
        ...
        for indicator, values in {**executive_indicators, **high_income_professions}.items():
            if indicator in combined_text:
                ...
                profession_analysis["investment_sophistication"] = max(
                    profession_analysis["investment_sophistication"], values["sophistication"]
                )
                profession_analysis["net_worth_contribution"] = max(
                    profession_analysis["net_worth_contribution"], values["income"] * values["net_worth_multiplier"]
                )
    ...
    async def _detect_luxury_buying_signals(self, lead_data: Dict[str, Any]) -> LuxuryBuyingSignal:
        ...
        recent_communications = [
            comm
            for comm in communication
            if datetime.fromisoformat(comm.get("date", datetime.now().isoformat()))
        ...
        recent_searches = [
            search
            for search in search_history
            if datetime.fromisoformat(search.get("date", datetime.now().isoformat()))
            > datetime.now() - timedelta(days=30)
        ]
        ...
        for comm in recent_communications:
            ...
            signal_scores[LuxuryBuyingSignal.IMMEDIATE] += (
                sum(1 for signal in immediate_signals if signal in content) * 3
            )
    ...
    def _score_referral_source(self, referral_source: str) -> float:
        ...
        medium_quality_sources = ["friend", "family", "colleague", "business partner", "realtor"]
    ...
    def _recommend_service_level(self, luxury_lead: LuxuryLead) -> str:
        ...
        if (
            score >= 80
            and wealth_tier in [WealthTier.ULTRA_UHNW, WealthTier.BILLIONAIRE]
            or lifestyle_expectations == "white_glove"
        ):
            return "white_glove"
