...
class EnhancedLeadScoringService:
    ...
    def __init__(self):
        ...
        self.urgency_indicators = [
            "asap", "urgent", "immediately", "right away", "need to move fast",
            "time sensitive", "deadline", "closing soon", "opportunity closing"
        ]

        self.authority_indicators = [
            "decision maker", "i decide", "my choice", "i'm in charge",
            "my business", "i own", "final say", "up to me"
        ]

        self.engagement_indicators = [
            "tell me more", "how does", "what about", "can you explain",
            "interested in", "sounds good", "like the idea"
        ]
    ...
    async def analyze_lead_comprehensive(
        ...
        try:
            ...
            source_analysis = await self._analyze_source_quality(
                source_info or {}, attribution_data or {}, location_id
            )
    ...
    def _analyze_behavioral_patterns(self, conversation_history: List[Dict[str, Any]]) -> BehavioralSignals:
        ...
        avg_word_count = sum(len(msg.get("content", "").split()) for msg in conversation_history) / len(conversation_history)
    ...
    def _calculate_attribution_confidence(
        self, source_info: Dict[str, Any], attribution_data: Dict[str, Any]
    ) -> float:
    ...
    def _detect_objection_patterns(self, text: str) -> List[str]:
        ...
        objection_indicators = [
            "too expensive", "can't afford", "need to think", "not ready",
            "talk to spouse", "timing isn't right", "economic uncertainty",
            "market might crash", "rates too high"
        ]

        return [obj for obj in objection_indicators if obj in text]

    def _prepare_ml_features(
        self, contact_id: str, conversation_history: List[Dict[str, Any]], base_intent_profile: Optional[LeadIntentProfile]
    ) -> Dict[str, Any]:
        """Prepare features for ML model prediction."""

        return {
            'conversation_length': len(conversation_history),
            'avg_message_length': sum(len(msg.get("content", "").split()) for msg in conversation_history) / max(1, len(conversation_history)),
            'question_count': sum(1 for msg in conversation_history if "?" in msg.get("content", "")),
            'frs_score': base_intent_profile.frs.total_score if base_intent_profile else 50,
            'pcs_score': base_intent_profile.pcs.total_score if base_intent_profile else 50,
            'urgency_mentions': len([msg for msg in conversation_history if any(urgent in msg.get("content", "").lower() for urgent in self.urgency_indicators)]),
            'authority_mentions': len([msg for msg in conversation_history if any(auth in msg.get("content", "").lower() for auth in self.authority_indicators)])
        }
    ...
    async def get_lead_score_history(
        ...
        return [
            ...
            for i, score in enumerate(historical_data.values()) if score
        ]
...
async def analyze_lead(
    ...
    **kwargs
    ...
    return await service.analyze_lead_comprehensive(
        contact_id, location_id, conversation_history, **kwargs
    )
