...
class ClientExpansionIntelligence:
    ...
    def __init__(self):
        ...
        logger.info(
            f"ClientExpansionIntelligence initialized - "
            f"Target: ${self.expansion_needed:,} MRR expansion"
        )

    async def generate_expansion_framework(
        self,
        ...
        try:
            ...
            for opp in top_20[:10]:  # Top 10 for detailed props
                prop = await self.generate_value_proposition(
                    opp.client_id,
                    opp
                )
                if prop:
            ...
            framework = {
                ...
                "success_metrics": {
                    ...
                    "target_clients_60_days": len([o for o in top_20 if o.readiness_level == ReadinessLevel.READY_SOON]),
                    "target_clients_90_days": len([o for o in top_20 if o.readiness_level == ReadinessLevel.NEEDS_NURTURE])
                }
            }
    ...
    async def assess_client_expansion(
        self,
        ...
        try:
            # Calculate readiness
            readiness_score, readiness_level = self.readiness_scorer.calculate_readiness_score(
                client_metrics
            )
            ...
            vehicles, fit_scores = self.vehicle_recommender.recommend_vehicles(
                client_metrics,
                expansion_tier
            )
            ...
            # Estimate timeline
            close_days = self.readiness_scorer.estimate_close_timeline(
                readiness_level,
                readiness_score
            )
    ...
    async def generate_value_proposition(
        ...
        try:
            ...
            # Map solutions
            solution_mapping = self._map_solutions_to_pain_points(
                pain_points,
                opportunity.recommended_vehicles
            )
            ...
            success_stories = await self._find_comparable_success_stories(
                client_metrics,
                opportunity
            )

            # List specific features
            features_needed = self._determine_features_needed(
                opportunity.recommended_vehicles
            )
            ...
            # Pricing strategy
            pricing, justification, discount = self._determine_pricing_strategy(
                opportunity,
                client_metrics
            )

            # Objection handling
            objections, responses = self._prepare_objection_handling(
                opportunity,
                client_metrics
            )
            ...
            next_steps = self._recommend_next_steps(
                opportunity.readiness_level,
                opportunity.strategic_priority
            )

            # Urgency factors
            urgency_factors = self._identify_urgency_factors(
                client_metrics,
                opportunity
            )
    ...
    def _determine_expansion_tier(
        ...
        if (
            ...
            metrics.satisfaction_score >= 4.5 and
            metrics.leads_processed_monthly >= 200 and
            metrics.usage_intensity >= 0.7 and
            metrics.executive_relationship_strength >= 8
        ):
        ...
        # Tier 2: 5x potential ($7.5-15K annual)
            ...
            metrics.satisfaction_score >= 4.0 and
            metrics.leads_processed_monthly >= 100 and
            metrics.usage_intensity >= 0.5
        ):
            return ExpansionTier.TIER_2_5X

        # Tier 3: 2x potential ($5-7.5K annual)
        elif (
            readiness_score >= 45 and
            metrics.satisfaction_score >= 3.5 and
    ...
    def _estimate_implementation_cost(
        self,
        vehicles: List[ExpansionVehicle]
    ) -> Decimal:
    ...
    async def _prioritize_opportunities(
        ...
        sorted_opps = sorted(
            opportunities,
            key=lambda o: (o.strategic_priority, float(o.expansion_potential)),
            reverse=True
        )
    ...
    def _calculate_revenue_projections(
        ...
        weighted_revenue = sum(
            float(o.expansion_potential) * o.success_probability
            for o in qualified
        )
        ...
        day_30 = sum(
            float(o.expansion_potential) * o.success_probability
            for o in qualified
            if o.estimated_close_days <= 30
        )

        day_60 = sum(
            float(o.expansion_potential) * o.success_probability
            for o in qualified
            if o.estimated_close_days <= 60
        )

        day_90 = sum(
            float(o.expansion_potential) * o.success_probability
            for o in qualified
            if o.estimated_close_days <= 90
        )
    ...
    def _map_solutions_to_pain_points(
        self,
    ...
    def _determine_features_needed(
        self,
        vehicles: List[ExpansionVehicle]
    ) -> List[str]:
