...
from ghl_real_estate_ai.services.ai_predictive_lead_scoring import PredictiveLeadScorer, LeadScore
    ...
    def cache_key(self) -> str:
        ...
        content = {
            ...
            "lead_data_hash": hashlib.md5(
                json.dumps(self.lead_data, sort_keys=True, default=str).encode()
            ).hexdigest(),
...
class RealTimeInferenceEngineV2:
    ...
    async def predict(self, request: InferenceRequest) -> InferenceResult:
        ...
        try:
            ...
            behavioral_signals, market_segment = await asyncio.gather(
                signal_task, market_task
            )

            # Step 3: Route to appropriate model (target: <50ms)
            score_result = await self._run_inference(
                request, behavioral_signals, market_segment
            )
            ...
            self.performance_monitor.record_inference(
                result.inference_time_ms, False, market_segment.value
            )
    ...
    async def _extract_behavioral_signals(self, request: InferenceRequest) -> Dict[str, float]:
        ...
        try:
            ...
            signals = await loop.run_in_executor(
                self.thread_pool,
                self.signal_processor.extract_signals,
                request.lead_data,
    ...
    async def _run_inference(
        ...
        try:
            ...
            model_result = await self.market_router.predict(
                request.lead_data,
                behavioral_signals,
                market_segment
            )
...
if __name__ == "__main__":
    async def demo():
        ...
        print(f"Performance: P95={metrics['p95_latency_ms']:.1f}ms, "
