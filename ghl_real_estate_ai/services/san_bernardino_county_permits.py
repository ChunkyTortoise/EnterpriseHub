...
class SanBernardinoCountyPermitService(DataSourceInterface):
    ...
    async def _analyze_permit_delays(self, permits: List[PermitRecord], location: str) -> List[SentimentSignal]:
        ...
        try:
            ...
            delayed_permits = [p for p in under_review if p.issue_date < datetime.now() - timedelta(days=60)]
    ...
    async def _analyze_infrastructure_projects(
        self, permits: List[PermitRecord], location: str
        ...
        try:
            ...
            infrastructure_permits = [
                p for p in permits if "infrastructure" in p.permit_type.lower() or "public works" in p.applicant.lower()
            ]
