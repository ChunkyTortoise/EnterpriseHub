...
class VoiceClaudeService:
    ...
    def __init__(self, market_id: str = "austin"):
        ...
        self.intent_patterns = {
            VoiceInteractionType.PROPERTY_INQUIRY: [
                "tell me about", "what's the price", "show me properties", "house details",
                "square footage", "bedrooms", "bathrooms", "neighborhood", "schools"
            ],
            VoiceInteractionType.LEAD_UPDATE: [
                "update lead", "change status", "add note", "mark as", "follow up",
                "interested", "not interested", "qualified", "ready to buy"
            ],
            VoiceInteractionType.SCHEDULE_SHOWING: [
                "schedule", "book showing", "set appointment", "tour", "visit",
                "available times", "calendar", "meet", "viewing"
            ],
            VoiceInteractionType.MARKET_QUESTION: [
                "market conditions", "home values", "trends", "appreciation",
                "neighborhood stats", "comparable sales", "market analysis"
            ],
            VoiceInteractionType.GENERAL_ASSISTANCE: [
                "help", "what can you do", "how do I", "explain", "assist"
            ]
        }
    ...
    async def process_voice_interaction(
        ...
        try:
            ...
            transcription_result = await self._transcribe_audio(
                ...
                voice_request.audio_format,
                voice_request.language,
                voice_request.sample_rate
            )
    ...
    async def _extract_entities_by_intent(
        ...
        try:
            if intent == VoiceInteractionType.PROPERTY_INQUIRY:
                ...
                # Location mentions
                location_keywords = [
                    "downtown", "south austin", "north austin", "west lake", "cedar park",
                    "lakeway", "bee cave", "dripping springs", "pflugerville", "round rock"
                ]
    ...
    def _build_voice_query(self, voice_command: VoiceCommand, session_context: VoiceSessionContext) -> str:
        ...
        if voice_command.command_type == VoiceInteractionType.PROPERTY_INQUIRY:
            context_instructions = "Focus on the most compelling property features and provide actionable insights for the buyer."
    ...
    async def _save_session_context(self, session_context: VoiceSessionContext):
        ...
        await cache.set(
            f"voice_session:{session_context.session_id}",
            session_data,
            ttl=7200
        )
