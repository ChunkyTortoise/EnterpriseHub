...
class WhatIfAnalyzer:
    ...
    async def analyze_scenario(self, 
                             ...
                             modified_features: Dict[str, float]) -> Dict[str, Any]:
        """
...
class SHAPExplainerService:
    ...
    async def explain_prediction(self,
                               ...
                               lead_id: str,
                               lead_name: str,
                               prediction_score: float) -> SHAPExplanation:
        """
        ...
        try:
            ...
            for feature, shap_val in shap_dict.items():
                ...
                if shap_val < -0.1:  # Negative impact threshold
                    risk_factors.append(f"{context.display_name}: {context.negative_explanation_template.format(value=feature_values[feature])}")
                elif shap_val > 0.1:  # Positive impact threshold
                    opportunities.append(f"{context.display_name}: {context.positive_explanation_template.format(value=feature_values[feature])}")
    ...
    def _prepare_waterfall_data(self, shap_dict: Dict[str, float], base_value: float, prediction: float) -> Dict[str, Any]:
    ...
    def _prepare_feature_impact_data(self, shap_dict: Dict[str, float], importance_dict: Dict[str, float]) -> Dict[str, Any]:
    ...
    async def perform_what_if_analysis(self,
                                     explanation: SHAPExplanation,
                                     modified_features: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            scenario_result = await self.what_if_analyzer.analyze_scenario(
...
async def get_shap_explanation(model, scaler, shap_explainer, feature_names, features, 
                             lead_id, lead_name, prediction_score) -> SHAPExplanation:
    """
    ...
    return await service.explain_prediction(
        model, scaler, shap_explainer, feature_names, 
        features, lead_id, lead_name, prediction_score
    )
