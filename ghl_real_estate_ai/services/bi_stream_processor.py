...
from ghl_real_estate_ai.services.event_publisher import get_event_publisher, EventPublisher
...
class BIStreamProcessor:
    ...
    async def start(self):
        ...
        for stream_name, stream_key in self.stream_names.items():
            task = asyncio.create_task(
                self._process_stream(stream_name, stream_key),
        ...
        for window_name, window_size in self.window_configs.items():
            task = asyncio.create_task(
                self._compute_sliding_window_aggregations(window_name, window_size),
                name=f"aggregate_{window_name}"
            )
            self.processing_tasks.append(task)
    ...
    async def _process_lead_interaction(self, event_data: Dict[str, Any], timestamp: datetime):
        ...
        if temperature:
            await self._increment_real_time_counter(
                f"leads:{temperature}:{location_id}",
                1,
                ttl=3600
            )

        # Store for OLAP processing (would integrate with database)
        await self._queue_olap_record('fact_lead_interactions', {
            'lead_id': lead_id,
            'timestamp': timestamp,
    ...
    async def _process_commission_event(self, event_data: Dict[str, Any], timestamp: datetime):
        ...
        await self._update_sliding_metric(
            f"pipeline:commission:{location_id}",
            commission_amount,
            timestamp
        )
        ...
        if commission_amount > 0:
            ...
            await self._update_sliding_metric(
                f"pipeline:jorge_revenue:{location_id}",
                jorge_commission,
                timestamp
            )

        # Queue OLAP record
        await self._queue_olap_record('fact_commission_events', {
            'lead_id': event_data.get('lead_id'),
            'deal_id': deal_id,
            'timestamp': timestamp,
    ...
    async def _process_bot_performance(self, event_data: Dict[str, Any], timestamp: datetime):
        ...
        await self._update_sliding_metric(
            f"bot:performance:{bot_type}:{location_id}",
            processing_time,
            timestamp
        )
        ...
        await self._increment_real_time_counter(
            f"bot:attempts:{bot_type}:{location_id}",
            1,
            ttl=3600
        )
        ...
        if success:
            await self._increment_real_time_counter(
                f"bot:successes:{bot_type}:{location_id}",
                1,
                ttl=3600
            )
        ...
        while self.is_running:
            try:
                ...
                await self.event_publisher.publish_performance_update(
                    'bi_stream_processor_events_processed',
                    self.metrics.events_processed
                )

                await self.event_publisher.publish_performance_update(
                    'bi_stream_processor_processing_time',
                    self.metrics.processing_time_ms,
                    'ms'
                )
...
class BIEnhancedEventPublisher(EventPublisher):
    ...
    async def publish_lead_update(self, lead_id: str, lead_data: Dict[str, Any], action: str = "updated", **kwargs):
        ...
        await self.publish_bi_event_to_stream('lead_interactions', {
            'lead_id': lead_id,
            'action': action,
            'lead_data': lead_data,
            **kwargs
    ...
    async def publish_commission_update(self, deal_id: str, commission_amount: float, pipeline_status: str, **kwargs):
        ...
        # Publish to BI stream for analytics
        await self.publish_bi_event_to_stream('commission_events', {
            'deal_id': deal_id,
            'commission_amount': commission_amount,
    ...
    async def publish_bot_status_update(self, bot_type: str, contact_id: str, status: str, **kwargs):
        ...
        # Publish to BI stream for analytics
        await self.publish_bi_event_to_stream('bot_performance', {
            'bot_type': bot_type,
    ...
    async def publish_system_health_update(self, component: str, status: str, response_time_ms: float, **kwargs):
        ...
        # Publish to BI stream for analytics
        await self.publish_bi_event_to_stream('system_health', {
            'component': component,
            'status': status,
