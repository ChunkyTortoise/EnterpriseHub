...
class TimingOptimizer:
    ...
    async def get_optimal_send_time(
        ...
        try:
            ...
            if lead_profile and lead_profile.confidence_score > 0.7:
                ...
                recommendation = await self._get_personalized_timing(
                    lead_profile, channel, base_time
                )
            else:
                # Use global patterns and heuristics
                recommendation = await self._get_heuristic_timing(
                    lead_id, channel, base_time, lead_data
                )
    ...
    async def _get_personalized_timing(
        self,
        lead_profile: LeadTimingProfile,
        channel: Channel,
        base_time: datetime
    ) -> TimingRecommendation:
        ...
        reasoning = f"Based on {len(self.engagement_history.get(lead_profile.lead_id, []))} " \
    ...
    async def _find_next_optimal_slot(
        ...
        for day_offset in range(max_days_ahead):
            ...
            for hour in preferred_hours:
                candidate_time = check_time.replace(
    ...
    async def _update_global_patterns(self, engagement_event: EngagementEvent):
        ...
        self.global_patterns['best_hours_by_channel'][channel_key] = [
            h for h in self.global_patterns['best_hours_by_channel'][channel_key]
    ...
    async def _adjust_for_lead_data(
        self,
    ...
    async def get_channel_performance_analysis(
        ...
        if lead_id and lead_id in self.engagement_history:
            events = [
                e for e in self.engagement_history[lead_id]
    ...
    async def _analyze_lead_patterns(
        self,
        ...
        return {
            ...
            'engagement_by_channel': {
                str(channel): len(events) for channel, events in by_channel.items()
            }
        }
    ...
    async def optimize_workflow_timing(
        ...
        for step in workflow_steps:
            ...
            # Get optimal timing
            timing_rec = await self.get_optimal_send_time(
                lead_data.get('id', ''), channel, current_time, lead_data
            )
    ...
    async def bulk_optimize_timing(
        ...
        for i in range(0, len(lead_ids), batch_size):
            ...
            tasks = [
                self.get_optimal_send_time(lead_id, channel, base_time)
