...
class IntelligentListingGenerator:
    ...
    async def generate_enhanced_listings(
        self, property_data: Dict[str, Any], tenant_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        market_context = self.neighborhood_engine.get_neighborhood_profile(
            address=property_data.get("address"), zip_code=property_data.get("zip_code")
        )

        # Step 2: Prepare variations
        variations = [
            {"tone": "emotional", "target": "owner-occupants", "focus": "family appeal, lifestyle, schools, community"},
            {
                "tone": "analytical",
                "target": "investors",
                "focus": "ROI, market positioning, price per sqft, rental potential",
            },
            {"tone": "premium", "target": "luxury", "focus": "architectural features, high-end finishes, exclusivity"},
        ]
    ...
    async def generate_seo_keywords(self, property_data: Dict[str, Any], tenant_id: Optional[str] = None) -> List[str]:
        ...
        try:
            response = await self.llm.agenerate(
                prompt=prompt, complexity=TaskComplexity.ROUTINE, tenant_id=tenant_id, max_tokens=100
            )
