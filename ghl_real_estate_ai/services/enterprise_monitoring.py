                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "title": "Triggered At",
                                "value": alert.triggered_at.isoformat() if alert.triggered_at else "Unknown",
                                "short": True,
                            },
                        ],
...
class EnterpriseMonitoringSystem:
    ...
    def record_metric(
        self, name: str, value: float, labels: Dict[str, str] = None, metric_type: MetricType = MetricType.GAUGE
    ):
        """Record a metric data point."""
        metric = Metric(name=name, value=value, timestamp=datetime.now(), labels=labels or {}, metric_type=metric_type)
    ...
    async def trigger_alert(self, alert_id: str, current_value: float, message: str = None):
        ...
        notification_message = (
            message or f"Alert {alert.name} triggered. Current value: {current_value}, Threshold: {alert.threshold}"
        )
    ...
    async def _perform_health_check(self, name: str) -> HealthStatus:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(health_check.check_func):
                result = await asyncio.wait_for(health_check.check_func(), timeout=health_check.timeout_seconds)
            else:
                loop = asyncio.get_event_loop()
                result = await asyncio.wait_for(
                    loop.run_in_executor(None, health_check.check_func), timeout=health_check.timeout_seconds
                )
    ...
    async def _send_notification(
        self, channel_name: str, channel: NotificationChannel, alert: Alert, message: str
    ) -> bool:
            ...
            try:
                ...
                for name, health_check in self.health_checks.items():
                    ...
                    if (
                        ...
                        or (datetime.now() - health_check.last_check).total_seconds() >= health_check.interval_seconds
                    ):
    ...
    async def _evaluate_alert_condition(self, alert: Alert):
        ...
        if "response_time" in condition and ">" in condition:
            ...
            errors = sum(
                m.value
                for m in self.metrics.get("api_errors", [])
                if (datetime.now() - m.timestamp).total_seconds() < 300
            )
            requests = sum(
                m.value
                for m in self.metrics.get("api_requests", [])
                if (datetime.now() - m.timestamp).total_seconds() < 300
            )
    ...
    async def _initialize_default_monitoring(self):
        ...
        self.register_health_check("cache_health", self._check_cache_health, interval_seconds=60)

        self.register_health_check("system_health", self._check_system_health, interval_seconds=30)
