...
class ClaudeJourneyOrchestrator:
    ...
    async def design_personalized_journey(self, lead_id: str, lead_profile: Dict[str, Any]) -> PersonalizedJourney:
        ...
        try:
            ...
            touchpoints = [
                JourneyTouchpoint(**tp) for tp in data.get("touchpoints", [])
            ]
    ...
    async def monitor_and_dispatch(self, lead_id: str, lead_name: str, current_state: Dict[str, Any]):
        ...
        if last_update:
            ...
            if hours_stalled >= 48:
                ...
                logger.info(f"Triggering autonomous Content Pulse for {lead_name} (stalled in {current_stage} for {hours_stalled:.1f}h)")
                ...
                if message:
                    await self.ghl_sync.ghl_client.send_message(
                        contact_id=lead_id,
                        message=message,
                        channel="SMS"
                    )
