...
class SendGridClient:
    ...
    async def _ensure_session(self):
        ...
        if not self.session:
            ...
            self.session = aiohttp.ClientSession(
                timeout=timeout, headers=headers, connector=aiohttp.TCPConnector(limit=100, ttl_dns_cache=300)
            )
    ...
    async def _load_suppressions(self):
        ...
        try:
            ...
            for suppression_type in [SuppressionType.UNSUBSCRIBE, SuppressionType.BOUNCE]:
                ...
                self._suppression_cache[suppression_type] = {entry.email for entry in suppressions}
    ...
    async def send_email(
        ...
        dynamic_template_data: Dict[str, Any] = None,
        lead_id: str = None,
                    ...
                    await log_communication(
                        {
                            ...
                            if isinstance(email_message.status, str)
                            else email_message.status.value,
                            "campaign_id": campaign_id,
                            "template_id": template_id,
                            "metadata": {
    ...
    async def add_to_suppression(
        self, emails: List[str], suppression_type: SuppressionType, reason: str = None
    ) -> bool:
        ...
        try:
            ...
            await self._make_request("POST", f"/suppression/{suppression_type.value}", data=suppression_data)
    ...
    async def remove_from_suppression(self, emails: List[str], suppression_type: SuppressionType) -> bool:
        ...
        try:
            for event in events:
                ...
                if event_type in ["unsubscribe", "spamreport", "bounce"]:
                    ...
                    await self.add_to_suppression([email], suppression_type, event.get("reason", f"auto_{event_type}"))
        ...
        try:
            ...
            return {
                ...
                "rates": {
                    ...
                    "bounce_rate": (total_stats["bounces"] / total_stats["requests"] * 100)
                ...
                "suppression_counts": {
                    suppression_type.value: len(emails) for suppression_type, emails in self._suppression_cache.items()
                },
            }
    ...
    async def send_bulk_emails(self, emails: List[Dict[str, Any]], batch_size: int = 100) -> List[Dict[str, Any]]:
