...
from ghl_real_estate_ai.services.dynamic_valuation_engine import (
    get_dynamic_valuation_engine, ValuationResult, ValuationConfidence
)
...
class PricingIntelligenceService:
    ...
    async def analyze_investment_opportunity(
        ...
        try:
            ...
            market_timing_data = await self._analyze_market_timing(
                property_data, actual_purchase_price
            )
            ...
            risk_assessment = await self._assess_investment_risks(
                property_data, investment_metrics, market_timing_data
            )
    ...
    async def generate_pricing_recommendation(
        ...
        try:
            ...
            recommended_price = self._calculate_strategic_price(
                valuation_result, pricing_strategy, market_metrics
            )

            # Generate price range analysis
            price_ranges = self._calculate_price_ranges(
                recommended_price, valuation_result, market_metrics
            )
            ...
            market_response = await self._estimate_market_response(
                recommended_price, property_data, market_metrics
            )
    ...
    def _generate_pricing_rationale(
        ...
        # Valuation support
        rationale.append(f"Based on comprehensive market analysis of {len(valuation_result.comparables)} comparable sales")
    ...
    def _generate_error_investment_opportunity(
        ...
        return InvestmentOpportunity(
            ...
            metrics=InvestmentMetrics(
                purchase_price=0, estimated_current_value=0,
                projected_1y_value=0, projected_5y_value=0,
                current_equity=0, equity_growth_1y=0, equity_growth_5y=0,
