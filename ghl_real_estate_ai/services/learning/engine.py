...
from .interfaces import (
    ILearningModel, FeatureVector, ModelPrediction, 
    ConfidenceLevel, ModelType, TrainingResult, LearningMode
)
...
class CollaborativeFilteringModel(ILearningModel):
    ...
    async def train(
        ...
        **kwargs
        ...
        for i, fv in enumerate(features):
            ...
            if uid not in user_ratings: user_ratings[uid] = []
            if pid not in item_ratings: item_ratings[pid] = []
    ...
    def get_feature_importance(self) -> Dict[str, float]: return {}
...
class ContentBasedModel(ILearningModel):
    ...
    async def load(self, path: str) -> bool:
        ...
        with open(path, "r") as f:
            ...
            self.user_profiles = {
                uid: np.array(vec) for uid, vec in data.get("user_profiles", {}).items()
            }
            self._is_trained = data.get("is_trained", False)
        return True
