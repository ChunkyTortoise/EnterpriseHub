...
class ModelRegistry:
    ...
    async def get_latest_model(self, model_type: str, status: str = 'production') -> Optional[Tuple[ModelMetadata, Any]]:
        """Get latest model of specific type and status"""
        
        matching_models = []
        for model_id, model_data in self.models.items():
            if (model_data.get('model_type') == model_type and 
                model_data.get('deployment_status') == status):
        ...
        try:
            ...
            if new_status == 'production':
                ...
                for mid, mdata in self.models.items():
                    if (mid != model_id and 
                        mdata.get('model_type') == model_type and 
                        mdata.get('deployment_status') == 'production'):
    ...
    async def get_model_lineage(self, model_id: str) -> List[ModelMetadata]:
        ...
        # Find children
        children = [mid for mid, mdata in self.models.items() 
                   if mdata.get('parent_model_id') == model_id]
...
class ModelMonitoring:
    ...
    async def record_prediction(self, model_id: str, prediction: Any, features: Dict[str, Any], 
                              actual_outcome: Optional[Any] = None, latency_ms: float = 0):
    ...
    async def get_performance_metrics(self, model_id: str, time_window: str = '24h') -> Optional[ModelPerformanceMetrics]:
        ...
        recent_data = [
            record for record in self.metrics_storage[model_id]
...
class ModelDeploymentManager:
    ...
    async def deploy_model(self, model_id: str, deployment_strategy: str = 'full', 
                          traffic_percentage: int = 100) -> bool:
    ...
    async def _deploy_canary(self, model_id: str, metadata: ModelMetadata, 
                           model_artifact: Any, traffic_percentage: int) -> bool:
...
class MLOpsPipeline:
    ...
    async def train_and_deploy_model(self, training_config: Dict[str, Any], 
        ...
        try:
            ...
            deployment_success = await self.deployment_manager.deploy_model(
                result['model_id'], deployment_strategy
            )
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        if deployment_stats['total_deployments'] > 0:
            deployment_success_rate = (deployment_stats['active_deployments'] + deployment_stats['canary_deployments']) / deployment_stats['total_deployments']
...
if __name__ == "__main__":
    ...
    async def test_mlops_pipeline():
        ...
        if job_status['status'] == 'completed':
            ...
            deployment_success = await mlops.deployment_manager.deploy_model(
                model_id, 'canary', traffic_percentage=25
            )
            ...
            if deployment_success:
                ...
                if metrics:
                    ...
                    print(f"   • Accuracy: {metrics.accuracy:.2f}" if metrics.accuracy else "   • Accuracy: Not available")
