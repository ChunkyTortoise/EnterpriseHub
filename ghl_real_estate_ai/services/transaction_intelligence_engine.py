...
class TransactionIntelligenceEngine:
    ...
    async def get_proactive_recommendations(
        self, transaction: RealEstateTransaction, milestones: List[TransactionMilestone], risk_threshold: float = 0.3
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            upcoming_milestones = [
                m for m in milestones if m.status == MilestoneStatus.NOT_STARTED and m.order_sequence <= 3
            ]
            ...
            logger.error(
                f"Failed to generate proactive recommendations for transaction {transaction.transaction_id}: {e}"
            )
            raise
    ...
    async def _predict_delay_probability(self, features: TransactionFeatures) -> float:
        ...
        if self.delay_classifier and self.feature_scaler:
            try:
                ...
                scaled_features = self.feature_scaler.transform(features.to_array())
                delay_probability = self.delay_classifier.predict_proba(scaled_features)[0][
                    1
    ...
    async def _analyze_risk_factors(
        self, transaction: RealEstateTransaction, milestones: List[TransactionMilestone], features: TransactionFeatures
    ) -> List[RiskFactor]:
    ...
    async def _generate_recommended_actions(
        self, risk_factors: List[RiskFactor], features: TransactionFeatures
    ) -> List[str]:
    ...
    async def _get_ai_insights(
        self, transaction: RealEstateTransaction, milestones: List[TransactionMilestone], risk_factors: List[RiskFactor]
    ) -> Dict[str, Any]:
