...
class InstantCMAGenerator:
    ...
    def _generate_insights(
        ...
        if asking_price > estimated_value * 1.05:
            insights.append(f"⚠️ Property is priced {((asking_price/estimated_value - 1) * 100):.1f}% above market value. Consider adjusting to ${estimated_value:,} for faster sale.")
        elif asking_price < estimated_value * 0.95:
            insights.append(f"💰 Property is priced {((1 - asking_price/estimated_value) * 100):.1f}% below market value. Great opportunity for buyers!")
        else:
        ...
        if market_analysis['market_type'] == "Seller's Market":
            insights.append(f"🔥 Strong seller's market with only {market_analysis['inventory_months']} months of inventory. Expect multiple offers.")
        elif market_analysis['market_type'] == "Buyer's Market":
            insights.append(f"🎯 Buyer's market with {market_analysis['inventory_months']} months of inventory. Pricing and marketing critical.")
        
        # Days on market insight
        avg_dom = market_analysis['avg_days_on_market']
        insights.append(f"📅 Properties are selling in an average of {avg_dom} days. Price competitively to meet this benchmark.")
