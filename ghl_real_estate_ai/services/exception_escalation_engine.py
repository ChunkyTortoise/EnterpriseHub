...
class ExceptionEscalationEngine:
    ...
    async def escalate_to_human(
        self, occurrence_id: str, escalation_tier: EscalationTier, reason: str, recommended_actions: List[str] = None
    ) -> str:
        """
    ...
    async def resolve_exception(
        self, occurrence_id: str, resolution_method: str, resolution_summary: str, resolved_by: str = "system"
    ) -> bool:
                ...
                try:
                    ...
                    if not success:
                        ...
                        if occurrence.resolution_attempts >= strategy.max_attempts:
                            await self._escalate_resolution_failure(
                                occurrence, strategy, f"Max attempts reached ({strategy.max_attempts})"
                            )
                            return
                        else:
                            # Retry the strategy
                    ...
                    await self._escalate_resolution_failure(
                        occurrence, strategy, f"Step execution failed: {str(step_error)}"
                    )
                    return
        ...
        try:
            ...
            if occurrence.confidence_score < 0.7:
                ...
                if ai_classification:
                    occurrence.confidence_score = max(
                        occurrence.confidence_score, ai_classification.get("confidence", 0.0)
                    )
    ...
    async def _generate_escalation_summary(self, occurrence: ExceptionOccurrence, reason: str) -> str:
        ...
        try:
            ...
            response = await self.llm_client.generate(prompt=prompt, max_tokens=250, temperature=0.6)

            return (
                response.content.strip()
                if response.content
                else f"Exception escalation: {occurrence.title}. Reason: {reason}"
            )
    ...
    async def _escalate_resolution_failure(
        self, occurrence: ExceptionOccurrence, strategy: ResolutionStrategy, reason: str
    ):
    ...
    def _update_auto_resolution_rate(self):
        ...
        if self.metrics["total_exceptions"] > 0:
            self.metrics["auto_resolution_rate"] = (
                self.metrics["auto_resolved_count"] / self.metrics["total_exceptions"]
            )
    ...
    def _update_average_resolution_time(self, new_time: float):
        ...
        if total_resolved > 1:
            self.metrics["average_resolution_time_minutes"] = (
                current_avg * (total_resolved - 1) + new_time
            ) / total_resolved
        else:
