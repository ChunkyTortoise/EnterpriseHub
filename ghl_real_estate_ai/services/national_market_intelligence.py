...
class NationalMarketIntelligence:
    ...
    def _load_existing_data(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded intelligence data: {len(self.market_metrics)} markets, "
    ...
    async def _calculate_opportunity_score(self, market_id: str, market_config: Dict[str, Any]) -> float:
        ...
        type_bonuses = {"tech_hub": 10, "tech_headquarters": 15, "finance_hub": 8, "corporate_hub": 12, "energy_hub": 6}
    ...
    def _estimate_tech_job_growth(self, market_config: Dict[str, Any]) -> float:
        ...
        tech_presence = sum(
            1 for tech_emp in tech_employers if any(tech_emp in emp_name for emp_name in employer_names)
        )
    ...
    def _calculate_qol_score(self, market_config: Dict[str, Any]) -> float:
        ...
        state_bonuses = {"CA": 8, "WA": 6, "CO": 10, "TX": 4, "AZ": 6}
        score += state_bonuses.get(state, 0)

        # Average neighborhood school ratings
        neighborhoods = market_config.get("neighborhoods", [])
        if neighborhoods:
            avg_school_rating = statistics.mean(n.get("school_rating", 7.0) for n in neighborhoods)
    ...
    async def _generate_competitive_analysis(self, market_id: str) -> None:
        ...
        comp_info = competitive_data.get(
            market_type,
            {
        ...
        analysis = CompetitiveAnalysis(
            ...
            market_share_distribution={comp: 100 / len(comp_info["competitors"]) for comp in comp_info["competitors"]},
    ...
    async def _generate_pricing_intelligence(self) -> None:
        ...
        for market_id, metrics in self.market_metrics.items():
            ratio = metrics.median_home_price / (
                ...
                .get("demographics", {})
                .get("median_household_income", 70000)
            )
        ...
        self.pricing_intelligence = PricingIntelligence(
            ...
            seasonal_variations={market_id: {"Q1": 0.95, "Q2": 1.05, "Q3": 1.0, "Q4": 1.0} for market_id in markets},
            roi_projections={
                market_id: metrics.opportunity_score / 10 for market_id, metrics in self.market_metrics.items()
            },
    ...
    async def predict_cross_market_demand(self, source_market: str, destination_market: str) -> Dict[str, Any]:
        ...
        pattern = next(
            (
                p
                for p in self.migration_patterns
                if p.source_market.lower() == source_market.lower()
                and p.destination_market.lower() == destination_market.lower()
            ),
            None,
        )
    ...
    async def get_national_market_overview(self) -> Dict[str, Any]:
        ...
        overview = {
            "summary": {
                ...
                "average_opportunity_score": statistics.mean(m.opportunity_score for m in self.market_metrics.values())
    ...
    def _analyze_market_trends(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "growth_markets": len(
                [
                    m
                    for m in self.market_metrics.values()
                    if m.market_trend in [MarketTrend.GROWING, MarketTrend.RAPIDLY_GROWING]
                ]
            ),
            "stable_markets": len([m for m in self.market_metrics.values() if m.market_trend == MarketTrend.STABLE]),
        }
    ...
    async def _get_pricing_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "price_forecasts": {
                market: forecast["1yr"] for market, forecast in self.pricing_intelligence.price_trend_forecast.items()
            },
    ...
    def _analyze_migration_flows(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_destinations": sorted(destination_volumes.items(), key=lambda x: x[1], reverse=True)[:5],
            "top_sources": sorted(source_volumes.items(), key=lambda x: x[1], reverse=True)[:5],
            "primary_industries": list(
                set(industry for pattern in self.migration_patterns for industry in pattern.primary_industries)
            ),
            "average_success_rate": statistics.mean(p.success_rate for p in self.migration_patterns)
    ...
    async def _get_competitive_overview(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "common_service_gaps": list(
                set(gap for analysis in self.competitive_analyses.values() for gap in analysis.service_gaps)
            ),
    ...
    async def _generate_expansion_recommendations(self) -> List[Dict[str, Any]]:
        ...
        top_markets = sorted(self.market_metrics.items(), key=lambda x: x[1].opportunity_score, reverse=True)[:3]
        ...
        for market_id, metrics in top_markets:
            ...
            recommendation = {
                "market_id": market_id,
                "market_name": metrics.market_name,
                "recommendation_type": "immediate_expansion"
                if metrics.opportunity_score > 80
                else "strategic_expansion",
                "opportunity_score": metrics.opportunity_score,
                "key_advantages": [
                ...
                "estimated_roi": self.pricing_intelligence.roi_projections.get(market_id, 0)
