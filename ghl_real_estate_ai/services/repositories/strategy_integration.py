...
class RepositoryPropertyMatcher:
    ...
    def __init__(self, property_data_service: PropertyDataService,
    ...
    async def score_properties_with_repository(self, lead_preferences: Dict[str, Any],
                                             context: Optional[Dict[str, Any]] = None,
                                             max_properties: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.strategy_matcher and STRATEGY_PATTERN_AVAILABLE:
                return await self._score_with_strategy_pattern(
                    properties, lead_preferences, context, max_properties
                )
            else:
                # Use basic scoring fallback
                return await self._score_with_basic_fallback(
                    properties, lead_preferences, max_properties
                )
    ...
    async def _score_with_strategy_pattern(self, properties: List[Dict[str, Any]],
    ...
    async def _score_with_basic_fallback(self, properties: List[Dict[str, Any]],
                                       lead_preferences: Dict[str, Any],
                                       max_properties: int) -> List[Dict[str, Any]]:
    ...
    def _calculate_basic_score(self, property_data: Dict[str, Any],
        ...
        amenity_matches = sum(1 for amenity in must_haves
                            if any(amenity.lower() in str(prop_amenity).lower()
    ...
    def _generate_basic_match_reasons(self, property_data: Dict[str, Any],
    ...
    def _format_property_for_ui(self, property_data: Dict[str, Any],
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            'repository_metrics': await self.data_service.get_performance_metrics(),
            'strategy_metrics': {}
        }
...
async def create_repository_property_matcher(
    ...
    if config_type == "demo":
        data_dir = data_sources_config.get("json_data_dir", "/Users/cave/enterprisehub/ghl_real_estate_ai/data/knowledge_base")
        ...
        data_service = await PropertyDataServiceFactory.create_production_service(
            mls_config, fallback_paths
        )
        ...
        data_service = await PropertyDataServiceFactory.create_hybrid_service(
            json_paths, mls_config, rag_config
        )
...
async def enhanced_generate_property_matches(lead_context: Dict,
                                           data_sources_config: Optional[Dict[str, Any]] = None) -> List[Dict]:
    """
