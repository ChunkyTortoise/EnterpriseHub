...
class PropertyQueryBuilder(IPropertyQueryBuilder):
    ...
    def add_custom_filter(
        self, field: str, operator: QueryOperator, value: Any, field_type: str = None
    ) -> "PropertyQueryBuilder":
        """Add custom filter"""
    ...
    def _validate_query(self):
        ...
        # Price validation
        if (
            self._query.min_price is not None
            and self._query.max_price is not None
            and self._query.min_price > self._query.max_price
        ):
            raise ValueError("min_price cannot be greater than max_price")

        # Bedroom validation
        if (
            self._query.min_bedrooms is not None
            and self._query.max_bedrooms is not None
            and self._query.min_bedrooms > self._query.max_bedrooms
        ):
            raise ValueError("min_bedrooms cannot be greater than max_bedrooms")

        # Bathroom validation
        if (
            self._query.min_bathrooms is not None
            and self._query.max_bathrooms is not None
            and self._query.min_bathrooms > self._query.max_bathrooms
        ):
            raise ValueError("min_bathrooms cannot be greater than max_bathrooms")

        # Square footage validation
        if (
            self._query.min_sqft is not None
            and self._query.max_sqft is not None
            and self._query.min_sqft > self._query.max_sqft
        ):
            raise ValueError("min_sqft cannot be greater than max_sqft")

        # Year built validation
        if (
            self._query.min_year_built is not None
            and self._query.max_year_built is not None
            and self._query.min_year_built > self._query.max_year_built
        ):
