...
class JorgeFollowUpScheduler:
    ...
    async def process_webhook_follow_up(
        self, contact_id: str, location_id: str, webhook_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            seller_data = await self._extract_seller_data_from_webhook(
                contact_id=contact_id, location_id=location_id, webhook_data=webhook_data
            )
            ...
            if not should_trigger:
                ...
                return {"status": "skipped", "reason": "Follow-up conditions not met", "contact_id": contact_id}
            ...
            try:
                ...
                await self.cache.predictive_prefetch(
                    contact_id=contact_id, conversation_stage=stage, rag_engine_ref=self.rag_engine
                )
            ...
            try:
                ...
                variant = await self.ab_testing.allocate_participant(
                    test_id=test_id, participant_id=contact_id, context=seller_data
                )
                if variant:
            ...
            if follow_up_result.get("next_follow_up"):
                await self._schedule_next_follow_up(
                    contact_id=contact_id, location_id=location_id, next_follow_up=follow_up_result["next_follow_up"]
                )
            ...
            return {"status": "error", "error": str(e), "contact_id": contact_id}

    async def process_scheduled_follow_ups(
        self, batch_size: int = 50, location_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            due_contacts = await self._get_contacts_due_for_follow_up(batch_size=batch_size, location_id=location_id)

            if not due_contacts:
                return {"status": "success", "processed": 0, "message": "No contacts due for follow-up"}
    ...
    async def create_ghl_workflow_triggers(self, location_id: str) -> Dict[str, Any]:
        """
                ...
                for workflow in workflows:
                    try:
                        ...
                        created_workflows.append(
                            {"name": workflow["name"], "status": "template_created", "requires_manual_setup": True}
                        )
    ...
    async def _get_contacts_due_for_follow_up(
        self, batch_size: int, location_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_seller_data_for_contact(self, contact_id: str, location_id: str) -> Dict[str, Any]:
    ...
    async def _execute_follow_up_actions(
        self, contact_id: str, location_id: str, actions: List[Dict[str, Any]], message: str
    ) -> None:
    ...
    async def _schedule_next_follow_up(self, contact_id: str, location_id: str, next_follow_up: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Next follow-up scheduled for {contact_id} - Date: {scheduled_date}, Type: {follow_up_type}"
            )

            # Could add a tag to trigger GHL workflow at the right time
            if self.ghl_client:
                await self.ghl_client.apply_actions(
                    contact_id, [{"type": "add_tag", "tag": f"Next-FollowUp-{scheduled_date.split('T')[0]}"}]
                )
    ...
    async def get_follow_up_statistics(
        self, location_id: Optional[str] = None, date_range_days: int = 30
    ) -> Dict[str, Any]:
