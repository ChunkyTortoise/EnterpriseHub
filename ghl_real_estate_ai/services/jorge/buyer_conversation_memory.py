...
class BuyerConversationMemory:
    ...
    async def save_state(self, contact_id: str, state: Dict[str, Any], ttl_override: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            if should_compress:
                ...
                storage_data = {"compressed": False, "data": json_data}
    ...
    async def load_state(self, contact_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if state_version != self.STATE_VERSION:
                logger.warning(f"State version mismatch for {contact_id}: {state_version} != {self.STATE_VERSION}")
    ...
    def _prepare_state_for_storage(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(conversation_history) > self.max_history_messages:
            ...
            logger.debug(f"Trimmed conversation history to {self.max_history_messages} messages")
