...
DEFAULT_PLAYBOOK = SwarmPlaybook(
    ...
    steps=[
        ...
        SwarmStep(channel="Email", delay_seconds=60, message_template="hot_lead_followup", action_name="Data Delivery"),
        SwarmStep(
            channel="Voice", delay_seconds=180, message_template="ai_voice_script_v1", action_name="Qualification Call"
        ),
    ],
)
...
class MultiChannelSwarmOrchestrator:
    ...
    async def _execute_playbook(
        self, lead_id: str, lead_data: Dict[str, Any], playbook: SwarmPlaybook, tenant_id: Optional[str]
    ):
    ...
    async def _execute_step(
        ...
        if step.channel == "SMS":
            ...
            await self.voice_client.initiate_outbound_call(phone_number=lead_data.get("phone"), lead_data=lead_data)
            await self._record_activity(
                "VoiceBot", f"Started Voice Call: {step.action_name} with {lead_name}", "Warning"
            )
