...
class RealEstateServiceComposer:
    ...
    async def create_development_environment(self) -> Dict[str, Any]:
        ...
        config = {
            "repositories": {
                "JsonRepository": {
                    ...
                    "enable_caching": True,
                    "cache_config": {"backend": "memory", "max_size": 500},
                }
            },
            "strategies": {
                "scoring": {
                    "strategies": {"basic": {"enabled": True}, "enhanced": {"enabled": True, "weight_budget": 0.4}}
                },
                "property_matcher": {
                ...
                "integration": {"strategy_name": "enhanced", "enable_monitoring": True},
            },
            "caching": {"memory": {"enabled": True, "max_size": 1000}},
        ...
        self.optimized_container = OptimizedDIContainer(self.container, enable_caching=True, enable_monitoring=True)

        # Configure performance optimizations
        RealEstatePerformanceOptimizer.configure_container_for_real_estate(self.optimized_container)
    ...
    async def create_production_environment(self, mls_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config = {
            ...
            "strategies": {
                ...
                "property_matcher": {"strategy_name": "enhanced", "enable_monitoring": True, "enable_caching": True},
            },
            "caching": {
                "redis": {
                    "enabled": True,
            ...
            "configuration": {"environment": "production", "debug": False},
        }
        ...
        async with orchestrator.container.create_scope_async() as scope_id:
            ...
            if "repository_matcher" in context:
                ...
                scored_properties = await repository_matcher.score_properties_with_repository(
                    lead_preferences=lead_preferences, context=lead_context, max_properties=20
                )
            else:
    ...
    async def demonstrate_configuration_driven_setup(self, config_file_path: str) -> Dict[str, Any]:
        ...
        orchestrator_config = {"repositories": {}, "strategies": {}, "monitoring": {"enabled": True}}
    ...
    async def demonstrate_testing_setup(self) -> Dict[str, Any]:
        ...
        from .testing_support import MockServiceFactory, TestDIContainer
        ...
        test_container.register_mock(
            "IPropertyRepository", MockServiceFactory.create_mock_repository(), name="MockRepository"
        )

        test_container.register_mock(
            "PropertyDataService", MockServiceFactory.create_mock_data_service(), name="MockDataService"
        )

        test_container.register_mock(
            "ScoringFactory", MockServiceFactory.create_mock_scoring_factory(), name="MockScoringFactory"
        )

        test_container.register_mock(
            "MemoryCacheBackend", MockServiceFactory.create_mock_cache_backend(), name="MockCache"
        )
    ...
    async def demonstrate_performance_monitoring(self, environment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not optimized_container:
            ...
            optimized_container = OptimizedDIContainer(
                environment["container"], enable_caching=True, enable_monitoring=True
            )
        ...
        benchmark_results = await RealEstatePerformanceOptimizer.benchmark_property_operations(
            optimized_container, iterations=25
        )
...
async def run_development_example():
    ...
    try:
        ...
        print(
            f"   - Top match score: {search_results['top_matches'][0]['match_score'] if search_results['top_matches'] else 'N/A'}"
        )

        return {"environment": dev_env, "search_results": search_results, "performance_results": perf_results}
