...
class DatabaseService:
    ...
    def __init__(self, database_url: str = None):
        ...
        self.connection_manager = DatabaseConnectionManager(
            connection_string=self.database_url, min_connections=5, max_connections=20, pool_name="service6_lead_engine"
        )
        ...
        async with self.connection_manager.get_connection() as conn:
            ...
            for index_sql in critical_indexes:
                try:
                    ...
                    logger.debug(
                        f"✅ Critical index created: {index_sql.split(' ON ')[1].split('(')[0] if ' ON ' in index_sql else 'index'}"
                    )
                except Exception as e:
                    # Only catch "already exists" errors, raise everything else
                    if "already exists" in str(e).lower():
                        logger.debug(
                            f"ℹ️ Critical index already exists: {index_sql.split(' ON ')[1].split('(')[0] if ' ON ' in index_sql else 'index'}"
                        )
                    else:
    ...
    async def update_communication_status(self, comm_id: str, status: str, delivered_at: datetime = None) -> bool:
    ...
    async def create_nurture_campaign(self, campaign_data: Dict[str, Any], created_by: str = None) -> str:
    ...
    async def enroll_lead_in_campaign(self, lead_id: str, campaign_id: str, next_action_at: datetime = None) -> str:
    ...
    async def get_lead_response_data(self, lead_id: str) -> Dict[str, Any]:
        ...
        async with self.get_connection() as conn:
            ...
            negative_sentiment = any(
                row.get("sentiment_score") is not None and row.get("sentiment_score", 0) < -0.3 for row in response_list
            )
            ...
            valid_sentiments = [
                r.get("sentiment_score", 0) for r in response_list if r.get("sentiment_score") is not None
            ]
        ...
        async with self.get_connection() as conn:
            ...
            for comm in recent_comms:
                ...
                if comm_dict["channel"] == "email":
                    ...
                    sms_responses.append({"timestamp": comm_dict["sent_at"], "status": comm_dict["status"]})
    ...
    async def log_pricing_tool_use(
        self, lead_id: str, property_details: Dict[str, Any], valuation_result: Optional[Dict[str, Any]] = None
    ) -> str:
        ...
        async with self.transaction() as conn:
            ...
            """,
                ...
                json.dumps(valuation_result) if valuation_result else None,
            )
            return usage_id
    ...
    async def execute_optimized_query(self, sql: str, *args, timeout: Optional[float] = None) -> Any:
        ...
        return await self.connection_manager.execute_query(sql, *args, timeout=timeout, record_execution=True)
    ...
    async def get_connection_health(self) -> Dict[str, Any]:
        ...
        return {"enterprise_connection_manager": health, "database_specific": await self.health_check()}
...
async def log_pricing_tool_use(
    lead_id: str, property_details: Dict[str, Any], valuation_result: Optional[Dict[str, Any]] = None
) -> str:
