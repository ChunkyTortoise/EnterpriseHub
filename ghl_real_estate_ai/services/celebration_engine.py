...
from ghl_real_estate_ai.services.transaction_event_bus import TransactionEventBus, TransactionEvent, EventType
from ghl_real_estate_ai.database.transaction_schema import (
    RealEstateTransaction, TransactionMilestone, TransactionCelebration,
...
class CelebrationEngine:
    ...
    async def trigger_milestone_celebration(
        ...
        try:
            ...
            logger.info(f"Celebration triggered for {milestone.milestone_name} in transaction {transaction.transaction_id}")
    ...
    async def trigger_progress_celebration(
        ...
        try:
            ...
            logger.info(f"Progress celebration triggered at {triggered_threshold}% for transaction {transaction.transaction_id}")
    ...
    async def trigger_closing_countdown_celebration(
        ...
        try:
            ...
            celebration_type = CelebrationType.FIREWORKS_ANIMATION if days_to_closing <= 3 else CelebrationType.SUCCESS_BANNER
    ...
    async def _personalize_celebration_content(
        ...
        try:
            ...
            return CelebrationContent(
                ...
                duration_seconds=4 if milestone.milestone_type in [MilestoneType.LOAN_APPROVAL, MilestoneType.CLOSING_COMPLETED] else 3
            )
    ...
    async def _execute_celebration(
        self,
        transaction: RealEstateTransaction,
        milestone: Optional[TransactionMilestone],
        config: CelebrationConfig
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_celebration_email_html(
        self,
        transaction: RealEstateTransaction,
        config: CelebrationConfig
    ) -> str:
