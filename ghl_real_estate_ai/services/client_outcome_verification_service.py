...
class ClientOutcomeVerificationService:
    ...
    async def verify_transaction_outcome(
        ...
        try:
            ...
            verification_data = await self._gather_verification_data(
                transaction_id, verification_sources
            )
            
            # Verify individual components
            price_verification = await self._verify_transaction_price(
                claimed_data, verification_data
            )
            
            timeline_verification = await self._verify_transaction_timeline(
                claimed_data, verification_data
            )
            
            commission_verification = await self._verify_commission_data(
                claimed_data, verification_data
            )
            ...
            overall_verification_level, overall_accuracy = await self._calculate_overall_verification(
                [price_verification, timeline_verification, commission_verification],
            ...
            transaction_verification = TransactionVerification(
                ...
                listed_price=Decimal(str(verification_data.get("verified_listed_price", claimed_data.get("listed_price", 0)))),
                sold_price=Decimal(str(verification_data.get("verified_sold_price", claimed_data.get("sold_price", 0)))),
                ...
                commission_paid=Decimal(str(verification_data.get("verified_commission", claimed_data.get("commission_paid", 0)))),
    ...
    async def verify_performance_metric(
        ...
        try:
            ...
            # Gather evidence
            evidence = await self._gather_metric_evidence(
                metric_type, transactions, evidence_data
            )
            
            # Calculate accuracy
            accuracy_percentage = await self._calculate_metric_accuracy(
                claimed_value, verified_value, metric_type
            )
            ...
            anomalies = await self._detect_metric_anomalies(
                metric_type, claimed_value, verified_value, transactions
            )
    ...
    async def get_verification_report(
        ...
        try:
            ...
            silver_verifications = sum(1 for v in verifications if v.overall_verification_level == VerificationLevel.SILVER)
            bronze_verifications = sum(1 for v in verifications if v.overall_verification_level == VerificationLevel.BRONZE)
            
            overall_verification_rate = (gold_verifications + silver_verifications + bronze_verifications) / total_verifications if total_verifications > 0 else 0
            ...
            report = {
                ...
                "verification_summary": {
                    ...
                    "verification_levels": {
                        ...
                        "pending": total_verifications - (gold_verifications + silver_verifications + bronze_verifications)
                    }
                },
    ...
    async def detect_verification_anomalies(
        self,
        agent_id: str,
        ...
        try:
            ...
            gold_rate = verification_levels.count(VerificationLevel.GOLD) / len(verification_levels) if verification_levels else 0
            ...
            source_usage = {}
            for verification in verifications:
                for outcome in [verification.price_verification, verification.timeline_verification, verification.commission_verification]:
    ...
    async def _verify_transaction_timeline(
        ...
        if "mls_listing_date" in verification_data and "mls_sold_date" in verification_data:
            evidence.append(VerificationEvidence(
                source=VerificationSource.MLS_DATA,
                data_hash=hashlib.md5(f"{verification_data['mls_listing_date']}_{verification_data['mls_sold_date']}".encode()).hexdigest(),
                timestamp=datetime.now(),
                confidence_score=0.95
            ))
    ...
    async def _get_transaction_records(self, transaction_id: str) -> Optional[Dict]:
        ...
        return {
            "verified_days_on_market": 19,
            "verified_commission": 13350
        }

    async def _get_county_records(self, transaction_id: str) -> Optional[Dict]:
        """Get county records data"""
        return {
            "county_sold_price": 445000,
            "county_recording_date": datetime(2024, 2, 5)
        }
        ...
        return [
            {
                ...
                "verified": True,
                "source": "post_transaction_survey"
            }
        ]
    ...
    async def _identify_satisfaction_discrepancies(
        ...
        if abs(claimed_rating - average_verified_rating) > self.variance_thresholds["rating_variance"]:
            discrepancies.append(f"Claimed rating differs from verified average by {abs(claimed_rating - average_verified_rating):.1f}")
    ...
    async def _get_agent_transactions(
        self,
        agent_id: str,
        start_date: datetime,
        end_date: datetime
    ...
    async def _calculate_metric_accuracy(
        self,
        claimed_value: float,
    ...
    async def _detect_metric_anomalies(
        ...
        variance = abs(claimed_value - verified_value) / max(abs(claimed_value), abs(verified_value)) if max(abs(claimed_value), abs(verified_value)) > 0 else 0
    ...
    async def _generate_verification_summary(
        ...
        return {
            ...
            "satisfaction_confidence": satisfaction_verification.confidence_score * 100 if satisfaction_verification else None,
            "evidence_sources": len(set([e.source for e in price_verification.evidence + timeline_verification.evidence + commission_verification.evidence])),
            "total_anomalies": len(price_verification.anomalies_detected) + len(timeline_verification.anomalies_detected) + len(commission_verification.anomalies_detected)
        }
    ...
    async def _calculate_data_quality_score(
        self,
        agent_id: str,
        verifications: List[TransactionVerification]
    ) -> float:
        ...
        total_evidence_sources = sum(
            len(set([e.source for e in v.price_verification.evidence + v.timeline_verification.evidence + v.commission_verification.evidence]))
    ...
    async def _generate_verification_recommendations(
        ...
        gold_rate = sum(1 for v in verifications if v.overall_verification_level == VerificationLevel.GOLD) / len(verifications)
        if gold_rate < 0.7:
