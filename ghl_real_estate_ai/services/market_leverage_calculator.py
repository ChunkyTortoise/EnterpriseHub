...
class MarketLeverageCalculator:
    ...
    async def calculate_market_leverage(
        ...
        await self.cache_service.set(cache_key, market_leverage.model_dump(), ttl=3600)
        ...
        try:
            ...
            for range_name, (min_price, max_price) in price_ranges.items():
                ...
                inventory_data = await self.market_service.get_inventory_analysis(
                    zip_code=zip_code, property_type=property_type, min_price=int(min_price), max_price=int(max_price)
                )
    ...
    async def _analyze_competitive_pressure(self, property_id: str, property_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_sales = len([p for p in similar_properties if p.get("status") == "pending"])

            price_reductions = len([p for p in similar_properties if p.get("price_drops", 0) > 0])

            avg_days_on_market = sum(p.get("days_on_market", 30) for p in similar_properties) / max(
                len(similar_properties), 1
            )
    ...
    async def _analyze_property_positioning(
        self, property_data: Dict[str, Any], listing_history: ListingHistory
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            comps_data = await self.market_service.get_comparable_sales(
                ...
                sqft_range=(property_data.get("sqft", 2000) * 0.8, property_data.get("sqft", 2000) * 1.2),
                days_back=90,
            )
