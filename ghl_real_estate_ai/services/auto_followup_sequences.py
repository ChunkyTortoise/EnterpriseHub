...
class AutoFollowUpSequences:
    ...
    def _personalize_content(
        ...
        for key, value in personalized.items():
            if isinstance(value, str):
                value = value.replace("{{first_name}}", contact_data.get("first_name", ""))
    ...
    def _calculate_predicted_impact(
        ...
        return {
            ...
            "confidence": sum(rec.get("confidence", 0) for rec in recommendations) / len(recommendations) if recommendations else 0
        }
