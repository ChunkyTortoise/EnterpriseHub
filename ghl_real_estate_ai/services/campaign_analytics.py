...
class CampaignTracker:
    ...
    def update_funnel_stage(self, campaign_id: str, stage: str, count: int = 1):
        """
    ...
    def compare_campaigns(self, campaign_ids: List[str]) -> Dict[str, Any]:
        ...
        rankings = {
            ...
            "cost_per_lead": sorted(
                campaigns_data,
                key=lambda x: x["performance"]["cost_per_lead"] or float("inf"),
            ),
            "leads_generated": sorted(
                campaigns_data,
                key=lambda x: x["performance"]["leads_generated"],
                reverse=True,
            ),
        }
        ...
        return {
            ...
            "rankings": {
                "by_roi": [
                    {
                        "name": c["campaign_info"]["name"],
                        "value": c["performance"]["roi"],
                    }
                    for c in rankings["roi"][:5]
                ],
                "by_conversion_rate": [
                    {
                        "name": c["campaign_info"]["name"],
                        "value": c["performance"]["conversion_rate"],
                    }
                    for c in rankings["conversion_rate"][:5]
                ],
                "by_cost_efficiency": [
                    {
                        "name": c["campaign_info"]["name"],
                        "value": c["performance"]["cost_per_lead"],
                    }
                    for c in rankings["cost_per_lead"][:5]
                ],
            },
