...
class ErrorMonitoringService:
    ...
    async def get_error_metrics(self, timeframe_minutes: int = 60) -> Dict[str, Any]:
        """Get error metrics for the specified timeframe."""

        cutoff_time = time.time() - (timeframe_minutes * 60)
        relevant_errors = [error for error in self.recent_errors if error.timestamp >= cutoff_time]
        ...
        # Critical errors
        critical_errors = sum(
            1 for error in relevant_errors if error.category in [ErrorCategory.SYSTEM, ErrorCategory.DATABASE]
        )
    ...
    async def get_error_trends(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        for i in range(hours):
            ...
            hour_errors = [error for error in self.recent_errors if hour_start <= error.timestamp < hour_end]
    ...
    async def get_error_patterns(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        patterns = sorted(self.error_patterns.values(), key=lambda p: p.occurrences, reverse=True)[:limit]
    ...
    async def get_top_errors(self, timeframe_minutes: int = 60, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for error in relevant_errors:
            ...
            error_counts[key]["latest_occurrence"] = max(error_counts[key]["latest_occurrence"], error.timestamp)
    ...
    async def mark_error_resolved(self, error_id: str, resolution_time: Optional[float] = None):
        ...
        logger.info(
            f"Error marked as resolved: {error_id}", extra={"error_id": error_id, "resolution_time": resolution_time}
        )
    ...
    async def _send_alert(
        self, severity: AlertSeverity, title: str, message: str, context: Optional[Dict[str, Any]] = None
    ):
        ...
        logger.warning(f"Error Alert [{severity.value.upper()}]: {title}", extra=alert_data)
    ...
    async def _pattern_analyzer(self):
        ...
        while True:
            try:
                ...
                self.error_patterns = {k: v for k, v in self.error_patterns.items() if v.last_seen > cutoff_time}
    ...
    async def _calculate_health_status(self) -> Dict[str, Any]:
        ...
        return {"status": status, "message": message, "metrics": recent_metrics}
