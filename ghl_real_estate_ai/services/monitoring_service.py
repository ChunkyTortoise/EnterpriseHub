...
class MonitoringService:
    ...
    def register_health_check(self, health_check: HealthCheck):
        ...
        self.circuit_breakers[health_check.service_name] = CircuitBreaker(
            failure_threshold=health_check.failure_threshold, recovery_timeout=60
        )
    ...
    def register_ghl_health_check(self, ghl_client):
        ...
        self.register_health_check(
            HealthCheck(
                service_name="ghl", service_type=ServiceType.GHL, check_function=check_ghl, timeout=15, interval=60
            )
        )
    ...
    def register_graph_health_check(self):
        ...
        async def check_graph():
            try:
                ...
                return {"status": "healthy", "response_time_ms": response_time, "module": "FalkorDB/RedisGraph"}
                ...
                return {
                    ...
                    "remediation": "Ensure RedisGraph/FalkorDB module is loaded in Redis server"
                    if status == "degraded"
                    else "Check Redis connectivity",
                }
    ...
    def register_redis_health_check(self, cache_service):
        ...
        self.register_health_check(
            HealthCheck(
                service_name="redis", service_type=ServiceType.REDIS, check_function=check_redis, timeout=5, interval=30
            )
        )
    ...
    async def run_health_check(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.performance_tracker.record(f"{service_name}_response_time", response_time, {"service": service_name})
            ...
            # Record error
            self.performance_tracker.record(f"{service_name}_errors", 1, {"service": service_name, "error": str(e)})
    ...
    def record_operation_time(self, operation_name: str, duration_ms: float, tags: Dict[str, str] = None):
        """Record operation execution time."""
        self.performance_tracker.record(f"{operation_name}_duration", duration_ms, tags or {})

    def record_api_call(self, api_name: str, duration_ms: float, success: bool, status_code: int = None):
    ...
    def record_database_query(self, query_type: str, duration_ms: float, success: bool, rows_affected: int = None):
    ...
    async def create_alert(
        self, service_name: str, severity: AlertSeverity, title: str, message: str, metadata: Dict[str, Any] = None
    ) -> str:
        """Create a new alert."""
    ...
    async def resolve_alert(self, alert_id: str, resolved_by: str = None) -> bool:
        ...
        # Update cache
        await self.cache_service.set(f"alert:{alert_id}", alert.dict(), ttl=86400)
    ...
    async def get_monitoring_dashboard_data(self) -> Dict[str, Any]:
        ...
        system_metrics = {
            ...
            "healthy_services": len(
                [r for r in health_results.get("services", {}).values() if r.get("status") == "healthy"]
            ),
