...
class MarketMetrics:
    ...
    def get_market_condition(self) -> MarketCondition:
        ...
        if (self.inventory_level < 2.0 and
            self.price_trend > 5.0 and
            self.days_on_market < 20):
            return MarketCondition.SELLERS_MARKET

        # Buyers market indicators
        if (self.inventory_level > 6.0 and
            self.price_trend < -2.0 and
            self.days_on_market > 60):
...
class WeightConfigService:
    ...
    def _calculate_feature_importance(self, performance_data: List[Dict]) -> Dict[str, float]:
        ...
        features = ['engagement_score', 'response_time', 'page_views', 'budget_match',
                   'timeline_urgency', 'property_matches', 'communication_quality']
                ...
                for key in keys:
                    ...
                    if test_data:
                        ...
                        if (test_config.is_active and
                            (test_config.end_date is None or
                             datetime.fromisoformat(test_config.end_date) > datetime.now())):
                            return test_config
            except Exception:
                pass
...
class MarketConditionAdjuster:
    ...
    async def get_current_market_metrics(
        self,
...
class DynamicScoringOrchestrator:
    ...
    async def score_lead_with_dynamic_weights(
        ...
        weights = await self.weight_config.get_weights_for_lead(
            tenant_id, segment, market_metrics
        )
    ...
    def _detect_lead_segment(self, lead_data: Dict[str, Any]) -> LeadSegment:
        ...
        # Investor indicators
        if ('investment' in intent or 'investor' in intent or
            'roi' in intent or 'rental' in intent or
            'multiple' in intent):
            return LeadSegment.INVESTOR

        # Seller indicators
        if ('sell' in intent or 'selling' in intent or
            'list' in intent):
            return LeadSegment.SELLER

        # Commercial indicators
        if ('commercial' in intent or 'business' in intent or
            budget > 2000000):
        ...
        for msg in messages:
            ...
            if ('first time' in content or 'never bought' in content or
                'first home' in content):
    ...
    async def record_conversion_outcome(
        ...
        await self.weight_config.record_lead_outcome(
            tenant_id, lead_id, segment, features, converted, conversion_value
        )
