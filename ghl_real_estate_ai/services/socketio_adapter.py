...
class JorgeSocketNamespace(AsyncNamespace):
    ...
    async def on_join_dashboard(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Dashboard joined: {session['username']} (client: {client_type}, version: {dashboard_version})"
            )
            ...
            await self.emit(
                "dashboard_status",
                {
                    ...
                    "user": {"username": session["username"], "role": session["role"].value},
    ...
    async def on_subscribe_contact(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                ...
                {"contact_id": contact_id, "location_id": location_id, "status": "subscribed"},
                room=sid,
            )
    ...
    async def on_request_bot_handoff(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                "bot_handoff_initiated",
                {"handoff_id": handoff_id, "target_bot": target_bot, "status": "initiated"},
                room=sid,
            )
    ...
    async def on_provide_coaching(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                "coaching_delivered",
                {"conversation_id": conversation_id, "coaching_type": coaching_type, "status": "delivered"},
                room=sid,
            )
    ...
    async def on_sync_context_request(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                "context_sync_confirmed", {"conversation_id": conversation_id, "status": "synchronized"}, room=sid
            )
    ...
    async def on_enable_omnipresent_monitoring(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                "omnipresent_monitoring_enabled",
                {"conversation_id": conversation_id, "status": "active", "capabilities": data.get("capabilities", [])},
                room=sid,
            )
    ...
    async def on_request_handoff(self, sid: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await self.emit(
                "handoff_status", {"status": "processing", "handoff_id": task.task_id, "target": target_agent}, room=sid
            )
    ...
    async def on_simulate_traffic(self, sid: str, data: Dict[str, Any] = None):
        ...
        try:
            ...
            await self.emit(
                "lead_update",
                {"lead_name": lead_name, "location": random.choice(locations), "pcs_score": random.randint(40, 95)},
                room=sid,
            )

            # 2. Send AI Intent Signal
            await self.emit(
                "realtime_intent_update",
                {
            ...
            await self.emit(
                "performance_metrics",
                {"current_hour_cost": random.uniform(10.0, 25.0), "total_tasks": random.randint(50, 200)},
                room=sid,
            )
    ...
    def _get_default_subscriptions(self, role: UserRole) -> Set[EventType]:
        ...
        if role == UserRole.ADMIN:
            ...
            return {EventType.DASHBOARD_REFRESH, EventType.SYSTEM_ALERT}
