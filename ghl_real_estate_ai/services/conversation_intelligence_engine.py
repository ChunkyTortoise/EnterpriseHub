...
from ghl_real_estate_ai.services.enhanced_conversation_intelligence import (
...
class ConversationIntelligenceEngine:
    ...
    async def analyze_conversation_realtime(self, 
                                          conversation_id: str,
                                          lead_id: str, 
                                          message: str,
                                          sender: str,
                                          message_timestamp: datetime = None) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._update_emotional_journey(
                lead_id, emotional_analysis, message, message_timestamp
            )
            
            # Update buying intent profile
            await self._update_buying_intent_profile(
                lead_id, buying_signals, sentiment_dimensions
            )
            
            # Store conversation context
            await self._store_conversation_context(
                conversation_id, lead_id, message, sender, {
    ...
    async def _assess_sentiment_dimensions(self, message: str) -> Optional[SentimentDimensions]:
        ...
        try:
            ...
            return SentimentDimensions(
                overall_sentiment=0.0, confidence=0.5, excitement=0.5,
                confidence_level=0.5, anxiety=0.5, frustration=0.5,
                satisfaction=0.5, trust=0.5, urgency=0.5,
                understanding=0.5, decision_readiness=0.5,
                information_seeking=0.5, social_validation=0.5,
                authority_respect=0.5, relationship_building=0.5,
                financial_concern=0.5, decision_paralysis=0.5,
                external_pressure=0.5
            )
    
    async def _evaluate_conversation_quality(self, 
                                           conversation_id: str, 
                                           message: str, 
    ...
    async def _identify_coaching_opportunities(self, 
                                             conversation_id: str, 
                                             message: str, 
                                             sender: str) -> List[str]:
    ...
    async def _update_emotional_journey(self, 
                                       lead_id: str, 
    ...
    async def _update_buying_intent_profile(self, 
                                          lead_id: str, 
                                          buying_signals: List[BuyingSignal],
                                          sentiment_dimensions: Optional[SentimentDimensions]):
    ...
    def _calculate_conversation_health_score(self, 
                                           emotional_analysis: Dict[str, Any],
        ...
        try:
            ...
            dashboard = {
                ...
                'journey_summary': {
                    ...
                    'dominant_emotion': max(journey.dominant_emotions, key=journey.dominant_emotions.get) if journey.dominant_emotions else 'neutral',
