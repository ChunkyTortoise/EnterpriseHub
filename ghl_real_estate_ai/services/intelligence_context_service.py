...
class IntelligenceContextService:
    ...
    async def preserve_intelligence(
        ...
        try:
            ...
            cache_key = self._generate_handoff_key(
                lead_id=lead_id,
            ...
            logger.error(
                f"Intelligence preservation failed for {lead_id}: {e}",
                exc_info=True
            )
    ...
    async def retrieve_intelligence_context(
        ...
        try:
            ...
            if location_id and snapshot.location_id != location_id:
                logger.warning(
                    f"Location mismatch for {lead_id}: "
                    f"requested {location_id}, cached {snapshot.location_id}"
                )
                return None
            ...
            logger.error(
                f"Intelligence retrieval failed for {lead_id}: {e}",
                exc_info=True
            )
    ...
    async def create_intelligence_snapshot(
        ...
        try:
            ...
            conversation_summary = await self._generate_conversation_summary(
                intelligence_data, bot_transition
            )
    ...
    async def get_transition_history(
        ...
        try:
            ...
            if cached_history:
                ...
                return TransitionHistory.create_empty(
                    lead_id=lead_id,
                    location_id=location_id or ""
                )
    ...
    async def _extract_preserved_intelligence(
        self,
        intelligence_data: Dict[str, Any]
    ) -> PreservedIntelligence:
    ...
    async def _generate_conversation_summary(
        ...
        try:
            ...
            if bot_transition.transition_reason == TransitionReason.QUALIFIED_BUYER:
                return (f"Seller qualified with FRS {qualification_scores.get('FRS', 0):.0f}/PCS "
                ...
                return (f"Buyer qualified, also interested in selling current property. "
                ...
                return (f"Complex situation requiring human agent intervention. "
                       f"Review conversation history and address concerns.")
            else:
                return (f"Bot transition initiated. {len(conversation_history)} messages exchanged, "
    ...
    def _calculate_confidence_level(self, intelligence_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            conversation_quality = intelligence_data.get('conversation_intelligence', {}).get('conversation_quality_score', 50.0)
    ...
    def _assess_intelligence_quality(self, snapshot: IntelligenceSnapshot) -> str:
        ...
        try:
            ...
            if (snapshot.preserved_intelligence.key_objections_detected or
                snapshot.preserved_intelligence.response_recommendations):
                score += 1

            # Preferences available
            if (snapshot.preserved_intelligence.budget_range or
                snapshot.preserved_intelligence.location_preferences):
                score += 1
    ...
    def _update_preservation_metrics(
        self,
        latency_ms: float,
        data_size_bytes: int,
        success: bool
        ...
        if latency_ms > 0:
            total_time = (
                self.metrics['avg_preservation_latency_ms'] *
                (self.metrics['total_preservations'] - 1) + latency_ms
            )
    ...
    def _update_retrieval_metrics(self, retrieval_latency_ms: float, success: bool) -> None:
        ...
        if retrieval_latency_ms > 0:
            total_time = (
                ...
                (self.metrics['total_retrievals'] - 1) + retrieval_latency_ms
            )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        cache_hit_rate = (
            self.metrics['cache_hits'] / cache_operations * 100
            if cache_operations > 0 else 0.0
        )

        preservation_success_rate = (
            self.metrics['successful_preservations'] / self.metrics['total_preservations'] * 100
            if self.metrics['total_preservations'] > 0 else 0.0
        )
        ...
        if self._preservation_latencies:
            ...
            p99_preservation = sorted_preservation[p99_idx] if p99_idx < len(sorted_preservation) else sorted_preservation[-1]
