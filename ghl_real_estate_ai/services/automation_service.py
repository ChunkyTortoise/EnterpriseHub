...
class AutomationService:
    ...
    def schedule_peak_outreach(
        ...
        trigger = WorkflowTrigger(
            ...
            conditions={"scheduled_time": next_occurrence.isoformat(), "peak_day": peak_day, "peak_hour": peak_hour},
        )
        ...
        actions = [
            WorkflowAction(
                ...
                config={"message": message_template, "contact_id": contact_id},
                conditions=[],
                delay_seconds=0,
            ),
            WorkflowAction(
                ...
                config={"tag": f"peak_outreach_{peak_day.lower()}"},
                conditions=[],
                delay_seconds=0,
            ),
        ]
    ...
    def track_timeline_action(self, contact_id: str, action_name: str, estimated_days_saved: int = 5) -> Dict[str, Any]:
        """
    ...
    def execute_quick_wins(self, contact_id: str, actions: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        for action in actions:
            ...
            # Create trigger
            trigger = WorkflowTrigger(
                trigger_type=TriggerType.BEHAVIOR_BASED.value, conditions={"source": "quick_wins"}
            )
            ...
            results.append({"action": action, "workflow_id": workflow.workflow_id, "status": "queued"})
    ...
    def find_contractors(
        self, contact_id: str, feature_needed: str, property_address: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Determine category
        category_key = next((k for k in contractor_categories.keys() if k in feature_needed.lower()), "renovation")
        ...
        trigger = WorkflowTrigger(trigger_type=TriggerType.BEHAVIOR_BASED.value, conditions={"source": "gap_analysis"})
        ...
        for contractor in contractors:
            contractor_message += (
                f"• {contractor['name']} - {contractor['rating']}⭐ ({contractor['reviews']} reviews)\n"
            )
    ...
    def find_alternative_properties(
        self, contact_id: str, feature_needed: str, current_criteria: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if "tour" in action_name.lower() or "showing" in action_name.lower():
            ...
            actions.append(
                WorkflowAction(
                    ...
                    action_type=ActionType.SEND_SMS.value,
                    config={
                        "message": "Getting pre-approved strengthens your offer! I can connect you with our preferred lenders today."
                    },
                    conditions=[],
                    delay_seconds=0,
                )
            ...
            actions.append(
                WorkflowAction(
                    ...
                    action_type=ActionType.SEND_SMS.value,
                    config={
                        "message": "I've curated a list of properties perfectly matched to your criteria. Sending them your way now!"
                    },
                    conditions=[],
                    delay_seconds=0,
                )
    ...
    def get_automation_history(
        self, contact_id: Optional[str] = None, automation_type: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """
...
def demo_automation_service():
    ...
    result = service.schedule_peak_outreach(
        contact_id=test_contact_id, peak_day="Tuesday", peak_hour=14, segment="active"
    )
    ...
    result = service.track_timeline_action(
        contact_id=test_contact_id, action_name="Schedule property tour within 48 hours"
    )
    ...
    result = service.find_contractors(contact_id=test_contact_id, feature_needed="pool")
    print(f"   ✅ {result['message']}")
    print(f"   👷 Contractors: {len(result['contractors'])}")

    # Test 5: Find alternatives
    print("\n5️⃣ Finding alternative properties...")
    result = service.find_alternative_properties(contact_id=test_contact_id, feature_needed="pool")
    print(f"   ✅ {result['message']}")
