...
class ProactiveCommunicationEngine:
    ...
    async def send_celebration_message(
        ...
        achievement: str,
        recipients: List[str] = None,
        celebration_type: str = "milestone"
    ) -> List[str]:
        """
        ...
        try:
            ...
            event = CommunicationEvent(
                ...
                event_data={
                    ...
                    "response_time_hours": (datetime.now() - message.sent_at).total_seconds() / 3600 if message.sent_at else 0
                }
            )
    ...
    async def _process_message_queue(self):
        ...
        try:
            ...
            ready_messages = [
                ...
                if (msg.send_immediately or 
                    (msg.scheduled_send_time and msg.scheduled_send_time <= datetime.now()) or
                    msg.scheduled_send_time is None) and
                msg.status == MessageStatus.PENDING and
                not self._is_quiet_hours(msg.recipient_id)
            ]
            
            # Sort by urgency and creation time
            ready_messages.sort(
                key=lambda m: (m.urgency.value, m.created_at),
                reverse=True
            )
            ...
            for i in range(0, len(ready_messages), self.batch_send_size):
                ...
                await asyncio.gather(
                    *[self._send_message(msg) for msg in batch],
    ...
    async def _initiate_call(self, message: CommunicationMessage) -> bool:
        ...
        try:
            ...
            # Update channel metrics
            self.metrics["messages_by_channel"]["phone_call"] = self.metrics["messages_by_channel"].get("phone_call", 0) + 1
    ...
    async def _send_push_notification(self, message: CommunicationMessage) -> bool:
        ...
        try:
            ...
            # Update channel metrics
            self.metrics["messages_by_channel"]["push_notification"] = self.metrics["messages_by_channel"].get("push_notification", 0) + 1
    ...
    async def _send_portal_message(self, message: CommunicationMessage) -> bool:
        ...
        try:
            ...
            # Update channel metrics
            self.metrics["messages_by_channel"]["portal_message"] = self.metrics["messages_by_channel"].get("portal_message", 0) + 1
    ...
    async def _generate_milestone_message(
        ...
        try:
            ...
            return response.content.strip() if response.content else self._get_fallback_milestone_message(milestone_name, milestone_status)
    ...
    async def _generate_celebration_message(
        ...
        try:
            ...
            return response.content.strip() if response.content else f"🎉 Congratulations! {achievement} is complete. This is a significant step forward in your home purchase!"
    ...
    async def _generate_issue_alert_message(
        ...
        try:
            ...
            return response.content.strip() if response.content else f"We wanted to alert you to {issue_type}. {issue_description}\n\nSuggested next steps:\n{actions_text}"
    ...
    async def _track_communication_event(self, transaction_id: str, event_type: str, description: str, message_id: Optional[str] = None):
    ...
    async def _handle_message_failure(self, message: CommunicationMessage):
        ...
        if message.delivery_attempts < message.max_attempts:
            # Schedule retry
            retry_delay = self.retry_delay_minutes[min(message.delivery_attempts - 1, len(self.retry_delay_minutes) - 1)]
