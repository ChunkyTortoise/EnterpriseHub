...
class InputValidator:
    ...
    def __init__(self):
        ...
        self.allowed_html_tags = ["p", "br", "strong", "em", "u", "ol", "ul", "li", "h1", "h2", "h3", "h4", "h5", "h6"]
    ...
    async def validate_string(
        ...
        if not isinstance(value, str):
            return ValidationResult(
                is_valid=False, errors=[f"{field_name} must be a string"], severity=ValidationSeverity.MEDIUM
            )
    ...
    async def validate_email(self, email: str, field_name: str = "email") -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=True, sanitized_value=validated_email.email, warnings=string_result.warnings
            )

        except EmailNotValidError as e:
            return ValidationResult(
                is_valid=False, errors=[f"{field_name}: {str(e)}"], severity=ValidationSeverity.MEDIUM
            )

    async def validate_url(
        self, url: str, field_name: str = "url", allowed_schemes: List[str] = None
    ) -> ValidationResult:
        """Validate URL"""
        ...
        try:
            ...
            return ValidationResult(
                is_valid=True, sanitized_value=string_result.sanitized_value, warnings=string_result.warnings
            )

        except Exception:
            return ValidationResult(
                is_valid=False, errors=[f"{field_name}: Invalid URL format"], severity=ValidationSeverity.MEDIUM
            )

    async def validate_json(
        self, json_str: str, field_name: str = "json", max_size: int = 1024 * 1024
    ) -> ValidationResult:
        """Validate JSON input"""
        # Size check
        ...
        try:
            ...
            return ValidationResult(is_valid=True, sanitized_value=parsed_json, warnings=string_result.warnings)
    ...
    async def validate_file_upload(
        self, filename: str, content_type: str, file_size: int, allowed_types: Optional[List[str]] = None
    ) -> ValidationResult:
        ...
        filename_result = await self.validate_string(
            filename, "filename", max_length=255, custom_rules=["path_traversal", "command_injection"]
        )
    ...
    async def validate_headers(self, headers: Dict[str, str]) -> ValidationResult:
        ...
        for name, value in headers.items():
            ...
            header_result = await self.validate_string(
                value, f"header_{name}", max_length=8192, custom_rules=["header_injection", "xss_script_tags"]
            )
    ...
    async def validate_query_params(self, params: Dict[str, str]) -> ValidationResult:
        ...
        for name, value in params.items():
            ...
            param_result = await self.validate_string(value, f"param_{name}", max_length=2048)
    ...
    def _sanitize_html(self, html_content: str) -> str:
        ...
        return bleach.clean(
            html_content, tags=self.allowed_html_tags, attributes=self.allowed_html_attributes, strip=True
        )
    ...
    def get_rule_stats(self) -> Dict[str, int]:
        ...
        return {"total_rules": len(self.validation_rules), "by_severity": severity_counts, "by_action": action_counts}
