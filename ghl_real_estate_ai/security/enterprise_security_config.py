...
class CORSConfig:
    ...
    forbidden_headers: Set[str] = field(default_factory=lambda: {
        "X-Forwarded-For",
        "X-Real-IP", 
        "X-Forwarded-Host",
        "X-Original-URL"
...
class AuditConfig:
    ...
    pii_fields: Set[str] = field(default_factory=lambda: {
        "email", "phone", "ssn", "tax_id", "credit_card",
...
class ComplianceConfig:
    ...
    enabled_standards: Set[ComplianceStandard] = field(default_factory=lambda: {
    ...
    classify_as_sensitive: Set[str] = field(default_factory=lambda: {
...
class EnterpriseSecurityConfig:
    ...
    def _apply_security_level(self):
        ...
        if self.security_level == SecurityLevel.DEVELOPMENT:
            ...
            self.headers.content_security_policy = "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self'; connect-src 'self'"
