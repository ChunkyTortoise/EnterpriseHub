...
class AzureAuthIntegration:
    ...
    def __init__(self):
        ...
        self.permission_mappings = {
            "admin": ["platform.admin", "users.manage", "settings.configure", "reports.access", "billing.manage"],
            "manager": ["team.manage", "leads.assign", "reports.access", "properties.manage"],
            "agent": ["leads.access", "properties.view", "reports.view"],
            "user": ["profile.edit", "dashboard.view"],
        }
    ...
    def generate_auth_url(self, tenant_id: str, state: Optional[str] = None, scopes: Optional[List[str]] = None) -> str:
        """
    ...
    async def handle_auth_callback(
        self, authorization_code: str, state: str, tenant_id: Optional[str] = None
    ) -> AuthSession:
        """
        ...
        try:
            ...
            tokens = await self._exchange_code_for_tokens(authorization_code, tenant_id or state_data["tenant_id"])
            ...
            graph_user = await self._get_graph_user_details(tokens["access_token"], user_info["oid"])
    ...
    async def _exchange_code_for_tokens(self, authorization_code: str, tenant_id: str) -> Dict[str, str]:
        ...
        access_token = self._generate_demo_jwt(
            {"aud": "https://graph.microsoft.com", "scope": "User.Read"}, expires_in=3600
        )
        ...
        refresh_token = base64.urlsafe_b64encode(f"refresh_{uuid.uuid4()}".encode()).decode()
    ...
    async def _get_graph_user_details(self, access_token: str, user_id: str) -> Dict[str, Any]:
        ...
        graph_user = {
            ...
            "assignedRoles": [{"roleDefinition": {"displayName": "Real Estate Manager"}}],
        }

        return graph_user

    async def _create_auth_user(self, id_token_claims: Dict[str, Any], graph_user: Dict[str, Any]) -> AuthUser:
    ...
    async def _create_auth_session(self, user: AuthUser, tokens: Dict[str, str]) -> AuthSession:
        ...
        # Cache session
        await self.cache.set(f"auth_session:{session_id}", asdict(session), ttl=int(tokens.get("expires_in", 3600)))

        # Cache user session mapping
        await self.cache.set(f"user_session:{user.user_id}", session_id, ttl=int(tokens.get("expires_in", 3600)))
    ...
    def generate_logout_url(self, tenant_id: str, post_logout_redirect: Optional[str] = None) -> str:
        ...
        logout_params = {"post_logout_redirect_uri": post_logout_redirect or "https://app.enterprisehub.ai/login"}
    ...
    async def sync_tenant_users(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process each user
            sync_results = {"total_users": len(azure_users), "synced_users": 0, "skipped_users": 0, "errors": []}
            ...
            for azure_user in azure_users:
                try:
                    ...
                    sync_results["errors"].append({"user_id": azure_user["id"], "error": str(e)})
                    sync_results["skipped_users"] += 1

            logger.info(
                f"User sync completed: {sync_results['synced_users']} synced, {sync_results['skipped_users']} skipped"
            )
