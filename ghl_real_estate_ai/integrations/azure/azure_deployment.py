...
class AzureDeployment:
    ...
    def _generate_template_parameters(
        self,
        customer_id: str,
        deployment_size: str
    ) -> Dict[str, Any]:
            ...
            if region == regions[0]:
                ...
                sql_database = ResourceConfig(
                    ...
                    properties={
                        ...
                        "maxSizeBytes": str(resource_config["sql_database_sku"]["capacity"] * 1024 * 1024 * 1024),  # GB to bytes
                        "zoneRedundant": False,
                        "readScale": "Disabled",
    ...
    def _generate_template_outputs(
        self,
        resources: List[ResourceConfig],
        primary_region: str
    ) -> Dict[str, Any]:
        ...
        if web_app:
            ...
            outputs["webAppName"] = {
                "type": "string", 
                "value": f"[reference('{web_app.resource_name}').name]"
            }
            
        # Find SQL server
        sql_server = next(
            (r for r in resources if r.resource_type == ResourceType.SQL_SERVER),
            None
        )
        if sql_server:
        ...
        storage_account = next(
            (r for r in resources if r.resource_type == ResourceType.STORAGE_ACCOUNT and primary_region in r.resource_name),
            None
        )
        if storage_account:
    ...
    async def _complete_deployment(
        ...
        if job_data:
            ...
            job_data["deployed_resources"] = deployed_resources
            job_data["actual_cost"] = job_data["estimated_cost"] * (0.9 + (len(deployed_resources) * 0.01))  # Simulate actual cost
    ...
    async def generate_arm_json(self, template_id: str) -> str:
        ...
        arm_template = {
            ...
            "metadata": {
                ...
                "author": "EnterpriseHub Azure Integration"
            },
        ...
        for resource in template.resources:
            ...
            if resource.dependencies:
                arm_resource["dependsOn"] = [
                    f"[resourceId('{self._get_resource_provider(dep)}', '{dep}')]"
