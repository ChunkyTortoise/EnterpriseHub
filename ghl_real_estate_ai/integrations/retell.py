...
class RetellClient:
    ...
    def __init__(self):
        ...
        self.circuit_breaker = circuit_manager.get_or_create_breaker("retell")
    ...
    async def create_call(
        ...
        """
        Initiate an outbound call to a lead via circuit breaker.
        """
        if not self.api_key:
            return self._mock_call_response(to_number)

        async def _make_retell_call():
            """Internal function to execute the call creation"""
        ...
        try:
            return await self.circuit_breaker.call(_make_retell_call)
        except Exception as e:
            logger.error(f"Retell call creation failed through circuit breaker: {e}")
            raise

    async def get_call_details(self, call_id: str) -> Dict[str, Any]:
        """
        Retrieve details of a specific call via circuit breaker.
        """
        if not self.api_key:
            return {"status": "completed", "transcript": "Mock transcript"}

        async def _fetch_call_details():
            """Internal function to fetch call details"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(f"Failed to get Retell call details: {response.status} - {error_text}")
                        raise Exception(f"Retell API Error: {error_text}")

        # Execute through circuit breaker
        try:
            return await self.circuit_breaker.call(_fetch_call_details)
        except Exception as e:
            logger.error(f"Retell call details fetch failed through circuit breaker: {e}")
            return {}
