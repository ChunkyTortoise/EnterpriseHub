...
from ghl_real_estate_ai.agent_system.v2.agents.analysis_agent import AnalysisDeps, analysis_agent
from ghl_real_estate_ai.agent_system.v2.agents.design_agent import DesignDeps, design_agent
...
async def research_node(state: ConductorState):
    ...
    try:
        result = await research_agent.run(
            f"Perform research on {state['property_address']} in {state.get('market') or 'Unknown'}", deps=deps
        )
        data = result.output.model_dump()

        # Phase 6: Log Performance
        performance_monitor.log_agent_run(
            "researcher", time.time() - start_time, {"input": 1200, "output": 800}, "success"
        )
...
async def analysis_node(state: ConductorState):
    ...
    try:
        result = await analysis_agent.run(
            f"Analyze investment potential based on this research: {state['research_data']}", deps=deps
        )
        data = result.output.model_dump()

        # Phase 6: Log Performance
        performance_monitor.log_agent_run(
            "analyst", time.time() - start_time, {"input": 2500, "output": 450}, "success"
        )

        await semantic_cache.set(query, data)
        return {"analysis_results": data, "status": "analysis_complete"}
    except Exception as e:
...
async def design_node(state: ConductorState):
    ...
    if cached:
        ...
        return {"design_data": cached, "staged_images": staged_images, "status": "design_complete_cached"}
    ...
    try:
        ...
        # Log Performance
        performance_monitor.log_agent_run(
            "designer", time.time() - start_time, {"input": 1500, "output": 600}, "success"
        )

        await semantic_cache.set(query, data)
        return {"design_data": data, "staged_images": staged_images, "status": "design_complete"}
    except Exception as e:
...
async def executive_node(state: ConductorState):
    ...
    try:
        ...
        # Log Performance
        performance_monitor.log_agent_run(
            "executive", time.time() - start_time, {"input": 3500, "output": 1200}, "success"
        )

        await semantic_cache.set(query, data)
        return {"executive_summary": data, "status": "executive_complete"}
    except Exception as e:
...
async def marketing_node(state: ConductorState):
    ...
    try:
        ...
        # Log Performance
        performance_monitor.log_agent_run(
            "marketer", time.time() - start_time, {"input": 3000, "output": 900}, "success"
        )

        return {"matched_leads": matched_leads, "marketing_campaigns": marketing_data, "status": "marketing_complete"}
    except Exception as e:
...
async def lead_recovery_node(state: ConductorState):
            ...
            recovery_results.append(
                {
                    ...
                    "sentiment": analysis.predictive_insights.get("sentiment")
    ...
    return {"service6_insights": {"recovery_analysis": recovery_results}, "status": "lead_recovery_complete"}
...
async def evaluation_node(state: ConductorState):
    ...
    return {"evaluations": evaluations, "status": "complete_polished"}
