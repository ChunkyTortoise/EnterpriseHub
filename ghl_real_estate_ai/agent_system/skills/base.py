...
class Skill:
    ...
    def __init__(
        self, func: Callable, name: Optional[str] = None, description: Optional[str] = None, tags: List[str] = None
    ):
        self.func = func
    ...
    def to_gemini_tool(self) -> Dict[str, Any]:
        ...
        return {
            "function_declarations": [{"name": self.name, "description": self.description, "parameters": self.schema}]
        }
...
class SkillRegistry:
    ...
    def _get_skill_collection(self, location_id: str = "global"):
        ...
        if location_id not in self._skill_collections:
            ...
            self._skill_collections[location_id] = self._chroma_client.get_or_create_collection(
                name=collection_name, metadata={"hnsw:space": "cosine", "location_id": location_id}
            )
    ...
    def find_relevant_skills(self, query: str, limit: int = 5, location_id: str = "global") -> List[Skill]:
        ...
        if collection:
            try:
                results = collection.query(query_texts=[query], n_results=min(limit, len(self.skills)))
