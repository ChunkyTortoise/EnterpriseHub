...
def format_pagerduty_payload(alert: Alert) -> dict:
    ...
    return {
        ...
        "payload": {
            ...
            "severity": "critical" if alert.severity == "critical" else ("warning" if alert.severity == "warning" else "info"),
        ...
        "links": [
            {
                "href": f"https://dashboard.enterprisehub.com/jorge/alerts/{alert.id}",
                "text": "View Alert"
            }
        ]
    }
        ...
        "links": [
            {
                ...
                "url": f"https://dashboard.enterprisehub.com/jorge/alerts/{alert.id}"
            }
        ]
    }
    ...
    if pagerduty_url:
        ...
        try:
            if mock:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(pagerduty_url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
                        response.raise_for_status()
    ...
    if opsgenie_url:
        ...
        try:
            if mock:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(opsgenie_url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as response:
                        response.raise_for_status()
...
def main():
    parser = argparse.ArgumentParser(
        description="Test custom webhook alert channel"
    )
    parser.add_argument(
        "--mock",
        action="store_true",
    ...
    parser.add_argument(
        "--no-mock",
        action="store_true",
        help="Disable mock mode (send actual webhooks)"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
