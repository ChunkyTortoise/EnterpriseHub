...
class TestBulkOperationsManager:
    ...
    def test_create_operation_score(self, manager):
        ...
        operation_id = manager.create_operation(
            operation_type="score", target_leads=["lead1"], parameters={"criteria": "engagement"}, created_by="system"
        )
    ...
    def test_create_operation_empty_leads(self, manager):
        ...
        operation_id = manager.create_operation(
            operation_type="tag", target_leads=[], parameters={"tags": ["test"]}, created_by="test_user"
        )
    ...
    def test_create_operation_large_batch(self, manager):
        ...
        operation_id = manager.create_operation(
            operation_type="export", target_leads=leads, parameters={"format": "csv"}, created_by="test_user"
        )
    ...
    def test_load_operations_existing_file(self, tmp_path):
        ...
        test_data = {"operations": [{"id": "test1"}], "templates": {"template1": {}}}
    ...
    def test_operation_types(self, manager):
        ...
        for op_type in types:
            operation_id = manager.create_operation(
                operation_type=op_type, target_leads=["lead1"], parameters={}, created_by="test"
            )
            assert operation_id is not None
...
class TestBulkOperationsEdgeCases:
    ...
    def test_invalid_operation_type(self):
        ...
        operation_id = manager.create_operation(
            operation_type="invalid_type", target_leads=["lead1"], parameters={}, created_by="test"
        )
        assert operation_id is not None
    ...
    def test_special_characters_in_leads(self):
        ...
        manager = BulkOperationsManager(location_id="test")
        operation_id = manager.create_operation(
            operation_type="tag", target_leads=["lead@#$%", "lead!@#"], parameters={"tags": ["test"]}, created_by="test"
        )
        assert operation_id is not None
...
class TestBulkOperationsIntegration:
    ...
    def test_multiple_operations_same_location(self):
        ...
        for i in range(5):
            operation_id = manager.create_operation(
                operation_type="tag", target_leads=[f"lead_{i}"], parameters={"tags": [f"tag_{i}"]}, created_by="tester"
            )
            assert operation_id is not None
