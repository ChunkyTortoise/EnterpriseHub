...
def create_rag(name):
    ...
    rag = RAGEngine(collection_name=f"test_collection_{name}", persist_directory=persist_dir)
    return rag, persist_dir


@pytest.mark.skip(
    reason="RAG tenant scoping returns 0 results - ChromaDB collection not properly persisting in test environment"
)
...
def test_multitenancy_no_global():
    ...
    try:
        temp_rag.add_documents(documents=["Tenant A info"], ids=["a1"], location_id="TENANT_A")
