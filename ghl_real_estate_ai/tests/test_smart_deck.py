...
async def test_smart_deck_excludes_seen_properties(swipe_manager_with_mock):
    ...
    # Get smart deck
    deck = await swipe_manager_with_mock.get_smart_deck(
        lead_id=lead_id,
        location_id=location_id,
        limit=10
    )
...
async def test_apply_negative_feedback_adjustments(swipe_manager_with_mock):
    ...
    # Apply adjustments
    adjusted = swipe_manager_with_mock._apply_negative_feedback_adjustments(
        lead_id, initial_prefs
    )
...
async def test_filter_negative_matches(swipe_manager_with_mock):
    ...
    # Filter
    filtered = swipe_manager_with_mock._filter_negative_matches(
        properties, lead_id, context
    )
