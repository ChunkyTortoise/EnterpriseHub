...
async def test_open_redirect_validation(mock_env_redirect, monkeypatch):
    ...
    with (
        patch("ghl_real_estate_ai.api.routes.enterprise_partnerships.enterprise_auth_service", mock_auth),
        patch("ghl_real_estate_ai.api.routes.enterprise_partnerships.logger") as mock_logger,
    ):
        res = await initiate_sso_login(
            ontario_mills="test.com", redirect_uri="https://trusted.com/callback", auth_service=mock_auth
        )
