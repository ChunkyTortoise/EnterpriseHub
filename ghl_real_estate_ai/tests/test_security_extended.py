...
from ghl_real_estate_ai.security.auth_manager import (
    AuthManager, SecurityConfig, User, Session, AuthProvider, SessionStatus, Role
)
from ghl_real_estate_ai.security.security_middleware import SecurityMiddleware, SecurityHeaders
...
class TestAuthManager:
    ...
    async def test_authenticate_user_locked(self, auth_manager):
        ...
        user = User(
            user_id="user_123", username="locked", email="", full_name="",
            roles=[], provider=AuthProvider.LOCAL
        )
    ...
    async def test_account_lockout_after_failures(self, auth_manager):
        ...
        user = User(
            user_id="u1", username="victim", email="", full_name="",
            roles=[], provider=AuthProvider.LOCAL
        )
...
class TestSecurityMiddleware:
    ...
    async def test_dispatch_threat_detection_blocking(self, middleware):
        ...
        async def call_next(req): return Response()
    ...
    async def test_exempt_paths(self, middleware):
        ...
        async def call_next(req): return Response(status_code=200)
