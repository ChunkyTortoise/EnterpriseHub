        ...
        try:
            ...
            if demo_bypass_enabled:
                ...
                try:
                    ...
                    logger.error("Failed to import test_auth helper. Install jwt library or enable ENABLE_DEMO_BYPASS=true")
                    raise
    ...
    async def run_scale_test(self, num_users: int, duration: int, ramp_up_rate: int = 100):
        ...
        async def simulate_traffic(client, client_id):
            while time.time() < test_end_time:
                try:
                    await client.emit('ping_performance', {'timestamp': time.time(), 'client_id': client_id}, namespace='/jorge')
