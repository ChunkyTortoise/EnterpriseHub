...
class MockGHLClient:
    ...
    async def send_message(
        self, contact_id: str, message: str, channel: str = None
    ) -> Dict:
...
class TestLeadBotE2EWithCompliance:
    ...
    async def test_full_webhook_processing_with_lead_mode(
        ...
        # Patch services
        with patch(
            "ghl_real_estate_ai.services.jorge.jorge_handoff_service.JorgeHandoffService"
            ...
            with patch(
                "ghl_real_estate_ai.services.compliance_guard.ComplianceGuard.audit_message"
            ) as mock_audit:
                ...
                # Process the message
                compliance_result = await mock_audit(
                    lead_response, {"contact_id": sample_webhook_event.contact_id}
                )
    ...
    @pytest.mark.asyncio
    async def test_compliance_guard_intercepts_violations(
        self, compliance_guard
    ):
    ...
    @pytest.mark.asyncio
    async def test_compliance_guard_passes_compliant_messages(
        self, compliance_guard
    ):
    ...
    def test_sms_length_truncation(self):
        ...
        assert len(truncated) <= max_sms_length, f"Truncated message length {len(truncated)} should be <= {max_sms_length}"
...
class TestLeadBotHandoffScenarios:
    ...
    @pytest.mark.asyncio
    async def test_handoff_analytics_recording(
        self, handoff_service, mock_analytics_service
    ):
        ...
        decision = await handoff_service.evaluate_handoff(
            ...
            contact_id=contact_id,
            conversation_history=[
                {"role": "user", "content": "I'm pre-approved and ready to buy!"}
            ],
            intent_signals=intent_signals,
        )
...
class TestMultiBotHandoffCascade:
    ...
    @pytest.mark.asyncio
    async def test_lead_to_buyer_to_seller_cascade(
        self, handoff_service, mock_analytics_service
    ):
        """
        ...
        actions_1 = await handoff_service.execute_handoff(
            decision=decision_1, contact_id=contact_id
        )
        ...
        actions_2 = await handoff_service.execute_handoff(
            decision=decision_2, contact_id=contact_id
        )
    ...
    @pytest.mark.asyncio
    async def test_tag_accumulation_across_handoffs(
        self, handoff_service, mock_analytics_service
    ):
        ...
        first_actions = await handoff_service.execute_handoff(
            first_decision, contact_id=contact_id
        )
        ...
        second_actions = await handoff_service.execute_handoff(
            second_decision, contact_id=contact_id
        )
        ...
        for action_set in [first_actions, second_actions]:
            for action in action_set:
                if action["type"] == "add_tag" and action["tag"].startswith(
                    "Handoff-"
                ):
