...
class TestReengagementEngine:
    ...
    def mock_dependencies(self):
        ...
        llm_client_mock.agenerate = AsyncMock(
    ...
    def engine(self, mock_dependencies):
        ...
        with (
            patch(
                "ghl_real_estate_ai.services.reengagement_engine.LLMClient",
                return_value=mock_dependencies["llm_client"],
            ),
            patch(
                "ghl_real_estate_ai.services.reengagement_engine.AnalyticsService",
                return_value=mock_dependencies["analytics"],
            ),
        ):
    ...
    async def test_detect_trigger_invalid_timestamp(self, engine):
        ...
        context = {"contact_id": "contact_123", "last_interaction_at": "invalid-date"}
        trigger = await engine.detect_trigger(context)
        assert trigger is None
    ...
    async def test_determine_lead_goal_inferred(self, engine):
        ...
        context = {
            ...
            "conversation_history": [{"role": "user", "content": "I am looking to purchase a property"}],
        }
        action, is_buyer, is_seller = engine._determine_lead_goal(context)
        assert action == "buy"
        assert is_buyer is True
    ...
    async def test_send_reengagement_no_trigger(self, engine):
        ...
        context = {"contact_id": "contact_123", "last_interaction_at": datetime.utcnow().isoformat()}
        result = await engine.send_reengagement_message("contact_123", "John", context)
    ...
    async def test_scan_for_recovery_eligible_leads(self, engine, mock_dependencies):
        ...
        assert (
            leads[0]["campaign_template"].campaign_type == RecoveryCampaignType.WIN_BACK_AGGRESSIVE
        )  # High value due to 1.2 prob
    ...
    async def test_send_recovery_campaign_success(self, engine, mock_dependencies):
        ...
        assert (
            call_args["context"]["last_recovery_campaign"]["campaign_type"]
            == RecoveryCampaignType.WIN_BACK_AGGRESSIVE.value
        )
