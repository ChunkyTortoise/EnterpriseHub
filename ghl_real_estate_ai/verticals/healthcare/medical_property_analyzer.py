...
class MedicalPropertyAnalyzer:
    ...
    async def analyze_healthcare_property(
        self,
        property_data: HealthcareProperty,
    ...
    async def _analyze_market_opportunity(self, property_data: HealthcareProperty) -> Tuple[float, Dict]:
        ...
        try:
            market_analysis = await self.llm_client.generate_response(
                market_prompt,
                max_tokens=1000
            )
    ...
    async def _analyze_financial_viability(
        ...
        base_revenue = property_data.square_footage * revenue_per_sqft.get(
            property_data.property_type, 400
        )
    ...
    async def _generate_strategic_recommendations(
        ...
        try:
            strategic_response = await self.llm_client.generate_response(
                strategy_prompt,
                max_tokens=1200
            )
