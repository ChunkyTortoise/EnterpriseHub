...
async def run_evaluation_suite():
    ...
    for case in test_cases:
        ...
        response = await client.agenerate(prompt=prompt_text, system_prompt=system_instr, model="gemini-1.5-flash")

        # 3. Evaluate response
        eval_result = await evaluator.evaluate_response(
            response_text=response.content, context=case["context"], rubric_type=case["rubric"]
        )
