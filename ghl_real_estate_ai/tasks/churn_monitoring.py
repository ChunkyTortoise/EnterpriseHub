...
class ChurnMonitorScheduler:
    ...
    async def run_daily_churn_assessment(self):
        ...
        try:
            ...
            assessments = await self.churn_service.batch_assess_contacts([c["id"] for c in active_contacts])
            ...
            logger.info(
                f"Daily churn assessment completed: {high_risk_count} high risk, {critical_risk_count} critical risk"
            )
    ...
    async def execute_scheduled_recovery_actions(self):
        ...
        try:
            ...
            logger.info(f"Recovery actions executed: {success_count} success, {failure_count} failure")
