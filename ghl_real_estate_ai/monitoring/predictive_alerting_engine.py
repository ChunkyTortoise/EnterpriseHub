...
class PredictiveAlertingEngine:
    ...
    def _initialize_prediction_models(self):
        ...
        self.anomaly_detectors = {
            ...
            'business_metrics': IsolationForest(
                contamination=0.08,
                random_state=42,
                n_estimators=120
            )
    ...
    async def train_models(self, historical_metrics: Dict[str, List[TimeSeriesMetric]]):
        ...
        try:
            ...
            perf_features = self._extract_performance_features(
                historical_metrics.get('performance', [])
            )
            ...
            conv_features = self._extract_conversation_features(
                historical_metrics.get('conversation', [])
            )
            ...
            biz_features = self._extract_business_features(
                historical_metrics.get('business', [])
            )
    ...
    async def predict_performance_issues(
        ...
        recent_history: List[TimeSeriesMetric]
        ...
        try:
            ...
            perf_prediction = await self._predict_performance_degradation(
            ...
            conv_prediction = await self._predict_conversation_drift(
                current_metrics, recent_history
            )
            if conv_prediction:
            ...
            lead_prediction = await self._predict_lead_quality_issues(
                current_metrics, recent_history
            )
            if lead_prediction:
            ...
            system_prediction = await self._predict_system_overload(
                current_metrics, recent_history
            )
            ...
            business_prediction = await self._predict_business_anomalies(
                current_metrics, recent_history
            )
    ...
    async def _predict_performance_degradation(
        ...
        try:
            ...
            response_times = [
                m.value for m in recent_history
    ...
    async def _predict_conversation_drift(
        ...
        try:
            ...
            qualification_rates = [
                m.value for m in recent_history
                if m.metric_name == 'lead_conversion_rate'
            ][-15:]
    ...
    async def _predict_lead_quality_issues(
        ...
        try:
            ...
            if current_score < 65 and current_score > 0:  # Below warning threshold
                ...
                lead_scores = [
                    m.value for m in recent_history
                    if m.metric_name == 'average_lead_score'
                ][-10:]
    ...
    async def _predict_business_anomalies(
        ...
        try:
            ...
            deal_value_deviation = abs(avg_deal_value - historical_deal_value) / historical_deal_value if historical_deal_value > 0 else 0
