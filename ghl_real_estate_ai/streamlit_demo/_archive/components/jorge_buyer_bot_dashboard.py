        ...
        try:
            if self.analytics:
                ...
                return {
                    ...
                    "performance": {"conversion_rate": 14.2, "avg_response_time": "42ms", "compliance_score": 100.0},
                }
            else:
    ...
    async def get_buyer_pipeline(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "priority": "high",
                "budget": "$350K",
                "status": "Warm - Qualifying",
                "frs": 85,
                "ms": 72,
                "bot_analyzed": True,
                "last_interaction": "Yesterday",
                "next_action": "Verify rental projections",
            },
            {
                "id": "buyer_003",
                "name": "Mike Rodriguez",
                ...
                "priority": "warm",
                "budget": "$380K",
                "status": "Lukewarm - Nurture",
                "frs": 55,
                "ms": 74,
                "bot_analyzed": True,
                "last_interaction": "3 days ago",
                "next_action": "Lender referral follow-up",
            },
            {
                "id": "buyer_004",
                "name": "Linda Williams",
    ...
    async def qualify_buyer(self, buyer_id: str, buyer_name: str, history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.bot.process_buyer_conversation(
                buyer_id=buyer_id, buyer_name=buyer_name, conversation_history=history
            )
...
def render_buyer_intent_section(api_client: JorgeBuyerLiveClient):
    ...
    with col2:
        st.metric("Avg FRS", f"{stats['avg_financial_readiness']}%")
    with col3:
        st.metric("Avg MS", f"{stats['avg_motivation_score']}%")
    with col4:
        st.metric("Match Velocity", f"{stats['properties_matched_total']}")
    with col5:
        st.metric("Pipe Value", f"${stats['pipeline_potential_value'] // 1200000:.1f}M")
    ...
    with col_a:
        ...
        fig = go.Figure(
            data=go.Scatterpolar(r=[85, 90, 70, 95, 80], theta=categories, fill="toself", line_color="#00E5FF")
        )
        ...
        with st.container():
            ...
            with act1:
                if not buyer.get("bot_analyzed"):
                        ...
                        if JORGE_BUYER_BOT_AVAILABLE:
                            with st.spinner(f"Jorge is qualifying {buyer['name']}..."):
                                ...
                                sample_history = [
                                    {
                                        "role": "user",
                                        "content": f"Hi, I'm looking for a home in {buyer.get('intent', 'the area')}. Budget is {buyer.get('budget', 'flexible')}.",
                                        "sender_name": buyer["name"],
                                    },
                                    {
                                        "role": "assistant",
                                        "content": "I can help with that. Are you pre-approved for a mortgage?",
                                    },
                                    {"role": "user", "content": "Yes, we have a pre-approval letter ready."},
                                ]

                                qualification_result = run_async(
                                    api_client.qualify_buyer(buyer["id"], buyer["name"], sample_history)
                                )

                                if qualification_result.get("success"):
                                    st.session_state[f"buyer_qualification_{buyer['id']}"] = qualification_result
                                    st.success(
                                        f"✅ Qualified! FRS: {qualification_result.get('scores', {}).get('frs_score', 0)}"
                                    )
                                    st.rerun()
                                else:
                                    st.error(
                    ...
                    st.markdown(
                        f'<div style="color: #10B981; font-weight: bold; text-align: center;">✅ ANALYZED</div>',
                        unsafe_allow_html=True,
                    )
            ...
            with act4:
                status_color = (
                    "#00FF00" if "Hot" in buyer["status"] else "#FFA500" if "Warm" in buyer["status"] else "#8B949E"
                )
                st.markdown(
                    f'<div style="text-align: center; border-left: 3px solid {status_color}; font-size: 0.7rem; font-weight: bold;">{buyer["status"]}</div>',
                    unsafe_allow_html=True,
                )
...
def render_jorge_buyer_bot_dashboard():
    ...
    with tab1:
        render_buyer_intent_section(api_client)
    with tab2:
        render_buyer_pipeline_section(api_client)
    with tab3:
        try:
