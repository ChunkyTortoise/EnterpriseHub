...
class RealTimeValueDashboard:
    ...
    def render_dashboard(
        ...
        try:
            st.set_page_config(
                page_title="Real-Time Value Dashboard",
                page_icon="📊",
                layout="wide"
            )
    ...
    def _render_roi_headline(self, roi_calculation) -> None:
        ...
        with headline_col2:
            roi_color = "#00FF00" if float(roi_calculation.roi_percentage) >= 200 else "#FFD700" if float(roi_calculation.roi_percentage) >= 100 else "#FF6B6B"
            st.markdown(f"""
            <div style="text-align: center;">
    ...
    def _render_value_tracking_section(self, roi_calculation) -> None:
        ...
        dimensions = [
            ...
            ("Information Advantage", float(roi_calculation.information_advantage_value), ValueDimension.INFORMATION_ADVANTAGE),
        ...
        if not df.empty:
            ...
            fig = px.pie(
                ...
                color_discrete_map={
                    row['Dimension']: self.value_dimension_colors[row['Enum']] 
    ...
    def _render_value_timeline(self, roi_calculation) -> None:
        ...
        dates = pd.date_range(
            start=roi_calculation.period_start,
            end=roi_calculation.period_end,
            freq='D'
        )
    ...
    def _render_roi_metrics(self, roi_calculation) -> None:
        ...
        if roi_calculation.payback_period_days:
            st.metric(
                ...
                value=f"{roi_calculation.payback_period_days} days"
            )
...
def render_real_time_value_dashboard(
    agent_id: str = "jorge_sales_austin",
    client_id: Optional[str] = None,
    transaction_id: Optional[str] = None
) -> None:
    """
...
def render_value_dashboard_demo() -> None:
    ...
    agent_id = st.sidebar.selectbox(
        "Agent",
        ["jorge_sales_austin", "agent_002", "agent_003"],
        index=0
    )
    
    client_id = st.sidebar.selectbox(
        "Client View",
        [None, "client_smith", "client_johnson", "client_davis"],
        index=0
    )
    
    transaction_id = st.sidebar.selectbox(
        "Transaction",
        [None, "txn_001", "txn_002", "txn_003"],
        index=0
    )
