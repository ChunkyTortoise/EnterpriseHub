...
try:
    from enhanced_smart_lead_scorer import BuyingStage, EnhancedSmartLeadScorer, LeadPriority, LeadScoreBreakdown
        ...
        try:
            ...
            return {
                "voice_ai": {
                    ...
                    "qualification_rate": (s_data["temp_breakdown"]["hot"] / s_data["total_interactions"] * 100)
                    if s_data["total_interactions"] > 0
                    else 0,
                    "transfer_rate": 12.8,
                },
                "marketing": {
                ...
                "integration_health": {"ghl_status": "healthy", "claude_status": "healthy", "overall_uptime": 99.2},
    ...
    async def analyze_lead(self, lead_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommended_actions": [
                ...
                "🏠 Curate 3-5 property matches",
            ],
            "talking_points": [
                "🎯 'Based on your search...'",
                "💰 'With your budget...'",
                "🏫 'I know the best neighborhoods...'",
            ],
...
def render_jorge_seller_bot_section(api_client: JorgeAPIClient):
    ...
    with col1:
        ...
        user_msg = st.text_area(
            "Seller Message", "I'll get back to you next week, I'm pretty busy right now.", key="seller_msg_input"
        )
    ...
    with col2:
        if "jorge_seller_result" in st.session_state:
            ...
            st.markdown(
                f'<div class="elite-card" style="padding: 1.5rem; border-top: 2px solid var(--negotiation-neon) !important;">',
...
def render_voice_ai_section(api_client: JorgeAPIClient):
    ...
    with col1:
        ...
        phone_number = st.text_input(
            "Phone Number", placeholder="+1 (555) 123-4567", help="Start an AI-powered lead qualification call"
        )

        caller_name = st.text_input("Caller Name (Optional)", placeholder="John Smith")
    ...
    with col2:
        ...
        with col2_1:
            st.metric("Qualified Leads", voice_data["qualified_leads"], delta="+5 this week")
            st.metric("Avg Score", f"{voice_data['avg_qualification_score']:.1f}%", delta="+3.2%")

        with col2_2:
            st.metric("Total Calls", voice_data["total_calls"], delta="+12 today")
            st.metric(
                "Success Rate",
        ...
        # Call outcomes chart
        outcomes_df = pd.DataFrame(list(voice_data["call_outcomes"].items()), columns=["Outcome", "Count"])
...
def render_followup_orchestrator(api_client: JorgeAPIClient):
    ...
    st.markdown(
        f"### {get_svg_icon('intelligence')} Ghost-in-the-Machine // Follow-Up Orchestrator", unsafe_allow_html=True
    )
    ...
    with col1:
        ...
        simulation_step = st.select_slider(
            "Current Sequence Position", options=["initial", "day_3", "day_7", "day_14", "day_30"], value="day_7"
        )
    ...
    with col2:
        ...
        with perf_col1:
            ...
            fig.add_trace(
                go.Bar(
                    x=touchpoint_data["Touchpoint"],
                    y=touchpoint_data["Engagement"],
                    name="Engagement %",
                    marker_color="#4ecdc4",
                )
            )
            fig.add_trace(
                go.Bar(
                    x=touchpoint_data["Touchpoint"],
                    y=touchpoint_data["Response"],
                    name="Response %",
                    marker_color="#ff6b6b",
                )
            )
            fig.update_layout(
                height=250, margin=dict(l=0, r=0, t=30, b=0), barmode="group", title="Engagement vs Response"
            )
            st.plotly_chart(style_obsidian_chart(fig), use_container_width=True)
        ...
        if sim_result and "response_content" in sim_result:
            ...
            if simulation_step == "day_3":
                ...
                st.info(
                    "💬 'Hi {Name}—just following up on your property. No pressure, but we have qualified buyers interested...'"
                )
            elif simulation_step == "day_7":
                st.markdown("**Channel:** Voice AI // **Tone:** Jorge Stall-Breaker")
                st.warning(
                    "🔊 'Hey {Name}, I know I've reached out, but market conditions are shifting. Would 15 mins make sense?'"
                )
...
def render_whisper_mode():
    ...
    with col2:
        st.markdown(
            '<div class="elite-card" style="padding: 1.5rem; border-top: 2px solid var(--negotiation-neon) !important;">',
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f'<div class="terminal-line"><span class="terminal-timestamp">[00:05]</span> <span style="color: #8B949E;">Jorge:</span> "Hey Sarah, I saw you were looking at 123 Maple St. What caught your eye?"</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="terminal-line"><span class="terminal-timestamp">[00:12]</span> <span style="color: #6366F1;">Sarah:</span> "Yeah, the kitchen is nice, but Zillow says it\'s overpriced for the area..."</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div class="terminal-line" style="border-right: 2px solid var(--negotiation-neon); width: fit-content; animation: blink 0.5s step-end infinite;">_</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_marketing_section():
    ...
    with col1:
        ...
        trigger_type = st.selectbox(
            "Campaign Trigger", ["NEW_LEAD", "LEAD_SCORE_THRESHOLD", "LIFECYCLE_STAGE", "MARKET_OPPORTUNITY"]
        )
    ...
    with col2:
        ...
        for camp in campaigns:
            with st.container():
                ...
                with col_status:
                    st.write("🟢" if camp["status"] == "Active" else "🟡")
                with col_metrics:
                    st.write(f"{camp['leads']} leads")
                st.divider()
        st.markdown("</div>", unsafe_allow_html=True)
...
def render_property_digital_twin(address: str):
    ...
    st.markdown(
        f"<div style=\"text-align: center; font-family: 'Space Grotesk'; font-size: 0.7rem; color: var(--elite-accent); letter-spacing: 0.2em; margin-bottom: 0.5rem;\">3D_DIGITAL_TWIN_ACTIVE</div>",
        unsafe_allow_html=True,
    )
    components.html(html_content, height=350)
    st.markdown(
        f'<div style="text-align: center; color: var(--text-secondary); font-size: 0.6rem;">{address.upper()} // INTERACTIVE NEURAL MESH</div>',
        unsafe_allow_html=True,
    )
...
def render_market_prediction_section():
    ...
    with col1:
        ...
        neighborhood = st.selectbox("Neighborhood", ["Rancho Cucamonga", "Upland", "Ontario", "Claremont", "Pomona"])
    ...
    with col2:
        st.markdown(
            '<div class="elite-card" style="padding: 1.5rem; display: flex; flex-direction: column; align-items: center; justify-content: center;">',
            unsafe_allow_html=True,
        )
...
def render_integration_dashboard(api_client: JorgeAPIClient):
    ...
    with col1:
        st.metric("🔴 Immediate", scoring_analytics["priority_breakdown"]["immediate"])
    with col2:
        st.metric("🟠 High", scoring_analytics["priority_breakdown"]["high"])
    with col3:
        st.metric("📊 Avg Score", f"{scoring_analytics['avg_score']:.1f}/100")
    with col4:
        st.metric("🎯 Accuracy", f"{scoring_analytics['accuracy_metrics']['prediction_accuracy']:.1f}%")
    with col5:
        st.metric("🧠 Analyzed", f"{scoring_analytics['total_leads_scored']:,}")
    ...
    with col2:
        if "latest_analysis" in st.session_state:
            ...
            if "intent_profile" in analysis:
                ...
                with c1:
                    st.metric("Motivation", f"{frs['motivation']['score']}%")
                with c2:
                    st.metric("Timeline", f"{frs['timeline']['score']}%")
                with c3:
                    st.metric("Condition", f"{frs['condition']['score']}%")
                with c4:
                    st.metric("Price", f"{frs['price']['score']}%")
...
def render_property_matching_integration_section():
    ...
    try:
        from ghl_real_estate_ai.streamlit_demo.components.jorge_property_matching_dashboard import (
...
def render_analytics_integration_section():
    ...
    try:
        from ghl_real_estate_ai.streamlit_demo.components.jorge_analytics_dashboard import (
...
def render_lead_pipeline_section(api_client: JorgeAPIClient):
    ...
    if (
        ...
        and current_time - st.session_state[cache_timestamp_key] < cache_duration
    ):
    ...
    with col_list:
        ...
        for lead in pipeline:
            with st.container():
                st.markdown(
                    f'<div class="elite-card" style="padding: 1.25rem; margin-bottom: 1rem;">', unsafe_allow_html=True
                )
                ...
                with col2:
                    ...
                    temp = (
                        "hot" if lead["priority"] == "immediate" else "warm" if lead["priority"] == "high" else "cold"
                    )
                ...
                with col3:
                    st.markdown(
                        f"<div style='text-align: right;'><span style='font-size: 1.2rem; font-weight: bold; color: var(--elite-accent);'>{lead['score']}%</span><br><span style='font-size: 0.7rem; color: var(--text-secondary);'>{lead['priority'].upper()}</span></div>",
                        unsafe_allow_html=True,
                    )
    ...
    with col_details:
        ...
        if lead_id:
            ...
            if lead:
                st.markdown(
                    f'<div class="elite-card" style="padding: 1.5rem; border-left: 4px solid var(--elite-accent) !important;">',
                    unsafe_allow_html=True,
                )
...
def render_phase1_intent_analysis(api_client: JorgeAPIClient):
    ...
    with col1:
        ...
        user_input = st.text_area(
            "Lead Message",
            "I need to sell fast because I'm relocating for a job transfer. In 30 days. It needs work, selling as-is.",
            height=100,
        )
    ...
    with col2:
        if "phase1_analysis" in st.session_state:
            ...
            if "cma_report_url" in st.session_state:
                st.markdown(
                    f'<a href="{st.session_state["cma_report_url"]}" target="_blank" style="text-decoration: none;"><div style="padding: 10px; background: var(--elite-accent); color: black; text-align: center; border-radius: 8px; font-weight: bold;">📥 DOWNLOAD ZILLOW-DEFENSE PDF</div></a>',
                    unsafe_allow_html=True,
                )
...
def render_war_room_section():
    ...
    with col1:
        st.metric("Active Leads", data["market_summary"]["total_active_leads"], delta="+12%")
    with col2:
        st.metric("Hottest Area", data["market_summary"]["hottest_area"])
    with col3:
        st.metric("Avg Heat Value", f"{data['market_summary']['avg_market_heat']}/100", delta="3.2")
    with col4:
        st.metric("MOAT Status", "SECURE", help="Intelligence Layer Integrity")
...
def render_jorge_lead_bot_dashboard():
    ...
    with tab2:
        render_lead_pipeline_section(api_client)
    with tab3:
        render_voice_ai_section(api_client)
    with tab4:
        render_jorge_seller_bot_section(api_client)
    with tab5:
        render_followup_orchestrator(api_client)
    with tab6:
    ...
    with tab10:
        render_phase1_intent_analysis(api_client)
    with tab11:
        render_war_room_section()
    with tab12:
        render_whisper_mode()
