...
def render_interactive_lead_map(leads_data: List[Dict[str, Any]], market: str='Austin'):
    ...
    with col_controls:
        ...
        view_mode = st.selectbox('View Mode:', ['Markers', 'Heat Map', 'Both'], key='map_view_selector', label_visibility='collapsed', help='Switch between marker view, heat map, or combined view')
    ...
    map_config = {'Austin': {'center_lat': 30.2672, 'center_lon': -97.7431, 'zoom': 11}, 'Rancho': {'center_lat': 34.12, 'center_lon': -117.57, 'zoom': 12}}
...
def create_plotly_map(leads_data: List[Dict[str, Any]], config: Dict[str, Any]) -> go.Figure:
    ...
    for lead in leads_data:
        ...
        hover_text = f'<b>{name}</b><br>Score: {score}<br>Budget: ${budget:,}<br>Area: {location}<br><i>Click for details</i>'
        texts.append(hover_text)
        lead_ids.append(lead.get('id', ''))
    fig = go.Figure()
    fig.add_trace(go.Scattermapbox(lat=lats, lon=lons, mode='markers', marker=dict(size=sizes, color=colors, opacity=0.9, sizemode='diameter'), text=texts, hovertemplate='%{text}<extra></extra>', customdata=lead_ids, name='Leads'))
    fig.update_layout(mapbox=dict(style='open-street-map', center=dict(lat=config['center_lat'], lon=config['center_lon']), zoom=config['zoom']), height=500, margin=dict(l=0, r=0, t=0, b=0), hovermode='closest', showlegend=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    return fig
...
def create_heat_map(leads_data: List[Dict[str, Any]], config: Dict[str, Any]) -> go.Figure:
    ...
    for lead in leads_data:
        ...
        heat_points.append({'lat': lat, 'lon': lon, 'intensity': heat_intensity, 'name': lead.get('name', 'Unknown'), 'score': score, 'budget': budget})
    fig = go.Figure()
    lats = [p['lat'] for p in heat_points]
    lons = [p['lon'] for p in heat_points]
    intensities = [p['intensity'] for p in heat_points]
    fig.add_trace(go.Densitymapbox(lat=lats, lon=lons, z=intensities, radius=30, colorscale=[[0, '#3b82f6'], [0.3, '#60a5fa'], [0.5, '#f59e0b'], [0.7, '#fb923c'], [1, '#ef4444']], opacity=0.6, hovertemplate='<b>Activity Density</b><br>Intensity: %{z:.2f}<extra></extra>', showscale=True, colorbar=dict(title='Activity<br>Level', thickness=15, len=0.7, x=1.02, tickvals=[0, 1, 2, 3], ticktext=['Low', 'Medium', 'High', 'Very High'])))
    fig.update_layout(mapbox=dict(style='open-street-map', center=dict(lat=config['center_lat'], lon=config['center_lon']), zoom=config['zoom']), height=500, margin=dict(l=0, r=0, t=0, b=0), hovermode='closest', showlegend=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    return fig
...
def create_combined_map(leads_data: List[Dict[str, Any]], config: Dict[str, Any]) -> go.Figure:
    ...
    for lead in leads_data:
        ...
        hover_text = f'<b>{name}</b><br>Score: {score}<br>Budget: ${budget:,}<br>Area: {location}<br><i>Click for details</i>'
        texts.append(hover_text)
    fig.add_trace(go.Scattermapbox(lat=lats, lon=lons, mode='markers', marker=dict(size=sizes, color=colors, opacity=0.9, sizemode='diameter'), text=texts, hovertemplate='%{text}<extra></extra>', name='Leads'))
    return fig
...
def render_lead_analysis(lead: Dict[str, Any]):
    ...
    activities = lead.get('activities', [{'time': '2 hours ago', 'action': 'Viewed property listing', 'icon': '👀'}, {'time': '1 day ago', 'action': 'Responded to SMS', 'icon': '💬'}, {'time': '3 days ago', 'action': 'Submitted inquiry form', 'icon': '📝'}])
...
def generate_sample_lead_data(market: str='Austin') -> List[Dict[str, Any]]:
    ...
    if market == 'Rancho':
        return [{'id': 'lead_001', 'name': 'Sarah Johnson', 'lat': 34.12, 'lon': -117.57, 'lead_score': 92, 'engagement_score': 85, 'budget': 1300000, 'location': 'Alta Loma', 'timeline': 'ASAP'}, {'id': 'lead_002', 'name': 'Mike Chen', 'lat': 34.11, 'lon': -117.58, 'lead_score': 65, 'engagement_score': 45, 'budget': 700000, 'location': 'Victoria Gardens', 'timeline': '6 months'}, {'id': 'lead_003', 'name': 'Emily Davis', 'lat': 34.1, 'lon': -117.56, 'lead_score': 42, 'engagement_score': 30, 'budget': 1000000, 'location': 'Rancho Cucamonga', 'timeline': 'Exploring'}]
    else:
        return [{'id': 'lead_001', 'name': 'Sarah Johnson', 'lat': 30.2672, 'lon': -97.7431, 'lead_score': 92, 'engagement_score': 85, 'budget': 800000, 'location': 'Downtown', 'timeline': 'ASAP'}, {'id': 'lead_002', 'name': 'Mike Chen', 'lat': 30.27, 'lon': -97.75, 'lead_score': 65, 'engagement_score': 45, 'budget': 450000, 'location': 'East Austin', 'timeline': '6 months'}, {'id': 'lead_003', 'name': 'Emily Davis', 'lat': 30.25, 'lon': -97.73, 'lead_score': 42, 'engagement_score': 30, 'budget': 300000, 'location': 'South Austin', 'timeline': 'Exploring'}]
...
def generate_lead_insights(lead: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    engagement = lead.get('engagement_score', 0)
    if score >= 80:
        insights.append({'icon': '🔥', 'text': 'High-priority lead! Strong buying signals detected.', 'type': 'success'})
    elif score >= 50:
        insights.append({'icon': '🎯', 'text': 'Qualified lead with moderate interest. Follow-up recommended.', 'type': 'info'})
    else:
        insights.append({'icon': '🌱', 'text': 'Early-stage lead. Nurture with educational content.', 'type': 'warning'})
    if budget >= 1000000:
        insights.append({'icon': '💎', 'text': f'Luxury buyer segment (${budget:,}). Show premium listings.', 'type': 'success'})
    elif budget > 0:
        insights.append({'icon': '💰', 'text': f'Budget confirmed at ${budget:,}. Property matcher active.', 'type': 'info'})
    if 'asap' in timeline or 'now' in timeline or '30' in timeline:
        insights.append({'icon': '⏰', 'text': 'Urgent timeline detected. Prioritize immediate showing.', 'type': 'warning'})
    if engagement >= 70:
        insights.append({'icon': '📈', 'text': f'High engagement ({engagement}%). Lead is actively searching.', 'type': 'success'})
    return insights
...
def generate_recommended_actions(lead: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    if score >= 80:
        actions.extend([{'id': 'call_now', 'label': 'Call Now', 'icon': '📞', 'toast': f'Calling {name}...'}, {'id': 'schedule_showing', 'label': 'Schedule Showing', 'icon': '📅', 'toast': 'Opening calendar...'}])
    actions.extend([{'id': 'sync_crm', 'label': 'Sync to CRM', 'icon': '🔄', 'toast': 'Syncing lead data to GoHighLevel...'}, {'id': 'send_properties', 'label': 'Send Property Matches', 'icon': '🏠', 'toast': f'Sending top 3 matches to {name}'}, {'id': 'send_sms', 'label': 'Send Follow-Up SMS', 'icon': '💬', 'toast': 'SMS queued in GHL'}, {'id': 'view_conversation', 'label': 'View Full Conversation', 'icon': '💭', 'toast': 'Loading conversation history...'}])
    return actions