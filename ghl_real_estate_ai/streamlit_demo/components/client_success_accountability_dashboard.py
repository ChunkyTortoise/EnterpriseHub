...
from ...services.client_success_scoring_service import (
...
def render_public_report_card(report, dashboard_data):
    ...
    """.format(
        f"{metrics['agent_success_score']:.0f}/100",
...
def render_client_roi_view(agent_id: str):
    ...
    with col4:
        st.metric(
            "Time Value",
            f"${roi_report.time_savings_value:,.0f}",
            help="Value of time savings from efficient process"
        )
...
def render_pricing_justification_view(agent_id: str):
    ...
    for factor_name, factor_data in justification["value_factors"].items():
        with st.expander(f"📊 {factor_name.replace('_', ' ').title()}", expanded=True):
            ...
            with col1:
                st.metric(
                    "Your Performance",
                    f"{factor_data['agent_performance']:.3f}",
                    help=factor_data['description']
                )
            
            with col2:
                st.metric(
                    "Market Average",
                    f"{factor_data['market_average']:.3f}"
                )
            
            with col3:
                advantage = factor_data.get('value_add') or factor_data.get('days_saved') or factor_data.get('quality_advantage', 0)
                st.metric(
                    "Your Advantage",
                    f"+{advantage:.3f}",
                    delta_color="normal"
    ...
    with col1:
        st.metric(
            "Additional Fee",
            f"${roi_data['additional_fee']:,.0f}",
            help="Extra cost for premium service"
        )
    
    with col2:
        st.metric(
            "Negotiation Value",
            f"${roi_data['negotiation_value']:,.0f}",
            help="Value from superior negotiation"
        )
    
    with col3:
        st.metric(
            "Time Value",
            f"${roi_data['time_value']:,.0f}",
            help="Value of time savings"
        )
...
def create_performance_trends_chart(report):
    ...
    dates = pd.date_range(
        start=report.report_period[0],
        end=report.report_period[1],
        freq='W'
    )
    ...
    # Add traces
    fig.add_trace(
        go.Scatter(x=dates, y=negotiation_trend, name="Negotiation %", line=dict(color='red')),
        row=1, col=1
    )
    
    fig.add_trace(
        go.Scatter(x=dates, y=satisfaction_trend, name="Satisfaction", line=dict(color='blue')),
        row=1, col=2
    )
    
    fig.add_trace(
        go.Scatter(x=dates, y=timeline_trend, name="Days to Close", line=dict(color='green')),
        row=2, col=1
    )
    
    # Calculate overall score trend
    overall_trend = [(n*20 + s*20 + (30-t)*3.33) for n, s, t in zip(negotiation_trend, satisfaction_trend, timeline_trend)]
    fig.add_trace(
        go.Scatter(x=dates, y=overall_trend, name="Overall Score", line=dict(color='purple')),
        row=2, col=2
    )
