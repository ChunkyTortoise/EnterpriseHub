        ...
        debates = [
            {
                ...
                "thought": "Lead sounds slightly frustrated with current interest rates.",
                "action_proposed": "Use empathetic approach",
                "confidence": 0.85,
            },
            {
                "timestamp": "2026-01-21T10:00:05",
                "agent": "scheduling_agent",
                "thought": "Qualification score is 6. Lead is ready for a tour.",
                "action_proposed": "Propose 3 tour slots",
                "confidence": 0.92,
            },
            {
...
def render_swarm_visualizer(lead_name: str = None, lead_data: dict = None):
    ...
    if lead_name and lead_name != "-- Select a Lead --":
        ...
        swarm_agents = [
            {"name": "CSO (Orchestrator)", "role": "Strategic Synthesis", "status": "thinking", "type": "Lead"},
            {"name": "Financial Analyst", "role": "Liquidity Assessment", "status": "active", "type": "Lead"},
            {"name": "Timeline Assessor", "role": "Urgency Modeling", "status": "active", "type": "Lead"},
            {"name": "Behavioral Psych", "role": "Lifestyle DNA", "status": "thinking", "type": "Lead"},
            {"name": "Risk Analyst", "role": "Barrier Detection", "status": "idle", "type": "Lead"},
            {"name": "Negotiation Engine", "role": "Counter-Strategy", "status": "idle", "type": "Lead"},
        ]
        G = nx.Graph()
        ...
        for ca in claude_agents:
            all_display_agents.append(
                {"name": ca["name"], "role": ca.get("role", "Specialist"), "status": "Idle", "type": "Claude"}
            )
        if not all_display_agents:
            all_display_agents = [
                {
                    "name": f"Agent-{i:02d}",
                    "role": random.choice(["Orchestrator", "Analyst", "Closer"]),
                    "status": "Idle",
                }
                for i in range(10)
            ]
        ...
        edge_trace = go.Scatter(
            x=edge_x, y=edge_y, line=dict(width=1, color="rgba(100, 116, 139, 0.3)"), hoverinfo="none", mode="lines"
        )
        node_x = []
        node_y = []
        for node in G.nodes():
        ...
        node_trace = go.Scatter(
            ...
            marker=dict(showscale=False, color=node_color, size=25, line=dict(width=2, color="white")),
        )
        fig = go.Figure(
        ...
        if tasks:
            ...
            st.dataframe(
                df_tasks.style.applymap(color_status, subset=["Status"]), use_container_width=True, hide_index=True
            )
    with col2:
