...
class CustomerSegmentationDashboard:
    ...
    def _render_sidebar_controls(self):
        ...
        segmentation_method = st.radio(
            "Choose Method", ["RFM Analysis", "Behavioral Clustering", "CLV-Based", "AI-Powered"]
        )
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    y=segment_clvs,
                    marker=dict(
                        color=segment_clvs, colorscale="Viridis", showscale=True, colorbar=dict(title="CLV ($)")
                    ),
    ...
    def _render_segment_recommendations(self, segment_name: str):
        ...
        segment_recs = recommendations.get(
            segment_name,
            [
...
if __name__ == "__main__":
    st.set_page_config(
        page_title="Customer Segmentation Dashboard", page_icon="👥", layout="wide", initial_sidebar_state="expanded"
    )
