...
class WorkflowDesignerComponent:
    ...
    def __init__(self):
        self.action_blocks = {
            "📱 Send SMS": {"action": "send_sms", "fields": ["message"], "icon": "📱", "color": "#4CAF50"},
            "📧 Send Email": {"action": "send_email", "fields": ["subject", "body"], "icon": "📧", "color": "#2196F3"},
            "⏰ Wait": {"action": "wait", "fields": ["delay_minutes"], "icon": "⏰", "color": "#FF9800"},
            "🏷️ Add Tag": {"action": "add_tag", "fields": ["tag"], "icon": "🏷️", "color": "#9C27B0"},
            "👤 Assign Agent": {"action": "assign_agent", "fields": ["agent_id"], "icon": "👤", "color": "#F44336"},
            "🔔 Notify": {
                "action": "notify_agent",
    ...
    def render_designer(self) -> Dict[str, Any]:
        ...
        with col2:
            workflow_trigger = st.selectbox(
                "Trigger Event",
                ["lead_created", "lead_updated", "no_response_48h", "appointment_scheduled", "property_viewed"],
            )
    ...
    def _render_step_config(self, idx: int, step: Dict[str, Any]):
        ...
        # Delay
        delay = st.number_input("Delay (minutes)", min_value=0, value=step.get("delay_minutes", 0), key=f"delay_{idx}")
        ...
        if action == "send_sms":
            ...
            subject = st.text_input("Subject", value=step["data"].get("subject", ""), key=f"subject_{idx}")
            body = st.text_area("Body", value=step["data"].get("body", ""), key=f"body_{idx}", height=150)
            ...
            tag = st.text_input("Tag Name", value=step["data"].get("tag", ""), key=f"tag_{idx}")
            step["data"]["tag"] = tag

        elif action == "assign_agent":
            agent = st.selectbox("Agent", ["Auto-assign", "Agent 1", "Agent 2", "Agent 3"], key=f"agent_{idx}")
            step["data"]["agent_id"] = agent

        elif action == "notify_agent":
            message = st.text_input("Notification Message", value=step["data"].get("message", ""), key=f"notify_{idx}")
            priority = st.select_slider("Priority", options=["low", "medium", "high", "urgent"], key=f"priority_{idx}")
            step["data"]["message"] = message
            ...
            a_type = st.selectbox(
                "Analysis Type",
                ["Lifestyle Alignment", "Investment Yield", "Churn Risk", "Sentiment"],
                key=f"atype_{idx}",
            )
    ...
    def _build_workflow(self, name: str, trigger: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "trigger": trigger,
            "steps": [
                {"order": idx + 1, "action": step["action"], "delay_minutes": step["delay_minutes"], **step["data"]}
                for idx, step in enumerate(st.session_state.workflow_steps)
            ],
