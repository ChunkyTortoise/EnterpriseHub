...
def render_marketplace_management():
    ...
    with tab2:
        ...
        governance_history = [
            {"time": "10:45 AM", "skill": "Zillow Trend Predictor", "action": "APPROVED", "reason": "Audit ID Verified"},
            {"time": "09:30 AM", "skill": "Unverified Scraper", "action": "REJECTED", "reason": "Missing Security Audit ID"},
            {"time": "08:15 AM", "skill": "PII Harvester", "action": "BLOCKED", "reason": "Malicious patterns detected in scan"}
        ]
        
        for action in governance_history:
            color = "green" if action['action'] == "APPROVED" else "red"
            st.markdown(f"**{action['time']}** | {action['skill']} | <span style='color:{color}'>{action['action']}</span> - {action['reason']}", unsafe_allow_html=True)
