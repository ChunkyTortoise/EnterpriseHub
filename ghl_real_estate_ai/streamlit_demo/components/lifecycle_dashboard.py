...
def render_full_lifecycle_dashboard():
    ...
    with col_sel1:
        selected_label = st.selectbox("Select Lead to View Lifecycle:", list(lead_options.keys()), index=0)
    ...
    for i, stage in enumerate(stages):
        with cols[i]:
            ...
            bg_color = (
                "rgba(16, 185, 129, 0.1)" if is_past else "rgba(99, 102, 241, 0.1)" if is_current else "transparent"
            )
    ...
    with col_log:
        ...
        if not events:
            ...
            events = [{"time": "Now", "source": "System", "event": "Lifecycle monitoring activated.", "type": "action"}]
