...
# Page configuration
st.set_page_config(
    page_title="Market Intelligence - Phase 7",
    page_icon="📊",
    layout="wide",
...
class MarketIntelligenceDashboard:
    ...
    def _render_header(self):
        ...
        with col2:
            st.metric(
                ...
                value="91.2%",
                delta="7-14 day prediction"
            )

        with col3:
            st.metric(
                label="🎯 Jorge's Market Share",
                value="12.5%",
                delta="+ 1.8% YoY growth"
            )
    ...
    def _render_sidebar(self):
        ...
        st.sidebar.selectbox(
            "📍 Geographic Focus",
            ["Austin Metro", "Central Austin", "North Austin", "South Austin", "West Austin", "Cedar Park", "Round Rock"],
            index=0
        )
    ...
    def _generate_market_data(self):
        ...
        competitors = {
            'Jorge Real Estate AI': {'market_share': 12.5, 'growth_rate': 0.18, 'commission_avg': 6.0, 'technology_score': 95},
            'Traditional Agent A': {'market_share': 15.2, 'growth_rate': 0.03, 'commission_avg': 5.5, 'technology_score': 45},
            'Discount Broker B': {'market_share': 18.7, 'growth_rate': 0.12, 'commission_avg': 3.0, 'technology_score': 60},
            'Premium Service C': {'market_share': 8.9, 'growth_rate': -0.05, 'commission_avg': 6.5, 'technology_score': 70},
    ...
    def _render_market_alerts(self, data):
        ...
        for alert in alerts:
            severity_color = "error" if alert['severity'] == 'critical' else "warning" if alert['severity'] == 'high' else "info"
    ...
    def _render_trend_detection(self, data):
        ...
        # Median price
        fig.add_trace(
            go.Scatter(x=trend_data['dates'], y=trend_data['median_price'],
                      mode='lines', name='Median Price', line=dict(color='#3b82f6')),
            row=1, col=1
        )

        # Inventory levels
        fig.add_trace(
            go.Scatter(x=trend_data['dates'], y=trend_data['inventory_levels'],
                      mode='lines', name='Inventory (Months)', line=dict(color='#ef4444')),
            row=1, col=2
        )

        # Buyer activity
        fig.add_trace(
            go.Scatter(x=trend_data['dates'], y=trend_data['buyer_activity'],
                      mode='lines', name='Buyer Activity', line=dict(color='#10b981')),
            row=2, col=1
        )

        # Jorge market share
        fig.add_trace(
            go.Scatter(x=trend_data['dates'], y=trend_data['jorge_market_share'],
                      mode='lines', name='Market Share %', line=dict(color='#8b5cf6')),
            row=2, col=2
        )
    ...
    def _render_inventory_analysis(self, data):
        ...
        fig = go.Figure(data=[
            go.Bar(x=price_segments, y=inventory_data,
