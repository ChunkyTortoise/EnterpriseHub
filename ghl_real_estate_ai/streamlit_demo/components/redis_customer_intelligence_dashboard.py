...
try:
    ...
    class StreamlitAuthenticator:
        def check_authentication(self): return True
        def require_permission(self, perm): return lambda f: f
...
class RedisCustomerIntelligenceDashboard:
        ...
        try:
            ...
            with col3:
                ...
                st.metric(
                    "Active Customers",
                    f"{total_customers:,}",
                    delta="12" if total_customers > 0 else None
                )
            ...
            with col4:
                ...
                st.metric(
                    "Last Update",
                    latest_update.split('T')[1][:8] if latest_update != "N/A" else "N/A"
                )
            ...
            recent_metrics = df.nlargest(20, 'timestamp')[
                ['customer_id', 'metric_type', 'value', 'timestamp']
            ].copy()
    ...
    def _render_metrics_charts(self, df: pd.DataFrame, selected_metrics: List[str]):
        ...
        fig.update_layout(
            height=600,
            showlegend=True,
            title_text="📈 Real-Time Metrics Trends",
            title_x=0.5
        )
    ...
    def _render_customer_segmentation(self):
        ...
        selected_segments = getattr(st.session_state, 'selected_segments', 
                                   ['high_value', 'growth_potential'])
        ...
        try:
            ...
            with col2:
                ...
                fig_box.update_layout(
                    title="Segment Score Distributions",
                    yaxis_title="Score",
                    xaxis_title="Segment Type"
                )
            ...
            segment_stats = df.groupby('segment_type').agg({
                'segment_score': ['mean', 'std', 'count'],
                'confidence': 'mean'
            }).round(3)
    ...
    def _render_journey_mapping(self):
        ...
        selected_stages = getattr(st.session_state, 'selected_stages', 
        ...
        try:
            ...
            with col1:
                ...
                fig_bar.update_layout(
                    title="Customers by Journey Stage",
            ...
            with col2:
                ...
                avg_conversion = df.groupby('current_stage')['conversion_probability'].mean().sort_values(ascending=False)
            ...
            if len(journey_flow) > 0:
                ...
                stages = list(set(journey_flow['current_stage'].tolist() + journey_flow['predicted_next_stage'].tolist()))
                ...
                fig_sankey = go.Figure(data=[go.Sankey(
                    ...
                    link=dict(
                        ...
                        target=[stage_indices[row['predicted_next_stage']] for _, row in journey_flow.iterrows()],
            ...
            if all_bottlenecks:
                ...
                fig_bottlenecks = go.Figure(data=[
                    go.Bar(
        ...
        try:
            ...
            if 'next_best_actions' in predictions_data:
                ...
                if nba_data:
                    ...
                    top_recommendations['expected_impact'] = top_recommendations['expected_impact'].apply(lambda x: f"{x:.2%}")
    ...
    def _render_system_health(self):
        ...
        try:
            ...
            log_events = [
                ...
                {"Timestamp": (datetime.now() - timedelta(minutes=2)).strftime("%H:%M:%S"), "Event": "Cache updated", "Level": "INFO"},
                {"Timestamp": (datetime.now() - timedelta(minutes=5)).strftime("%H:%M:%S"), "Event": "Health check passed", "Level": "INFO"},
                {"Timestamp": (datetime.now() - timedelta(minutes=8)).strftime("%H:%M:%S"), "Event": "New customer data received", "Level": "INFO"},
                {"Timestamp": (datetime.now() - timedelta(minutes=12)).strftime("%H:%M:%S"), "Event": "Analytics computation completed", "Level": "INFO"},
            ]
