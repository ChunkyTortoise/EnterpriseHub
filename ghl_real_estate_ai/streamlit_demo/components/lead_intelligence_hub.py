...
try:
    ...
    from ghl_real_estate_ai.streamlit_demo.components.elite_refinements import (
...
def render_lead_intelligence_hub(services, mock_data, claude, market_key, selected_market, elite_mode=False):
    ...
    if elite_mode:
        if CLAUDE_SERVICES_AVAILABLE and claude_services:
            st.success(
                "🧠 **Claude AI Integration Active**: Real-time conversation intelligence, semantic property matching, and autonomous qualification enabled."
            )
        else:
    ...
    with st.expander("🔍 Advanced Lead Filters", expanded=True):
        ...
        with col_filter1:
            conversation_health_filter = st.selectbox(
                "Conversation Health", ["All", "Excellent", "Good", "Concerning", "Poor"], key="health_filter"
            )
        with col_filter2:
        ...
        with col_filter3:
            closing_readiness_filter = st.selectbox(
                "Closing Readiness", ["All", "High", "Medium", "Low"], key="closing_filter"
            )
        with col_filter4:
            trust_level_filter = st.selectbox(
                "Trust Level", ["All", "Strong", "Established", "Building", "Weak"], key="trust_filter"
            )
    enhanced_lead_data = {}
    ...
    for lead_name in lead_names[1:]:
        ...
        health_status = {"Excellent": "Optimal", "Good": "Stable", "Concerning": "At Risk", "Poor": "Critical"}.get(
            health, "Unknown"
        )
        emotion_status = {
            "Excited": "High Intent",
            "Analytical": "Deep Research",
            "Cautious": "Refining",
            "Frustrated": "Action Required",
            "Ready": "Closing",
        }.get(emotion, "Discovery")
    ...
    selected_display_name = st.selectbox(
        "Choose a lead to analyze:", lead_display_names, index=default_display_idx, key="hub_lead_selector_enhanced"
    )
            ...
            with st.spinner(f"🧠 Claude is performing multi-dimensional analysis for {selected_lead_name}..."):
                try:
                    analysis_result = run_async(
                        enhanced_intelligence.get_comprehensive_lead_analysis(selected_lead_name, lead_context)
                    )
    ...
    if selected_lead_name != "-- Select a Lead --":
        ...
        analysis_result = None
    st.markdown("---")
    tab1, tab_swarm, tab2, tab3, tab_sentiment, tab4, tab5, tab6, tab7, tab8, tab_journey, tab9, tab10 = st.tabs(
        [
            "Lead Scoring",
            "Agent Swarm",
            "Deep Dossier",
    ...
    with tab1:
        ...
        with col_map:
            try:
                from components.interactive_lead_map import generate_sample_lead_data, render_interactive_lead_map
                ...
                if market_key == "Rancho":
                    map_data = pd.DataFrame(
                        {
                            "lat": [34.12, 34.11, 34.1, 34.13, 34.115],
                            "lon": [-117.57, -117.58, -117.56, -117.59, -117.575],
                    ...
                    map_data = pd.DataFrame(
                        {
                            "lat": [30.2672, 30.27, 30.25, 30.28, 30.26],
                            "lon": [-97.7431, -97.75, -97.73, -97.76, -97.74],
                            "type": ["Hot", "Hot", "Warm", "Cold", "Hot"],
                            "value": [100, 80, 50, 20, 90],
                        }
                    )
                    ...
                    def create_dna_radar(values, categories=categories):
                        ...
                        fig.update_layout(
                            polar=dict(
                                ...
                                angularaxis=dict(
                                    gridcolor="rgba(255,255,255,0.1)", tickfont=dict(size=11, color="#E6EDF3")
                                ),
            ...
            if claude_services.get("conversation") and elite_mode and (selected_lead_name != "-- Select a Lead --"):
                ...
                try:
                    ...
                    auto_refresh_enabled = st.checkbox(
                        "🔄 Live Updates", value=True, help="Auto-refresh conversation intelligence every 30 seconds"
                    )
                    conversation_engine.render_enhanced_intelligence_dashboard(
                        thread_id=thread_id, messages=conversation_history, lead_context=lead_context
                    )
        ...
        if claude_services.get("qualification") and elite_mode and (selected_lead_name != "-- Select a Lead --"):
            ...
            st.info(
                "🎯 **Claude Insight**: This lead shows immediate buying signals with high lifestyle compatibility. Recommend priority agent assignment within 24 hours."
            )
        elif analysis_result:
            ...
            with st.container(border=True):
                ...
                with c_elite1:
                    ...
                    st.progress(
                        analysis_result.frs_score / 100, text=f"Readiness Index: {analysis_result.frs_score:.0f}%"
                    )
                ...
                with c_elite2:
                    ...
                    st.progress(
                        analysis_result.pcs_score / 100, text=f"Commitment Index: {analysis_result.pcs_score:.0f}%"
                    )
            ...
            try:
                ...
                with f_col1:
                    if st.button("👍", key=f"up_{selected_lead_name}"):
                        run_async(
                            rlhf_service.record_feedback(
                                f"trace_{selected_lead_name}", 1, context={"lead": selected_lead_name}
                            )
                        )
        ...
        if analysis_result:
            st.caption(
                f"📊 Claude Confidence: {analysis_result.confidence_score:.0%} | Analysis Time: {analysis_result.analysis_time_ms}ms"
            )
            st.markdown("---")
        ...
        if selected_lead_name != "-- Select a Lead --":
            ...
            with sync_col1:
                st.write(
                    "Push Claude's deep psychological analysis and lifestyle dimensions directly to Jorge's GoHighLevel CRM."
                )
                st.caption(
                    "This enables GHL-native automation based on Trust Scores, Investment Mindsets, and 16+ Lifestyle Dimensions."
                )
            with sync_col2:
                if GHL_SYNC_AVAILABLE:
                    if st.button("🔄 Sync DNA to CRM", type="primary", use_container_width=True):
                        with st.spinner("🧬 Mapping psychological DNA to GHL..."):
                            ...
                            if analysis_result:
                                dna_payload = {
                                    "factors": getattr(analysis_result, "factor_scores", {}),
                                    "dimensions": getattr(analysis_result, "lifestyle_dimensions", {}),
                                    "metadata": {
                                        "confidence": getattr(analysis_result, "confidence_score", 0),
                                        "strategic_summary": getattr(analysis_result, "strategic_summary", ""),
                                    },
                                }
                            else:
                                dna_payload = {
                            ...
                            if sync_result["status"] == "success":
                                st.success(
                                    f"✅ DNA Dossier synced! {sync_result['fields_updated']} fields updated in GHL."
                                )
                                st.balloons()
                            else:
            ...
            with col_swarm_ctrl:
                        ...
                        if results_key in st.session_state:
                            ...
                            with st.container(border=True):
                                ...
                                with col_syn2:
                                    ...
                                    st.warning(
                                        f"**Bottleneck:** {synthesis.get('conversion_bottleneck', 'None detected')}"
                                    )
                ...
                with c4:
                    ...
                    if "error" in risk:
                        ...
                        risk_color = (
                            "#ef4444" if "High" in risk_level else "#f59e0b" if "Medium" in risk_level else "#10b981"
                        )
                        render_obsidian_card(
                            title="Risk Analyst",
                            content=f"""
    ...
    with tab3:
        try:
            ...
            st.info(
                "💡 The full Churn Prediction Engine provides 26 behavioral features, multi-horizon risk scoring, and automated intervention orchestration."
            )
    with tab_sentiment:
        ...
        if selected_lead_name == "-- Select a Lead --":
            ...
            mock_history = [
                ...
                {
                    "role": "assistant",
                    "content": "It can be fast-paced! What specifically is concerning you?",
            ...
            with st.spinner("🧠 Analyzing conversation sentiment drift..."):
                try:
                    ...
                    with col_drift1:
                        ...
                        st.metric(
                            "Sentiment Drift", f"{drift_score:.2f}", delta=f"{drift_score:.2f}", delta_color=drift_color
                        )

                        # Gauge for sentiment
                        current_sentiment = drift_result.get("recent_sentiment", 0.0)
                        st.markdown(
                            f"**Current Sentiment Node:** {'🔴' if current_sentiment < 0 else '🟡' if current_sentiment < 0.5 else '🟢'}"
                        )
                    ...
                    with col_drift2:
                        if drift_result.get("alert") == "COLD_LEAD":
                            ...
                            re_engage_msg = sentiment_engine.get_reengagement_message(
                                selected_lead_name.split(" ")[0], drift_result.get("objection_hint")
                            )
                    ...
                    def create_sentiment_traj(df):
                        fig_traj = px.line(
                            df, x="Message", y="Sentiment", title="Rolling Sentiment (Window: 3)", markers=True
                        )
                        fig_traj.add_hline(y=0, line_dash="dash", line_color="white", opacity=0.3)
                        return fig_traj

                    plotly_optimizer.render_chart(
                        create_sentiment_traj, traj_data, key=f"sentiment_traj_{selected_lead_name}"
                    )
    ...
    with tab4:
        ...
        if selected_lead_name == "-- Select a Lead --":
            ...
            try:
                ...
                with col_trans1:
                    ...
                    st.success(
                        f"**Predicted Next Stage:** {transitions.get('predicted_next_stage', 'Unknown').title()}"
                    )
        ...
        try:
            ...
            if selected_lead_name != "-- Select a Lead --":
                ...
                must_haves = lead_context.get("extracted_preferences", {}).get(
                    "must_haves", ["swimming pool", "3-car garage", "updated kitchen", "good schools"]
                )
                if must_haves:
        ...
        if elite_mode:
            ...
            try:
                ...
                if leads_for_segmentation:
                    result = run_async(
                        services["segmentation"].segment_leads(leads_for_segmentation, method="behavioral")
                    )
                    if result["segments"]:
...
def render_real_time_conversation_coach(claude_services, lead_context, selected_lead_name):
    ...
    if analytics_key not in st.session_state:
        st.session_state[analytics_key] = {
    ...
    with col_analytics2:
        ...
        st.metric(
            "Buying Intent",
            f"{intent_level:.0%}",
            delta="+10%" if len(coaching_analytics["intent_trajectory"]) > 1 else None,
        )
    with col_analytics3:
        emotional_state = coaching_analytics["emotional_state"]
        emotion_emoji = {"curious": "🤔", "excited": "😊", "concerned": "😟", "analytical": "🧐", "ready": "🎯"}.get(
            emotional_state, "😐"
        )
    ...
    with col_coaching:
            ...
            with st.spinner("Analyzing thread state..."):
                ...
                for m in coaching_messages:
                    formatted_msgs.append(
                        {"role": "user" if m["role"] == "lead" else "assistant", "content": m["content"]}
                    )
                thread_analysis = run_async(
                    conversation_engine.analyze_conversation_thread(
                        f"coaching_{selected_lead_name}", formatted_msgs, lead_context
                    )
                )
        ...
        if coaching_messages and latest_lead_message:
            ...
            for i, suggestion in enumerate(suggestions, 1):
                if st.button(
                    f"💬 Option {i}",
                    key=f"coaching_suggestion_{i}_{selected_lead_name}",
                    use_container_width=True,
                    help=suggestion,
                ):
                    coaching_messages.append(
                        {"role": "agent", "content": suggestion, "timestamp": datetime.now(), "suggested": True}
                    )
                    st.rerun()
        ...
        if len(coaching_messages) > 2:
            intent_improvement = (
                coaching_analytics["intent_trajectory"][-1] - coaching_analytics["intent_trajectory"][0]
            ) * 100
        ...
        if len(st.session_state.coaching_messages) > 2:
            intent_improvement = (
                st.session_state.coaching_analytics["intent_trajectory"][-1]
                - st.session_state.coaching_analytics["intent_trajectory"][0]
            ) * 100
            st.metric("Intent Improvement", f"+{intent_improvement:.0f}%")
            response_effectiveness = min(
                100, len([m for m in st.session_state.coaching_messages if m["role"] == "agent"]) * 15
            )
            st.metric("Response Effectiveness", f"{response_effectiveness}%")
    ...
    with col_chat:
        ...
        if "conversation_messages" not in st.session_state:
            st.session_state.conversation_messages = [
                {
                    "role": "assistant",
                    "content": f"Hi {selected_lead_name.split(' ')[0]}! I saw you've been looking at properties in the area. What kind of home are you hoping to find?",
                }
            ]
...
def apply_lead_filters(
    enhanced_lead_data: dict, health_filter: str, emotion_filter: str, closing_filter: str, trust_filter: str
) -> dict:
    """
