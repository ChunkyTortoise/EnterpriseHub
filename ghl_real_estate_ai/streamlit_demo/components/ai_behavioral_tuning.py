    ...
    with st.expander(f"🎛️ {phase_name} AI Control Deck", expanded=False):
        ...
        with col2:
            ...
            question_style = st.segmented_control(
                "Inquiry Depth", ["Subtle", "Balanced", "Direct"], default="Balanced", key=f"{phase_name}_qual_style"
            )
        st.markdown("---")
        ...
        with col_save:
            if st.button(
                "🚀 Commit Behavior Changes", use_container_width=True, type="primary", key=f"{phase_name}_save"
            ):
