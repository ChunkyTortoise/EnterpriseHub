...
# Page configuration
st.set_page_config(
    page_title="Conversation Analytics - Phase 7", page_icon="💬", layout="wide", initial_sidebar_state="expanded"
)
...
class ConversationAnalyticsDashboard:
    ...
    def _render_header(self):
        ...
        with col1:
            st.metric(label="🎯 Conversion Rate", value="28.4%", delta="+ 3.2% vs baseline")

        with col2:
            st.metric(label="😊 Avg Sentiment", value="8.3/10", delta="+ 0.7 improvement")

        with col3:
            st.metric(label="🏆 Jorge Method Score", value="94.2%", delta="Industry leading")

        with col4:
            st.metric(label="⚡ Response Quality", value="91.8%", delta="+ 4.1% this month")

        with col5:
            st.metric(label="💰 Commission Defense", value="96.1%", delta="6% rate maintained")
    ...
    def _render_sidebar(self):
        ...
        # Time range selector
        st.sidebar.selectbox("📅 Time Range", ["Last 24 Hours", "Last 7 Days", "Last 30 Days", "Last Quarter"], index=2)
        ...
        st.sidebar.select_slider(
            "🔍 Analytics Depth", options=["Basic", "Standard", "Advanced", "Deep Learning"], value="Advanced"
        )
    ...
    def _render_realtime_metrics(self, data):
        ...
        with col1:
            st.metric("Active Conversations", "23", delta="+ 5 from yesterday")

        with col2:
            st.metric("Conversations Today", "156", delta="+ 18% vs average")

        with col3:
            st.metric("Avg Response Time", "1.2s", delta="- 0.3s improvement")

        with col4:
            st.metric("Success Rate Today", "89.7%", delta="+ 2.1% vs target")
    ...
    def _render_jorge_methodology_analysis(self, data):
        ...
        with col2:
            ...
            st.warning(
                f"**Improvement Opportunity**: Buyer needs identification at {jorge_perf['buyer_needs_identification']:.1%}"
            )
    ...
    def _render_commission_defense_analysis(self, data):
        ...
        with col1:
            st.metric("Commission Defense Success", "96.1%", delta="+1.1% vs target")

        with col2:
            st.metric("Avg Commission Rate", "6.0%", delta="No degradation")

        with col3:
            st.metric("Negotiation Success", "92.3%", delta="+3.2% improvement")
    ...
    def _render_coaching_recommendations(self, data):
        ...
        for rec in recommendations:
            priority_color = (
                "red" if rec["priority"] == "High" else "orange" if rec["priority"] == "Medium" else "green"
            )
