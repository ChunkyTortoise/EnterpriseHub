...
class InteractiveAnalytics:
    ...
    def __init__(self, realtime_service, state_manager):
        ...
        if "analytics_state" not in st.session_state:
            st.session_state.analytics_state = {
                ...
                "selected_filters": {"time_range": "30d", "agent": "all", "source": "all", "status": "all"},
    ...
    def _render_filter_panel(self):
        ...
        with col4:
            status = st.selectbox(
                "🎯 Status", ["all", "hot", "warm", "cold", "converted", "lost"], index=0, key="analytics_status"
            )
    ...
    def _render_key_metrics(self, data: Dict[str, Any]):
        ...
        metrics = [
            ...
            {
                "label": "Avg Latency",
                "value": f"{data['avg_response_time']}m",
                "change": f"{data['response_time_change']:+}m",
    ...
    def _render_leads_trend_chart(self, data: Dict[str, Any]):
        ...
        df_trend = pd.DataFrame({"Date": dates, "Total Nodes": leads_data["total"], "Hot Signals": leads_data["hot"]})
        ...
        def create_funnel_fig(data_dict):
            fig = go.Figure(
                go.Funnel(
                    ...
                    marker=dict(
                        color=["#6366F1", "#8b5cf6", "#f59e0b", "#10b981", "#ef4444"],
    ...
    def _render_agent_performance_summary(self, data: Dict[str, Any]):
        ...
        def create_agent_fig(data_dict):
            ...
            fig.add_trace(
                go.Bar(x=agents_list, y=conversions, name="Conversions", marker=dict(color="#10b981")), row=1, col=1
            )
            fig.add_trace(
                go.Bar(x=agents_list, y=response_times, name="Latency", marker=dict(color="#6366F1")), row=2, col=1
            )
            return fig
            ...
            for i, (stage, value) in enumerate(zip(stages, values)):
                ...
                fig.add_trace(
                    go.Funnel(
                        ...
                        textinfo="value+percent initial",
                        marker=dict(
                            color=f"rgba(99, 102, 241, {color_intensity})",
                            line=dict(width=1, color="rgba(255,255,255,0.2)"),
                        ),
                        name=stage,
                    )
    ...
    def _render_source_analysis(self):
        ...
        def create_source_trend(trend_dict):
            ...
            for source in trend_dict:
                fig.add_trace(
                    go.Scatter(
                        x=trend_dict[source]["dates"],
                        y=trend_dict[source]["values"],
                        mode="lines+markers",
                        name=source,
    ...
    def _render_time_trends(self):
        ...
        def create_time_series_subplots(data_dict):
            ...
            fig.add_trace(
                go.Scatter(x=dates, y=data_dict["time_series"]["leads"], name="Leads", line=dict(color="#6366F1")),
                row=1,
                col=1,
            )
            fig.add_trace(
                go.Scatter(
                    x=dates, y=data_dict["time_series"]["conversions"], name="Conversions", line=dict(color="#10b981")
                ),
                row=1,
                col=2,
            )
            fig.add_trace(
                go.Scatter(
                    x=dates,
                    y=data_dict["time_series"]["response_time"],
                    name="Response Time",
            ...
            fig.add_trace(
                go.Scatter(x=dates, y=data_dict["time_series"]["revenue"], name="Revenue", line=dict(color="#f59e0b")),
                row=2,
                col=2,
            )
    ...
    def _get_analytics_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conversion_funnel": {"Leads": 1000, "Qualified": 450, "Proposal": 180, "Negotiation": 85, "Closed": 32},
            ...
            "source_distribution": {"Website": 350, "Facebook": 280, "Google Ads": 220, "Referral": 150, "Direct": 100},
            "agent_performance": {
