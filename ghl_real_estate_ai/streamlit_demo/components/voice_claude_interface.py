...
def render_voice_claude_interface():
        ...
        with command_history_container:
            if voice_status["last_command"]:
                ...
                sample_history = [
                    {
                        "time": "2 minutes ago",
                        "command": "Hey Claude, show me my leads",
                        "response": "Displaying your 5 active leads with priority ranking",
                    },
                    {
                        "time": "5 minutes ago",
                        "command": "Hey Claude, what's my performance?",
                        "response": "Your conversion rate is up 15% this week",
                    },
                    {
                        "time": "8 minutes ago",
                        "command": "Hey Claude, go to dashboard",
                        "response": "Navigating to dashboard with today's insights",
                    },
                ]
    ...
    with demo_col1:
        ...
        test_command = st.text_input("Type a command to test voice processing:", placeholder="show me my leads")

        if st.button("Process Command"):
            if test_command.strip():
                with st.spinner("Processing voice command..."):
                    # Simulate processing the command
                    response = run_async(companion.process_voice_command(test_command))
    ...
    if st.session_state.live_call_active:
        with st.container(border=True):
            ...
            with hud_col1:
                ...
                pulse_color = (
                    "rgba(0, 255, 65, 0.3)" if st.session_state.last_mention == "safety" else "rgba(99, 102, 241, 0.1)"
                )

                fig.update_layout(
                    polar=dict(
                        radialaxis=dict(
                            visible=True, range=[0, 1], showticklabels=False, gridcolor="rgba(255,255,255,0.1)"
                        ),
            ...
            with hud_col2:
                ...
                if sim_speech:
                    if (
                        "safe" in sim_speech.lower()
                        or "safety" in sim_speech.lower()
                        or "security" in sim_speech.lower()
                    ):
