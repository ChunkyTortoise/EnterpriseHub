...
class RealTimeNegotiationCoaching:
        ...
        with col1:
            ...
            with st.form("start_coaching_session"):
                ...
                negotiation_phase = st.selectbox(
                    "Negotiation Phase",
                    ["Initial Contact", "Offer Presentation", "Counter-Negotiation", 
                     "Final Terms", "Contract Review"]
                )
                ...
                with col1a:
                    buyer_type = st.selectbox(
                        "Buyer Type",
                        ["First-Time Buyer", "Move-Up Buyer", "Investor", "Cash Buyer"]
                    )
                ...
                communication_channel = st.selectbox(
                    ...
                    ["Phone Call", "In-Person Meeting", "Video Call", "Text/Email"]
                )
                ...
                if st.form_submit_button("🚀 Start Coaching Session", type="primary"):
                    self.start_coaching_session(
                        property_id, negotiation_phase, buyer_type, 
                        urgency_level, communication_channel, initial_context
                    )
                    st.rerun()
    ...
    def render_conversation_interface(self):
        ...
        with col1:
            ...
            with st.form("add_conversation", clear_on_submit=True):
                ...
                conversation_text = st.text_area(
                    "What was said?",
                    placeholder="Enter the conversation details...",
                    height=100
                )
                ...
                with col1a:
                    emotional_tone = st.selectbox(
                        "Emotional Tone",
                        ["Neutral", "Positive", "Concerned", "Excited", "Frustrated", "Confident"]
                    )
                
                with col1b:
                    urgency_change = st.selectbox(
                        "Urgency Change",
                ...
                if st.form_submit_button("📝 Add Entry", type="primary"):
                    self.add_conversation_entry(
                        speaker, conversation_text, emotional_tone, urgency_change
                    )
                    st.rerun()
            ...
            current_situation = st.selectbox(
                "Current Situation",
                ["Need immediate guidance", "Handling objection", "Making counter-offer",
                 "Closing negotiation", "Addressing concerns", "Building rapport"]
            )
        ...
        if st.session_state.coaching_conversation_log:
            for i, entry in enumerate(reversed(st.session_state.coaching_conversation_log[-10:])):
                with st.expander(f"{entry['timestamp'].strftime('%H:%M:%S')} - {entry['speaker']}", 
    ...
    def start_coaching_session(self, property_id: str, phase: str, buyer_type: str, 
                             urgency: str, channel: str, context: str):
    ...
    def end_coaching_session(self):
        ...
        if st.session_state.active_coaching_session:
            session_summary = {
                ...
                'final_confidence': st.session_state.coaching_insights_history[-1].get('confidence', 0) if st.session_state.coaching_insights_history else 0
            }
            
            st.success(f"✅ Coaching session ended for {session_summary['property_id']}")
            st.info(f"Session lasted {session_summary['duration'].seconds // 60} minutes with {session_summary['conversation_entries']} conversation entries")
