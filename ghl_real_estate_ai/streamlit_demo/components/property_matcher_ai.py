...
def render_property_matcher(lead_context: Dict, elite_mode: bool=False, analysis_result=None):
    ...
    for idx, property in enumerate(matches[:3]):
        with st.expander(f"{property['icon']} {property['address']} - {property['match_score']}% Match", expanded=idx == 0):
            ...
            with col_info:
                ...
                for factor in factors:
                    ...
                    with col_label:
                        ...
                        st.markdown(f"<div style='font-size: 1.2rem; padding-top: 0.5rem;'>{factor['icon']} {status_icon}</div>", unsafe_allow_html=True)
                    with col_bar:
...
def generate_property_matches(lead_context: Dict, elite_mode: bool=False) -> List[Dict]:
    ...
    try:
        ...
        for prop in raw_matches:
            formatted_matches.append({'id': prop.get('id', 'unknown'), 'address': f"{prop.get('address', {}).get('street', 'Address')}, {prop.get('address', {}).get('city', 'City')}", 'price': prop.get('price', 750000), 'beds': prop.get('bedrooms', 3), 'baths': prop.get('bathrooms', 2.5), 'sqft': prop.get('sqft', 2100), 'neighborhood': prop.get('address', {}).get('neighborhood', 'Area'), 'icon': _get_property_icon(prop), 'match_score': int(prop.get('match_score', 0) * 100), 'budget_match': prop.get('match_score', 0) > 0.7, 'location_match': True, 'features_match': True, 'match_reasons': [pm.generate_match_reasoning(prop, extracted)], 'match_type': prop.get('match_type', 'Standard'), 'ai_reasoning': prop.get('ai_reasoning', '')})
...
def _load_demo_properties() -> List[Dict]:
    ...
    """
    return [{'id': 'prop-001', 'address': '123 Oak Street', 'price': 750000, 'bedrooms': 3, 'bathrooms': 2.5, 'sqft': 2100, 'neighborhood': 'Downtown', 'property_type': 'Single Family', 'year_built': 2018, 'recently_renovated': True, 'amenities': ['garage', 'updated_kitchen', 'hardwood_floors'], 'days_on_market': 12}, {'id': 'prop-002', 'address': '456 Maple Ave', 'price': 780000, 'bedrooms': 3, 'bathrooms': 2, 'sqft': 2300, 'neighborhood': 'Domain', 'property_type': 'Single Family', 'year_built': 2020, 'recently_renovated': False, 'amenities': ['pool', 'garage', 'outdoor_space'], 'days_on_market': 8}, {'id': 'prop-003', 'address': '789 Cedar Lane', 'price': 725000, 'bedrooms': 4, 'bathrooms': 2.5, 'sqft': 2400, 'neighborhood': 'South Congress', 'property_type': 'Townhome', 'year_built': 2019, 'recently_renovated': False, 'amenities': ['garage', 'outdoor_space'], 'days_on_market': 15}, {'id': 'prop-004', 'address': '321 Pine Boulevard', 'price': 795000, 'bedrooms': 3, 'bathrooms': 3, 'sqft': 2500, 'neighborhood': 'Westlake', 'property_type': 'Single Family', 'year_built': 2021, 'recently_renovated': False, 'amenities': ['garage', 'pool', 'hardwood_floors', 'updated_kitchen'], 'days_on_market': 5}, {'id': 'prop-005', 'address': '555 Elm Drive', 'price': 680000, 'bedrooms': 3, 'bathrooms': 2, 'sqft': 1950, 'neighborhood': 'Mueller', 'property_type': 'Single Family', 'year_built': 2017, 'recently_renovated': True, 'amenities': ['garage', 'outdoor_space', 'updated_kitchen'], 'days_on_market': 20}, {'id': 'prop-006', 'address': '888 Willow Way', 'price': 850000, 'bedrooms': 4, 'bathrooms': 3.5, 'sqft': 2800, 'neighborhood': 'Tarrytown', 'property_type': 'Single Family', 'year_built': 2022, 'recently_renovated': False, 'amenities': ['garage', 'pool', 'hardwood_floors', 'updated_kitchen', 'outdoor_space'], 'days_on_market': 3}, {'id': 'prop-007', 'address': '999 Highland Ave', 'price': 720000, 'bedrooms': 3, 'bathrooms': 2, 'sqft': 2050, 'neighborhood': 'Highland', 'property_type': 'Single Family', 'year_built': 2016, 'recently_renovated': True, 'amenities': ['garage', 'hardwood_floors'], 'days_on_market': 18}]
...
def _get_fallback_properties(lead_context: Dict) -> List[Dict]:
    ...
    if lead_id == 'tech_professional_sarah':
        return [{'address': '2847 Glenwood Trail', 'price': 525000, 'beds': 3, 'baths': 2.5, 'sqft': 2180, 'neighborhood': 'Teravista (Round Rock)', 'icon': '🏡', 'match_score': 95, 'budget_match': True, 'location_match': True, 'features_match': True, 'match_reasons': ['Under SF-adjusted budget ($550k)', '12-min commute to Apple campus', 'Gigabit fiber internet ready', 'Dedicated home office space', 'Round Rock ISD (9/10 rating)']}, {'address': '15823 Fontana Lake Dr', 'price': 585000, 'beds': 4, 'baths': 3, 'sqft': 2650, 'neighborhood': 'Avery Ranch', 'icon': '🏠', 'match_score': 88, 'budget_match': False, 'location_match': True, 'features_match': True, 'match_reasons': ['Premium location near Brushy Creek', 'Modern open floor plan', 'Large secondary office/flex space', 'Walkable to local coffee shops', 'High-speed networking installed']}]
    elif lead_id == 'investor_david':
        return [{'address': '14821 Willow Ridge Dr', 'price': 285000, 'beds': 3, 'baths': 2, 'sqft': 1420, 'neighborhood': 'Manor', 'icon': '💰', 'match_score': 98, 'budget_match': True, 'location_match': True, 'features_match': True, 'match_reasons': ['Projected $270/mo positive cash flow', 'Turnkey condition (built 2018)', 'High rental demand (95% area occupancy)', 'Rapid appreciation forecast (12%+)', 'Manor tech-corridor growth zone']}, {'address': '101 Cedar Rd', 'price': 320000, 'beds': 2, 'baths': 1, 'sqft': 1100, 'neighborhood': 'Del Valle', 'icon': '🏬', 'match_score': 91, 'budget_match': True, 'location_match': True, 'features_match': True, 'match_reasons': ['Proximity to Tesla Giga Austin', 'Low maintenance exterior', 'Ideal entry-level rental', 'Strategic land-value hold', 'Near Austin Airport expansion']}]
    location = extracted.get('location', 'Austin')
    beds = extracted.get('bedrooms', 3)
    return [{'address': '123 Oak Street', 'price': 750000, 'beds': 3, 'baths': 2.5, 'sqft': 2100, 'neighborhood': 'Downtown', 'icon': '🏡', 'match_score': 92, 'budget_match': True, 'location_match': True, 'features_match': True, 'match_reasons': [f'Within budget (${budget:,})', f'Location compatibility ({location})', f'Exact bedroom count ({beds} beds)', 'Newly renovated kitchen', 'Walk to shops and restaurants']}]