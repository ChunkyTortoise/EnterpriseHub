...
def render_conversation_simulator(services, selected_lead_name):
    ...
    with st.expander("⚙️ Simulation Settings", expanded=False):
        ...
        with col_s1:
            scenario = st.selectbox(
                "Training Scenario:", 
                ["General Inquiry", "Objection Handling", "Qualification Run", "Scheduling Conflict", "Aggressive Negotiation"]
            )
        with col_s2:
    ...
    with st.form(key="simulator_input_form", clear_on_submit=True):
        ...
        with col_in:
            prompt = st.text_input(f"Message as {selected_lead_name}...", placeholder="Type your message here...", label_visibility="collapsed")
        with col_btn:
                ...
                with status_placeholder.status("🧠 Claude is thinking...", expanded=True) as status:
                    ...
                    if CLAUDE_AVAILABLE:
                        ...
                        try:
                            ...
                            full_response = f"I apologize, I'm having trouble connecting to my intelligence core. Error: {str(e)}"
                    else:
                        # Fallback
                        try:
