...
class CostTrackingDashboard:
        ...
        if self.conversation_optimizer:
            ...
            return CostMetrics(
                ...
                optimization_score=self._calculate_optimization_score(
                    conv_metrics, cache_metrics, budget_metrics, perf_metrics
                ),
            )
        else:
    ...
    def render_overview_cards(self, metrics: CostMetrics):
        ...
        with col2:
            st.metric("Tokens Saved", f"{metrics.total_tokens_saved:,}", delta="+387K today", delta_color="inverse")

        with col3:
            st.metric("Cache Hit Rate", f"{metrics.cache_hit_rate:.1f}%", delta="+2.3%", delta_color="inverse")

        with col4:
            st.metric(
                "Optimization Score", f"{metrics.optimization_score:.1f}/100", delta="+5.2", delta_color="inverse"
            )
    ...
    def render_cost_trends_chart(self):
        ...
        df = pd.DataFrame(
            {"Date": days, "Daily_Savings": daily_savings, "Cumulative_Savings": np.cumsum(daily_savings)}
        )
    ...
    def render_budget_tracking(self, metrics: CostMetrics):
        ...
        st.dataframe(
            df_budgets[["Tenant", "Used ($)", "Budget ($)", "Utilization (%)", "Status"]], use_container_width=True
        )
