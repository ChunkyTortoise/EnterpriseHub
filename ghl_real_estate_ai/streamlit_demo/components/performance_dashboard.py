...
class PerformanceDashboard:
    ...
    def _render_tab_navigation(self):
        ...
        tabs = [('overview', '📊 Overview'), ('agents', '👥 Agents'), ('campaigns', '📈 Campaigns'), ('goals', '🎯 Goals')]
        col1, col2, col3, col4 = st.columns(4)
        columns = [col1, col2, col3, col4]
        for i, (tab_key, tab_label) in enumerate(tabs):
            with columns[i]:
                if st.button(tab_label, key=f'perf_tab_{tab_key}', use_container_width=True, type='primary' if st.session_state.performance_tab == tab_key else 'secondary'):
    ...
    def _render_kpi_grid(self, data: Dict[str, Any]):
        """Render KPI grid"""
        kpis = [{'icon': '💰', 'label': 'Monthly Revenue', 'value': f"${data['monthly_revenue']:,.0f}", 'change': data['revenue_change'], 'trend': 'positive' if data['revenue_change'] > 0 else 'negative'}, {'icon': '🎯', 'label': 'Conversion Rate', 'value': f"{data['conversion_rate']:.1f}%", 'change': data['conversion_change'], 'trend': 'positive' if data['conversion_change'] > 0 else 'negative'}, {'icon': '📞', 'label': 'Calls Made', 'value': f"{data['calls_made']:,}", 'change': data['calls_change'], 'trend': 'positive' if data['calls_change'] > 0 else 'negative'}, {'icon': '📧', 'label': 'Emails Sent', 'value': f"{data['emails_sent']:,}", 'change': data['emails_change'], 'trend': 'positive' if data['emails_change'] > 0 else 'negative'}, {'icon': '⏱️', 'label': 'Avg Response Time', 'value': f"{data['avg_response_time']}min", 'change': data['response_time_change'], 'trend': 'negative' if data['response_time_change'] > 0 else 'positive'}, {'icon': '📈', 'label': 'Lead Score Avg', 'value': f"{data['avg_lead_score']:.0f}", 'change': data['lead_score_change'], 'trend': 'positive' if data['lead_score_change'] > 0 else 'negative'}]
    ...
    def _render_performance_trend_chart(self, data: Dict[str, Any]):
        ...
        fig = make_subplots(rows=2, cols=1, subplot_titles=['REVENUE & NODES', 'CONVERSION & LATENCY'], vertical_spacing=0.15, specs=[[{'secondary_y': True}], [{'secondary_y': True}]])
        fig.add_trace(go.Scatter(x=dates, y=metrics['revenue'], name='Revenue', line=dict(color='#10b981', width=3), hovertemplate='<b>Date:</b> %{x}<br><b>Revenue:</b> $%{y:,.0f}<extra></extra>'), row=1, col=1)
        fig.add_trace(go.Scatter(x=dates, y=metrics['leads'], name='Nodes', line=dict(color='#6366F1', width=2), yaxis='y2', hovertemplate='<b>Date:</b> %{x}<br><b>Nodes:</b> %{y}<extra></extra>'), row=1, col=1)
        fig.add_trace(go.Scatter(x=dates, y=metrics['conversion_rate'], name='Conversion', line=dict(color='#f59e0b', width=3), hovertemplate='<b>Date:</b> %{x}<br><b>Conversion:</b> %{y:.1f}%<extra></extra>'), row=2, col=1)
        fig.add_trace(go.Scatter(x=dates, y=metrics['response_time'], name='Latency', line=dict(color='#8b5cf6', width=2), yaxis='y4', hovertemplate='<b>Date:</b> %{x}<br><b>Latency:</b> %{y} min<extra></extra>'), row=2, col=1)
    ...
    def _render_revenue_breakdown_chart(self, data: Dict[str, Any]):
        ...
        fig = go.Figure(data=[go.Pie(labels=list(revenue_sources.keys()), values=list(revenue_sources.values()), hole=0.6, marker=dict(colors=['#6366F1', '#8b5cf6', '#f59e0b', '#10b981', '#ef4444'], line=dict(color='rgba(255,255,255,0.1)', width=1)), hovertemplate='<b>Source:</b> %{label}<br><b>Revenue:</b> $%{value:,.0f}<extra></extra>')])
        st.plotly_chart(style_obsidian_chart(fig), use_container_width=True)
    ...
    def _render_team_summary(self, data: Dict[str, Any]):
        ...
        for i, agent in enumerate(agents[:5]):
            ...
            performance_score = agent['conversions'] * 10 + agent['calls_made'] * 0.1 + agent['emails_sent'] * 0.05 - agent['avg_response_time'] * 0.5
    ...
    def _render_agent_comparison_chart(self, agents: List[Dict[str, Any]]):
        ...
        metrics = {'Conversions': [agent['conversions'] for agent in agents], 'Calls': [agent['calls_made'] for agent in agents], 'Signals': [agent['emails_sent'] for agent in agents], 'Score': [agent['avg_lead_score'] for agent in agents]}
        fig = go.Figure()
        colors = ['#6366F1', '#8b5cf6', '#f59e0b', '#10b981']
        for i, (metric_name, values) in enumerate(metrics.items()):
            fig.add_trace(go.Bar(name=metric_name, x=agent_names, y=values, marker_color=colors[i], hovertemplate=f'<b>Agent:</b> %{{x}}<br><b>{metric_name}:</b> %{{y}}<extra></extra>'))
        st.plotly_chart(style_obsidian_chart(fig), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        for i, agent in enumerate(agents):
            with cols[i % 2]:
                with st.container():
                    ...
                    with col3:
                        st.metric('Latency', f"{agent['avg_response_time']}m", delta=f"{agent['response_time_change']:+}m")
                    trend_data = agent['performance_trend']
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=list(range(len(trend_data))), y=trend_data, mode='lines', name='Score', line=dict(color='#6366F1', width=3), fill='tozeroy', fillcolor='rgba(99, 102, 241, 0.1)'))
                    st.plotly_chart(style_obsidian_chart(fig), use_container_width=True)
    ...
    def _render_campaigns_tab(self):
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=[c['name'] for c in campaigns], y=[c['roi'] for c in campaigns], marker=dict(color=[c['roi'] for c in campaigns], colorscale='RdYlGn', showscale=True, colorbar=dict(title='ROI (%)')), hovertemplate='<b>Campaign:</b> %{x}<br><b>ROI:</b> %{y:.1f}%<extra></extra>'))
    ...
    def _render_goals_tab(self):
        ...
        for goal_name, forecast in forecast_data.items():
            fig.add_trace(go.Scatter(x=forecast['dates'], y=forecast['projected'], mode='lines', name=f'{goal_name} (PROJ)', line=dict(dash='dash', color='#6366F1'), hovertemplate=f'<b>Goal:</b> {goal_name}<br><b>Date:</b> %{{x}}<br><b>Projected:</b> %{{y:.0f}}<extra></extra>'))
            fig.add_trace(go.Scatter(x=forecast['dates'][:len(forecast['actual'])], y=forecast['actual'], mode='lines+markers', name=f'{goal_name} (ACTUAL)', line=dict(width=3, color='#10b981'), hovertemplate=f'<b>Goal:</b> {goal_name}<br><b>Date:</b> %{{x}}<br><b>Actual:</b> %{{y:.0f}}<extra></extra>'))
        st.plotly_chart(style_obsidian_chart(fig), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    def _get_performance_data(self) -> Dict[str, Any]:
        ...
        import random
        return {'monthly_revenue': random.randint(180000, 250000), 'revenue_change': random.uniform(-5, 15), 'conversion_rate': random.uniform(18, 28), 'conversion_change': random.uniform(-2, 5), 'calls_made': random.randint(1200, 1800), 'calls_change': random.uniform(-10, 20), 'emails_sent': random.randint(3500, 5000), 'emails_change': random.uniform(-8, 25), 'avg_response_time': random.randint(8, 18), 'response_time_change': random.uniform(-5, 3), 'avg_lead_score': random.randint(72, 88), 'lead_score_change': random.uniform(-3, 8), 'revenue_by_source': {'Website': random.randint(45000, 75000), 'Facebook': random.randint(35000, 60000), 'Google Ads': random.randint(40000, 70000), 'Referral': random.randint(25000, 45000), 'Direct': random.randint(15000, 30000)}, 'agent_summary': [{'name': 'Sarah Johnson', 'conversions': random.randint(25, 35), 'calls_made': random.randint(180, 250), 'emails_sent': random.randint(350, 450), 'avg_response_time': random.randint(8, 15)}, {'name': 'Mike Chen', 'conversions': random.randint(20, 30), 'calls_made': random.randint(160, 220), 'emails_sent': random.randint(320, 420), 'avg_response_time': random.randint(10, 18)}, {'name': 'Lisa Rodriguez', 'conversions': random.randint(28, 38), 'calls_made': random.randint(190, 260), 'emails_sent': random.randint(380, 480), 'avg_response_time': random.randint(6, 12)}, {'name': 'David Kim', 'conversions': random.randint(18, 28), 'calls_made': random.randint(150, 210), 'emails_sent': random.randint(300, 400), 'avg_response_time': random.randint(12, 20)}, {'name': 'Emily Davis', 'conversions': random.randint(22, 32), 'calls_made': random.randint(170, 230), 'emails_sent': random.randint(340, 440), 'avg_response_time': random.randint(9, 16)}], 'agents_detailed': self._generate_detailed_agent_data(), 'campaigns': self._generate_campaign_data(), 'goals': [{'name': 'Monthly Revenue Goal', 'current': random.randint(180000, 220000), 'target': 250000, 'target_date': '2026-01-31'}, {'name': 'Conversion Rate Target', 'current': random.uniform(22, 26), 'target': 30, 'target_date': '2026-01-31'}, {'name': 'Lead Response Time', 'current': random.uniform(10, 14), 'target': 8, 'target_date': '2026-01-31'}, {'name': 'Team Productivity', 'current': random.uniform(85, 92), 'target': 95, 'target_date': '2026-01-31'}]}
    ...
    def _generate_detailed_agent_data(self) -> List[Dict[str, Any]]:
        ...
        detailed_data = []
        for agent in agents:
            detailed_data.append({'name': agent, 'conversions': random.randint(18, 38), 'conversions_change': random.randint(-3, 8), 'calls_made': random.randint(150, 260), 'calls_change': random.randint(-15, 25), 'emails_sent': random.randint(300, 480), 'emails_change': random.randint(-20, 35), 'avg_response_time': random.randint(6, 20), 'response_time_change': random.randint(-5, 3), 'avg_lead_score': random.randint(70, 90), 'performance_trend': [random.randint(70, 95) for _ in range(30)]})
        return detailed_data

    def _generate_campaign_data(self) -> List[Dict[str, Any]]:
        """Generate campaign performance data"""
        import random
        campaigns = ['Facebook Lead Gen', 'Google Search Ads', 'LinkedIn Campaign', 'Email Nurture Sequence', 'Referral Program', 'Website SEO']
        campaign_data = []
        for campaign in campaigns:
            cost = random.randint(2000, 8000)
            revenue = random.randint(cost * 1.2, cost * 5)
            roi = (revenue - cost) / cost * 100
            campaign_data.append({'name': campaign, 'cost': cost, 'revenue': revenue, 'roi': roi, 'conversion_rate': random.uniform(12, 35), 'status': random.choice(['Active', 'Paused', 'Completed'])})
    ...
    def _generate_performance_trends(self):
        ...
        trends = {'revenue': [random.randint(6000, 12000) for _ in range(30)], 'leads': [random.randint(25, 65) for _ in range(30)], 'conversion_rate': [random.uniform(15, 30) for _ in range(30)], 'response_time': [random.randint(8, 25) for _ in range(30)]}
        return (dates, trends)
