...
class AlertCenter:
    ...
    def __init__(self, realtime_service, state_manager):
        ...
        self.alert_config = {AlertType.HOT_LEAD: {'icon': '🔥', 'priority': AlertPriority.CRITICAL, 'sound': True, 'actions': ['call_lead', 'schedule_meeting', 'send_email']}, AlertType.STALE_LEAD: {'icon': '⚠️', 'priority': AlertPriority.HIGH, 'sound': False, 'actions': ['reactivate_campaign', 'schedule_call', 'update_status']}, AlertType.BOOKING_REMINDER: {'icon': '📅', 'priority': AlertPriority.HIGH, 'sound': True, 'actions': ['confirm_appointment', 'send_reminder', 'reschedule']}, AlertType.FOLLOW_UP_NEEDED: {'icon': '📞', 'priority': AlertPriority.MEDIUM, 'sound': False, 'actions': ['create_task', 'schedule_call', 'send_email']}, AlertType.BUDGET_QUALIFIED: {'icon': '💰', 'priority': AlertPriority.HIGH, 'sound': True, 'actions': ['priority_flag', 'schedule_meeting', 'notify_agent']}, AlertType.SYSTEM_ISSUE: {'icon': '🚨', 'priority': AlertPriority.CRITICAL, 'sound': True, 'actions': ['check_system', 'notify_admin', 'create_ticket']}}
    ...
    def _get_alerts_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            recent_events = self.realtime_service.get_recent_events(event_type='new_alert', limit=50, since=datetime.now() - timedelta(hours=24))
            ...
            for event in recent_events:
                ...
                alerts.append({'id': event.id, 'type': alert_type, 'message': data.get('message', 'Alert notification'), 'priority': self.alert_config.get(AlertType(alert_type), {'priority': AlertPriority.MEDIUM})['priority'].value, 'timestamp': event.timestamp, 'lead_id': data.get('lead_id'), 'read': False, 'action_required': data.get('action_required', True)})
    ...
    def _render_filter_bar(self):
        ...
        with col1:
            priority_filter = st.selectbox('Priority', ['all', 'critical', 'high', 'medium', 'low'], index=0, key='alert_priority_filter')
            st.session_state.alert_filters['priority'] = priority_filter
        with col2:
            type_filter = st.selectbox('Type', ['all', 'hot_lead', 'stale_lead', 'booking_reminder', 'follow_up_needed'], index=0, key='alert_type_filter')
    ...
    def _filter_alerts(self, alerts_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        time_deltas = {'1h': timedelta(hours=1), '6h': timedelta(hours=6), '24h': timedelta(hours=24), '7d': timedelta(days=7), '30d': timedelta(days=30)}
    ...
    def _render_alert_actions(self, alert: Dict[str, Any], actions: List[str]):
        ...
        action_configs = {'call_lead': {'label': '📞 Call', 'type': 'primary'}, 'schedule_meeting': {'label': '📅 Schedule', 'type': 'primary'}, 'send_email': {'label': '📧 Email', 'type': 'secondary'}, 'reactivate_campaign': {'label': '🔄 Reactivate', 'type': 'primary'}, 'schedule_call': {'label': '📞 Schedule Call', 'type': 'primary'}, 'update_status': {'label': '📝 Update', 'type': 'secondary'}, 'confirm_appointment': {'label': '✅ Confirm', 'type': 'primary'}, 'send_reminder': {'label': '📬 Remind', 'type': 'secondary'}, 'reschedule': {'label': '🔄 Reschedule', 'type': 'secondary'}, 'create_task': {'label': '📋 Task', 'type': 'secondary'}, 'priority_flag': {'label': '🏃 Priority', 'type': 'primary'}, 'notify_agent': {'label': '👨\u200d💼 Notify', 'type': 'secondary'}, 'acknowledge': {'label': '✓ Done', 'type': 'secondary'}}
    ...
    def _handle_alert_action(self, alert: Dict[str, Any], action: str):
        """Handle alert action"""
        action_messages = {'call_lead': '📞 Initiating call to lead...', 'schedule_meeting': '📅 Opening calendar to schedule meeting...', 'send_email': '📧 Opening email template...', 'reactivate_campaign': '🔄 Reactivating lead campaign...', 'schedule_call': '📞 Scheduling follow-up call...', 'update_status': '📝 Opening status update form...', 'confirm_appointment': '✅ Appointment confirmed!', 'send_reminder': '📬 Reminder sent!', 'reschedule': '🔄 Opening reschedule options...', 'create_task': '📋 Creating follow-up task...', 'priority_flag': '🏃 Lead flagged as priority!', 'notify_agent': '👨\u200d💼 Agent notification sent!', 'acknowledge': '✓ Alert acknowledged!'}
        message = action_messages.get(action, f"Action '{action}' executed!")
        st.success(message)
        alert['read'] = True
        self.realtime_service.emit_event(event_type='user_action', data={'alert_id': alert['id'], 'action': action, 'lead_id': alert.get('lead_id'), 'timestamp': datetime.now().isoformat()}, priority=2)
        time.sleep(1)
        st.rerun()
    ...
    def _toggle_alert_read_status(self, alert: Dict[str, Any]):
        ...
        self.realtime_service.emit_event(event_type='user_action', data={'alert_id': alert['id'], 'action': 'toggle_read', 'new_status': status, 'timestamp': datetime.now().isoformat()}, priority=1)
        time.sleep(0.5)
        st.rerun()
