...
if __name__ == "__main__":
    st.set_page_config(
        page_title="Advanced Churn Recovery Dashboard",
        page_icon="🛡️",
        layout="wide",
...
def generate_campaign_performance() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        'Channel': ['Email Sequence', 'SMS Urgent', 'Phone Callback', 'Property Alert', 'Market Update', 'Consultation Offer'],
...
def render_executive_kpis(kpis: ChurnKPIs):
    ...
    with col1:
        color = "#ef4444" if kpis.overall_churn_rate > 0.09 else "#f59e0b" if kpis.overall_churn_rate > 0.07 else "#22c55e"
        st.markdown(f"""
        <div class="kpi-card">
...
def render_recovery_pipeline(pipeline_data: pd.DataFrame):
    ...
    with col1:
        ...
        fig = go.Figure(go.Funnel(
            ...
            marker=dict(
                color=['#ef4444', '#f59e0b', '#1f77b4', '#8b5cf6', '#22c55e']
            )
        ))
    ...
    with col2:
        ...
        for _, row in pipeline_data.iterrows():
            stage_color = "#22c55e" if row['Conversion_Rate'] > 70 else "#f59e0b" if row['Conversion_Rate'] > 60 else "#ef4444"
...
def render_roi_insights():
    ...
    with col2:
        ...
        for _, row in invest_df.iterrows():
            priority_color = "#22c55e" if row['Priority_Score'] > 2 else "#f59e0b" if row['Priority_Score'] > 1.5 else "#ef4444"
