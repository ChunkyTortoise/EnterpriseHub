...
class SalesCopilotHub:
    ...
    def _render_negotiation_assistant(self):
        ...
        with col_input:
            ...
            objection_text = st.text_area(
                "Live Objection / Barrier",
                placeholder="e.g., 'The property taxes in Rancho Cucamonga are making me nervous...'",
                height=120,
            )

            persona = st.select_slider(
                "Strategy Aggression:", options=["Consultative", "Balanced", "Assertive"], value="Balanced"
            )
            ...
            if st.button("✨ Generate Tactical Counter", use_container_width=True, type="primary"):
                with st.spinner("Analyzing objection through lead psychology lens..."):
                    ...
                    st.session_state.last_negotiation_response = {
                        ...
                        "talking_points": [
                            "Address tax specific concerns",
                            "Pivot to long-term equity",
                            "Highlight scarcity",
                        ],
                        "confidence": 0.88,
    ...
    def _render_meeting_intel(self):
        ...
        with col1:
            ...
            m_type = st.selectbox(
                "Engagement Type", ["Listing Presentation", "Buyer Consultation", "Investor Pitch", "Final Closing"]
            )
            lead_options = st.session_state.get("lead_options", {})
    ...
    def _render_deal_probability(self):
        ...
        with col1:
            fig = px.scatter(
                ...
                text="lead",
                size_max=60,
                color_discrete_map={"High": "#10B981", "Medium": "#6366F1", "Low": "#F59E0B", "Stagnant": "#EF4444"},
                labels={"prob": "Win Probability (%)", "value": "Projected Deal Value ($)"},
            )
            fig.update_traces(
                textposition="middle center",
                textfont=dict(color="#FFFFFF", size=10),
                marker=dict(line=dict(width=2, color="rgba(255,255,255,0.2)"), opacity=0.8),
            )
            fig.update_layout(
        ...
        with col2:
            ...
            if st.button("🔥 Run Stress Test Analysis", key="run_stress_test"):
                with st.spinner("Claude is identifying potential deal-killers..."):
                    ...
                    st.markdown(
                        "• **Action:** Lead with the 2-1 Buy-down strategy to offset immediate cash flow concerns."
                    )
    ...
    def _render_investor_roi(self):
        ...
        with c2:
            ...
            fig = px.area(
                x=years,
                y=values,
                title="Institutional Growth Forecast (5YR)",
                labels={"x": "Fiscal Year", "y": "Asset Valuation"},
            )
