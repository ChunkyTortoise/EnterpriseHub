...
class SHAPExplainabilityDashboard:
    ...
    def _render_explanation_section(self, lead_name: str, lead_context: Dict[str, Any]) -> None:
        ...
        with col2:
            if st.button("🔍 Explain Decision", key=f"explain_btn_{lead_name}", type="primary"):
                st.session_state[f"shap_loading_{lead_name}"] = True
                st.rerun()

        with col3:
            if st.button("🔄 Refresh", key=f"refresh_shap_{lead_name}"):
                st.session_state[f"shap_explanation_{lead_name}"] = None
                st.rerun()
    ...
    def _load_shap_explanation(self, lead_name: str, lead_context: Dict[str, Any], explanation_type: str) -> None:
        ...
        with st.spinner(f"🧠 Analyzing AI decision for {lead_name}..."):
            try:
                ...
                score, confidence, _ = run_async(predictor.predict_lead_score(lead_context))
                ...
                st.success(f"✅ AI decision explained in {explanation.explanation_time_ms:.0f}ms {cache_status}")
    ...
    def _render_quick_overview(self, explanation: SHAPExplanation) -> None:
        ...
        with col1:
            create_metric_card(
                ...
                card_config=CardConfig(background_color="#1E40AF" if explanation.prediction >= 70 else "#DC2626"),
            )
        ...
        with col2:
            ...
            if top_driver:
                create_metric_card(
                    ...
                    card_config=CardConfig(background_color="#059669" if top_driver["impact"] > 0 else "#DC2626"),
                )
                ...
                with st.container():
                    render_obsidian_card(
                        ...
                        card_config=CardConfig(
                            border_color="#059669" if driver["impact"] > 0 else "#DC2626", show_border=True
                        ),
                    )
    ...
    def _render_what_if_section(self, lead_name: str, explanation: SHAPExplanation) -> None:
        ...
        with col1:
            ...
            for feature in modifiable_features:
                ...
                if feature == "response_time_hours":
                    ...
                    new_val = (
                        1 if st.checkbox(f"{display_name}", value=False, key=f"whatif_{feature}_{lead_name}") else 0
                    )
        ...
        with col2:
            ...
            if st.button("🔮 Run What-If Analysis", key=f"whatif_btn_{lead_name}", type="primary"):
                with st.spinner("🧮 Analyzing scenario..."):
                    try:
                        ...
                        scenario_result = run_async(
                            self.shap_service.perform_what_if_analysis(explanation, modified_values)
                        )
    ...
    def _display_what_if_results(self, scenario_result: Dict[str, Any]) -> None:
        ...
        with col1:
            st.metric("Original Score", f"{scenario_result['original_score']:.0f}", delta=None)
        ...
        with col3:
            ...
            impact_level = (
                "High"
                if abs(scenario_result["score_change"]) > 10
                else "Medium"
                if abs(scenario_result["score_change"]) > 5
                else "Low"
            )
            st.metric("Impact Level", impact_level, delta="Improvement" if improvement else "Decline")
    ...
    def _render_performance_metrics(self) -> None:
        ...
        try:
            ...
            with col1:
                st.metric("Explanations Generated", metrics["explanations_generated"])

            with col2:
                st.metric("Cache Hit Rate", f"{metrics['cache_hit_rate_percent']:.1f}%")

            with col3:
                st.metric("Avg Time", f"{metrics['avg_explanation_time_ms']:.0f}ms")

            with col4:
                st.metric("What-If Scenarios", metrics["what_if_scenarios"])
    ...
    def _get_category_emoji(self, category: str) -> str:
        ...
        emoji_map = {"behavioral": "🧠", "financial": "💰", "temporal": "⏰", "engagement": "🗣️", "other": "📊"}
...
def main():
    ...
    """
    st.set_page_config(page_title="SHAP Explainability Dashboard", layout="wide", initial_sidebar_state="expanded")
    ...
    demo_lead_context = {
        ...
        "extracted_preferences": {"budget": "$300-400k", "location": "downtown area", "timeline": "3 months"},
    }
