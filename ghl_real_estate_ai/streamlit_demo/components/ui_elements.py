...
def render_action_card(title, description, icon, key_suffix=""):
    ...
    with col2:
        if st.button(
            "Run Action", key=f"run_action_{key_suffix}_{title.lower().replace(' ', '_')}", use_container_width=True
        ):
...
def render_insight_card(title, value, description, status="info", action_label=None, action_key=None):
    ...
    border_colors = {"success": "#10B981", "warning": "#F59E0B", "info": "#6366f1", "danger": "#EF4444"}
