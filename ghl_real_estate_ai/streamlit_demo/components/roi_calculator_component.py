...
class ROICalculator:
    ...
    def calculate_scenario_roi(self, scenario: InvestmentScenario, 
                             lead_volume: int, current_recovery_rate: float) -> ROICalculation:
        ...
        payback_months = total_investment / (monthly_benefit - monthly_cost) if monthly_benefit > monthly_cost else float('inf')
    ...
    with st.container():
        ...
        with col3:
            ...
            projection_months = st.selectbox(
                ...
                [6, 12, 18, 24, 36],
                index=1,
                help="Months for ROI projection"
            )
...
def render_scenario_analysis(params: Dict, scenarios: List[InvestmentScenario]):
    ...
    for scenario in scenarios:
        roi_calc = calculator.calculate_scenario_roi(
            scenario, 
            params['monthly_leads'], 
            params['current_recovery_rate']
        )
...
def render_channel_optimization(params: Dict, channels: List[ChannelInvestment]):
    ...
    with col2:
        ...
        fig = px.bar(roi_data.sort_values('ROI', ascending=True),
                    x='ROI',
                    y='Channel',
                    orientation='h',
                    color='ROI',
    ...
    for idx, channel in enumerate(channels):
        ...
        roi_class = "metric-positive" if channel.roi_projection > 300 else "metric-warning" if channel.roi_projection > 200 else "metric-negative"
    ...
    for month in months:
        ...
        scenarios = {
            'Conservative': {
                'recovery_lift': 0.08,
                'monthly_cost': 2500,
                'setup_cost': 5000 if month == 1 else 0
            },
            'Balanced': {
                'recovery_lift': 0.15,
                'monthly_cost': 4800,
                'setup_cost': 12000 if month == 1 else 0
            },
            'Aggressive': {
                'recovery_lift': 0.25,
                'monthly_cost': 8500,
                'setup_cost': 25000 if month == 1 else 0
            }
        }
