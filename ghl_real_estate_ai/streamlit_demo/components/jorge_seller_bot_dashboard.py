...
class JorgeSellerLiveClient:
                ...
                return [
                    {
                        "id": "seller_001",
                        "name": "Michael Chen",
                        "property": "4521 Oak Valley Dr",
                        "score": 89.3,
                        "priority": "immediate",
                        "motivation": "Job relocation",
                        "status": "Hot Lead - Ready",
                        "expected_roi": 420,
                        "valuation": 678000,
                        "bot_analyzed": True,
                    ...
                    },
                    {
                        ...
                        "property": "1847 Maple Street",
                        "score": 71.8,
                        "priority": "high",
                        "motivation": "Upsizing family",
                        "status": "Qualification Active",
                        "expected_roi": 340,
                        "valuation": 485000,
                        "bot_analyzed": True,
                        "last_interaction": "Yesterday",
                        "next_action": "Send market analysis",
                    },
                    {
                        "id": "seller_003",
                        "name": "David Kim",
                        "property": "892 Pine Creek Ln",
                        "score": 52.4,
                        "priority": "warm",
                        "motivation": "Market timing",
                    ...
                    },
                    {
                        ...
                        "valuation": 0,
                        "bot_analyzed": False,
                        "last_interaction": "Just contacted",
                        "next_action": "Run Jorge qualification",
                    },
                ]
        except Exception as e:
    ...
    async def qualify_seller(self, seller_id: str, conversation_history: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.bot.process_seller_message(
                lead_id=seller_id, lead_name=seller_name, history=conversation_history
            )
    ...
    def _fallback_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {"conversion_rate": 0, "avg_response_time": "N/A", "compliance_score": 0},
        }
        ...
        return [
            {
                ...
                "next_action": "Check service connectivity",
            }
        ]
...
def render_negotiation_simulator():
    ...
    with col3:
        ...
        if empathy > 80:
            predicted_days = max(1, predicted_days - 2)
        if aggression > 80:
            predicted_days += 3
...
def render_seller_negotiation_section(api_client: JorgeSellerAPIClient):
    ...
    with col1:
        st.metric("Active Deals", stats["active_negotiations"])
    with col2:
        st.metric("Vague Detected", stats["vague_answers_caught"])
    with col3:
        st.metric("Take-Away Closes", stats["take_away_closes_triggered"])
    with col4:
        st.metric("ROI Potential", f"{stats['expected_roi_total']}%")
    with col5:
        st.metric("Comm. Capture", f"${stats['total_commission_potential'] // 1000}K")
...
def render_seller_pipeline_section(api_client: JorgeSellerLiveClient):
    ...
    for seller in pipeline:
        with st.container():
            ...
            with col2:
                temp = (
                    "hot" if seller["priority"] == "immediate" else "warm" if seller["priority"] == "high" else "cold"
                )
                if seller.get("bot_analyzed"):
                        ...
                        if JORGE_SELLER_BOT_AVAILABLE:
                            with st.spinner(f"Jorge is analyzing {seller['name']}..."):
                                ...
                                sample_history = [
                                    {
                                        "role": "user",
                                        "content": f"Hi, I'm interested in selling my property at {seller['property']}",
                                        "sender_name": seller["name"],
                                    },
                                    {
                                        "role": "assistant",
                                        "content": "Great! I'd love to help. What's driving you to sell?",
                                    },
                                    {"role": "user", "content": "We're looking to move closer to family."},
                                ]

                                qualification_result = run_async(
                                    api_client.qualify_seller(seller["id"], sample_history)
                                )

                                if qualification_result.get("success"):
                                    st.session_state[f"qualification_{seller['id']}"] = qualification_result
                                    st.success(
                                        f"✅ {seller['name']} qualified! Temperature: {qualification_result.get('temperature', 'unknown')}"
                                    )
                                    st.rerun()
                                else:
                                    st.error(
                                        f"❌ Qualification failed: {qualification_result.get('error', 'Unknown error')}"
                                    )
                        else:
                            st.warning("Jorge Bot service unavailable")
                else:
                    # Show qualification status
                    temp_color = {"hot": "#00FF00", "warm": "#FFA500", "lukewarm": "#FFFF00", "cold": "#FF6B6B"}.get(
                        seller.get("priority", "cold"), "#8B949E"
                    )
                    st.markdown(
                        f"""
            ...
            with action_col3:
                ...
                if seller.get("bot_analyzed"):
                    ...
                    st.button(
                        "📊 Analysis",
                        key=f"details_disabled_{seller['id']}",
                        disabled=True,
                        help="Run qualification first",
                    )
...
def render_jorge_seller_bot_dashboard():
    ...
    if qualification_updates:
        ...
        for update in qualification_updates[-3:]:  # Show last 3 updates
            ...
            st.toast(f"🎯 {contact_id}: Q{current_question} - {temperature.title()} Lead", icon="🤖")
    ...
    with tab1:
        render_seller_negotiation_section(api_client)
    with tab2:
        render_seller_pipeline_section(api_client)
    with tab3:
        st.markdown('<div class="elite-card" style="padding: 1.5rem;">', unsafe_allow_html=True)
        st.subheader("Security & Compliance")
        render_dossier_block(
            "MOAT scrubbing active. All PII data is anonymized before neural processing.", title="DATA_SECURITY_REPORT"
        )
        st.markdown("</div>", unsafe_allow_html=True)

    # Terminal with real-time bot activity
    terminal_logs = [{"prefix": "[MOAT]", "message": "PII scrubbing successful for session S-9921."}]
