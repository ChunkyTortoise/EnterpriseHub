...
def render_roi_dashboard():
    ...
    try:
        ...
        with col1:
            ...
            fig_savings = px.area(df_daily, x='timestamp', y='savings', 
            ...
            df_melted = df_agg.melt(id_vars='model', value_vars=['cache_read_input_tokens', 'misses'],
                                   var_name='Type', value_name='Tokens')
            
            fig_ratio = px.bar(df_melted, x='model', y='Tokens', color='Type', 
        ...
        with col2:
            ...
            # Pie chart of models
            fig_models = px.pie(df, values='cost_usd', names='model', hole=0.5,
            ...
            healing_data = pd.DataFrame({
                "Request": ["REQ-A", "REQ-B", "REQ-C", "REQ-D", "REQ-E"],
                "Iterations": [1, 3, 1, 2, 1]
            })
            
            fig_healing = px.bar(healing_data, x="Request", y="Iterations", 
                                title="Healing Iterations (Recent)",
                                color="Iterations",
                                color_continuous_scale=["#10B981", "#F59E0B", "#EF4444"])
            st.plotly_chart(style_obsidian_chart(fig_healing), use_container_width=True)

            render_dossier_block(f"""
