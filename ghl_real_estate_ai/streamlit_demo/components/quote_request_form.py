...
def render_quote_request_form():
    ...
    with st.form("quote_request_form"):
        ...
        with col2:
            ...
            preferred_contact = st.selectbox("Preferred Contact Method", ["Email", "Phone", "Video Call"])
        ...
        with col3:
            timeline = st.selectbox(
                "Desired Timeline",
                ["ASAP (1-2 weeks)", "Standard (3-5 weeks)", "Flexible (6+ weeks)", "Ongoing retainer"],
            )
        ...
        has_existing_data = st.radio("Do you have existing data/systems to integrate?", ["Yes", "No", "Not sure"])

        team_size = st.selectbox("Expected users/team size", ["Just me", "2-10 people", "11-50 people", "50+ people"])

        discovery_call = st.checkbox("I'd like to schedule a 30-minute discovery call", value=True)
