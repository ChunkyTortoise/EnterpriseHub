...
class ResponsiveLayoutManager:
        ...
        for i in range(0, num_metrics, cols_per_row):
            ...
            for j in range(cols_per_row):
                ...
                if metric_idx < num_metrics:
                    ...
                    with cols[j]:
                        st.metric(label=metric['label'], value=metric['value'], delta=metric.get('delta', None), delta_color=metric.get('delta_color', 'normal'))
    ...
    def mobile_optimized_dataframe(self, df, **kwargs):
        ...
        if st.session_state.get('is_mobile', False):
            ...
            if len(df.columns) > max_cols:
                ...
                selected_columns = st.multiselect('Select columns to display:', all_columns, default=default_columns, key='mobile_df_columns')
                if selected_columns:
    ...
    def responsive_plotly_chart(self, fig, **kwargs):
        ...
        if st.session_state.get('is_mobile', False):
            fig.update_layout(height=adaptive_height, margin=dict(l=20, r=20, t=30, b=20), font=dict(size=10), legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='center', x=0.5))
        else:
