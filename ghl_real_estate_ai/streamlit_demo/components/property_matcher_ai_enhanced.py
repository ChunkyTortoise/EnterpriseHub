...
def render_enhanced_property_matcher(lead_context: Dict):
    ...
    for idx, property in enumerate(matches[:3]):
        ...
        with st.expander(f"{property['icon']} {property['address']} - {property['match_score']}% Match {confidence_color}", expanded=idx == 0):
...
def generate_enhanced_property_matches(lead_context: Dict) -> List[Dict]:
    ...
    try:
        ...
        ml_preferences = {'budget': extracted.get('budget', 800000), 'location': extracted.get('location', 'Downtown'), 'bedrooms': extracted.get('bedrooms', 3), 'property_type': extracted.get('property_type', 'Single Family'), 'must_haves': extracted.get('must_haves', ['garage']), 'nice_to_haves': extracted.get('nice_to_haves', ['pool', 'good_schools'])}
        ml_matches = ml_matcher.find_enhanced_matches(ml_preferences, limit=5, min_confidence=60.0)
        formatted_matches = []
        for match in ml_matches:
            ...
            formatted_match = {'address': street_address, 'price': match.get('price', 750000), 'beds': match.get('bedrooms', 3), 'baths': match.get('bathrooms', 2.5), 'sqft': match.get('sqft', 2100), 'neighborhood': neighborhood, 'icon': _get_property_icon(match), 'match_score': int(confidence_score.overall), 'budget_match': confidence_score.budget_match > 80, 'location_match': confidence_score.location_match > 80, 'features_match': confidence_score.feature_match > 80, 'match_reasons': confidence_score.reasoning[:5], 'confidence_level': confidence_score.get_confidence_level(), 'ml_breakdown': {'budget_confidence': confidence_score.budget_match, 'location_confidence': confidence_score.location_match, 'feature_confidence': confidence_score.feature_match, 'market_confidence': confidence_score.market_context}}
    ...
    return [{'address': '123 Oak Street', 'price': 750000, 'beds': 3, 'baths': 2.5, 'sqft': 2100, 'neighborhood': 'Downtown', 'icon': '🏡', 'match_score': 94, 'budget_match': True, 'location_match': True, 'features_match': True, 'confidence_level': 'high', 'match_reasons': [f'Excellent value at ${(budget - 750000) / 1000:.0f}k under your ${budget / 1000:.0f}k budget', f'Prime location in {location} as requested', 'Has all your must-have features: garage, good schools', f'Perfect {beds}-bedroom layout matches your needs', 'Hot property - recently listed and likely to move quickly'], 'ml_breakdown': {'budget_confidence': 92.0, 'location_confidence': 95.0, 'feature_confidence': 96.0, 'market_confidence': 88.0}}, {'address': '456 Maple Ave', 'price': 780000, 'beds': 3, 'baths': 2, 'sqft': 2300, 'neighborhood': 'Domain', 'icon': '🏠', 'match_score': 88, 'budget_match': True, 'location_match': False, 'features_match': True, 'confidence_level': 'high', 'match_reasons': [f'Perfectly priced within your ${budget / 1000:.0f}k budget', 'Pool and covered patio as requested', f'Spacious {beds} bedrooms with bonus space', 'Top-rated school district (rated 9/10)', 'Adjacent to tech corridor for easy commuting'], 'ml_breakdown': {'budget_confidence': 89.0, 'location_confidence': 75.0, 'feature_confidence': 92.0, 'market_confidence': 85.0}}, {'address': '789 Cedar Lane', 'price': 725000, 'beds': 4, 'baths': 2.5, 'sqft': 2400, 'neighborhood': 'South Congress', 'icon': '🏘️', 'match_score': 85, 'budget_match': True, 'location_match': True, 'features_match': False, 'confidence_level': 'high', 'match_reasons': [f'Great value at ${(budget - 725000) / 1000:.0f}k under budget - Excellent value!', 'Trendy neighborhood with walkable amenities', 'Bonus 4th bedroom perfect for home office', 'Large backyard for outdoor activities', 'Recently updated kitchen and bathrooms'], 'ml_breakdown': {'budget_confidence': 95.0, 'location_confidence': 88.0, 'feature_confidence': 78.0, 'market_confidence': 82.0}}]
