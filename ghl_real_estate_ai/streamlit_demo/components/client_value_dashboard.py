...
def get_services():
    ...
    return {"pricing_optimizer": DynamicPricingOptimizer(), "roi_calculator": ROICalculatorService()}
...
def load_pricing_analytics(location_id: str, days: int = 30):
    ...
    try:
        return run_async(services["pricing_optimizer"].get_pricing_analytics(location_id, days))
    finally:
        loop.close()
...
def load_roi_report(location_id: str, days: int = 30):
    ...
    asyncio.set_event_loop(loop)
    try:
        return run_async(services["roi_calculator"].generate_client_roi_report(location_id, days))
    finally:
        loop.close()
...
def render_client_value_dashboard():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["💰 ROI Analysis", "📊 Pricing Analytics", "🆚 Human vs AI", "🎯 Competitive Position", "📈 Projections"]
    )
...
def render_executive_summary(pricing_data: Dict, roi_data: Any):
    ...
    with col1:
        st.metric(
            "Current ARPU", f"${current_arpu:.0f}", delta=f"{arpu_improvement:+.1f}% vs baseline", delta_color="normal"
        )
...
def render_roi_analysis_tab(roi_data: Any):
    ...
    with col1:
        ...
        costs_data = {
            ...
            "Amount": [roi_data.ai_total_cost, roi_data.human_equivalent_cost, roi_data.total_savings],
...
def render_pricing_analytics_tab(pricing_data: Dict):
    ...
    with col1:
        ...
        if tier_performance and any(p["count"] > 0 for p in tier_performance.values()):
            ...
            fig_pie = go.Figure(
                data=[go.Pie(labels=labels, values=values, hole=0.3, marker_colors=colors[: len(labels)])]
            )
...
def render_human_vs_ai_tab(location_id: str):
    ...
    asyncio.set_event_loop(loop)
    try:
        comparisons = run_async(services["roi_calculator"].calculate_human_vs_ai_comparison(location_id))
    finally:
        loop.close()
...
def render_competitive_analysis_tab(roi_data: Any, pricing_data: Dict):
    ...
    with col1:
        st.metric("Cost Advantage vs Industry", f"{jorge_advantage:.0f}%", delta="Lower than industry average")

        # Industry benchmark comparison
        st.markdown("### 📊 Industry Benchmarks")

        benchmark_data = {
            "Metric": ["Response Time", "Availability", "Cost per Lead", "Accuracy Rate"],
            "Industry Average": ["2 hours", "40 hrs/week", "$45.00", "65%"],
            "Jorge's AI": ["30 seconds", "24/7 (168 hrs/week)", "$3.75", "92%"],
            "Advantage": ["97% faster", "4.2x more available", "92% lower cost", "42% more accurate"],
        }
    ...
    with col2:
        ...
        fig_radar.add_trace(
            go.Scatterpolar(r=jorge_scores, theta=categories, fill="toself", name="Jorge's AI", line_color="#2ca02c")
        )

        fig_radar.add_trace(
            go.Scatterpolar(
                r=industry_scores, theta=categories, fill="toself", name="Industry Average", line_color="#ff7f0e"
            )
        )
        ...
        try:
            calc_result = run_async(
                services["roi_calculator"].get_savings_calculator(
                    leads_per_month=leads_per_month, messages_per_lead=messages_per_lead, human_hourly_rate=human_rate
                )
            )
        finally:
            loop.close()

        st.metric("Monthly Savings", f"${calc_result['cost_comparison']['monthly_savings']:,.0f}")
        st.metric("Net Monthly Benefit", f"${calc_result['jorge_ai_investment']['net_monthly_benefit']:,.0f}")
        st.metric("ROI Percentage", f"{calc_result['jorge_ai_investment']['roi_percentage']:.0f}%")
