...
class NotificationSystem:
    ...
    def _render_notification_settings(self):
        ...
        with col2:
            ...
            settings["auto_dismiss_info"] = st.slider(
                "Info notifications (seconds)",
                0, 30, settings["auto_dismiss_info"],
                key="auto_dismiss_info"
            )
            
            settings["auto_dismiss_warning"] = st.slider(
                "Warning notifications (seconds)",
                0, 60, settings["auto_dismiss_warning"],
                key="auto_dismiss_warning"
            )
            
            settings["auto_dismiss_error"] = st.slider(
                "Error notifications (0 = manual only)",
                0, 120, settings["auto_dismiss_error"],
    ...
    def _render_notification_list(self):
        ...
        filtered_notifications = [
            ...
            if settings["categories_enabled"].get(n.category, True)
        ]
    ...
    def _render_notification_item(self, notification: Notification):
        ...
        if (notification.auto_dismiss_seconds and 
            notification.auto_dismiss_seconds > 0 and
    ...
    def render_toast_notifications(self):
        ...
        for notification in st.session_state.notifications:
            if (notification.severity in ["critical", "error"] and 
                not notification.dismissed and
                (current_time - notification.timestamp).total_seconds() < 10):  # Show for 10 seconds
    ...
    def _create_conversation_notification(self, data: Dict[str, Any], priority: str):
        ...
        self.add_notification(
            ...
            severity=severity,
            category="lead",
            source_event_type="conversation_update"
        )
