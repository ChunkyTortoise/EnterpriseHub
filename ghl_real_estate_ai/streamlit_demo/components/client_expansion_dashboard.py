...
def render_client_expansion_dashboard():
    ...
    show_tier_filter = st.sidebar.multiselect(
        "Expansion Tiers",
        options=[ExpansionTier.TIER_1_10X.value, ExpansionTier.TIER_2_5X.value, ExpansionTier.TIER_3_2X.value],
        default=[ExpansionTier.TIER_1_10X.value, ExpansionTier.TIER_2_5X.value, ExpansionTier.TIER_3_2X.value],
    )

    show_readiness_filter = st.sidebar.multiselect(
        "Readiness Levels",
        options=[ReadinessLevel.READY_NOW.value, ReadinessLevel.READY_SOON.value, ReadinessLevel.NEEDS_NURTURE.value],
        default=[ReadinessLevel.READY_NOW.value, ReadinessLevel.READY_SOON.value],
    )
    ...
    if st.sidebar.button("🔄 Generate Expansion Framework", type="primary"):
        with st.spinner("Analyzing 50+ clients and generating expansion opportunities..."):
            framework = run_async(expansion_service.generate_expansion_framework(include_action_plan=True))
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["📊 Overview", "🎯 Top 20 Opportunities", "📈 Revenue Projections", "✅ Action Plan", "💡 Value Propositions"]
    )
...
def render_overview_metrics(framework: dict):
    ...
    with col1:
        st.metric("Current MRR", f"${overview.get('current_mrr', 0):,.0f}", delta=None)
    ...
    with col3:
        st.metric("Opportunities", overview.get("opportunities_identified", 0), delta=None)
    ...
    with col4:
        ...
        st.metric("Weighted Pipeline", f"${weighted_potential:,.0f}", delta=f"{target_achievement:.0f}% of target")
...
def render_overview_tab(framework: dict, tier_filter: list):
    ...
    st.dataframe(
        df_tiers.style.format(
            {"Total Potential ($)": "${:,.0f}", "Avg Potential ($)": "${:,.0f}", "Success Rate (%)": "{:.1f}%"}
        ),
...
def render_opportunities_tab(framework: dict, tier_filter: list, readiness_filter: list):
    ...
    st.dataframe(
        df_opps.style.background_gradient(subset=["Priority", "Score"], cmap="RdYlGn").format(
            {"Current ARR": "${:,.0f}", "Target ARR": "${:,.0f}", "Expansion $": "${:,.0f}", "Score": "{:.1f}"}
        ),
        use_container_width=True,
        height=600,
    )
    ...
    with col1:
        st.metric("Total Opportunities", len(df_opps), delta=None)

    with col2:
        total_expansion = df_opps["Expansion $"].sum()
        st.metric("Total Potential", f"${total_expansion:,.0f}", delta=None)

    with col3:
        avg_deal = df_opps["Expansion $"].mean()
        st.metric("Avg Deal Size", f"${avg_deal:,.0f}", delta=None)

    with col4:
        avg_close = df_opps["Close Days"].mean()
        st.metric("Avg Close Time", f"{avg_close:.0f} days", delta=None)
...
def render_revenue_projections_tab(framework: dict):
    ...
    with col1:
        st.metric("Total Potential", f"${projections.get('total_potential', 0):,.0f}", delta=None)

    with col2:
        st.metric("Weighted (Risk-Adjusted)", f"${projections.get('weighted_potential', 0):,.0f}", delta=None)

    with col3:
        target_pct = projections.get("target_achievement_percentage", 0)
        st.metric("Target Achievement", f"{target_pct:.0f}%", delta=f"{target_pct - 100:.0f}pp vs target")
    ...
    timeline_data["Cumulative ARR"] = [
        sum(timeline_data["Projected ARR"][: i + 1]) for i in range(len(timeline_data["Projected ARR"]))
    ]

    timeline_data["Monthly MRR"] = [arr / 12 for arr in timeline_data["Cumulative ARR"]]
    ...
    st.dataframe(
        df_timeline.style.format(
            {"Projected ARR": "${:,.0f}", "Cumulative ARR": "${:,.0f}", "Monthly MRR": "${:,.0f}"}
        ),
        use_container_width=True,
    )
...
def render_action_plan_tab(framework: dict):
    ...
    with col1:
        st.metric("Target Revenue", f"${float(action_plan['total_target_revenue']):,.0f}", delta=None)

    with col2:
        st.metric("Target Clients", action_plan["target_client_count"], delta=None)

    with col3:
        kpis = action_plan.get("kpis", {})
        st.metric("Target Close Rate", f"{kpis.get('target_close_rate', 0) * 100:.0f}%", delta=None)
    ...
    if actions_30:
        ...
        st.dataframe(df_actions_30.style.format({"potential_value": "${:,.0f}"}), use_container_width=True)
    ...
    if actions_60:
        ...
        st.dataframe(df_actions_60.style.format({"potential_value": "${:,.0f}"}), use_container_width=True)
    ...
    if actions_90:
        ...
        st.dataframe(df_actions_90.style.format({"potential_value": "${:,.0f}"}), use_container_width=True)
...
def render_value_propositions_tab(framework: dict):
    ...
    with col1:
        st.metric("Current ROI", f"{roi_data.get('current_roi_percentage', 0):,.0f}%")
    ...
    with col3:
        st.metric("Payback Period", f"{roi_data.get('payback_period_months', 0)} months")
    ...
    with col1:
        st.metric("Incremental Revenue", f"${roi_data.get('incremental_revenue_impact', 0):,.0f}")

    with col2:
        st.metric("3-Year Value", f"${roi_data.get('3_year_value', 0):,.0f}")
    ...
    with col1:
        st.metric("Proposed Monthly", f"${float(selected_vp.get('proposed_pricing', 0)):,.2f}")

    with col2:
        st.metric("Payment Terms", selected_vp.get("payment_terms", "Monthly"))
...
if __name__ == "__main__":
    st.set_page_config(page_title="Client Expansion Intelligence", page_icon="💰", layout="wide")
