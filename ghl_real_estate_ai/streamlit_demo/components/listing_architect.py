...
def render_listing_architect():
    ...
    with col_input:
        ...
        with st.container(border=True):
            ...
            style = st.selectbox(
                "Preferred Narrative", ["Luxury/Elegant", "Family/Warm", "Investor/ROI", "Modern/Minimalist"]
            )
            ...
            target_audience = st.radio(
                "Target Persona", ["Young Professionals", "Growing Families", "Retirees", "Institutional Investors"]
            )
            ...
            if st.button("🚀 Architect Campaign", use_container_width=True, type="primary"):
                with st.spinner("Claude is analyzing local market trends and crafting copy..."):
                    ...
                    try:
                        ...
                        listings = run_async(st.session_state.listing_generator.generate_enhanced_listings(prop_data))
    ...
    with col_output:
        if st.session_state.get("listing_generated") and "architect_results" in st.session_state:
            ...
            with tab_z:
                ...
                if "Luxury" in style:
                    selected_result = results[2]
                elif "Investor" in style:
                    selected_result = results[1]

                render_dossier_block(
                    selected_result["text"], title=f"PUBLIC MLS DESCRIPTION ({selected_result['tone'].upper()})"
                )
...
{fb_copy}
            ...
            # Reach chart
            df_reach = pd.DataFrame({"Channel": ["Organic", "Paid Social", "SMS"], "Reach": [1450, 5200, 186]})
            fig = px.bar(
                df_reach,
                x="Channel",
                y="Reach",
