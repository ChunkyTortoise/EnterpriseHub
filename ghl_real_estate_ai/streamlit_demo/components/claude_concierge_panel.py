...
try:
    from ghl_real_estate_ai.services.claude_concierge_orchestrator import get_claude_concierge_orchestrator, ConciergeMode, ConciergeResponse
...
def render_claude_concierge_panel(hub_name: str):
    ...
    with st.sidebar.expander("💡 Current Intelligence", expanded=True):
        ...
        try:
            ...
            response = run_async(orchestrator.generate_live_guidance(
            ...
            st.info("Jorge, Sarah Chen is showing high intent (FRS: 88). Recommend initiating Price Negotiation sequence in Seller Command.")

    # Reactive Chat Interface
    with st.sidebar.expander("💬 Ask Claude", expanded=False):
        user_query = st.text_input("Query platform state...", placeholder="Ex: Is Sarah ready to sell?", key="concierge_chat_input")
        if user_query:
            st.markdown(f"*Analyzing platform state for: '{user_query}'*")
            # In a real app, this would call orchestrator.chat_with_concierge
            st.markdown("Based on my coordination with the Lead and Seller bots, Sarah is highly qualified but waiting for a CMA. I recommend generating the CMA now.")
