...
def sparkline(data: list, color: str = "#6366F1", height: int = 50):
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            visible=False,
            fixedrange=True,
            range=[min(data) * 0.9 if min(data) != 0 else -0.1, max(data) * 1.1 if max(data) != 0 else 0.1],
        ),
    )
    return fig
...
try:
    ...
    from ghl_real_estate_ai.streamlit_demo.components.ai_behavioral_tuning import (
        get_behavior_config,
...
class AutomationStudioHub:
    ...
    def render_hub(self):
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["🖥️ Automation Control", "🎭 Persona Lab", "🎨 Visual Designer", "🧠 Neural Triggers", "🏪 Marketplace"]
        )
    ...
    def _render_automation_control(self):
        ...
        log_events = [
            {
                "time": "10:45 AM",
                "event": "Preference Extracted",
                "detail": "Budget: $1.3M (Sarah Johnson)",
                "field": "contact.budget",
                "status": "Synced",
            },
            {
                "time": "10:46 AM",
                "event": "Custom Field Update",
                "detail": "Area: Alta Loma",
                "field": "contact.preferred_area",
                "status": "Synced",
            },
            {
                "time": "11:02 AM",
                "event": "Phase 2 Match",
                "detail": "Sent 3 RC Listings via SMS",
                "field": "contact.tags",
                "status": "Synced",
            },
            {
                "time": "11:15 AM",
                "event": "VAPI Connection",
                "detail": "Voice Call Initiated",
                "field": "contact.last_call",
                "status": "Active",
            },
        ]
...
def run_async(coro):
    ...
    def _render_persona_lab(self):
        ...
        with col_tune:
            ...
            persona_name = st.selectbox(
                "Current Persona",
                [
                    "The Professional closer (Default)",
                    "Friendly Neighborhood Guide",
                    "Data-Driven Analyst",
                    "Aggressive Hustler",
                ],
            )
            ...
            focus = st.multiselect(
                "Prioritize Data Clusters:",
                ["Market Trends", "School Ratings", "Investment ROI", "Lifestyle & Coffee", "Neighborhood Safety"],
                default=["Market Trends", "Investment ROI"],
            )

            current_p = st.text_area(
                "Custom System Prompt Instructions:",
                value="You are a helpful assistant on Jorge's team. Be friendly and keep it concise.",
                height=100,
            )
                ...
                with st.spinner("Claude is optimizing neural weights and persona instructions..."):
                    try:
                        ...
                        opt_context = {
                            ...
                            "parameters": {"tone": tone, "empathy": empathy, "persistence": persistence},
                            "focus_areas": focus,
                        ...
                        prompt = (
                            f"Optimize this real estate AI persona based on these parameters: {json.dumps(opt_context)}"
                        )
                        ...
                        st.session_state.optimized_persona = (
                            "Fallback: Stick to a balanced, consultative approach with high follow-up frequency."
                        )
        ...
        with col_preview:
            ...
            st.info(
                "💡 **Claude's Note:** Persona Lab adjustments affect the 'Tone' and 'Demeanor' of all outgoing AI communications."
            )
    ...
    def _render_neural_triggers(self):
        ...
        triggers = [
            ...
            {
                "event": "Sentiment drop in SMS thread",
                "action": "Escalate to Human Agent + Record VAPI Prompt",
                "status": "Active",
            },
            {
                "event": "Lead asks about 'Interest Rates'",
                "action": "Push Mortgage ROI Calculator link",
                "status": "Active",
            },
            {
                "event": "No activity for 72h after showing",
                "action": "Trigger Churn Early Warning Sequence",
                "status": "Active",
            },
        ]
