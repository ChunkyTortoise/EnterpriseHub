...
# UI Setup
def setup_ui():
    st.set_page_config(page_title="GHL AI Analytics", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
def filter_conversations_by_date(conversations: List[Dict], start_date: datetime, end_date: datetime) -> List[Dict]:
...
def create_lead_score_distribution_chart(conversations: List[Dict]) -> go.Figure:
    ...
    fig = go.Figure(
        data=[go.Histogram(x=scores, nbinsx=20, marker_color=COLORS["primary"], opacity=0.8, name="Lead Scores")]
    )
...
def main():
    ...
    date_preset = st.sidebar.selectbox("Quick Select", ["Last 7 Days", "Last 30 Days", "Last 90 Days", "Custom"])
    ...
    # Lead type filter
    lead_types = st.sidebar.multiselect("Lead Classification", ["hot", "warm", "cold"], default=["hot", "warm", "cold"])
    ...
    with tab1:
        ...
        with col2:
            st.metric("Avg Lead Score", f"{metrics['avg_lead_score']:.1f}", delta=f"{metrics['hot_leads']} hot leads")
        ...
        if saved > 0:
            st.info(
                f"🚀 **Efficiency Boost:** The AI caching layer has saved **${saved:.4f}** in API costs today for this tenant."
            )
        ...
        if filtered_conversations:
            ...
            st.info(
                "No conversations found for the selected filters. Try adjusting your date range or tenant selection."
            )
    ...
    with tab2:
        ...
        if not tenants:
            ...
            selected_tenant_detail = st.selectbox("Select Tenant for Detailed View", [t["name"] for t in tenants])
            ...
            if tenant_detail:
                ...
                if tenant_convs_filtered:
                    ...
                    recent_convs = sorted(tenant_convs_filtered, key=lambda x: x["start_time"], reverse=True)[:10]
                        ...
                        conv_table.append(
                            {
                                ...
                                "Date": datetime.fromisoformat(conv["start_time"].replace("Z", "+00:00")).strftime(
                                    "%Y-%m-%d %H:%M"
                                ),
    ...
    with tab3:
        ...
        if not system_health:
            ...
            with col1:
                ...
                st.metric("Uptime", f"{uptime:.2f}%", delta="Excellent" if uptime > 99.5 else "Good")
            ...
            with col3:
                ...
                st.metric("API Calls (24h)", f"{api_calls:,}", delta=f"{api_calls / 24:.0f}/hour")

            with col4:
                response_time = system_health.get("avg_response_time_ms", 0)
                st.metric("Avg Response Time", f"{response_time}ms", delta="Fast" if response_time < 500 else "Slow")
            ...
            with col2:
                ...
                st.metric(
                    "Compliance Rate", f"{compliance * 100:.1f}%", delta="Compliant" if compliance > 0.95 else "Review"
                )
            with col3:
            ...
            with col2:
                st.metric(
                    "Cost Saved (Cache)",
                    f"${usage.get('saved_cost', 0):.4f}",
                    delta=f"{usage.get('cache_hits', 0)} hits",
                )
            with col3:
                st.metric("Total Tokens", f"{usage.get('total_tokens', 0):,}")
            with col4:
                hit_rate = (
                    (usage.get("cache_hits", 0) / usage.get("total_requests", 1)) * 100
        ...
        with tab4:
            ...
            if selected_tenant == "All Tenants":
                ...
                if not active_campaigns:
                        ...
                        with st.form("create_campaign"):
                            ...
                            with col2:
                                target_conversions = st.number_input(
                                    "Target Conversions", min_value=0, value=10, step=1
                                )
                            with col3:
                    ...
                    selected_campaign_name = st.selectbox("Select Campaign", options=list(campaign_names.keys()))
                    ...
                    if "error" not in performance:
                        ...
                        with col1:
                            ...
                            fig_roi.add_trace(
                                go.Bar(
                                    name="Spent", x=["Campaign"], y=[roi_data["total_spent"]], marker_color="#ff4b4b"
                                )
                            )
                        ...
                        with col1:
                            ...
                            st.markdown(
                                f"{leads_data['actual']} / {leads_data['target']} ({leads_data['achievement_rate']:.0f}%)"
                            )
                        ...
                        with col2:
                            ...
                            st.progress(progress / 100)
                            st.markdown(
                                f"{conv_data['actual']} / {conv_data['target']} ({conv_data['achievement_rate']:.0f}%)"
                            )
                        ...
                        with col3:
                            ...
                            st.progress(progress / 100)
                            st.markdown(
                                f"{roi_target['actual']:.2f}x / {roi_target['target']:.2f}x ({roi_target['achievement_rate']:.0f}%)"
                            )
                    ...
                    if channel_data:
                        ...
                        with col1:
                            fig_channel_leads = go.Figure(
                                data=[go.Bar(x=channels, y=channel_leads, marker_color="#3498db")]
                            )
                        ...
                        with col2:
                            fig_channel_roi = go.Figure(
                                data=[go.Bar(x=channels, y=channel_roi, marker_color="#2ecc71")]
                            )
        ...
        with tab5:
            ...
            if selected_tenant == "All Tenants":
                ...
                if not tracker.journeys:
                    ...
                    with col3:
                        conversion_rate = (
                            (funnel_counts.get("converted", 0) / funnel_data["total_journeys"] * 100)
                            if funnel_data["total_journeys"] > 0
                            else 0
                        )
                        st.metric("Conversion Rate", f"{conversion_rate:.1f}%")
                    with col4:
                        lost_rate = (
                            (funnel_counts.get("lost", 0) / funnel_data["total_journeys"] * 100)
                            if funnel_data["total_journeys"] > 0
                            else 0
                        )
                    ...
                    if journey_list:
                        ...
                        selected_journey_name = st.selectbox(
                            "Select a lead to view their journey:", options=list(journey_names.keys())
                        )
                        ...
                        if selected_journey_name:
                            ...
                            if "error" not in summary:
                                ...
                                for i, stage_entry in enumerate(timeline):
                                    ...
                                    entered_time = datetime.fromisoformat(stage_entry["entered_at"]).strftime(
                                        "%b %d, %I:%M %p"
                                    )
                                    ...
                                    if stage_entry["events"]:
                                        with st.expander(f"View {len(stage_entry['events'])} events"):
                                            for event in stage_entry["events"]:
                                                event_time = datetime.fromisoformat(event["timestamp"]).strftime(
                                                    "%I:%M %p"
                                                )
                                ...
                                for moment in key_moments:
                                    moment_time = datetime.fromisoformat(moment["timestamp"]).strftime(
                                        "%b %d, %I:%M %p"
                                    )
                                    moment_type_icons = {
                                        "first_contact": "👋",
    ...
    if BULK_OPERATIONS_AVAILABLE:
        with tab6:
            ...
            if selected_tenant == "All Tenants":
                ...
                with col1:
                    selection_method = st.radio("Selection Method", ["Manual Entry", "Filter Criteria", "Upload CSV"])

                with col2:
                    if selection_method == "Manual Entry":
                        contact_ids_input = st.text_area(
                            "Enter Contact IDs (one per line)", placeholder="contact_123\ncontact_456\ncontact_789"
                        )
                ...
                if selected_op_type == "score":
                    ...
                    parameters = {"context": {"budget": default_budget, "timeline": default_timeline}}
                    ...
                    parameters = {"action": "add" if action == "Add Tags" else "remove", "tags": tags}
                    ...
                    parameters = {"assigned_to": assigned_to}
                    ...
                    parameters = {"stage": new_stage, "reason": reason}
                    ...
                    parameters = {"format": export_format, "fields": fields}
                        ...
                        if target_leads:
                            ...
                            with col_c:
                                st.metric(
                                    ...
                                    delta="Errors" if results.get("failed", 0) > 0 else None,
                                )
                            ...
                            if results.get("errors"):
                                with st.expander("View Errors"):
                                    for error in results["errors"][:10]:
                                        st.error(
                                            f"Contact: {error.get('contact_id', 'N/A')} - {error.get('error', 'Unknown error')}"
                                        )
                ...
                with st.expander("➕ Create New Template"):
                    with st.form("create_template"):
                        ...
                        tmpl_category = st.selectbox(
                            "Category", ["general", "welcome", "followup", "reengagement", "appointment"]
                        )
