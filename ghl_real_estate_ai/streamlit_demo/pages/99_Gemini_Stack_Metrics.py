...
if df is None or df.empty:
    ...
    with row1_col1:
        ...
        fig_cost = px.pie(cost_by_task, values='cost_usd', names='task_type', hole=0.4,
    ...
    with row1_col2:
        ...
        fig_models = px.bar(calls_by_model, x='model', y='count', 
    ...
    fig_daily = px.line(daily_metrics, x='date', y='cost_usd', markers=True,
                       title="Daily Spend (USD)", labels={'cost_usd': 'Cost ($)'})
    ...
    if pro_usage > flash_usage:
        st.sidebar.info("High Pro model usage detected. Can any of these tasks be handled by Flash with a better prompt?")
