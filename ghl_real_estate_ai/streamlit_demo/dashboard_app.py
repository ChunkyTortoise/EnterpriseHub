...
# Page configuration
st.set_page_config(
    page_title="EnterpriseHub BI Dashboard",
    page_icon="🏠",
    layout="wide",
...
def show_overview():
    ...
    with col2:
        ...
        fig = px.bar(rev_data, x='month', y='revenue', 
...
def show_lead_analytics(leads):
    ...
    fig = px.histogram(filtered, x='score', color='temperature',
                       color_discrete_map={'Hot': '#FF6B6B', 'Warm': '#FFD93D', 'Cold': '#6BCB77'})
    st.plotly_chart(fig, use_container_width=True)
    
    # Data table
    st.subheader("Lead Details")
    st.dataframe(filtered[['lead_id', 'name', 'score', 'temperature', 'source', 'status', 'budget']],
...
def show_bot_performance():
    ...
    with col1:
        ...
        fig = px.bar(bots, x='bot', y='avg_response_time', 
                     color_discrete_sequence=['#667eea'])
        st.plotly_chart(fig, use_container_width=True)
    
    with col2:
        st.subheader("Handoffs by Bot")
        fig = px.bar(bots, x='bot', y='handoffs', color='bot',
...
def show_forecast():
    ...
    fig.add_trace(go.Scatter(
        x=forecast['date'], y=forecast['p90'],
        mode='lines', name='P90 (Optimistic)',
        line=dict(color='green', width=1),
        fill='tonexty'
    ))
    
    fig.add_trace(go.Scatter(
        x=forecast['date'], y=forecast['p50'],
        mode='lines', name='P50 (Most Likely)',
        line=dict(color='blue', width=2),
        fill='tonexty'
    ))
    
    fig.add_trace(go.Scatter(
        x=forecast['date'], y=forecast['p10'],
        mode='lines', name='P10 (Conservative)',
