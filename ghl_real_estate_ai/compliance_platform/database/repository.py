...
from .models import Base, DBModelRegistration, DBRiskAssessment, DBPolicyViolation, DBComplianceScore
...
class BaseRepository(Generic[T]):
    ...
    async def get(self, id: Any) -> Optional[T]:
        stmt = select(self.model_cls).where(self.model_cls.model_id == str(id)) if hasattr(self.model_cls, 'model_id') else select(self.model_cls).get(id)
...
class ModelRepository(BaseRepository[DBModelRegistration]):
    ...
    async def get_with_relations(self, model_id: str) -> Optional[DBModelRegistration]:
        stmt = select(DBModelRegistration).options(
