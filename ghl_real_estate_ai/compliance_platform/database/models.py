...
from sqlalchemy import String, Float, Boolean, Integer, DateTime, ForeignKey, JSON, Enum as SQLEnum, Text
...
class DBModelRegistration(Base):
    ...
    compliance_status: Mapped[ComplianceStatus] = mapped_column(SQLEnum(ComplianceStatus), default=ComplianceStatus.UNDER_REVIEW)
    applicable_regulations: Mapped[List[RegulationType]] = mapped_column(JSON, default=list) # Stored as list of strings
    ...
    compliance_score: Mapped[Optional["DBComplianceScore"]] = relationship(back_populates="model", uselist=False, cascade="all, delete-orphan")
...
class DBPolicyViolation(Base):
    ...
    model_id: Mapped[str] = mapped_column(ForeignKey("model_registrations.model_id")) # Adding FK to model for easier queries
    ...
    remediation_actions: Mapped[List["DBRemediationAction"]] = relationship(back_populates="violation", cascade="all, delete-orphan")
...
class DBAuditRecord(Base):
    ...
    metadata_info: Mapped[Dict[str, Any]] = mapped_column(JSON, default=dict, name="metadata") # "metadata" is reserved in SQLAlchemy Base
