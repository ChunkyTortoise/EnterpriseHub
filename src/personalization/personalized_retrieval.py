...
from src.personalization.user_profile import UserProfile, Preference, Interest, PreferenceCategory
...
class RankingEngine:
    ...
    async def rank_items(
        ...
        # Filter items first
        filtered_items = await self.content_filter.filter_items(
            items, profile, item_extractor
        )
        
        # Score items
        scored_items = await self._score_items(
            filtered_items, profile, base_scorer, item_extractor
        )
    ...
    async def _apply_diversity(self, items: List[ScoredItem[T]]) -> List[ScoredItem[T]]:
        ...
        while remaining and len(diversified) < self.config.max_items:
            ...
            for item in remaining:
                ...
                combined_score = (
                    item.final_score * (1 - self.config.diversity_weight) +
    ...
    async def retrieve(
        ...
        for expanded_query in expanded_queries:
            items = await asyncio.get_event_loop().run_in_executor(
                None, item_provider, expanded_query
            )
            for item in items:
        ...
        ranked_items = await self.ranking_engine.rank_items(
            items, profile, base_scorer, item_extractor
        )
    ...
    async def retrieve_batch(
        ...
        for query in queries:
            results[query] = await self.retrieve(
                query, profile, item_provider, base_scorer, item_extractor
            )
        return results
    ...
    async def explain_ranking(
        ...
        for pref in profile.preferences.values():
            ...
            if pref.name in item_data and item_data[pref.name] == pref.value:
                ...
                explanation["reasoning"].append(
                    f"Matched preference '{pref.name}' = '{pref.value}'"
                )
