...
from src.caching.redis_client import RedisClient, BaseRedisClient
    ...
    def to_csv(self) -> str:
        ...
        if self.metrics:
            fieldnames = [
                ...
                "avg_latency_ms", "p95_latency_ms", "entry_count"
            ]
...
class CacheAnalytics:
    ...
    async def record_hit(
        ...
        await self._record_event(
            cache_name, MetricType.HIT, response_time_ms, size_bytes
        )
    ...
    async def record_miss(
        ...
        """Record a cache miss."""
        await self._record_event(
            cache_name, MetricType.MISS, computation_time_ms, 0
        )
    ...
    async def record_error(self, cache_name: str, error: Exception) -> None:
        ...
        await self._record_event(
            cache_name, MetricType.ERROR, 0, 0, error=str(error)
        )
    
    async def record_latency(self, cache_name: str, latency_ms: float) -> None:
        """Record operation latency."""
        await self._record_event(
            cache_name, MetricType.LATENCY, latency_ms, 0
        )
    ...
    async def _get_recent_metrics(
        ...
        events = [
            e for e in self._metrics[cache_name]
    ...
    async def get_cost_analysis(
        ...
        return CostAnalysis(
            ...
            cost_per_miss=self.cost_per_miss
        )
        ...
        for name, cache_metrics in metrics.items():
            ...
            if cache_metrics.hit_rate < 0.5:
                recommendations.append(
                    f"{name}: Low hit rate ({cache_metrics.hit_rate:.1%}). "
                    "Consider increasing TTL or cache size."
                )
            elif cache_metrics.hit_rate > 0.95:
                recommendations.append(
                    f"{name}: Very high hit rate ({cache_metrics.hit_rate:.1%}). "
            ...
            if total_ops > 0:
                ...
                if eviction_rate > 0.1:
                    recommendations.append(
                        f"{name}: High eviction rate ({eviction_rate:.1%}). "
...
# Factory function
async def create_cache_analytics(
