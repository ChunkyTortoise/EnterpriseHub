...
class QueryCache:
    ...
    async def invalidate_by_tag(self, tag: str) -> int:
        ...
        async with self._lock:
            # Remove from memory
            keys_to_remove = [
                key for key, result in self._memory_cache.items()
            ...
            if self.redis:
                try:
                    cursor = 0
                    while True:
                        cursor, keys = await self.redis.scan_keys(
                            f"{self.key_prefix}:*",
                            count=100,
                            cursor=cursor
                        )
    ...
    def register_warming_task(
        ...
        self._warming_queue = deque(
            sorted(self._warming_queue, key=lambda t: t.priority, reverse=True)
        )
    ...
    async def _preemptive_warming(self) -> None:
        ...
        frequent_queries = [
            (key, result) for key, result in self._memory_cache.items()
    ...
    async def get_popular_queries(self, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        sorted_queries = sorted(
            [(r.query, r.access_count) for r in self._memory_cache.values()],
            key=lambda x: x[1],
            reverse=True
        )
