...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run all EnterpriseHub synthetic benchmarks")
    parser.add_argument(
        "--iterations",
        type=int,
        default=1_000,
    ...
    for ep_name, passed in api_targets.items():
        ...
        _row(f"API {ep_name} P99 < {ep.target_ms:.0f}ms", passed, f"{ep.p99:.2f}ms")

    print("  " + "-" * 68)
    total_checks = len(cache_targets) + len(orch_targets) + len(api_targets)
    passed_checks = sum(cache_targets.values()) + sum(orch_targets.values()) + sum(api_targets.values())
