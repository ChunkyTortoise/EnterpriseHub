...
HANDOFFS = [
    {
        ...
        "contact": "Sarah Chen",
        "confidence": 0.92,
        "trigger": "Pre-approval confirmed + budget stated",
    },
    {
        "from": "Lead Bot",
        "to": "Seller Bot",
        "contact": "Robert Chen",
        "confidence": 0.88,
        "trigger": "CMA request + 'sell my house'",
    },
    {
        "from": "Lead Bot",
        "to": "Buyer Bot",
        ...
        "confidence": 0.95,
        "trigger": "Cash buyer + immediate timeline",
    },
    {
        "from": "Buyer Bot",
        "to": "Lead Bot",
        "contact": "Emily Torres",
        "confidence": 0.71,
        "trigger": "Budget uncertainty + early stage",
    },
]
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "[![Tests](https://img.shields.io/badge/tests-6497-brightgreen)](https://github.com/ChunkyTortoise/EnterpriseHub)"
    )
...
if hub == "Executive Dashboard":
    ...
    with c1:
        kpi_card("Monthly Revenue", "$285K", delta=12, prefix="")
    with c2:
        kpi_card("Active Leads", "847", delta=8)
    with c3:
        kpi_card("Conversion Rate", "23", delta=5, suffix="%")
    with c4:
        kpi_card("Avg Deal Size", "$425K", delta=-2)
    ...
    for _, row in df.iterrows():
        ...
        with c1:
            st.markdown(
                f"**{row['name']}**<br><span style='color:#94a3b8;font-size:0.8rem'>{row['email']}</span>",
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(f"Score: **{row['score']}**/100")
        with c3:
            st.markdown(temp_badge(row["temp"]), unsafe_allow_html=True)
        with c4:
            st.markdown(f"${row['budget']:,}")
        with c5:
            st.caption(" | ".join(row["signals"][:2]))
...
# ---------------------------------------------------------------------------
    ...
    with c1:
        kpi_card("Lead Score", str(lead["score"]), suffix="/100")
    with c2:
        kpi_card("Temperature", lead["temp"])
    with c3:
        kpi_card("Budget", f"${lead['budget']:,}")
    ...
    with col_right:
        ...
        st.info(
            f"**{lead['name']}** scored **{lead['score']}** → classified as **{lead['temp']}-Lead** → assigned to **{lead['bot']} Bot**"
        )
...
# ---------------------------------------------------------------------------
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi_card("Tokens/Workflow", "7,800", delta=-92, prefix="")
    with c2:
        kpi_card("Monthly Cost", "$460", delta=-89, prefix="")
    with c3:
        kpi_card("Cache Hit Rate", "87", suffix="%")
    with c4:
        kpi_card("Orchestrator Overhead", "<200", suffix="ms")
    ...
    with col_left:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Bar(name="Before", x=["Tokens/Workflow", "Monthly Cost ($)"], y=[93000, 4200], marker_color="#ef4444")
        )
        fig.add_trace(
            go.Bar(name="After", x=["Tokens/Workflow", "Monthly Cost ($)"], y=[7800, 460], marker_color="#10b981")
        )
        fig.update_layout(
            barmode="group",
            template="plotly_dark",
    ...
    with col_right:
        ...
        savings = [60, 25, 15]
        fig2 = go.Figure(
            go.Pie(labels=techniques, values=savings, marker=dict(colors=["#6366f1", "#06b6d4", "#10b981"]), hole=0.4)
        )
        fig2.update_layout(
            template="plotly_dark",
            height=350,
...
st.markdown(
    ...
    "6,497 test functions &bull; FastAPI + Streamlit + PostgreSQL + Redis + Claude AI"
    "</div>",
    unsafe_allow_html=True,
)
