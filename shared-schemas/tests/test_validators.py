...
class TestValidateTenantLimits:
    def test_within_limits(self):
        limits = TenantLimits(
            max_users=1, max_queries_per_day=50, storage_gb=0.3, max_api_keys=1, rate_limit_rpm=10
        )
    ...
    def test_exceeds_users(self):
        limits = TenantLimits(
            max_users=10, max_queries_per_day=50, storage_gb=0.3, max_api_keys=1, rate_limit_rpm=10
        )
        violations = validate_tenant_limits(TenantTier.FREE, limits)
