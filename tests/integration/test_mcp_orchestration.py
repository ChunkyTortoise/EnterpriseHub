...
async def test_mcp_tool_discovery():
    ...
    orchestrator.mcp_servers = {
        "LeadIntelligence": mock_lead_server,
        "AnalyticsIntelligence": mock_analytics_server
    }
...
async def test_mcp_tool_execution():
    ...
    tool_call = {
        "id": "call_123",
        "name": "get_lead_score_breakdown",
        "args": {"lead_context": {"test": "data"}}
    }
...
async def test_multi_turn_orchestration():
    ...
    turn1_response = LLMResponse(
        ...
        tool_calls=[{
            "id": "tc_01",
            "name": "analyze_lead",
            "args": {"lead_name": "John Doe", "lead_context": {}}
        }]
    )
...
async def test_ghl_sync_on_action():
    ...
    turn1_response = LLMResponse(
        ...
        tool_calls=[{
            "id": "tc_action",
            "name": "generate_lead_outreach_script",
            "args": {"lead_id": "real_contact_123"}
        }]
    )
    
    turn2_response = LLMResponse(
        content="Script generated and synced.",
        provider=LLMProvider.CLAUDE,
        model="claude-3-5-sonnet"
    )
