...
from tests.mocks.external_services import (
    ...
    MockApolloClient, MockTwilioClient, MockSendGridClient,
    MockWebhookPayloads, create_test_lead_data, create_mock_service6_response
)
from tests.fixtures.sample_data import LeadProfiles, AnalyticsData
...
class TestLeadIngestionToConversionWorkflow:
    ...
    async def test_high_intent_lead_fast_track_workflow(self, integrated_service6_stack):
        ...
        call_result = await services['twilio'].make_call(
            to=lead_data['phone'],
            twiml_url='https://enterprisehub.ai/voice/high-intent-script'
        )
    ...
    async def test_webhook_to_campaign_enrollment_workflow(self, integrated_service6_stack):
        ...
        enrichment_result = await services['apollo'].enrich_lead(
            ...
            webhook_payload['data'].get('phone')
        )
        ...
        with patch.object(services['database'], 'enroll_in_campaign') as mock_enroll:
            ...
            enrollment_success = await services['database'].enroll_in_campaign(
                lead_id, campaign_id, enrollment_data
            )
        ...
        await services['database'].update_lead_score(
            ...
            {'analysis_metadata': 'webhook_triggered'}
        )
    ...
    async def test_realtime_scoring_to_immediate_response_workflow(self, integrated_service6_stack):
        ...
        immediate_action_required = (
            scoring_result.primary_score >= 80.0 and 
            scoring_result.confidence >= 0.8
        )
        ...
        if response_received:
            ...
            await services['database'].update_lead_score(
                lead_activity['lead_id'],
                updated_score,
                {'response_to_realtime_outreach': True}
            )
    ...
    async def test_error_recovery_and_graceful_degradation_workflow(self, integrated_service6_stack):
        ...
        services['database'].get_lead_activity_data = AsyncMock(
            side_effect=Exception("Database connection timeout")
        )
        
        # Mock Apollo API rate limiting
        services['apollo'].enrich_lead = AsyncMock(
            side_effect=Exception("Apollo API rate limit exceeded")
        )
        ...
        try:
            sms_result = await services['twilio'].send_sms(
                to=lead_data['phone'],
                message="Hello! I wanted to follow up on your property interest."
            )
        ...
        services['database'].get_lead_activity_data = AsyncMock(
            return_value={'activity': 'recovered_data'}
        )
