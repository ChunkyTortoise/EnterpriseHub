...
from ghl_real_estate_ai.agents.enhanced_bot_orchestrator import EnhancedBotOrchestrator, ConversationSession
    ...
    async def test_orchestrator_buyer_conversation_flow(self, orchestrator, mock_conversation_session):
        ...
        with patch('ghl_real_estate_ai.agents.enhanced_bot_orchestrator.JorgeBuyerBot') as mock_buyer_bot_class:
            ...
            result = await orchestrator._orchestrate_buyer_conversation(
                session=mock_conversation_session,
                message="I'm ready to see some properties",
                intent_update=None
            )
...
class TestSMSComplianceIntegration:
    ...
    def sms_compliance_service(self):
        ...
        with patch.multiple(
            'ghl_real_estate_ai.services.sms_compliance_service',
            get_cache_service=Mock,
            get_event_publisher=Mock
        ):
    ...
    async def test_sms_validation_before_buyer_message(self, sms_compliance_service):
        ...
        with patch.object(sms_compliance_service, '_is_opted_out', return_value=False), \
            ...
            validation = await sms_compliance_service.validate_sms_send(
                ...
                message_content="Your property matches are ready for review."
            )
    ...
    async def test_sms_opt_out_during_buyer_conversation(self, sms_compliance_service):
        ...
        with patch.object(sms_compliance_service, '_store_opt_out_audit', new_callable=AsyncMock), \
             patch.object(sms_compliance_service.cache, 'set', new_callable=AsyncMock), \
             patch.object(sms_compliance_service.event_publisher, 'publish_sms_opt_out_processed', new_callable=AsyncMock), \
             patch.object(sms_compliance_service.event_publisher, 'publish_sms_compliance_event', new_callable=AsyncMock):
    ...
    async def test_sms_frequency_limit_integration(self, sms_compliance_service):
        ...
        with patch.object(sms_compliance_service, '_is_opted_out', return_value=False), \
             ...
             patch.object(sms_compliance_service.event_publisher, 'publish_sms_frequency_limit_hit', new_callable=AsyncMock):

            validation = await sms_compliance_service.validate_sms_send(
                phone_number=phone_number,
                message_content="Another property update"
            )
    ...
    async def test_property_match_events_integration(self):
        ...
        with patch('ghl_real_estate_ai.services.event_publisher.get_event_publisher') as mock_get_publisher:
            ...
            mock_event_publisher.publish_property_match_update.assert_called_once_with(
                contact_id="test_buyer",
                properties_matched=2,
                match_criteria={"bedrooms": 3}
            )
