...
class TestOpenRouterClient:
    ...
    def test_cost_tracking(self):
        ...
        headers = Headers({"x-openrouter-generation-cost": "0.0025", "x-openrouter-total-cost": "0.015"})
    ...
    async def test_async_generation_mock(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            mock_response.headers = {"x-openrouter-generation-cost": "0.001", "x-openrouter-total-cost": "0.005"}
            mock_response.raise_for_status = Mock()

            mock_client.return_value.__aenter__.return_value.post = AsyncMock(return_value=mock_response)

            result = await client.generate_async(prompt="Test prompt", system_prompt="You are helpful")
    ...
    def test_sync_generation_fallback(self):
        ...
        client = OpenRouterClient(
            api_key="test-key", default_model="invalid/model", fallback_models=["anthropic/claude-3.5-sonnet"]
        )
...
class TestLLMClientOpenRouter:
    ...
    async def test_openrouter_real_async_generation(self):
        ...
        result = await client.agenerate(
            prompt="Say 'Hello from OpenRouter' and nothing else.", max_tokens=50, temperature=0.0
        )
    ...
    def test_openrouter_real_sync_generation(self):
        ...
        result = client.generate(prompt="Say 'Test successful' and nothing else.", max_tokens=50, temperature=0.0)
