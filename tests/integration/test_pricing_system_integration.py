...
class TestPricingSystemIntegration:
    ...
    async def test_complete_webhook_to_pricing_flow(
        self, 
        client, 
        ...
        with patch('ghl_real_estate_ai.api.routes.webhook.verify_ghl_signature', return_value=True), \
            ...
            mock_claude.return_value.process_webhook.return_value = Mock(
                lead_score=3,
                extracted_data=sample_lead_context["extracted_data"],
                classification="hot"
            )
    ...
    async def test_pricing_api_endpoints_integration(self, client):
        ...
        with patch('ghl_real_estate_ai.api.middleware.jwt_auth.get_current_user', return_value={"user_id": "test_user"}), \
             patch('ghl_real_estate_ai.services.dynamic_pricing_optimizer.DynamicPricingOptimizer') as mock_pricing:
    ...
    async def test_concurrent_pricing_calculations(self, pricing_optimizer):
        ...
        contexts = [
            {"contact_id": f"contact_{i}", "location_id": "test_location", "score": 3}
            for i in range(5)
        ]
        ...
        tasks = [
            pricing_optimizer.calculate_lead_price(
                ctx["contact_id"], 
                ctx["location_id"], 
                {"score": ctx["score"]}
            )
