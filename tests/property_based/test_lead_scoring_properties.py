...
class TestLeadScoringInvariants:
    ...
    @given(st.dictionaries(
        keys=st.sampled_from(['budget', 'location', 'timeline', 'bedrooms', 'bathrooms', 'must_haves', 'financing', 'motivation', 'home_condition']),
        values=st.one_of(
    ...
    def test_percentage_score_mapping_invariant(self, preferences: Dict[str, Any]):
        ...
        assert percentage_score == expected_score, f"Question count {question_count} should map to {expected_score}% but got {percentage_score}%"
    ...
    @given(
        ...
        additional_key=st.sampled_from(['budget', 'location', 'timeline', 'bedrooms', 'bathrooms', 'must_haves', 'financing', 'motivation', 'home_condition']),
    ...
    def test_monotonic_property(self, base_prefs: Dict[str, Any], additional_key: str, additional_value: str):
        ...
        assert enhanced_score >= base_score, f"Adding {additional_key} decreased score from {base_score} to {enhanced_score}"
        assert enhanced_score <= base_score + 1, f"Adding {additional_key} increased score too much: {base_score} to {enhanced_score}"
    ...
    @given(
        ...
        conversation_history=st.lists(
            st.dictionaries(
                keys=st.just('role'),
                values=st.sampled_from(['user', 'assistant'])
            ),
            max_size=20
        ),
    ...
    @settings(max_examples=100, deadline=5000)
    def test_context_robustness(self, preferences: Dict[str, Any], conversation_history: List[Dict], created_at: datetime):
        ...
        try:
            ...
            assert score == reasoning_result["score"], f"Inconsistency between calculate methods: {score} vs {reasoning_result['score']}"
            assert score == reasoning_result["questions_answered"], f"Score should equal questions_answered: {score} vs {reasoning_result['questions_answered']}"
...
class LeadScoringStateMachine(RuleBasedStateMachine):
    ...
    @rule(target=preferences, 
          key=st.sampled_from(['budget', 'location', 'timeline', 'bedrooms', 'bathrooms', 'must_haves', 'financing', 'motivation', 'home_condition']),
          value=st.text(min_size=1, max_size=100))
    def add_preference(self, key: str, value: str):
        """Rule: Add a preference to the system"""
        return {key: value}
    
    @rule(target=contexts,
          prefs=preferences)
...
class TestEnhancedLeadIntelligenceProperties:
    ...
    def test_performance_metrics_consistency(self, lead_name: str, preferences: Dict[str, Any]):
        ...
        assert 0 <= initial_metrics["cache_hit_rate"] <= 1, f"Invalid cache hit rate: {initial_metrics['cache_hit_rate']}"
