...
def premium_showcase():
    ...
    with patch.object(PremiumSwarmShowcase, '_get_swarm_service', return_value=None), \
...
def test_generate_agent_insight_demographic(sample_agent, sample_lead_data):
    ...
    assert 'income' in insight['primary_finding'].lower() or any('income' in ki.lower() for ki in insight['key_insights'])
...
def test_render_live_swarm_analysis(mock_session_state, mock_streamlit, premium_showcase):
    ...
    with patch('streamlit.selectbox', return_value="Sarah Chen - Tech Executive"), \
...
def test_render_swarm_configuration(mock_streamlit, premium_showcase):
    ...
    with patch('streamlit.tabs', return_value=[Mock(), Mock(), Mock()]), \
...
def test_agent_insights_quality_validation():
    ...
    key_agents = ["Demographic Analyzer", "Behavioral Profiler", "Intent Detector", 
                  "Financial Analyzer", "Risk Assessor"]
    ...
    for agent_data in showcase.demo_agents:
        if agent_data['name'] in key_agents:
            ...
            if agent_data['name'] == "Demographic Analyzer":
                assert any(term in primary_finding + key_insights_text for term in ['income', 'age', 'location', 'demographic'])
            elif agent_data['name'] == "Financial Analyzer":
                assert any(term in primary_finding + key_insights_text for term in ['financial', 'value', 'debt', 'credit', 'buying power'])
