...
def mock_national_registry():
    ...
    base_registry.get_market_config.return_value = {
        ...
        'neighborhoods': [
            {'name': 'Cherry Creek', 'school_rating': 9.1},
            {'name': 'Boulder', 'school_rating': 9.3}
        ]
    }
...
def intelligence_service(mock_cache_service, mock_national_registry, mock_claude_assistant):
    ...
    with patch('ghl_real_estate_ai.services.national_market_intelligence.get_cache_service', return_value=mock_cache_service), \
         patch('ghl_real_estate_ai.services.national_market_intelligence.get_national_market_registry', return_value=mock_national_registry), \
         patch('ghl_real_estate_ai.services.national_market_intelligence.ClaudeAssistant', return_value=mock_claude_assistant), \
...
class TestNationalMarketIntelligence:
    ...
    def test_determine_market_trend(self, intelligence_service):
        ...
        config_rapid = {
            'price_appreciation_1y': 16.0,
            'demographics': {'population_growth_rate': 0.05}
        }
        ...
        # Test growing market
        config_growing = {
            'price_appreciation_1y': 9.0,
            'demographics': {'population_growth_rate': 0.03}
        }
        ...
        config_stable = {
            ...
            'demographics': {'population_growth_rate': 0.02}
        }
        ...
        config_declining = {
            ...
            'demographics': {'population_growth_rate': -0.01}
        }
    ...
    def test_calculate_col_index(self, intelligence_service):
        ...
        config_high = {
            ...
            'demographics': {'median_household_income': 70000}
        }
        ...
        config_low = {
            ...
            'demographics': {'median_household_income': 90000}
        }
    ...
    def test_estimate_migration_volume(self, intelligence_service):
        ...
        intelligence_service.market_metrics['source'] = MarketMetrics(
            market_id='source', market_name='Source', population=1000000,
            median_home_price=400000, price_appreciation_1y=5.0, price_appreciation_3y=15.0,
            inventory_days=30, employment_rate=0.94, income_growth_rate=0.03,
            corporate_headquarters_count=5, tech_job_growth=0.05,
            cost_of_living_index=95.0, quality_of_life_score=75.0,
            market_trend=MarketTrend.STABLE, opportunity_score=65.0,
            last_updated=datetime.now()
        )

        intelligence_service.market_metrics['destination'] = MarketMetrics(
            market_id='destination', market_name='Destination', population=1500000,
            median_home_price=600000, price_appreciation_1y=8.0, price_appreciation_3y=24.0,
            inventory_days=28, employment_rate=0.96, income_growth_rate=0.04,
            corporate_headquarters_count=8, tech_job_growth=0.08,
            cost_of_living_index=110.0, quality_of_life_score=85.0,
            market_trend=MarketTrend.GROWING, opportunity_score=80.0,
            last_updated=datetime.now()
        )
    ...
    async def test_get_top_market_opportunities(self, intelligence_service):
        ...
        intelligence_service.market_metrics['high_opportunity'] = MarketMetrics(
            market_id='high_opportunity', market_name='High Opportunity',
            population=2000000, median_home_price=500000, price_appreciation_1y=10.0,
            price_appreciation_3y=30.0, inventory_days=25, employment_rate=0.96,
            income_growth_rate=0.04, corporate_headquarters_count=10, tech_job_growth=0.12,
            cost_of_living_index=105.0, quality_of_life_score=90.0,
            market_trend=MarketTrend.RAPIDLY_GROWING, opportunity_score=90.0,
            last_updated=datetime.now()
        )

        intelligence_service.market_metrics['low_opportunity'] = MarketMetrics(
            market_id='low_opportunity', market_name='Low Opportunity',
            population=800000, median_home_price=300000, price_appreciation_1y=2.0,
            price_appreciation_3y=6.0, inventory_days=45, employment_rate=0.90,
            income_growth_rate=0.01, corporate_headquarters_count=2, tech_job_growth=0.02,
            cost_of_living_index=85.0, quality_of_life_score=65.0,
            market_trend=MarketTrend.STABLE, opportunity_score=35.0,
            last_updated=datetime.now()
        )
    ...
    def test_analyze_market_trends(self, intelligence_service):
        ...
        intelligence_service.market_metrics = {
            'growing1': MarketMetrics(
                'growing1', 'Growing 1', 1000000, 400000, 8.0, 24.0, 30, 0.95, 0.03, 5, 0.06,
                100.0, 80.0, MarketTrend.GROWING, 70.0, datetime.now()
            ),
            'growing2': MarketMetrics(
                'growing2', 'Growing 2', 1200000, 450000, 9.0, 27.0, 28, 0.96, 0.04, 6, 0.07,
                105.0, 82.0, MarketTrend.GROWING, 75.0, datetime.now()
            ),
            'stable1': MarketMetrics(
                'stable1', 'Stable 1', 900000, 350000, 4.0, 12.0, 35, 0.94, 0.02, 3, 0.03,
                90.0, 70.0, MarketTrend.STABLE, 60.0, datetime.now()
            )
        }
    ...
    async def test_generate_expansion_recommendations(self, intelligence_service):
        ...
        intelligence_service.market_metrics = {
            'premium_market': MarketMetrics(
                'premium_market', 'Premium Market', 2000000, 600000, 12.0, 36.0, 25, 0.97, 0.05, 12, 0.15,
                120.0, 92.0, MarketTrend.RAPIDLY_GROWING, 88.0, datetime.now()
            ),
            'good_market': MarketMetrics(
                'good_market', 'Good Market', 1500000, 500000, 8.0, 24.0, 30, 0.95, 0.03, 8, 0.08,
                110.0, 85.0, MarketTrend.GROWING, 78.0, datetime.now()
            )
        }

        intelligence_service.competitive_analyses = {
            'premium_market': CompetitiveAnalysis(
                'premium_market', ['Competitor A'], {}, 0.7, [], {}, [], [], [],
                'moderate', OpportunityLevel.PREMIUM
            ),
            'good_market': CompetitiveAnalysis(
                'good_market', ['Competitor B'], {}, 0.6, [], {}, [], [], [],
                'low', OpportunityLevel.HIGH
            )
        }
