...
async def strategic_consultant():
    ...
    with patch('ghl_real_estate_ai.services.strategic_claude_consultant.get_database', return_value=AsyncMock()), \
         patch('ghl_real_estate_ai.services.strategic_claude_consultant.get_enterprise_tenant_service', return_value=AsyncMock()):
...
async def test_generate_strategic_recommendations(strategic_consultant, sample_business_context):
    ...
    for rec in recommendations:
        ...
        assert rec.strategic_priority in [StrategicPriority.REVENUE_OPTIMIZATION, StrategicPriority.OPERATIONAL_EFFICIENCY]
...
async def test_revenue_prediction(strategic_consultant, sample_historical_data):
    ...
    insight = await strategic_consultant._predict_revenue(
        sample_historical_data["revenue"],
        90
    )
...
async def test_execute_autonomous_workflow(strategic_consultant):
    ...
    result = await strategic_consultant.execute_autonomous_workflow(
...
async def test_end_to_end_consulting_workflow(strategic_consultant, sample_business_context, sample_historical_data):
    ...
    roi_result = await strategic_consultant.calculate_roi_attribution(
        intervention_date,
        {"revenue": sample_historical_data["revenue"]}
    )
    assert "overall_roi" in roi_result
