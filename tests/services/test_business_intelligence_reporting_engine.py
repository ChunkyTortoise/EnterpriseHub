...
class TestBusinessIntelligenceReportingEngine:
    ...
    def sample_business_data(self):
        ...
        return {
            ...
            "leads": {"total": 450, "qualified": 280, "converted": 65, "conversion_rate": 14.4},
            "properties": {"listed": 125, "sold": 32, "average_price": 675000, "days_on_market": 45},
            "agents": [
            ...
            return_value=json.dumps(
                {
                    ...
                    "risk_factors": ["Market cooling trends", "Interest rate sensitivity"],
                }
            )
        )
            ...
            return_value=json.dumps(
                {
                    ...
                    "profitability_metrics": {"gross_margin": 25.5, "net_margin": 12.3},
                    "financial_insights": [
            ...
            return_value=json.dumps(
                {
                    ...
                    "market_opportunities": ["First-time homebuyer segment", "Commercial real estate expansion"],
                    "threat_assessment": ["New market entrants", "Economic uncertainty"],
                }
            )
        )
    ...
    async def test_create_kpi_dashboard(self, engine, sample_business_data):
        ...
        dashboard = await engine.create_kpi_dashboard(
            data=sample_business_data, dashboard_config={"refresh_interval": 300}
        )
    ...
    async def test_generate_custom_report(self, engine, sample_business_data):
            ...
            return_value=json.dumps(
                {
                    ...
                    "findings": ["Top performers drive 60% of revenue", "Training programs show 20% improvement"],
                    "recommendations": ["Implement peer mentoring", "Expand successful training modules"],
                }
            )
        )
        ...
        custom_report = await engine.generate_custom_report(template=template, data=sample_business_data)
    ...
    async def test_export_report_data(self, engine):
        ...
        for export_format in ["pdf", "excel", "json"]:
            exported_data = await engine.export_report_data(report=report, format_type=export_format)
    ...
    async def test_performance_metrics_tracking(self, engine):
        ...
        engine.cache_service.get = AsyncMock(return_value=None)
        engine.cache_service.set = AsyncMock()
        engine.claude_client.generate = AsyncMock(return_value=json.dumps({"executive_summary": "Test summary"}))
    ...
    try:
        with patch("ghl_real_estate_ai.services.business_intelligence_reporting_engine.CacheService"):
            with patch("ghl_real_estate_ai.services.business_intelligence_reporting_engine.ClaudeClient"):
                ...
                engine.cache_service.set = AsyncMock()
                engine.claude_client.generate = AsyncMock(
                    return_value=json.dumps({"executive_summary": "Test summary", "key_highlights": ["Test highlight"]})
                )

                sample_data = {"revenue": {"current_month": 1000000}, "leads": {"total": 100, "converted": 15}}
