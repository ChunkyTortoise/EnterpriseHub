...
class TestRealtimeNotificationEngine:
    ...
    async def test_send_notification_basic(self, notification_engine, sample_notification_event):
        ...
        for provider in notification_engine.providers.values():
            provider.send_notification.return_value = {
                "success": True,
                "provider_message_id": "test_msg_123"
            }
    ...
    async def test_ai_notification_optimization(self, notification_engine, sample_notification_event):
        ...
        # Mock user context
        notification_engine._get_user_context = AsyncMock(return_value={
            "typical_active_hours": ["09:00", "17:00"],
            "engagement_history": {"email_open_rate": 0.75}
        })
    ...
    async def test_user_preferences_update(self, notification_engine, sample_notification_preferences):
        ...
        result = await notification_engine.update_user_preferences(
            "user_123", sample_notification_preferences
        )
...
class TestNotificationEngineIntegration:
    ...
    async def test_end_to_end_notification_flow(self, notification_engine):
        ...
        notification_engine._get_recipient_info = AsyncMock(return_value={
            "user_id": "user_123",
            "email": "test@example.com",
            "phone": "+1234567890"
        })
        
        notification_engine._get_user_preferences = AsyncMock(return_value=NotificationPreferences(
            user_id="user_123",
...
class TestNotificationOptimization:
    ...
    async def test_ai_timing_optimization(self, notification_engine):
        ...
        user_context = {
            ...
            "timezone": "UTC",
            "engagement_history": {
                "best_response_times": ["10:00", "14:00", "16:00"]
            }
        }
    ...
    async def test_channel_optimization_by_content(self, notification_engine):
        ...
        text_channel = await notification_engine._determine_optimal_channel(
            contact, "TEXT", text_event.message
        )
        
        urgent_channel = await notification_engine._determine_optimal_channel(
            contact, "TEXT", urgent_event.message
        )
