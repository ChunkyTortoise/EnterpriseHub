...
class TestPricingIntelligenceService:
    ...
    async def test_investment_opportunity_analysis(
        self, pricing_service, sample_property_data, mock_valuation_engine, mock_market_service, mock_cache_service
    ):
        ...
        result = await pricing_service.analyze_investment_opportunity(
            sample_property_data, purchase_price=650000, rental_analysis=True
        )
    ...
    async def test_investment_metrics_calculation(
        self, pricing_service, sample_property_data, mock_valuation_result, mock_market_service
    ):
        ...
        metrics = await pricing_service._calculate_investment_metrics(
            sample_property_data, purchase_price, mock_valuation_result, include_rental
        )
    ...
    @pytest.mark.asyncio
    async def test_market_timing_analysis(self, pricing_service, sample_property_data, mock_market_service):
        """Test market timing analysis for buy/sell decisions"""

        pricing_service.market_service = mock_market_service

        timing_data = await pricing_service._analyze_market_timing(sample_property_data, purchase_price=650000)
    ...
    @pytest.mark.asyncio
    async def test_opportunity_score_calculation(self, pricing_service, mock_valuation_result):
        ...
        score = await pricing_service._calculate_opportunity_score(
            high_return_metrics, timing_data, mock_valuation_result
        )
    ...
    async def test_pricing_recommendation_generation(
        self, pricing_service, sample_property_data, mock_valuation_engine, mock_market_service
    ):
        ...
        listing_goals = {"timeline": "normal", "priority": "balanced"}

        result = await pricing_service.generate_pricing_recommendation(
            sample_property_data, listing_goals=listing_goals, market_positioning="competitive"
        )
    ...
    async def test_rental_income_estimation(self, pricing_service):
        ...
        property_data = {"estimated_value": 600000, "sqft": 1800, "neighborhood": "Central"}
    ...
    @pytest.mark.asyncio
    async def test_risk_assessment(self, pricing_service, sample_property_data):
        ...
        timing_data = {"timing_recommendation": MarketTiming.HOLD_MONITOR, "timing_score": 40.0}
    ...
    @pytest.mark.asyncio
    async def test_market_response_estimation(self, pricing_service, sample_property_data, mock_market_service):
    ...
    @pytest.mark.asyncio
    async def test_competitive_advantage_identification(self, pricing_service, sample_property_data):
        ...
        mock_comparables = [Mock(sqft=1800), Mock(sqft=1900), Mock(sqft=2000)]

        advantages = await pricing_service._identify_competitive_advantages(property_with_amenities, mock_comparables)
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, pricing_service, mock_market_service):
    ...
    async def test_business_impact_metrics(
        ...
        result = await pricing_service.analyze_investment_opportunity(
            high_value_property, purchase_price=1500000, rental_analysis=True
        )
        ...
        assert result.investment_grade in [InvestmentGrade.GOOD, InvestmentGrade.EXCELLENT, InvestmentGrade.EXCEPTIONAL]

    @pytest.mark.asyncio
    async def test_market_condition_adaptability(self, pricing_service, sample_property_data, mock_valuation_engine):
        """Test adaptability to different market conditions"""

        pricing_service.valuation_engine = mock_valuation_engine

        # Test different market conditions
        market_conditions = [MarketCondition.STRONG_SELLERS, MarketCondition.BALANCED, MarketCondition.STRONG_BUYERS]
        ...
        for condition in market_conditions:
            ...
            result = await pricing_service.generate_pricing_recommendation(sample_property_data)
        ...
        assert (
            seller_rec.pricing_strategy != buyer_rec.pricing_strategy
            or seller_rec.recommended_price != buyer_rec.recommended_price
        )
    ...
    async def test_roi_calculation_accuracy(self, pricing_service):
        ...
        test_property = {"estimated_value": 500000, "sqft": 2000, "year_built": 2015}
...
class TestPricingIntelligencePerformance:
    ...
    async def test_investment_analysis_speed(self):
        ...
        result = await service.analyze_investment_opportunity(
            sample_property, purchase_price=480000, rental_analysis=True
        )
    ...
    async def test_pricing_recommendation_speed(self):
        ...
        result = await service.generate_pricing_recommendation(sample_property)
