...
class TestAdvancedAnalyticsVisualizationEngine:
    ...
    async def test_generate_predictive_insights(self, engine, sample_data):
        ...
        engine.claude_client.generate = AsyncMock(
            return_value=json.dumps(
                {
                    "predictions": [
                        {"metric": "revenue", "predicted_value": 2500000, "confidence": 0.85, "timeframe": "30_days"}
                    ],
                    "insights": [
                        {"type": "trend", "description": "Revenue growth trending upward", "confidence": 0.92}
                    ],
                }
            )
        )
    ...
    async def test_create_dashboard(self, engine, sample_data):
        ...
        engine.claude_client.generate = AsyncMock(
            return_value=json.dumps(
                {"insights": ["Strong performance this quarter"], "recommendations": ["Focus on high-value leads"]}
            )
        )

        dashboard = await engine.create_dashboard(
            data=sample_data, dashboard_type="executive", filters={"date_range": "30d"}
        )
    ...
    async def test_export_analytics_data(self, engine, sample_data):
        ...
        export_data = await engine.export_analytics_data(data=sample_data, format_type="json", include_charts=True)
    ...
    try:
        with patch("ghl_real_estate_ai.services.advanced_analytics_visualization_engine.CacheService"):
            with patch("ghl_real_estate_ai.services.advanced_analytics_visualization_engine.ClaudeClient"):
                ...
                engine.claude_client.generate = AsyncMock(
                    return_value=json.dumps(
                        {"insights": ["Test insight"], "predictions": [{"metric": "test", "value": 100}]}
                    )
                )

                sample_data = {"leads": [{"id": "1", "score": 85}], "properties": [{"id": "1", "price": 500000}]}

                # Run full pipeline
                dashboard = await engine.create_dashboard(data=sample_data, dashboard_type="operational")
