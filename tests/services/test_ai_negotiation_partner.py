...
def _make_mock_seller_psychology(**overrides):
    """Create a valid SellerPsychologyProfile Pydantic model for tests."""
    defaults = {
        "motivation_type": SellerMotivationType.FINANCIAL,
        "urgency_level": UrgencyLevel.MODERATE,
        "urgency_score": 50.0,
        "behavioral_pattern": ListingBehaviorPattern.REALISTIC_PRICING,
        "emotional_attachment_score": 40.0,
        "financial_pressure_score": 50.0,
        "flexibility_score": 50.0,
        "response_time_hours": 4.0,
        "communication_style": "professional",
        "primary_concerns": ["price", "timeline"],
        "negotiation_hot_buttons": ["closing_costs"],
        "relationship_importance": 50.0,
    }
    defaults.update(overrides)
    return SellerPsychologyProfile(**defaults)


def _make_mock_market_leverage(**overrides):
    """Create a valid MarketLeverage Pydantic model for tests."""
    defaults = {
        "overall_leverage_score": 60.0,
        "market_condition": MarketCondition.BALANCED,
        "inventory_levels": {"500k-750k": 4.5},
        "competitive_pressure": 50.0,
        "seasonal_advantage": 5.0,
        "property_uniqueness_score": 50.0,
        "comparable_sales_strength": 60.0,
        "price_positioning": "fairly_priced",
        "financing_strength": 75.0,
        "cash_offer_boost": 0.0,
        "quick_close_advantage": 0.0,
    }
    defaults.update(overrides)
    return MarketLeverage(**defaults)


def _make_mock_strategy(**overrides):
    """Create a valid NegotiationStrategy Pydantic model for tests."""
    defaults = {
        "primary_tactic": NegotiationTactic.PRICE_FOCUSED,
        "confidence_score": 75.0,
        "recommended_offer_price": Decimal("712500"),
        "offer_range": {"min": Decimal("690000"), "target": Decimal("712500"), "max": Decimal("735000")},
        "key_terms_to_emphasize": ["competitive_price"],
        "concessions_to_request": ["quick_close"],
        "relationship_building_approach": "professional_analytical",
        "opening_strategy": "Present offer with market analysis",
        "response_to_counter": "Focus on market data",
        "negotiation_timeline": "standard_timeline",
        "primary_talking_points": [],
        "objection_responses": {},
    }
    defaults.update(overrides)
    return NegotiationStrategy(**defaults)


def _make_mock_win_probability(**overrides):
    """Create a valid WinProbabilityAnalysis Pydantic model for tests."""
    defaults = {
        "win_probability": 72.0,
        "confidence_interval": {"lower": 65.0, "upper": 79.0},
        "factor_weights": {},
        "risk_factors": [],
        "success_drivers": [],
        "scenarios": {},
        "optimal_offer_analysis": {},
    }
    defaults.update(overrides)
    return WinProbabilityAnalysis(**defaults)
...
class TestNegotiationIntelligence:
    ...
    async def test_complete_analysis_workflow(self, negotiation_partner, mock_analysis_request):
        ...
        mock_leverage = _make_mock_market_leverage(overall_leverage_score=75.0)
        ...
        with patch.object(negotiation_partner, '_gather_analysis_data') as mock_gather, \
            ...
            mock_strategy_call.return_value = mock_strategy
            mock_win_call.return_value = mock_win
    ...
    async def test_analysis_performance_target(self, negotiation_partner, mock_analysis_request):
        ...
        mock_psych = _make_mock_seller_psychology()
        mock_leverage = _make_mock_market_leverage()
...
class TestRealTimeCoaching:
    ...
    async def test_realtime_coaching_basic(self, negotiation_partner):
        ...
        # Setup active negotiation with properly-configured mocks
        mock_intelligence = MagicMock()
        mock_intelligence.negotiation_strategy.primary_tactic = NegotiationTactic.PRICE_FOCUSED
        mock_intelligence.seller_psychology.motivation_type = SellerMotivationType.FINANCIAL
        mock_intelligence.seller_psychology.urgency_level = UrgencyLevel.MODERATE
        mock_intelligence.market_leverage.overall_leverage_score = 60.0
        mock_intelligence.win_probability.win_probability = 72.0
    ...
    async def test_coaching_counter_offer_scenario(self, negotiation_partner):
        ...
        # Setup active negotiation with specific strategy and numeric attribute values
        mock_intelligence = MagicMock()
        mock_intelligence.negotiation_strategy.primary_tactic = NegotiationTactic.PRICE_FOCUSED
        mock_intelligence.seller_psychology.motivation_type = SellerMotivationType.FINANCIAL
        mock_intelligence.seller_psychology.urgency_level = UrgencyLevel.HIGH
        mock_intelligence.market_leverage.overall_leverage_score = 75.0
        mock_intelligence.win_probability.win_probability = 80.0
        ...
        with patch.object(negotiation_partner, '_analyze_conversation_context') as mock_analyze, \
            ...
            assert "counter" in result.immediate_guidance.lower() or "strategy" in result.immediate_guidance.lower()
...
class TestStrategyUpdates:
    ...
    async def test_strategy_update_new_information(self, negotiation_partner):
        ...
        # Setup active negotiation with proper attribute values
        mock_intelligence = MagicMock()
        mock_intelligence.property_id = "PROP_12345"
        mock_intelligence.lead_id = "LEAD_67890"
        mock_intelligence.tenant_id = "tenant_test"
        mock_intelligence.seller_psychology = _make_mock_seller_psychology()
        mock_intelligence.market_leverage = _make_mock_market_leverage()
        ...
        with patch.object(negotiation_partner.psychology_analyzer, 'analyze_seller_psychology') as mock_psych, \
            ...
            mock_summary.return_value = ("Updated summary", ["insight"], ["action"])
...
class TestIntegrationScenarios:
    ...
    async def test_distressed_seller_scenario(self, negotiation_partner):
        ...
        with patch.object(negotiation_partner, '_gather_analysis_data') as mock_gather, \
            ...
            mock_strategy_call.return_value = mock_strategy
            mock_win_call.return_value = mock_win
            mock_summary.return_value = ("Distressed seller summary", ["urgency is critical"], ["act fast"])
            mock_dna.return_value = None
            mock_voss.return_value = {"voss_level": "tactical_empathy", "generated_response": None}
    ...
    async def test_emotional_seller_scenario(self, negotiation_partner):
        ...
        mock_leverage = _make_mock_market_leverage(overall_leverage_score=55.0)
        mock_strategy = _make_mock_strategy(
            primary_tactic=NegotiationTactic.RELATIONSHIP_BUILDING,
            relationship_building_approach="personal_empathetic",
        )
        ...
        with patch.object(negotiation_partner, '_gather_analysis_data') as mock_gather, \
            ...
            mock_strategy_call.return_value = mock_strategy
            mock_win_call.return_value = mock_win
            mock_summary.return_value = ("Emotional seller summary", [], [])
            mock_dna.return_value = None
            mock_voss.return_value = {"voss_level": "tactical_empathy", "generated_response": None}
