...
class TestAdvancedPropertyMatchingEngine:
    ...
    async def test_behavioral_property_matching(
        self, property_matching_engine, mock_lead_preferences,
        mock_conversation_history, mock_property_inventory
    ):
        """Test behavioral property matching with ML-enhanced scoring."""

        with patch.object(property_matching_engine, '_get_property_inventory', return_value=mock_property_inventory), \
    ...
    @pytest.mark.asyncio
    async def test_behavioral_weight_calculation(
        self, property_matching_engine, mock_conversation_history
    ):
    ...
    async def test_matching_feedback_processing(self, property_matching_engine):
        ...
        with patch.object(property_matching_engine, '_update_behavioral_model') as mock_update, \
    ...
    async def test_health_status_monitoring(self, property_matching_engine):
        ...
        with patch.object(property_matching_engine, '_check_ml_model_health') as mock_ml_health, \
            ...
            mock_data_health.return_value = {'status': 'healthy', 'last_refresh': '2025-01-25T09:00:00', 'property_count': 1247}
...
class TestConversationIntelligenceService:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_conversation_analysis(
        self, conversation_service, mock_conversation_data
    ):
        """Test comprehensive conversation analysis with insights generation."""

        with patch.object(conversation_service, '_analyze_sentiment_timeline') as mock_sentiment, \
            ...
            assert insights.dominant_sentiment in ['very_positive', 'positive', 'neutral', 'slightly_negative', 'negative', 'very_negative']
    ...
    @pytest.mark.asyncio
    async def test_objection_detection_and_responses(
        self, conversation_service, mock_conversation_data
    ):
        """Test objection detection and response recommendation system."""

        with patch.object(conversation_service, '_classify_objection_type') as mock_classify, \
    ...
    @pytest.mark.asyncio
    async def test_jorge_methodology_coaching_opportunities(
        self, conversation_service, mock_jorge_conversation
    ):
        """Test coaching opportunity identification for Jorge methodology."""

        with patch.object(conversation_service, '_analyze_jorge_methodology_effectiveness') as mock_jorge, \
            ...
            assert coaching_op.opportunity_type in ['objection_handling', 'closing_technique', 'rapport_building', 'active_listening']
    ...
    @pytest.mark.asyncio
    async def test_sentiment_timeline_tracking(
        self, conversation_service, mock_conversation_data
    ):
        ...
        with patch.object(conversation_service, '_process_sentiment_timeline') as mock_timeline:
            ...
            for point in timeline:
                ...
                assert point.sentiment_classification in ['very_positive', 'positive', 'neutral', 'slightly_negative', 'negative', 'very_negative']
    ...
    async def test_health_status_monitoring(self, conversation_service):
        ...
        with patch.object(conversation_service, '_check_sentiment_model_health') as mock_sentiment_health, \
...
class TestClientPreferenceLearningEngine:
    ...
    async def test_conversation_preference_learning(
        ...
        with patch.object(preference_learning_engine, '_extract_preferences_from_conversation') as mock_extract, \
    ...
    @pytest.mark.asyncio
    async def test_property_interaction_learning(
        self, preference_learning_engine, mock_property_interaction_data
    ):
        """Test preference learning from property interaction behavior."""

        with patch.object(preference_learning_engine, '_analyze_interaction_patterns') as mock_analyze, \
    ...
    @pytest.mark.asyncio
    async def test_preference_profile_retrieval(
        self, preference_learning_engine, mock_existing_preference_profile
    ):
        """Test preference profile retrieval with completeness scoring."""

        with patch.object(preference_learning_engine, '_get_profile_from_database') as mock_get_profile, \
    ...
    @pytest.mark.asyncio
    async def test_preference_match_prediction(
        self, preference_learning_engine, mock_existing_preference_profile
    ):
        ...
        with patch.object(preference_learning_engine, 'get_preference_profile', return_value=mock_existing_preference_profile), \
    ...
    @pytest.mark.asyncio
    async def test_preference_drift_detection(
        self, preference_learning_engine, mock_existing_preference_profile
    ):
        ...
        with patch.object(preference_learning_engine, '_get_historical_preferences') as mock_history, \
            ...
            drift_detections = await preference_learning_engine.analyze_preference_drift(
                ...
                min_significance_threshold=0.7
            )
            ...
            style_drift = next(
                (d for d in drift_detections if 'style' in str(d.preference_category).lower()),
                None
            )
    ...
    async def test_health_status_monitoring(self, preference_learning_engine):
        ...
        with patch.object(preference_learning_engine, '_check_learning_accuracy') as mock_accuracy, \
