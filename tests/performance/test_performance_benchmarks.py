...
class TestAIResponsePerformance:
    ...
    async def test_claude_semantic_caching_performance(self, claude_assistant):
        ...
        response1 = await claude_assistant.explain_match_with_claude(
        ...
        start_time = time.perf_counter()
        response2 = await claude_assistant.explain_match_with_claude(
            property_data, lead_preferences
        )
    ...
    async def test_ai_response_time_target(self, claude_assistant):
        ...
        for description, preferences in test_queries:
            ...
            start_time = time.perf_counter()
            response = await claude_assistant.explain_match_with_claude(
...
class TestCachePerformance:
    ...
    async def test_cached_computation_performance(self, cache_service):
        ...
        start_time = time.perf_counter()
        result1 = await cache_service.cached_computation(
            "expensive_test", expensive_computation, 300, 10
        )
        first_call_time = time.perf_counter() - start_time
        
        # Second call (cache hit)
        start_time = time.perf_counter()
        result2 = await cache_service.cached_computation(
            "expensive_test", expensive_computation, 300, 10
        )
        second_call_time = time.perf_counter() - start_time
...
if __name__ == "__main__":
    ...
    pytest.main([
        ...
        "-v",
        "--tb=short",
        "-k", "test_claude_semantic_caching_performance or test_cache_warming_performance"
    ])