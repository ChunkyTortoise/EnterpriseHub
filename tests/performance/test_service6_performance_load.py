...
from tests.mocks.external_services import (
    MockClaudeClient, MockRedisClient, MockDatabaseService,
...
class TestLeadAnalysisPerformance:
    ...
    async def test_sustained_lead_analysis_throughput(self, performance_orchestrator):
        ...
        assert summary['response_times']['p95'] <= 300, f"P95 latency {summary['response_times']['p95']:.1f}ms should be <=300ms"
        assert summary['reliability']['success_rate'] >= 0.99, f"Success rate {summary['reliability']['success_rate']:.3f} should be >=99%"
    ...
    async def test_real_time_scoring_latency(self, performance_orchestrator):
        ...
        assert summary['response_times']['p95'] <= 200, f"RT scoring P95 {summary['response_times']['p95']:.1f}ms should be <=200ms"
        assert summary['response_times']['p99'] <= 500, f"RT scoring P99 {summary['response_times']['p99']:.1f}ms should be <=500ms"
        assert summary['response_times']['mean'] <= 100, f"RT scoring mean {summary['response_times']['mean']:.1f}ms should be <=100ms"
...
class TestDatabasePerformance:
    ...
    async def test_database_crud_performance(self, performance_database):
        ...
        for i in range(200):
            ...
            async def update_with_timing(lead_id, score):
                start_time = time.time()
                try:
                    result = await performance_database.update_lead_score(
                        lead_id, score, {'test': 'performance_test'}
                    )
                    end_time = time.time()
                    metrics.record_operation(start_time, end_time, success=result)
                    return result
        ...
        assert summary['response_times']['mean'] <= 100, f"DB avg latency {summary['response_times']['mean']:.1f}ms should be <=100ms"
        assert summary['response_times']['p95'] <= 200, f"DB P95 latency {summary['response_times']['p95']:.1f}ms should be <=200ms"
    ...
    async def test_concurrent_database_operations(self, performance_database):
        ...
        for i in range(60):
            lead_data = create_test_lead_data({
                'lead_id': f'CONCURRENT_NEW_{i:03d}',
        ...
        for i in range(40):
            async def concurrent_update(lead_id, score):
                ...
                try:
                    result = await performance_database.update_lead_score(
                        lead_id, score, {'concurrent_test': True}
                    )
                    end_time = time.time()
                    metrics.record_operation(start_time, end_time, success=result)
                    return result
        ...
        assert concurrent_throughput >= 50, f"Concurrent DB throughput {concurrent_throughput:.1f} ops/sec should be >=50/sec"
        assert summary['response_times']['p95'] <= 250, f"Concurrent DB P95 {summary['response_times']['p95']:.1f}ms should be <=250ms"
