...
class PerformanceTestSuite:
    ...
    async def load_test_lead_processing(self, 
                                      concurrent_users: int = 10,
                                      duration_seconds: int = 300,
                                      target_rps: float = 10.0) -> LoadTestResult:
        """
    ...
    async def _simulate_lead_analysis(self, worker_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/api/score_lead",
                json=lead_data,
...
async def test_target_load_performance():
    ...
    async with PerformanceTestSuite() as suite:
        ...
        assert result.p95_response_time_ms < 30000, f"P95 response time {result.p95_response_time_ms:.0f}ms exceeds 30s target"
...
async def test_stress_performance():
    ...
    async with PerformanceTestSuite() as suite:
        ...
        assert any("20_users" in level for level in passing_levels), \
...
if __name__ == "__main__":
    ...
    async def run_performance_tests():
        ...
        async with PerformanceTestSuite() as suite:
            ...
            baseline = await suite.load_test_lead_processing(
                concurrent_users=1, duration_seconds=60, target_rps=1.0
            )
            baseline_targets = suite.validate_performance_targets(baseline)
            print(f"   ✅ Response Time: {baseline.avg_response_time_ms:.0f}ms avg, {baseline.p95_response_time_ms:.0f}ms P95")
            print(f"   ✅ Throughput: {baseline.throughput_rps:.2f} RPS")
            print(f"   ✅ Error Rate: {baseline.error_rate:.1%}")
            print(f"   {'✅' if baseline_targets['overall_pass'] else '❌'} Overall: {'PASS' if baseline_targets['overall_pass'] else 'FAIL'}")
            ...
            print(f"   ✅ Response Time: {target_load.avg_response_time_ms:.0f}ms avg, {target_load.p95_response_time_ms:.0f}ms P95")
            print(f"   ✅ Throughput: {target_load.throughput_rps:.2f} RPS")
            print(f"   ✅ Error Rate: {target_load.error_rate:.1%}")
            print(f"   {'✅' if target_targets['overall_pass'] else '❌'} Overall: {'PASS' if target_targets['overall_pass'] else 'FAIL'}")
            
            # Performance summary
            print("\n📊 Performance Summary")
            print("=====================")
            print(f"Target: <30s response times     {'✅ MET' if baseline.p95_response_time_ms < 30000 else '❌ MISSED'}")
