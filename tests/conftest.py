...
async def mock_external_services():
    ...
    from tests.mocks.external_services import MockApolloClient, MockClaudeClient, MockSendGridClient, MockTwilioClient
...
@pytest.fixture
async def integrated_service6_stack(
    mock_database_service, mock_redis_cache, mock_external_services, service6_test_config
):
...
def _patch_compliance_guard(mocker):
    mocker.patch(
        ...
        new=AsyncMock(return_value=(_get_compliance_status(), "ok", [])),
    )
