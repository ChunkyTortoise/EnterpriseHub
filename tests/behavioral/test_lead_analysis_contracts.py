...
from tests.mocks.external_services import create_test_lead_data, MockClaudeClient, MockRedisClient
...
class TestHighIntentLeadContracts:
    ...
    def test_qualified_buyer_with_urgency_contract(self):
        ...
        assert any(urgency_signal in reasoning_text.lower() 
                  for urgency_signal in ["cash", "60 days", "close", "timeline"]), \
        ...
        priority_actions = [action for action in actions if any(
            priority_word in action.lower() 
            for priority_word in ["priority", "immediately", "24", "sms", "notify"]
        )]
...
class TestLeadTemperatureConsistency:
    ...
    def test_progressive_lead_warming_contract(self):
        ...
        stages = [
            ...
            {'location': 'Austin', 'timeline': 'soon', 'budget': '500000', 'bedrooms': '3', 'financing': 'pre-approved'}  # Financing
        ]
        ...
        for i in range(1, len(scores)):
            assert scores[i] >= scores[i-1], f"Score decreased from stage {i-1} to {i}: {scores[i-1]} -> {scores[i]}"
        ...
        for prop in properties:
            try:
                ...
                if prop['price'] <= 350000:
                    ...
                    assert any(positive_word in psychological_fit.lower() 
                    ...
                    budget_concern_mentioned = any(concern in psychological_fit.lower() 
                    ...
                    if not budget_concern_mentioned:
                        ...
                        assert any(caution_word in psychological_fit.lower() 
...
class TestBusinessOutcomeValidation:
    ...
    def test_conversion_probability_alignment_contract(self):
        ...
        for scenario in high_conversion_scenarios:
            ...
            immediate_actions = [a for a in actions if any(
                immediate_word in a.lower() 
                for immediate_word in ["immediate", "24", "sms", "notify", "priority"]
            )]
            assert len(immediate_actions) >= 1, \
                f"{scenario['profile']} missing immediate actions: {actions}"
            
            # Validate business outcome alignment
            high_value_actions = [a for a in actions if any(
                value_word in a.lower() 
                for value_word in ["showing", "tour", "pre-approval", "offer", "contract"]
            )]
            assert len(high_value_actions) >= 1, \
                f"{scenario['profile']} missing high-value actions: {actions}"
    ...
    def test_churn_risk_mitigation_contract(self):
        ...
        for profile_data in churn_risk_profiles:
            ...
            retention_actions = [a for a in actions if any(
                retention_word in a.lower() 
                for retention_word in ["nurture", "educational", "value", "build", "trust"]
            )]
            assert len(retention_actions) >= 1, \
                f"{profile_data['profile']} missing retention actions: {actions}"
            
            # Ensure no aggressive sales actions for churn risk
            aggressive_actions = [a for a in actions if any(
                aggressive_word in a.lower() 
                for aggressive_word in ["immediately", "pressure", "close", "urgent"]
            )]
...
class TestCrossServiceBehavioralConsistency:
    ...
    def test_recommendation_scoring_alignment_contract(self):
        ...
        for scenario in score_scenarios:
            ...
            matching_actions = sum(1 for expected in scenario['expected_actions'] 
                                 if expected in action_text)
            assert matching_actions >= 2, \
                f"{scenario['name']} actions don't align with score {score}: {actions}"
...
class TestBusinessRuleCompliance:
    ...
    def test_data_privacy_contract(self):
        ...
        reasoning_text = reasoning.get("reasoning", "")
        sensitive_requests = any(sensitive_word in reasoning_text.lower() 
