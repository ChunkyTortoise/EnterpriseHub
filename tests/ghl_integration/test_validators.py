...
class TestWebhookValidator:
    ...
    async def test_validate_hmac_success(self, validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "my_secret"}):
            payload = b'{"test": "data"}'
            expected_sig = hmac.new(b"my_secret", payload, hashlib.sha256).hexdigest()
    ...
    async def test_validate_hmac_with_prefix(self, validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "my_secret"}):
            payload = b'{"test": "data"}'
            expected_sig = "sha256=" + hmac.new(b"my_secret", payload, hashlib.sha256).hexdigest()
    ...
    async def test_validate_dev_mode_skip(self, validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "my_secret", "GHL_SKIP_SIGNATURE_VERIFICATION": "true"}):
            payload = b'{"test": "data"}'
...
class TestMultiValidator:
    ...
    async def test_full_validation_success(self, multi_validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "test_secret"}):
            ...
            result = await multi_validator.validate(payload, sig, {"X-GHL-Timestamp": "2026-02-11T10:00:00Z"})
    ...
    async def test_full_validation_invalid_signature(self, multi_validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "test_secret"}):
            ...
            result = await multi_validator.validate(payload, "invalid_sig", {"X-GHL-Timestamp": "2026-02-11T10:00:00Z"})
    ...
    async def test_full_validation_invalid_ip(self, multi_validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "test_secret", "GHL_ALLOWED_IPS": "1.2.3.4,5.6.7.8"}):
            ...
            result = await multi_validator.validate(
                payload, sig, {"X-GHL-Timestamp": "2026-02-11T10:00:00Z"}, client_ip="9.10.11.12"
            )
    ...
    async def test_full_validation_valid_ip(self, multi_validator):
        ...
        with patch.dict(os.environ, {"GHL_WEBHOOK_SECRET": "test_secret", "GHL_ALLOWED_IPS": "1.2.3.4,5.6.7.8"}):
            ...
            result = await multi_validator.validate(
                payload, sig, {"X-GHL-Timestamp": "2026-02-11T10:00:00Z"}, client_ip="1.2.3.4"
            )
