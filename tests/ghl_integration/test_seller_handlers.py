...
class TestSellerHandlers:
    ...
    async def test_handle_seller_inquiry_with_seller_tag(self):
        ...
        with patch("ghl_integration.handlers.seller_handlers._generate_cma", new_callable=AsyncMock) as mock_cma:
            mock_cma.return_value = {
                "success": True,
                "estimated_value": 750000
            }
            with patch("ghl_integration.handlers.seller_handlers._send_seller_greeting", new_callable=AsyncMock):
                with patch("ghl_integration.handlers.seller_handlers._update_ghl_seller_fields", new_callable=AsyncMock):
    ...
    async def test_handle_seller_inquiry_without_address(self):
        ...
        with patch("ghl_integration.handlers.seller_handlers._send_seller_greeting", new_callable=AsyncMock) as mock_greeting:
            with patch("ghl_integration.handlers.seller_handlers._store_seller_state", new_callable=AsyncMock):
                with patch("ghl_integration.handlers.seller_handlers._emit_seller_event", new_callable=AsyncMock):
    ...
    async def test_handle_listing_created_seller_pipeline(self, opportunity_create_payload):
        ...
        with patch("ghl_integration.handlers.seller_handlers._get_seller_pipeline_ids", return_value=["pipeline_seller_001"]):
            with patch("ghl_integration.handlers.seller_handlers._get_contact_details", new_callable=AsyncMock) as mock_contact:
                mock_contact.return_value = {
                    "name": "Jane Doe",
                    "email": "jane@example.com"
                }
                with patch("ghl_integration.handlers.seller_handlers._send_listing_confirmation", new_callable=AsyncMock):
                    with patch("ghl_integration.handlers.seller_handlers._store_seller_state", new_callable=AsyncMock):
                        with patch("ghl_integration.handlers.seller_handlers._update_ghl_seller_fields", new_callable=AsyncMock):
                            with patch("ghl_integration.handlers.seller_handlers._emit_seller_event", new_callable=AsyncMock):
    ...
    async def test_handle_listing_created_non_seller_pipeline(self):
        ...
        with patch("ghl_integration.handlers.seller_handlers._get_seller_pipeline_ids", return_value=["pipeline_other"]):
            payload = {
                "data": {
    ...
    async def test_handle_seller_response_success(self):
        ...
        mock_state = {
            "contact_id": "contact_seller_001",
            "qualification_stage": "Q0",
            "name": "Jane Seller"
        }
        
        with patch("ghl_integration.handlers.seller_handlers._get_seller_state", new_callable=AsyncMock) as mock_get_state:
            mock_get_state.return_value = mock_state
            with patch("ghl_integration.handlers.seller_handlers._process_qualification_message", new_callable=AsyncMock) as mock_process:
                ...
                with patch("ghl_integration.handlers.seller_handlers._send_seller_message", new_callable=AsyncMock):
                    with patch("ghl_integration.handlers.seller_handlers._store_seller_state", new_callable=AsyncMock) as mock_store:
                        with patch("ghl_integration.handlers.seller_handlers._update_ghl_seller_fields", new_callable=AsyncMock):
                            with patch("ghl_integration.handlers.seller_handlers._emit_seller_event", new_callable=AsyncMock):
    ...
    async def test_handle_seller_response_no_state(self):
        ...
        with patch("ghl_integration.handlers.seller_handlers._get_seller_state", return_value=None):
            with patch("ghl_integration.handlers.seller_handlers._initialize_seller_from_contact", new_callable=AsyncMock) as mock_init:
                mock_init.return_value = {
                    "contact_id": "contact_seller_new",
                    "qualification_stage": "Q0"
                }
                with patch("ghl_integration.handlers.seller_handlers._process_qualification_message", new_callable=AsyncMock) as mock_process:
...
class TestSellerHelperFunctions:
    ...
    def test_extract_address_from_message(self):
        ...
        data = {
            "message": "I live at 456 Oak Avenue in Rancho Cucamonga"
        }
