...
class TestTwilioWebhookSecurity:
    ...
    def generate_twilio_signature(self, url: str, params: Dict[str, str], auth_token: str) -> str:
        ...
        signature = hmac.new(auth_token.encode("utf-8"), data.encode("utf-8"), hashlib.sha1).digest()
    ...
    def test_valid_twilio_signature(self, sample_twilio_payload, twilio_auth_token):
        ...
        valid_signature = self.generate_twilio_signature(webhook_url, sample_twilio_payload, twilio_auth_token)
    ...
    def test_twilio_url_tampering_detection(self, sample_twilio_payload, twilio_auth_token):
        ...
        valid_signature = self.generate_twilio_signature(original_url, sample_twilio_payload, twilio_auth_token)
...
class TestSendGridWebhookSecurity:
    ...
    def generate_sendgrid_signature(self, payload: str, timestamp: str, verification_key: str) -> str:
        ...
        signature = hmac.new(verification_key.encode("utf-8"), signed_payload.encode("utf-8"), hashlib.sha256).digest()
        return base64.b64encode(signature).decode("utf-8")
    ...
    def test_valid_sendgrid_signature(self, sample_sendgrid_payload, sendgrid_verification_key):
        ...
        valid_signature = self.generate_sendgrid_signature(payload_str, timestamp, sendgrid_verification_key)
    ...
    def test_sendgrid_timestamp_validation(self, sample_sendgrid_payload, sendgrid_verification_key):
        ...
        valid_signature = self.generate_sendgrid_signature(payload_str, old_timestamp, sendgrid_verification_key)
    ...
    def test_missing_sendgrid_headers(self, sample_sendgrid_payload):
        ...
        mock_request = Mock()
        mock_request.headers = {"X-Twilio-Email-Event-Webhook-Timestamp": str(int(time.time()))}
        mock_request.body = payload_str.encode("utf-8")

        assert verify_sendgrid_signature(mock_request) is False

        # Test missing timestamp header
        mock_request.headers = {"X-Twilio-Email-Event-Webhook-Signature": "some_signature"}
...
class TestWebhookSecurityIntegration:
    ...
    async def test_ghl_webhook_endpoint_security(self, test_client, sample_ghl_payload, ghl_secret):
        ...
        valid_signature = hmac.new(ghl_secret.encode("utf-8"), payload_str.encode("utf-8"), hashlib.sha256).hexdigest()

        try:
            response = test_client.post(
                "/webhooks/ghl", json=sample_ghl_payload, headers={"X-GHL-Signature": f"sha256={valid_signature}"}
            )
    ...
    async def test_unauthorized_webhook_access_blocked(self, test_client, sample_ghl_payload):
        ...
        try:
            response = test_client.post(
                "/webhooks/ghl", json=sample_ghl_payload, headers={"X-GHL-Signature": invalid_signature}
            )
    ...
    def test_webhook_signature_algorithms_strength(self):
        ...
        ghl_signature = hmac.new(test_secret.encode("utf-8"), test_data.encode("utf-8"), hashlib.sha256).hexdigest()
        assert len(ghl_signature) == 64  # SHA256 produces 64-char hex string

        # SendGrid uses HMAC-SHA256 (strong)
        sendgrid_signature = hmac.new(test_secret.encode("utf-8"), test_data.encode("utf-8"), hashlib.sha256).digest()
        assert len(base64.b64encode(sendgrid_signature).decode("utf-8")) > 40

        # Twilio uses HMAC-SHA1 (acceptable for legacy compatibility)
        twilio_signature = hmac.new(test_secret.encode("utf-8"), test_data.encode("utf-8"), hashlib.sha1).digest()
    ...
    @pytest.mark.parametrize("provider,secret_length", [("ghl", 32), ("twilio", 32), ("sendgrid", 32), ("apollo", 32)])
...
class TestWebhookRateLimiting:
    ...
    async def test_webhook_rate_limiting(self, test_client, sample_ghl_payload, ghl_secret):
        ...
        try:
            ...
            for _ in range(100):  # Send 100 requests rapidly
                response = test_client.post("/webhooks/ghl", json=sample_ghl_payload, headers=headers)
    ...
    def test_webhook_payload_size_limits(self, test_client):
        ...
        try:
            response = test_client.post(
                "/webhooks/ghl", json=large_payload, headers={"X-GHL-Signature": "sha256=dummy_signature"}
            )
