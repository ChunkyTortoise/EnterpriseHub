...
class TestWebhookSignatureValidation:
    ...
    def test_ghl_webhook_valid_signature(self, signature_validator):
        ...
        expected_signature = hmac.new(
            signature_validator.ghl_secret.encode(),
            signature_string.encode(),
            hashlib.sha256
        ).hexdigest()
        
        # Act
        is_valid = signature_validator.validate_ghl_signature(
    ...
    def test_ghl_webhook_tampered_payload(self, signature_validator):
        ...
        is_valid = signature_validator.validate_ghl_signature(
            tampered_json, valid_signature, timestamp
        )
    ...
    def test_twilio_webhook_signature_validation(self, signature_validator):
        ...
        expected_signature = base64.b64encode(
            hmac.new(
                signature_validator.twilio_auth_token.encode(),
                signature_string.encode(),
                hashlib.sha1
            ).digest()
        ).decode()
        ...
        # Act
        is_valid = signature_validator.validate_twilio_signature(
            params, expected_signature, url
        )
    ...
    def test_sendgrid_webhook_signature_validation(self, signature_validator):
        ...
        expected_signature = base64.b64encode(
            hmac.new(
                signature_validator.sendgrid_webhook_secret.encode(),
        ...
        # Act
        is_valid = signature_validator.validate_sendgrid_signature(
            payload, expected_signature, timestamp
        )
...
class TestReplayAttackPrevention:
    ...
    def test_reject_old_timestamp(self, replay_validator):
        ...
        # Act
        is_valid_time = replay_validator.validate_timestamp(
            old_timestamp, max_age_seconds=300
        )
    ...
    def test_reject_future_timestamp(self, replay_validator):
        ...
        # Act
        is_valid_time = replay_validator.validate_timestamp(
            future_timestamp, max_age_seconds=300
        )
...
class TestInputValidationAndSanitization:
    ...
    def test_oversized_payload_rejection(self, input_validator):
        ...
        oversized_payload = {
            ...
            'malicious_field': oversized_data
        }
        
        # Act
        is_valid = input_validator.validate_payload_size(
...
class TestDataEncryptionAndPIIProtection:
    ...
    def test_gdpr_data_deletion(self, encryption_handler):
        ...
        deletion_result = encryption_handler.gdpr_delete_data(
            lead_data['lead_id'], 
...
class TestAuthenticationAndAuthorization:
    ...
    async def test_webhook_source_authorization(self, auth_handler):
        ...
        authorized_sources = [
            'webhook.gohighlevel.com',
            'webhooks.twilio.com',
            'webhook.sendgrid.com',
            'api.apollo.io'
        ]
...
class TestComplianceAndAuditTrail:
    ...
    async def test_security_incident_logging(self, audit_logger):
        ...
        incident_log = await audit_logger.log_security_incident(
            incident_type=security_incident['incident_type'],
            details=security_incident,
            severity='medium'
        )
