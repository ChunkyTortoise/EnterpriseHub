...
class Service6TestRunner:
    ...
    def generate_performance_report(self, results: List[TestResult]) -> Dict:
        ...
        performance_metrics = {
            ...
            "tests_per_second": sum(r.passed + r.failed for r in results) / sum(r.duration_seconds for r in results) if results else 0
        }
    ...
    def generate_coverage_report(self, results: List[TestResult]) -> Dict:
        ...
        weighted_coverage = sum(
            r.coverage_percentage * (r.passed + r.failed) 
            for r in coverage_results
        ) / total_tests if total_tests > 0 else 0
        
        coverage_by_mode = {
            r.mode: r.coverage_percentage 
            for r in coverage_results
        }
    ...
    def print_final_report(self, suite_report: TestSuiteReport):
        ...
        print(f"   Success Rate: {Fore.GREEN if success_rate > 90 else Fore.YELLOW}{success_rate:.1f}%{Style.RESET_ALL}")
        ...
        if suite_report.overall_coverage_percentage is not None:
            ...
            print(f"   Overall Coverage: {coverage_color}{suite_report.overall_coverage_percentage:.1f}%{Style.RESET_ALL}")
        ...
        overall_success = suite_report.total_failed == 0 and (
            suite_report.overall_coverage_percentage is None or 
            suite_report.overall_coverage_percentage >= 80
        )
...
Examples:
  ...
  python tests/run_service6_tests.py performance --parallel=2
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Coverage percentage threshold (default: 80)"
    )
