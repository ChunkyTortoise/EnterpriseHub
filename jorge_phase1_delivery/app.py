...
if selected_hub == "🏢 Executive Command Center":
    ...
    with tab2:
        ...
        with col_p1:
            ...
            with st.expander("Manually Ingest Property"):
                n_title = st.text_input("Title", "New Luxury Home")
                n_price = st.number_input("Price", value=850000)
                n_beds = st.number_input("Bedrooms", value=4)
                n_baths = st.number_input("Bathrooms", value=3)
                n_loc = st.text_input("Neighborhood", "Alta Loma")
                
                if st.button("Ingest to AI Inventory"):
                    from services.demo_state import DemoStateManager
                    ...
                    new_prop = {
                        ...
                        "bedrooms": n_beds,
                        "bathrooms": n_baths,
                        "address": {"street": "123 New Listing Way", "city": "Rancho Cucamonga", "neighborhood": n_loc},
                        "description": "Just added to the market! Must see.",
                        "match_score": 0.99 # Force it to top for demo
                    }
                    state_mgr.add_listing(new_prop)
                    st.success(f"Ingested {n_title}! Check the Portal tab.")
                    st.rerun()

        with col_p2:
            st.markdown(f"#### Top Matches for {selected_lead_name}")
            
            # Load custom listings first
            from services.demo_state import DemoStateManager
            state_mgr = DemoStateManager()
            custom_matches = state_mgr.get_all_listings()
            
            # Standard matches
            standard_matches = services["property_matcher"].find_matches(current_prefs)
            
            # Combine them (custom first)
                ...
                for match in matches:
                    ...
                    with st.container(border=True):
                        ...
                        with c2:
                            ...
                            # Generate the "Why" logic live
                            why_reason = services["property_matcher"].generate_match_reasoning(match, current_prefs)
                            st.info(f"🤖 **AI Logic:** \"{why_reason}\"")
                            ...
                            if st.button(f"🚀 Send SMS", key=f"send_{match['id']}", use_container_width=True):
                                ...
                                if ai_tone == "Direct/Casual":
                                    sms_text = f"Hey {selected_lead_name.split()[0]}! Found one. {match['title']}. {why_reason} Check it: {match.get('listing_url')}"
    ...
    with tab3:
        ...
        with center_col:
            ...
            with portal_container:
                ...
                st.markdown("<p style='text-align: center; font-size: 0.9rem; opacity: 0.8;'>I've filtered the best homes for you.</p>", unsafe_allow_html=True)
                
                st.slider("Max Budget", 500000, 2000000, current_prefs.get('budget', 1000000), step=50000, key="portal_slider_preview")
                ...
                st.button("Update My Search", use_container_width=True, key="portal_btn_preview")
