...
class PDFReportGenerator:
    ...
    def generate_property_analysis_report(
        ...
        analysis_data = self._structure_property_analysis_data(property_data, market_comparison, financial_analysis)

        # Create PDF document
        output_path = (
            self.output_dir
            / f"property_analysis_{property_data.get('id', 'unknown')}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
        )
    ...
    def generate_market_comparison_report(
        ...
        output_path = (
            self.output_dir
            / f"market_comparison_{target_area.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
        )
        doc = SimpleDocTemplate(str(output_path), pagesize=letter)
    ...
    def generate_financial_projection_report(
        ...
        output_path = (
            self.output_dir
            / f"financial_projection_{property_data.get('id', 'unknown')}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
        )
        doc = SimpleDocTemplate(str(output_path), pagesize=letter)
    ...
    def _create_roi_projection_chart(self, projections: Dict[str, Any], filename: str) -> Path:
        ...
        for i, (scenario, roi_values) in enumerate(roi_data.items()):
            ax.plot(
                years, roi_values, linewidth=3, color=colors[i], label=f"{scenario} Scenario", marker="o", markersize=6
            )
    ...
    def _add_property_cover_page(self, story: List, analysis_data: PropertyAnalysisData):
        ...
        # Title
        title = Paragraph("PROPERTY ANALYSIS REPORT", self.styles["title"])
        story.append(title)
        ...
        subtitle = Paragraph(f"Property Address: {address_str}", self.styles["subtitle"])
        ...
        # Report date
        date_para = Paragraph(f"Report Generated: {datetime.now().strftime('%B %d, %Y')}", self.styles["body"])
    ...
    def _add_financial_analysis_section(
        self, story: List, analysis_data: PropertyAnalysisData, charts_included: List[str]
    ):
        ...
        assumptions_data = [
            ...
            [
                "Down Payment",
                f"{financial_metrics.get('down_payment_percent', 20):.0f}%",
                f"${financial_metrics.get('down_payment_amount', 0):,}",
            ],
            ...
            [
                "Property Management",
                f"{financial_metrics.get('management_fee_percent', 8):.0f}%",
        ...
        cash_flow_data = [
            ...
            [
                "Less: Vacancy Loss",
                f"-${financial_metrics.get('vacancy_loss', 0):,}",
                f"({financial_metrics.get('vacancy_rate', 5):.0f}%)",
            ],
            ...
            [
                "Property Management",
                f"-${financial_metrics.get('management_fee', 0):,}",
                f"({financial_metrics.get('management_fee_percent', 8):.0f}%)",
            ],
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, -1),
                        self.colors["success"]
                        if financial_metrics.get("monthly_cash_flow", 0) > 0
    ...
    def _structure_property_analysis_data(
        self, property_data: Dict[str, Any], market_comparison: List[Dict[str, Any]], financial_analysis: Dict[str, Any]
    ...
    def _analyze_lead_performance_data(
        self, leads_data: List[Dict[str, Any]], time_period: str, breakdown_by_segment: bool
    ...
    def _calculate_investment_metrics(
        self, property_data: Dict[str, Any], financial_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _assess_investment_risks(
        ...
        return {"risk_factors": risks, "risk_score": risk_score, "risk_level": risk_level}
    ...
    def _create_custom_styles(self) -> Dict[str, ParagraphStyle]:
        ...
        custom_styles = {
            ...
            "body": ParagraphStyle(
                "CustomBody", parent=styles["Normal"], fontSize=11, alignment=TA_JUSTIFY, spaceAfter=8
            ),
            "bullet": ParagraphStyle(
                "CustomBullet", parent=styles["Normal"], fontSize=11, leftIndent=20, bulletIndent=10, spaceAfter=4
            ),
    ...
    def _generate_market_comparison_data(
        self, target_area: str, comparable_areas: List[str], analysis_period: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_velocity": {"target_days_on_market": 28, "comparable_days_on_market": [32, 35]},
            "inventory_levels": {"target_months_supply": 3.1, "comparable_months_supply": [3.8, 4.2]},
        }
    ...
    def _calculate_financial_projections(
        ...
        projections = {"scenarios": {}, "projection_years": projection_years, "base_assumptions": investment_parameters}
...
def demo_pdf_generation():
    ...
    sample_property = {
        "id": "PROP_001",
        "price": 625000,
        "address": {"street": "123 Oak Hill Dr", "city": "Rancho Cucamonga", "state": "TX", "zip": "78745"},
        "bedrooms": 3,
        "bathrooms": 2.5,
        "sqft": 2100,
        "year_built": 2018,
    ...
    sample_market_comparison = [
        {"address": "456 Pine St", "price": 595000, "sqft": 1980, "price_per_sqft": 301, "days_on_market": 22},
        {"address": "789 Cedar Ave", "price": 645000, "sqft": 2150, "price_per_sqft": 300, "days_on_market": 8},
    ]
