...
class ReflectionConfig:
    ...
    gap_keywords: Set[str] = field(
        default_factory=lambda: {
            "unknown", "unclear", "not mentioned", "not specified",
            "missing", "incomplete", "insufficient", "limited",
            "no information", "not available", "unavailable",
        }
    )
...
class ConfidenceScorer:
    ...
    def calculate(
        ...
        completeness = self._calculate_completeness(
            tool_results, query_intent, answer_text
        )
    ...
    def _calculate_result_diversity(
        self, 
        tool_results: List[ToolResult]
    ) -> float:
    ...
    def _calculate_completeness(
        ...
        if answer_text:
            ...
            gap_count = sum(
                1 for keyword in self.config.gap_keywords 
...
class ReflectionEngine:
    ...
    def identify_gaps(
        ...
        for step in query_plan.steps:
            ...
            keyword_matches = sum(
                ...
                if len(kw) > 3 and kw in answer_lower
            )
    ...
    def generate_correction_strategies(
        ...
        for gap in assessment.gaps:
            strategies.append(CorrectionStrategy(
                target_dimension=QualityDimension.COMPLETENESS,
    ...
    def should_iterate(
        ...
        if confidence_score and confidence_score.is_high_confidence(
    ...
    def _assess_completeness(
        ...
        has_results = any(
            r.success and r.data for r in tool_results
        )
    ...
    def _assess_coherence(self, answer: str) -> float:
        ...
        flow_score = min(
            sum(1 for indicator in flow_indicators if indicator in answer.lower()) / 3,
            1.0
        )
    ...
    def _assess_citations(
        ...
        has_sources = any(
            r.success and r.data and isinstance(r.data, dict)
    ...
    def _assess_currency(self, answer: str, query: str) -> float:
        ...
        is_time_sensitive = any(
            kw in query_lower for kw in time_sensitive_keywords
        )
