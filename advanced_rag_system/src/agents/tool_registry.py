...
class BaseTool(ABC):
    ...
    async def _check_rate_limit(self) -> None:
        ...
        if self._last_call_time:
            time_since_last = (
                datetime.now() - self._last_call_time
            ).total_seconds()
        ...
        try:
            ...
            return ToolResult(
                ...
                data={
                    ...
                    "total_found": len(results),
                    "query": query,
                    "provider": "serper" if self._serper_key else 
                               "tavily" if self._tavily_key else "mock",
                },
                execution_time_ms=execution_time,
            )
...
class ToolRegistry:
    ...
    def _rank_results(
        ...
        all_items.sort(
            key=lambda x: x.get("score", x.get("position", 0)),
            reverse=True
        )
