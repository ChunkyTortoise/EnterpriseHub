...
class QueryPlannerConfig:
    ...
    calculation_keywords: Set[str] = field(default_factory=lambda: {
        "calculate", "compute", "sum", "total", "average", "mean",
        "median", "percentage", "ratio", "difference", "multiply",
        "divide", "add", "subtract", "how much", "how many",
        "cost", "price", "value", "worth", "budget", "expense"
    })
    comparison_keywords: Set[str] = field(default_factory=lambda: {
        "compare", "versus", "vs", "difference between", "similarities",
        "better than", "worse than", "advantages", "disadvantages",
        "pros and cons", "which is", "which one", "or", "between"
    })
...
class QueryPlanner:
    ...
    def analyze_intent(self, query: str) -> IntentAnalysis:
        ...
        calc_keywords_found = sum(
            1 for kw in self.config.calculation_keywords 
            if kw in query_lower
        )
        if calc_keywords_found > 0:
            scores[QueryIntent.CALCULATION] = scores.get(
                QueryIntent.CALCULATION, 0.0
            ) + min(calc_keywords_found * 0.2, 0.5)
        
        # Keyword-based scoring for comparison
        comp_keywords_found = sum(
            1 for kw in self.config.comparison_keywords 
            if kw in query_lower
        )
        if comp_keywords_found > 0:
            scores[QueryIntent.COMPARISON] = scores.get(
                QueryIntent.COMPARISON, 0.0
            ) + min(comp_keywords_found * 0.2, 0.5)
        ...
        return IntentAnalysis(
            ...
            entities=entities,
            keywords=keywords,
            requires_calculation=QueryIntent.CALCULATION in scores 
                and scores[QueryIntent.CALCULATION] >= 0.3,
            requires_comparison=QueryIntent.COMPARISON in scores 
                and scores[QueryIntent.COMPARISON] >= 0.3,
    ...
    def decompose_query(
        ...
        if intent_analysis.requires_comparison:
            ...
            parts = re.split(
                r'\s+(?:versus|vs|compared to|or|and)\s+',
                query,
                flags=re.IGNORECASE
            )
            if len(parts) >= 2:
        ...
        if intent_analysis.requires_calculation:
            ...
            calc_pattern = re.search(
                r'(average|sum|total|calculate|compute)\s+(?:of\s+)?(.+?)(?:\?|$)',
                query_lower
            )
            if calc_pattern:
    ...
    def create_plan(self, query: str) -> QueryPlan:
        ...
        for i, sub_query in enumerate(sub_queries[:self.config.max_steps], 1):
            ...
            primary_tool = tool_selections[0] if tool_selections else ToolSelection(
                tool_name="vector_search",
                confidence=0.5,
        ...
        parallelizable = (
            self.config.enable_parallelization 
            and len(steps) > 1 
            and not any(step.dependencies for step in steps)
        )
    ...
    def _extract_keywords(self, query: str) -> List[str]:
        ...
        stop_words = {
            ...
            "why", "how", "that", "with", "from", "they", "them",
            "have", "this", "will", "your", "about", "could", "would"
        }
