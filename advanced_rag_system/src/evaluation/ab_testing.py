...
class WeightedSplitter(TrafficSplitter):
    ...
    def assign(
        ...
        **kwargs
        ...
        weights = {}
        for variant in experiment.variants:
            weights[variant.id] = self.weights.get(
                variant.id,
...
class ThompsonSamplingSplitter(TrafficSplitter):
    ...
    def assign(
        ...
        **kwargs
        ...
        samples = {}
        for variant in experiment.variants:
            alpha, beta = self.variant_params.get(
                variant.id,
...
class StatisticalTester:
    ...
    def _chi2_cdf(self, x: float, k: int) -> float:
        ...
        return 1 - np.exp(-x / 2) * sum(
            (x / 2) ** i / np.math.factorial(i) for i in range(k // 2 + 1)
        )
    ...
    def compare_variants(
        ...
        if metric_config.metric_type == MetricType.BINARY:
            ...
            test_result = self.statistical_tester.chi_square_test(
                int(treatment_successes), len(treatment_values),
                int(control_successes), len(control_values)
            )
        else:
            # Use t-test for continuous metrics
            test_result = self.statistical_tester.t_test(
...
class ExperimentManager:
    ...
    async def get_results(self, experiment_id: str) -> Optional[ExperimentResults]:
        ...
        for variant in exp.variants:
            ...
            variant_results[variant.id] = self.aggregator.aggregate_variant_results(
                var_obs, variant
            )
        ...
        if exp.primary_metric and control_id:
            ...
            for variant in exp.variants:
                if variant.id != control_id:
                    treatment_obs = [
                        obs for obs in variant_observations.get(variant.id, [])
                        if obs.metric_name == exp.primary_metric.name
                    ]
                    control_obs = [
                        obs for obs in variant_observations.get(control_id, [])
                        if obs.metric_name == exp.primary_metric.name
                    ]

                    if treatment_obs and control_obs:
                        primary_comparison = self.aggregator.compare_variants(
                            treatment_obs, control_obs,
        ...
        return ExperimentResults(
            ...
            duration_hours=(
                (exp.end_time or datetime.utcnow()) - exp.start_time
            ).total_seconds() / 3600 if exp.start_time else 0,
