...
class TestComponentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_query_enhancement_reranking_integration(
        self, mock_hyde_generator, mock_reranker, sample_search_results
    ):
...
class TestDataFlow:
    ...
    async def test_document_flow_through_pipeline(self, sample_document_chunks, mock_reranker):
        ...
        with patch.object(searcher.hybrid_searcher, "search", new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [SearchResult(chunk=original_chunk, score=0.95, rank=1, distance=0.05)]
    ...
    async def test_metadata_preservation(self, sample_document_chunks, mock_reranker):
        ...
        with patch.object(searcher.hybrid_searcher, "search", new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [SearchResult(chunk=chunk_with_metadata, score=0.9, rank=1, distance=0.1)]
