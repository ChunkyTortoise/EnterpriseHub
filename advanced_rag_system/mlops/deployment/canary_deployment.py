    ...
    async def collect_metrics(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                canary_metrics = await self._collect_endpoint_metrics(
                    session, canary_endpoint, "canary"
                )

                # Collect current metrics
                current_metrics = await self._collect_endpoint_metrics(
                    session, current_endpoint, "current"
                )
...
class CanaryDeploymentController:
    ...
    async def _run_initial_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unhealthy_endpoints = [
                endpoint for endpoint, status in health_results.items()
            ...
            canary_metrics, current_metrics = await self.metrics_collector.collect_metrics(
                self.canary_endpoint + "/metrics",
                self.current_endpoint + "/metrics",
                traffic_stats
            )
    ...
    async def _analyze_canary_performance(self) -> Dict[str, Any]:
        ...
        try:
            # Collect current metrics
            traffic_stats = self.traffic_splitter.get_traffic_statistics(
                self.config.observation_window_minutes
            )
            ...
            # Analyze comparison
            analysis = self.metrics_collector.analyze_metrics_comparison(
                canary_metrics, current_metrics
            )
    ...
    def _make_deployment_decision(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (current_percentage >= self.config.max_canary_percentage and
                ...
                if error_rate_ok and response_time_ok and success_rate_ok:
                    return {
                        ...
                        'reason': "All criteria met for full deployment"
                    }

            # Ramp up if performance is good
            if (analysis['health_status'] == HealthStatus.HEALTHY.value and
                current_percentage < self.config.max_canary_percentage):

                new_percentage = min(
                    current_percentage + self.config.ramp_up_step,
...
async def create_rag_canary_deployment():
    ...
    health_checks = [
        ...
        HealthCheck(
            endpoint="http://canary-rag-api:8000/api/v1/embedding/health",
            expected_status_code=200
        )
    ]
