...
async def get_call_history(
    ...
    """
    try:
        calls = await twilio_client.get_call_history(
...
async def send_sms(
    ...
    try:
        _validate_phone_number(to_number)
        message = await twilio_client.send_sms(
            to_number=to_number,
            body=body,
            media_url=media_url
        )
...
async def get_sms_history(
    ...
    """
    try:
        messages = await twilio_client.get_sms_history(
            phone_number=phone_number,
            limit=limit,
            start_date=start_date
        )
...
async def transcribe_call_recording(recording_url: str, language: str = "en") -> str:
    ...
    """
    try:
        result = await transcription_client.transcribe_audio(
...
async def route_call(
    ...
    try:
        ...
        if lead_score and lead_score >= 80:
            ...
            priority = "low"
            greeting = "Welcome! Press 1 for property listings, 2 for market information, or stay on the line for an agent."
